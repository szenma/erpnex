...
class TransactionBase(StatusUpdater):
	...
	def validate_with_previous_doc(self, ref):
		...
		for key, val in ref.products():
			is_child = val.get("is_child_table")
			ref_doc = {}
			product_ref_dn = []
			for d in self.get_all_children(self.doctype + " Product"):
				ref_dn = d.get(val["ref_dn_field"])
				if ref_dn:
					if is_child:
						self.compare_values({key: [ref_dn]}, val["compare_fields"], d)
						if ref_dn not in product_ref_dn:
							product_ref_dn.append(ref_dn)
	...
	def compare_values(self, ref_doc, fields, doc=None):
		for reference_doctype, ref_dn_list in ref_doc.products():
	...
	def validate_rate_with_reference_doc(self, ref_details):
		...
		for ref_dt, ref_dn_field, ref_link_field in ref_details:
			reference_names = [d.get(ref_link_field) for d in self.get("products") if d.get(ref_link_field)]
			reference_details = self.get_reference_details(reference_names, ref_dt + " Product")
	...
	def get_link_filters(self, for_doctype):
		if hasattr(self, "prev_link_mapper") and self.prev_link_mapper.get(for_doctype):
			...
			values = filter(None, tuple(product.as_dict()[fieldname] for product in self.products))
	...
	def reset_default_field_value(self, default_field: str, child_table: str, child_table_field: str):
		...
		example:
		        doc = {
		                ...
		                "products": [{"from_warehouse": "warehouse B"}, {"from_warehouse": "warehouse A"}],
		        }
		        Since this has dissimilar values in child table, the default field will be erased.

		        doc.reset_default_field_value("set_from_warehouse", "products", "from_warehouse")
		"""
