		...
		"chart_of_accounts": {
			"*": {
				"product_tax_templates": [
					...
					for title, data in templates.products()
				],
				"*": [
					{
						"title": title,
...
def from_detailed_data(company_name, data):
	...
	product_tax_templates = tax_templates.get("product_tax_templates") or tax_templates.get("*", {})
	...
	if product_tax_templates:
		for template in product_tax_templates:
			make_product_tax_template(company_name, template)
...
def make_taxes_and_charges_template(company_name, doctype, template):
	...
	for tax_row in template.get("taxes"):
		...
		for fieldname, default_value in tax_row_defaults.products():
...
def make_product_tax_template(company_name, template):
	"""Create an Product Tax Template.

	This requires a separate method because Product Tax Template is structured
