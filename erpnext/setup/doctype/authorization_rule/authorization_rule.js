...
frappe.ui.form.on("Authorization Rule", {
	...
	set_master_type: function(frm) {
		if(frm.doc.based_on==="Customerwise Discount") {
			...
			frm.set_value("customer_or_product", "Customer");
		} else if(frm.doc.based_on==="Productwise Discount") {
			unhide_field("master_name");
			frm.set_value("customer_or_product", "Product");
		} else {
			frm.set_value("customer_or_product", "");
...
cur_frm.fields_dict['master_name'].get_query = function(doc) {
	if (doc.based_on == 'Customerwise Discount')
	...
	else if (doc.based_on == 'Productwise Discount')
		return {
			doctype: "Product",
		...
		return {
			filters: [
				['Product', 'name', '=', 'cheating done to avoid null']
			]
		}
}
