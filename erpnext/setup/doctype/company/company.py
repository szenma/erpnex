...
class Company(NestedSet):
	...
	def validate(self):
		...
		self.validate_provisional_account_for_non_stock_products()
	...
	def validate_provisional_account_for_non_stock_products(self):
		if not self.get("__islocal"):
			if (
				cint(self.enable_provisional_accounting_for_non_stock_products) == 1
				and not self.default_provisional_account
			):
				frappe.throw(
					_("Set default {0} account for non stock products").format(frappe.bold("Provisional Account"))
				)
			...
			make_property_setter(
				...
				"hidden",
				not self.enable_provisional_accounting_for_non_stock_products,
				"Check",
	...
	def on_trash(self):
		...
		for doctype in ["Mode of Payment Account", "Product Default"]:
			frappe.db.sql("delete from `tab{0}` where company = %s".format(doctype), self.name)

		# clear default accounts, warehouses from product
		warehouses = frappe.db.sql_list("select name from tabWarehouse where company=%s", self.name)
		if warehouses:
			frappe.db.sql(
				"""delete from `tabProduct Reorder` where warehouse in (%s)"""
		...
		if boms:
			...
			for dt in ("BOM Operation", "BOM Product", "BOM Scrap Product", "BOM Explosion Product"):
				frappe.db.sql(
		...
		frappe.db.sql("delete from `tabProduct Tax Template` where company=%s", self.name)
