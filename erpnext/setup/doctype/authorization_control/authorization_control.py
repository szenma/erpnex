...
class AuthorizationControl(TransactionBase):
	def get_appr_user_role(self, det, doctype_name, total, based_on, condition, product, company):
	...
	def validate_auth_rule(self, doctype_name, total, based_on, cond, company, product=""):
		chk = 1
		add_cond1, add_cond2 = "", ""
		if based_on == "Productwise Discount":
			add_cond1 += " and master_name = " + frappe.db.escape(cstr(product))
			productwise_exists = frappe.db.sql(
			...
			if productwise_exists:
				self.get_appr_user_role(
					productwise_exists, doctype_name, total, based_on, cond + add_cond1, product, company
				)
				chk = 0
		if chk == 1:
			if based_on == "Productwise Discount":
			...
			self.get_appr_user_role(appr, doctype_name, total, based_on, cond + add_cond2, product, company)
	...
	def bifurcate_based_on_type(self, doctype_name, total, av_dis, based_on, doc_obj, val, company):
		...
		if based_on == "Productwise Discount":
			if doc_obj:
				for t in doc_obj.get("products"):
					self.validate_auth_rule(
						doctype_name, t.discount_percentage, based_on, add_cond, company, t.product_code
					)
		else:
	...
	def validate_approving_authority(self, doctype_name, company, total, doc_obj=""):
		...
		for r in based_on:
			if r in final_based_on and r != "Productwise Discount":
