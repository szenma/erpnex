...
class EmailDigest(Document):
	...
	def get_msg_html(self):
		...
		if self.get("purchase_orders_products_overdue"):
			(
				context.purchase_order_list,
				context.purchase_orders_products_overdue_list,
			) = self.get_purchase_orders_products_overdue_list()
			if not context.purchase_order_list:
				frappe.throw(_("No products to be received are overdue"))
	...
	def set_style(self, context):
		...
		context.line_product = "padding: 5px 0px; margin: 0; border-bottom: 1px solid #d1d8dd;"
	...
	def get_notifications(self):
		...
		notifications = sorted(notifications.get("open_count_doctype", {}).products(), key=lambda a: a[1])
	...
	def get_purchase_orders_products_overdue_list(self):
		fields_po = "distinct `tabPurchase Order Product`.parent as po"
		fields_poi = (
			"`tabPurchase Order Product`.parent, `tabPurchase Order Product`.schedule_date, product_code,"
			"received_qty, qty - received_qty as missing_qty, rate, amount"
		)

		sql_po = """select {fields} from `tabPurchase Order Product`
			left join `tabPurchase Order` on `tabPurchase Order`.name = `tabPurchase Order Product`.parent
			where status<>'Closed' and `tabPurchase Order Product`.docstatus=1 and CURRENT_DATE > `tabPurchase Order Product`.schedule_date
			and received_qty < qty order by `tabPurchase Order Product`.parent DESC,
			`tabPurchase Order Product`.schedule_date DESC""".format(
			fields=fields_po
		)

		sql_poi = """select {fields} from `tabPurchase Order Product`
			left join `tabPurchase Order` on `tabPurchase Order`.name = `tabPurchase Order Product`.parent
			where status<>'Closed' and `tabPurchase Order Product`.docstatus=1 and CURRENT_DATE > `tabPurchase Order Product`.schedule_date
			and received_qty < qty order by `tabPurchase Order Product`.idx""".format(
			fields=fields_poi
		)
		purchase_order_list = frappe.db.sql(sql_po, as_dict=True)
		purchase_order_products_overdue_list = frappe.db.sql(sql_poi, as_dict=True)

		for t in purchase_order_products_overdue_list:
		...
		return purchase_order_list, purchase_order_products_overdue_list
