...
def get_data(filters, conditions):
	...
	if filters.get("group_by"):
		...
		if filters.get("group_by") == "Product":
			sel_col = "t2.product_code"
		elif filters.get("group_by") == "Customer":
			sel_col = "t1.party_name" if conditions.get("trans") == "Quotation" else "t1.customer"
		elif filters.get("group_by") == "Supplier":
			sel_col = "t1.supplier"

		if filters.get("based_on") in ["Product", "Customer", "Supplier"]:
			inc = 2
		else:
			inc = 1
		data1 = frappe.db.sql(
			""" select %s from `tab%s` t1, `tab%s Product` t2 %s
		...
		for d in range(len(data1)):
			...
			row = frappe.db.sql(
				"""select DISTINCT(%s) from `tab%s` t1, `tab%s Product` t2 %s
			...
			for i in range(len(row)):
				...
				row1 = frappe.db.sql(
					""" select %s , %s from `tab%s` t1, `tab%s Product` t2 %s
							where t2.parent = t1.name and t1.company = %s and %s between %s and %s
...
def based_wise_columns_query(based_on, trans):
	...
	if based_on == "Product":
		based_on_details["based_on_cols"] = ["Product:Link/Product:120", "Product Name:Data:120"]
		based_on_details["based_on_select"] = "t2.product_code, t2.product_name,"
		based_on_details["based_on_group_by"] = "t2.product_code"
		based_on_details["addl_tables"] = ""

	elif based_on == "Product Group":
		based_on_details["based_on_cols"] = ["Product Group:Link/Product Group:120"]
		based_on_details["based_on_select"] = "t2.product_group,"
		based_on_details["based_on_group_by"] = "t2.product_group"
		based_on_details["addl_tables"] = ""
