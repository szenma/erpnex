...
from erpnext.accounts.doctype.pricing_rule.utils import (
	apply_pricing_rule_for_free_products,
...
from erpnext.controllers.print_settings import (
	set_print_templates_for_product_table,
	set_print_templates_for_taxes,
)
from erpnext.controllers.sales_and_purchase_return import validate_return
from erpnext.exceptions import InvalidCurrency
from erpnext.setup.utils import get_exchange_rate
from erpnext.stock.doctype.product.product import get_uom_conv_factor
...
class AccountsController(TransactionBase):
	...
	def get_print_settings(self):
		...
		products_field = self.meta.get_field("products")

		if products_field and products_field.fieldtype == "Table":
			print_setting_fields += ["compact_product_print", "print_uom_after_quantity"]
	...
	def on_trash(self):
		...
		rpi = frappe.qb.DocType("Repost Payment Ledger Products")
	...
	def validate_deferred_income_expense_account(self):
		...
		for product in self.get("products"):
			if product.get("enable_deferred_revenue") or product.get("enable_deferred_expense"):
				if not product.get(field_map.get(self.doctype)):
					...
					if not default_deferred_account:
						frappe.throw(
							_(
								"Row #{0}: Please update deferred revenue/expense account in product row or default account in company master"
							).format(product.idx)
						)
					else:
						product.set(field_map.get(self.doctype), default_deferred_account)
	...
	def before_print(self, settings=None):
		if self.doctype in [
			...
			if self.get("group_same_products"):
		...
		set_print_templates_for_product_table(self, settings)
	...
	def validate_party_accounts(self):
		...
		if self.doctype == "Sales Invoice":
			...
			product_field = "income_account"
		else:
			party_account_field = "credit_to"
			product_field = "expense_account"

		for product in self.get("products"):
			if product.get(product_field) == self.get(party_account_field):
				frappe.throw(
					_("Row {0}: {1} {2} cannot be same as {3} (Party Account) {4}").format(
						product.idx,
						frappe.bold(frappe.unscrub(product_field)),
	...
	def validate_inter_company_reference(self):
		...
		if self.is_internal_transfer():
			...
			label = "Delivery Note Product" if self.doctype == "Purchase Receipt" else "Sales Invoice Product"
	...
	def set_missing_product_details(self, for_validate=False):
		"""set missing product values"""
		...
		if hasattr(self, "products"):
			...
			if self.doctype in ["Quotation", "Sales Order", "Delivery Note", "Sales Invoice"]:
				document_type = "{} Product".format(self.doctype)
			...
			for product in self.get("products"):
				if product.get("product_code"):
					...
					args.update(product.as_dict())

					args["doctype"] = self.doctype
					args["name"] = self.name
					args["child_docname"] = product.name
					...
					ret = get_product_details(args, self, for_validate=True, overwrite_warehouse=False)

					for fieldname, value in ret.products():
						if product.meta.get_field(fieldname) and value is not None:
							if product.get(fieldname) is None or fieldname in force_product_fields:
								product.set(fieldname, value)

							elif fieldname in ["cost_center", "conversion_factor"] and not product.get(fieldname):
								product.set(fieldname, value)

							elif fieldname == "serial_no":
								# Ensure that serial numbers are matched against Stock UOM
								product_conversion_factor = product.get("conversion_factor") or 1.0
								product_qty = abs(product.get("qty")) * product_conversion_factor

								if product_qty != len(get_serial_nos(product.get("serial_no"))):
									product.set(fieldname, value)
					...
					if self.doctype in ["Purchase Invoice", "Sales Invoice"] and product.meta.get_field(
						"is_fixed_asset"
					):
						product.set("is_fixed_asset", ret.get("is_fixed_asset", 0))

					# Double check for cost center
					# Products add via promotional scheme may not have cost center set
					if hasattr(product, "cost_center") and not product.get("cost_center"):
						product.set(
							"cost_center", self.get("cost_center") or erpnext.get_default_cost_center(self.company)
						)

					if ret.get("pricing_rules"):
						self.apply_pricing_rule_on_products(product, ret)
						self.set_pricing_rule_details(product, ret)
				else:
					# Transactions line product without product code

					uom = product.get("uom")
					stock_uom = product.get("stock_uom")
					if bool(uom) != bool(stock_uom):  # xor
						product.stock_uom = product.uom = uom or stock_uom

					# UOM cannot be zero so substitute as 1
					product.conversion_factor = (
						get_uom_conv_factor(product.get("uom"), product.get("stock_uom"))
						or product.get("conversion_factor")
						or 1
					)
	...
	def apply_pricing_rule_on_products(self, product, pricing_rule_args):
		if not pricing_rule_args.get("validate_applied_rule", 0):
			# if user changed the discount percentage then set user's discount percentage ?
			if pricing_rule_args.get("price_or_product_discount") == "Price":
				product.set("pricing_rules", pricing_rule_args.get("pricing_rules"))
				if pricing_rule_args.get("apply_rule_on_other_products"):
					other_products = json.loads(pricing_rule_args.get("apply_rule_on_other_products"))
					if other_products and product.product_code not in other_products:
						return

				product.set("discount_percentage", pricing_rule_args.get("discount_percentage"))
				product.set("discount_amount", pricing_rule_args.get("discount_amount"))
				if pricing_rule_args.get("pricing_rule_for") == "Rate":
					product.set("price_list_rate", pricing_rule_args.get("price_list_rate"))

				if product.get("price_list_rate"):
					product.rate = flt(
						product.price_list_rate * (1.0 - (flt(product.discount_percentage) / 100.0)),
						product.precision("rate"),
					)

					if product.get("discount_amount"):
						product.rate = product.price_list_rate - product.discount_amount

				if product.get("apply_discount_on_discounted_rate") and pricing_rule_args.get("rate"):
					product.rate = pricing_rule_args.get("rate")

			elif pricing_rule_args.get("free_product_data"):
				apply_pricing_rule_for_free_products(self, pricing_rule_args.get("free_product_data"))

		elif pricing_rule_args.get("validate_applied_rule"):
			for pricing_rule in get_applied_pricing_rules(product.get("pricing_rules")):
				pricing_rule_doc = frappe.get_cached_doc("Pricing Rule", pricing_rule)
				for field in ["discount_percentage", "discount_amount", "rate"]:
					if product.get(field) < pricing_rule_doc.get(field):
						title = get_link_to_form("Pricing Rule", pricing_rule)

						frappe.msgprint(
							_("Row {0}: user has not applied the rule {1} on the product {2}").format(
								product.idx, frappe.bold(title), frappe.bold(product.product_code)
							)
						)

	def set_pricing_rule_details(self, product_row, args):
		...
		for pricing_rule in pricing_rules:
			self.append(
				"pricing_rules",
				{
					...
					"child_docname": product_row.name,
					"rule_applied": True,
				},
			)
	...
	def get_gl_dict(self, args, account_currency=None, product=None):
		...
		for dimension in accounting_dimensions:
			...
			if product and product.get(dimension):
				dimension_dict[dimension] = product.get(dimension)
	...
	def validate_qty_is_not_zero(self):
		if self.doctype != "Purchase Receipt":
			for product in self.products:
				if not product.qty:
					frappe.throw(_("Product quantity can not be zero"))
	...
	def get_advance_entries(self, include_unallocated=True):
		...
		order_list = list(set(d.get(order_field) for d in self.get("products") if d.get(order_field)))
	...
	def unlink_ref_doc_from_po(self):
		so_products = []
		for product in self.products:
			so_products.append(product.name)
			...
			set(
				frappe.get_all(
					...
					filters={
						...
						"sales_order_product": ["in", so_products],
						"docstatus": ["<", 2],
					},
					pluck="parent",
				)
		...
		if linked_po:
			frappe.db.set_value(
				"Purchase Order Product",
				{"sales_order": self.name, "sales_order_product": ["in", so_products], "docstatus": ["<", 2]},
				{"sales_order": None, "sales_order_product": None},
			)
	...
	def get_amount_and_base_amount(self, product, enable_discount_accounting):
		amount = product.net_amount
		base_amount = product.base_net_amount
	...
	def make_discount_gl_entries(self, gl_entries):
		...
		if enable_discount_accounting:
			for product in self.get("products"):
				if product.get("discount_amount") and product.get("discount_account"):
					discount_amount = product.discount_amount * product.qty
					if self.doctype == "Purchase Invoice":
						income_or_expense_account = (
							product.expense_account
							if (not product.enable_deferred_expense or self.is_return)
							else product.deferred_expense_account
						)
					else:
						income_or_expense_account = (
							product.income_account
							if (not product.enable_deferred_revenue or self.is_return)
							else product.deferred_revenue_account
						)

					account_currency = get_account_currency(product.discount_account)
					gl_entries.append(
						self.get_gl_dict(
							{
								"account": product.discount_account,
								"against": supplier_or_customer,
								dr_or_cr: flt(
									discount_amount * self.get("conversion_rate"), product.precision("discount_amount")
								),
								dr_or_cr + "_in_account_currency": flt(discount_amount, product.precision("discount_amount")),
						...
						self.get_gl_dict(
							{
								...
								rev_dr_cr
								+ "_in_account_currency": flt(discount_amount, product.precision("discount_amount")),
								"cost_center": product.cost_center,
								"project": product.project or self.project,
							},
							account_currency,
							product=product,
						)
	...
	def validate_multiple_billing(self, ref_dt, product_ref_dn, based_on):
		...
		reference_names = [d.get(product_ref_dn) for d in self.get("products") if d.get(product_ref_dn)]
		reference_details = self.get_billing_reference_details(
			reference_names, ref_dt + " Product", based_on
		)

		for product in self.get("products"):
			if not product.get(product_ref_dn):
				continue

			ref_amt = flt(reference_details.get(product.get(product_ref_dn)), self.precision(based_on, product))

			if not ref_amt:
				frappe.msgprint(
					_("System will not check over billing since amount for Product {0} in {1} is zero").format(
						product.product_code, ref_dt
					),
					title=_("Warning"),
					indicator="orange",
				)
			...
			already_billed = self.get_billed_amount_for_product(product, product_ref_dn, based_on)

			total_billed_amt = flt(
				flt(already_billed) + flt(product.get(based_on)), self.precision(based_on, product)
			)

			allowance, product_allowance, global_qty_allowance, global_amount_allowance = get_allowance_for(
				product.product_code, product_allowance, global_qty_allowance, global_amount_allowance, "amount"
			)
			...
			if overbill_amt > 0.01 and role_allowed_to_over_bill not in user_roles:
				if self.doctype != "Purchase Invoice":
					self.throw_overbill_exception(product, max_allowed_amt)
				elif not cint(
	...
	def get_billed_amount_for_product(self, product, product_ref_dn, based_on):
		"""
		Returns Sum of Amount of
		Sales/Purchase Invoice Products
		that are linked to `product_ref_dn` (`dn_detail` / `pr_detail`)
		...
		product_doctype = frappe.qb.DocType(product.doctype)
		based_on_field = frappe.qb.Field(based_on)
		join_field = frappe.qb.Field(product_ref_dn)

		result = (
			frappe.qb.from_(product_doctype)
			.select(Sum(based_on_field))
			.where(join_field == product.get(product_ref_dn))
			.where(
				Criterion.any(
					[  # select all products from other invoices OR current invoices
						Criterion.all(
							[  # for selecting products from other invoices
								product_doctype.docstatus == 1,
								product_doctype.parent != self.name,
							]
						),
						Criterion.all(
							[  # for selecting products from current invoice, that are linked to same reference
								product_doctype.docstatus == 0,
								product_doctype.parent == self.name,
								product_doctype.name != product.name,
							]
						),
					]
				)
	...
	def throw_overbill_exception(self, product, max_allowed_amt):
		frappe.throw(
			_(
				"Cannot overbill for Product {0} in row {1} more than {2}. To allow over-billing, please set allowance in Accounts Settings"
			).format(product.product_code, product.idx, max_allowed_amt)
		)
	...
	def get_stock_products(self):
		stock_products = []
		product_codes = list(set(product.product_code for product in self.get("products")))
		if product_codes:
			stock_products = frappe.db.get_values(
				"Product", {"name": ["in", product_codes], "is_stock_product": 1}, pluck="name", cache=True
			)
	...
	def group_similar_products(self):
		group_product_qty = {}
		group_product_amount = {}
		# to update serial number in print
		count = 0

		for product in self.products:
			group_product_qty[product.product_code] = group_product_qty.get(product.product_code, 0) + product.qty
			group_product_amount[product.product_code] = group_product_amount.get(product.product_code, 0) + product.amount

		duplicate_list = []
		for product in self.products:
			if product.product_code in group_product_qty:
				count += 1
				product.qty = group_product_qty[product.product_code]
				product.amount = group_product_amount[product.product_code]

				if product.qty:
					product.rate = flt(flt(product.amount) / flt(product.qty), product.precision("rate"))
				else:
					product.rate = 0

				product.idx = count
				del group_product_qty[product.product_code]
			else:
	...
	def set_payment_schedule(self):
		...
		if not self.get("payment_schedule"):
			if (
				...
				for product in data:
					self.append("payment_schedule", product)
	...
	def get_order_details(self):
		if self.doctype == "Sales Invoice":
			po_or_so = self.get("products")[0].get("sales_order")
			po_or_so_doctype = "Sales Order"
			po_or_so_doctype_name = "sales_order"

		else:
			po_or_so = self.get("products")[0].get("purchase_order")
	...
	def linked_order_has_payment_terms(self, po_or_so, fieldname, doctype):
		if po_or_so and self.all_products_have_same_po_or_so(po_or_so, fieldname):
	...
	def all_products_have_same_po_or_so(self, po_or_so, fieldname):
		for product in self.get("products"):
			if product.get(fieldname) != po_or_so:
				return False
	...
	def check_finance_books(self, product, asset):
		if (
			len(asset.finance_books) > 1
			and not product.get("finance_book")
			and not self.get("finance_book")
			and asset.finance_books[0].finance_book
		):
			frappe.throw(
				_("Select finance book for the product {0} at row {1}").format(product.product_code, product.idx)
			)
...
def validate_inclusive_tax(tax, doc):
	def _on_previous_row_error(row_range):
		throw(
			_("To include tax in row {0} in Product rate, taxes in rows {1} must also be included").format(
				tax.idx, row_range
			)
		)
	...
	if cint(getattr(tax, "included_in_print_rate", None)):
		if tax.charge_type == "Actual":
			...
			throw(
				_("Charge of type 'Actual' in row {0} cannot be included in Product Rate or Paid Amount").format(
					tax.idx
				)
			)
...
def set_child_tax_template_and_map(product, child_product, parent_doc):
	args = {
	...
	child_product.product_tax_template = _get_product_tax_template(args, product.taxes)
	if child_product.get("product_tax_template"):
		child_product.product_tax_rate = get_product_tax_map(
			parent_doc.get("company"), child_product.product_tax_template, as_json=True
		)


def add_taxes_from_tax_template(child_product, parent_doc, db_insert=True):
	add_taxes_from_product_tax_template = frappe.db.get_single_value(
		"Accounts Settings", "add_taxes_from_product_tax_template"
	)

	if child_product.get("product_tax_rate") and add_taxes_from_product_tax_template:
		tax_map = json.loads(child_product.get("product_tax_rate"))
...
def set_order_defaults(
	parent_doctype, parent_doctype_name, child_doctype, child_docname, trans_product
):
	"""
	Returns a Sales/Purchase Order Product child product containing the default values
	"""
	p_doc = frappe.get_doc(parent_doctype, parent_doctype_name)
	child_product = frappe.new_doc(child_doctype, p_doc, child_docname)
	product = frappe.get_doc("Product", trans_product.get("product_code"))

	for field in ("product_code", "product_name", "description", "product_group"):
		child_product.update({field: product.get(field)})

	date_fieldname = "delivery_date" if child_doctype == "Sales Order Product" else "schedule_date"
	child_product.update({date_fieldname: trans_product.get(date_fieldname) or p_doc.get(date_fieldname)})
	child_product.stock_uom = product.stock_uom
	child_product.uom = trans_product.get("uom") or product.stock_uom
	child_product.warehouse = get_product_warehouse(product, p_doc, overwrite_warehouse=True)
	conversion_factor = flt(
		get_conversion_factor(product.product_code, child_product.uom).get("conversion_factor")
	)
	child_product.conversion_factor = flt(trans_product.get("conversion_factor")) or conversion_factor

	if child_doctype == "Purchase Order Product":
		# Initialized value will update in parent validation
		child_product.base_rate = 1
		child_product.base_amount = 1
	if child_doctype == "Sales Order Product":
		child_product.warehouse = get_product_warehouse(product, p_doc, overwrite_warehouse=True)
		if not child_product.warehouse:
			frappe.throw(
				_("Cannot find {} for product {}. Please set the same in Product Master or Stock Settings.").format(
					frappe.bold("default warehouse"), frappe.bold(product.product_code)
				)
			)

	set_child_tax_template_and_map(product, child_product, p_doc)
	add_taxes_from_tax_template(child_product, p_doc)
	return child_product


def validate_child_on_delete(row, parent):
	"""Check if partially transacted product (row) is being deleted."""
	if parent.doctype == "Sales Order":
		if flt(row.delivered_qty):
			frappe.throw(
				_("Row #{0}: Cannot delete product {1} which has already been delivered").format(
					row.idx, row.product_code
				)
			)
		if flt(row.work_order_qty):
			frappe.throw(
				_("Row #{0}: Cannot delete product {1} which has work order assigned to it.").format(
					row.idx, row.product_code
				)
			)
		if flt(row.ordered_qty):
			frappe.throw(
				_("Row #{0}: Cannot delete product {1} which is assigned to customer's purchase order.").format(
					row.idx, row.product_code
				)
			)
	...
	if parent.doctype == "Purchase Order" and flt(row.received_qty):
		frappe.throw(
			_("Row #{0}: Cannot delete product {1} which has already been received").format(
				row.idx, row.product_code
			)
		)
	...
	if flt(row.billed_amt):
		frappe.throw(
			_("Row #{0}: Cannot delete product {1} which has already been billed.").format(
				row.idx, row.product_code
			)
		)
...
def update_bin_on_delete(row, doctype):
	"""Update bin for deleted product (row)."""
	...
	if doctype == "Sales Order":
		qty_dict["reserved_qty"] = get_reserved_qty(row.product_code, row.warehouse)
	else:
		if row.material_request_product:
			qty_dict["indented_qty"] = get_indented_qty(row.product_code, row.warehouse)

		qty_dict["ordered_qty"] = get_ordered_qty(row.product_code, row.warehouse)

	if row.warehouse:
		update_bin_qty(row.product_code, row.warehouse, qty_dict)


def validate_and_delete_children(parent, data) -> bool:
	deleted_children = []
	updated_product_names = [d.get("docname") for d in data]
	for product in parent.products:
		if product.name not in updated_product_names:
			deleted_children.append(product)
...
@frappe.whitelist()
def update_child_qty_rate(parent_doctype, trans_products, parent_doctype_name, child_docname="products"):
	def check_doc_permissions(doc, perm_type="create"):
		try:
			...
			frappe.throw(
				_("You do not have permissions to {} products in a {}.").format(
	...
	def get_new_child_product(product_row):
		child_doctype = "Sales Order Product" if parent_doctype == "Sales Order" else "Purchase Order Product"
		return set_order_defaults(
			parent_doctype, parent_doctype_name, child_doctype, child_docname, product_row
		)

	def validate_quantity(child_product, new_data):
		if not flt(new_data.get("qty")):
			frappe.throw(
				_("Row # {0}: Quantity for Product {1} cannot be zero").format(
					new_data.get("idx"), frappe.bold(new_data.get("product_code"))
				),
				title=_("Invalid Qty"),
			)

		if parent_doctype == "Sales Order" and flt(new_data.get("qty")) < flt(child_product.delivered_qty):
			frappe.throw(_("Cannot set quantity less than delivered quantity"))

		if parent_doctype == "Purchase Order" and flt(new_data.get("qty")) < flt(
			child_product.received_qty
		):
			frappe.throw(_("Cannot set quantity less than received quantity"))

	def should_update_supplied_products(doc) -> bool:
		"""Subcontracted PO can allow following changes *after submit*:

		1. Change rate of subcontracting - regardless of other changes.
		2. Change qty and/or add new products and/or remove products
		        Exception: Transfer/Consumption is already made, qty change not allowed.
		"""

		supplied_products_processed = any(
			product.supplied_qty or product.consumed_qty or product.returned_qty for product in doc.supplied_products
		)

		update_supplied_products = (
			any_qty_changed or products_added_or_removed or any_conversion_factor_changed
		)
		if update_supplied_products and supplied_products_processed:
			frappe.throw(_("Product qty can not be updated as raw materials are already processed."))

		return update_supplied_products

	data = json.loads(trans_products)

	any_qty_changed = False  # updated to true if any product's qty changes
	products_added_or_removed = False  # updated to true if any new product is added or removed
	...
	_removed_products = validate_and_delete_children(parent, data)
	products_added_or_removed |= _removed_products
	...
	for d in data:
		...
		if not d.get("docname"):
			new_child_flag = True
			products_added_or_removed = True
			check_doc_permissions(parent, "create")
			child_product = get_new_child_product(d)
		else:
			check_doc_permissions(parent, "write")
			child_product = frappe.get_doc(parent_doctype + " Product", d.get("docname"))

			prev_rate, new_rate = flt(child_product.get("rate")), flt(d.get("rate"))
			prev_qty, new_qty = flt(child_product.get("qty")), flt(d.get("qty"))
			prev_con_fac, new_con_fac = flt(child_product.get("conversion_factor")), flt(
				d.get("conversion_factor")
			)
			prev_uom, new_uom = child_product.get("uom"), d.get("uom")

			if parent_doctype == "Sales Order":
				prev_date, new_date = child_product.get("delivery_date"), d.get("delivery_date")
			elif parent_doctype == "Purchase Order":
				prev_date, new_date = child_product.get("schedule_date"), d.get("schedule_date")
		...
		validate_quantity(child_product, d)
		if flt(child_product.get("qty")) != flt(d.get("qty")):
			any_qty_changed = True

		child_product.qty = flt(d.get("qty"))
		rate_precision = child_product.precision("rate") or 2
		conv_fac_precision = child_product.precision("conversion_factor") or 2
		qty_precision = child_product.precision("qty") or 2

		if flt(child_product.billed_amt, rate_precision) > flt(
			flt(d.get("rate"), rate_precision) * flt(d.get("qty"), qty_precision), rate_precision
		):
			frappe.throw(
				_("Row #{0}: Cannot set Rate if amount is greater than billed amount for Product {1}.").format(
					child_product.idx, child_product.product_code
				)
			)
		else:
			child_product.rate = flt(d.get("rate"), rate_precision)

		if d.get("conversion_factor"):
			if child_product.stock_uom == child_product.uom:
				child_product.conversion_factor = 1
			else:
				child_product.conversion_factor = flt(d.get("conversion_factor"), conv_fac_precision)

		if d.get("uom"):
			child_product.uom = d.get("uom")
			conversion_factor = flt(
				get_conversion_factor(child_product.product_code, child_product.uom).get("conversion_factor")
			)
			child_product.conversion_factor = (
				flt(d.get("conversion_factor"), conv_fac_precision) or conversion_factor
			)

		if d.get("delivery_date") and parent_doctype == "Sales Order":
			child_product.delivery_date = d.get("delivery_date")

		if d.get("schedule_date") and parent_doctype == "Purchase Order":
			child_product.schedule_date = d.get("schedule_date")

		if flt(child_product.price_list_rate):
			if flt(child_product.rate) > flt(child_product.price_list_rate):
				#  if rate is greater than price_list_rate, set margin
				#  or set discount
				child_product.discount_percentage = 0

				if parent_doctype in sales_doctypes:
					child_product.margin_type = "Amount"
					child_product.margin_rate_or_amount = flt(
						child_product.rate - child_product.price_list_rate, child_product.precision("margin_rate_or_amount")
					)
					child_product.rate_with_margin = child_product.rate
			else:
				child_product.discount_percentage = flt(
					(1 - flt(child_product.rate) / flt(child_product.price_list_rate)) * 100.0,
					child_product.precision("discount_percentage"),
				)
				child_product.discount_amount = flt(child_product.price_list_rate) - flt(child_product.rate)

				if parent_doctype in sales_doctypes:
					child_product.margin_type = ""
					child_product.margin_rate_or_amount = 0
					child_product.rate_with_margin = 0

		child_product.flags.ignore_validate_update_after_submit = True
		if new_child_flag:
			parent.load_from_db()
			child_product.idx = len(parent.products) + 1
			child_product.insert()
		else:
			child_product.save()
	...
	if parent_doctype == "Purchase Order":
		...
		if parent.is_old_subcontracting_flow:
			if should_update_supplied_products(parent):
