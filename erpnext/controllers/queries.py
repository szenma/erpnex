...
def product_query(doctype, txt, searchfield, start, page_len, filters, as_dict=False):
	...
	# Get searchfields from meta and use in Product Link field query
	...
	if "description" in searchfields:
		columns += """, if(length(tabProduct.description) > 40, \
			concat(substr(tabProduct.description, 1, 40), "..."), description) as description"""
	...
	if filters and isinstance(filters, dict):
		if filters.get("customer") or filters.get("supplier"):
			...
			product_rules_list = frappe.get_all(
				"Party Specific Product", filters={"party": party}, fields=["restrict_based_on", "based_on_value"]
			)

			filters_dict = {}
			for rule in product_rules_list:
				if rule["restrict_based_on"] == "Product":
	...
	if frappe.db.count(doctype, cache=True) < 50000:
		# scan description only if products are less than 50000
		description_cond = "or tabProduct.description LIKE %(txt)s"

	return frappe.db.sql(
		"""select
			tabProduct.name {columns}
		from tabProduct
		where tabProduct.docstatus < 2
			and tabProduct.disabled=0
			and tabProduct.has_variants=0
			and (tabProduct.end_of_life > %(today)s or ifnull(tabProduct.end_of_life, '0000-00-00')='0000-00-00')
			and ({scond} or tabProduct.product_code IN (select parent from `tabProduct Barcode` where barcode LIKE %(txt)s)
				{description_cond})
			{fcond} {mcond}
		order by
			if(locate(%(_txt)s, name), locate(%(_txt)s, name), 99999),
			if(locate(%(_txt)s, product_name), locate(%(_txt)s, product_name), 99999),
			idx desc,
			name, product_name
...
def bom(doctype, txt, searchfield, start, page_len, filters):
	doctype = "BOM"
	conditions = []
	fields = get_fields(doctype, ["name", "product"])
	...
	if args.get("warehouse"):
		...
		batch_nos = frappe.db.sql(
			...
			where
				...
				and sle.product_code = %(product_code)s
...
def get_account_list(doctype, txt, searchfield, start, page_len, filters):
	...
	if isinstance(filters, dict):
		for key, val in filters.products():
...
def get_blanket_orders(doctype, txt, searchfield, start, page_len, filters):
	return frappe.db.sql(
		...
		from `tabBlanket Order` bo, `tabBlanket Order Product` boi
		where
			boi.parent = bo.name
			and boi.product_code = {product_code}
			and bo.blanket_order_type = '{blanket_order_type}'
			and bo.company = {company}
			and bo.docstatus = 1""".format(
			product_code=frappe.db.escape(filters.get("product")),
...
def warehouse_query(doctype, txt, searchfield, start, page_len, filters):
	# Should be used when product code is passed in filters.
	doctype = "Warehouse"
...
def get_batch_numbers(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters and filters.get("product"):
		query += " and product = {product}".format(product=frappe.db.escape(filters.get("product")))
...
@frappe.whitelist()
@frappe.validate_and_sanitize_search_inputs
def product_manufacturer_query(doctype, txt, searchfield, start, page_len, filters):
	product_filters = [
		["manufacturer", "like", "%" + txt + "%"],
		["product_code", "=", filters.get("product_code")],
	]

	product_manufacturers = frappe.get_all(
...
def get_purchase_receipts(doctype, txt, searchfield, start, page_len, filters):
	query = """
		select pr.name
		from `tabPurchase Receipt` pr, `tabPurchase Receipt Product` prproduct
		where pr.docstatus = 1 and prproduct.parent = pr.name
		and pr.name like {txt}""".format(
		txt=frappe.db.escape("%{0}%".format(txt))
	)

	if filters and filters.get("product_code"):
		query += " and prproduct.product_code = {product_code}".format(
			product_code=frappe.db.escape(filters.get("product_code"))
		)
...
def get_purchase_invoices(doctype, txt, searchfield, start, page_len, filters):
	query = """
		select pi.name
		from `tabPurchase Invoice` pi, `tabPurchase Invoice Product` piproduct
		where pi.docstatus = 1 and piproduct.parent = pi.name
		and pi.name like {txt}""".format(
		txt=frappe.db.escape("%{0}%".format(txt))
	)

	if filters and filters.get("product_code"):
		query += " and piproduct.product_code = {product_code}".format(
			product_code=frappe.db.escape(filters.get("product_code"))
		)
...
def get_tax_template(doctype, txt, searchfield, start, page_len, filters):
	...
	product_doc = frappe.get_cached_doc("Product", filters.get("product_code"))
	product_group = filters.get("product_group")
	company = filters.get("company")
	taxes = product_doc.taxes or []

	while product_group:
		product_group_doc = frappe.get_cached_doc("Product Group", product_group)
		taxes += product_group_doc.taxes or []
		product_group = product_group_doc.parent_product_group

	if not taxes:
		return frappe.get_all(
			"Product Tax Template", filters={"disabled": 0, "company": company}, as_list=True
		)
	else:
		...
		taxes = _get_product_tax_template(args, taxes, for_validate=True)
