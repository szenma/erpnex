...
def get_list_for_transactions(
	...
	if txt:
		if meta.get_field("products"):
			if meta.get_field("products").options:
				child_doctype = meta.get_field("products").options
				for product in frappe.get_all(child_doctype, {"product_name": ["like", "%" + txt + "%"]}):
					child = frappe.get_doc(child_doctype, product.name)
...
def post_process(doctype, data):
	...
	for d in data:
		...
		doc.products_preview = ", ".join(d.product_name for d in doc.products if d.product_name)
		result.append(doc)
