...
frappe.ui.form.on("Warranty Claim", {
	setup: function(frm) {
		...
		frm.add_fetch('serial_no', 'product_name', 'product_name');
		...
		frm.add_fetch('product_code', 'description', 'description');
	},
	onload: function(frm) {
		if(!frm.doc.status) {
...
cur_frm.fields_dict['serial_no'].get_query = function(doc, cdt, cdn) {
	...
	if(doc.product_code) {
		cond = ['Serial No', 'product_code', '=', doc.product_code];
		filter.push(cond);
	}
	if(doc.customer) {
...
cur_frm.fields_dict['product_code'].get_query = function(doc, cdt, cdn) {
	if(doc.serial_no) {
		return{
			doctype: "Serial No",
			fields: "product_code",
