...
def get_leaderboards():
	leaderboards = {
		...
		"Product": {
			...
			"method": "erpnext.startup.leaderboard.get_all_products",
			"icon": "stock",
		},
		"Supplier": {
			"fields": [
...
def get_all_customers(date_range, company, field, limit=None):
	if field == "outstanding_amount":
		...
		if field == "total_sales_amount":
			select_field = "sum(so_product.base_net_amount)"
		elif field == "total_qty_sold":
			select_field = "sum(so_product.stock_qty)"
		...
		return frappe.db.sql(
			...
			FROM `tabSales Order` as so JOIN `tabSales Order Product` as so_product
				ON so.name = so_product.parent
...
@frappe.whitelist()
def get_all_products(date_range, company, field, limit=None):
	if field in ("available_stock_qty", "available_stock_value"):
		select_field = "sum(actual_qty)" if field == "available_stock_qty" else "sum(stock_value)"
		return frappe.db.get_all(
			"Bin",
			fields=["product_code as name", "{0} as value".format(select_field)],
		...
		if field == "total_sales_amount":
			select_field = "sum(order_product.base_net_amount)"
			select_doctype = "Sales Order"
		elif field == "total_purchase_amount":
			select_field = "sum(order_product.base_net_amount)"
			select_doctype = "Purchase Order"
		elif field == "total_qty_sold":
			select_field = "sum(order_product.stock_qty)"
			select_doctype = "Sales Order"
		...
		return frappe.db.sql(
			"""
			select order_product.product_code as name, {0} as value
			from `tab{1}` sales_order join `tab{1} Product` as order_product
				on sales_order.name = order_product.parent
			where sales_order.docstatus = 1
				and sales_order.company = %s {2}
			group by order_product.product_code
			order by value desc
			limit %s
		""".format(
...
def get_all_suppliers(date_range, company, field, limit=None):
	if field == "outstanding_amount":
		...
		if field == "total_purchase_amount":
			select_field = "sum(purchase_order_product.base_net_amount)"
		elif field == "total_qty_purchased":
			select_field = "sum(purchase_order_product.stock_qty)"
		...
		return frappe.db.sql(
			...
			FROM `tabPurchase Order` as purchase_order LEFT JOIN `tabPurchase Order Product`
				as purchase_order_product ON purchase_order.name = purchase_order_product.parent
			where
