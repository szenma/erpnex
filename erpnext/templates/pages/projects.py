...
def get_context(context):
	...
	project.tasks = get_tasks(
		project.name, start=0, product_status="open", search=frappe.form_dict.get("search")
	)
...
def get_tasks(project, start=0, search=None, product_status=None):
...
@frappe.whitelist()
def get_task_html(project, start=0, product_status=None):
	return frappe.render_template(
		"erpnext/templates/includes/projects/project_tasks.html",
		{
			"doc": {
				...
				"tasks": get_tasks(project, start, product_status=product_status),
			}
		},
		is_path=True,
	)
