...
from erpnext.e_commerce.redisearch_utils import (
	WEBSITE_PRODUCT_CATEGORY_AUTOCOMPLETE,
	WEBSITE_PRODUCT_INDEX,
	WEBSITE_PRODUCT_NAME_AUTOCOMPLETE,
	is_redisearch_enabled,
	make_key,
)
from erpnext.e_commerce.shopping_cart.product_info import set_product_info_for_website
from erpnext.setup.doctype.product_group.product_group import get_product_for_list_in_html
...
def get_product_list(search=None, start=0, limit=12):
	...
	for product in data:
		set_product_info_for_website(product)

	return [get_product_for_list_in_html(r) for r in data]


def get_product_data(search=None, start=0, limit=12):
	# limit = 12 because we show 12 products in the grid view
	# base query
	query = """
		SELECT
			web_product_name, product_name, product_code, brand, route,
			website_image, thumbnail, product_group,
			description, web_long_description as website_description,
			website_warehouse, ranking
		FROM `tabWebsite Product`
		WHERE published = 1
		"""

	# search term condition
	if search:
		query += """ and (product_name like %(search)s
				or web_product_name like %(search)s
...
def product_search(query, limit=10, fuzzy_search=True):
	...
	ac = AutoCompleter(make_key(WEBSITE_PRODUCT_NAME_AUTOCOMPLETE), conn=red)
	client = Client(make_key(WEBSITE_PRODUCT_INDEX), conn=red)
...
def get_category_suggestions(query):
	...
	ac = AutoCompleter(make_key(WEBSITE_PRODUCT_CATEGORY_AUTOCOMPLETE), conn=frappe.cache())
