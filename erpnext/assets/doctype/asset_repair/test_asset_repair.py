...
class TestAssetRepair(unittest.TestCase):
	...
	def setUpClass(cls):
		set_depreciation_settings_in_company()
		create_asset_data()
		create_product("_Test Stock Product")
	...
	def test_stock_product_total_value(self):
		asset_repair = create_asset_repair(stock_consumption=1)

		for product in asset_repair.stock_products:
			total_value = flt(product.valuation_rate) * flt(product.consumed_quantity)
			self.assertEqual(product.total_value, total_value)
	...
	def test_total_repair_cost(self):
		...
		for product in asset_repair.stock_products:
			total_repair_cost += product.total_value
	...
	def test_stock_products(self):
		...
		self.assertTrue(asset_repair.stock_products)
	...
	def test_decrease_stock_quantity(self):
		...
		self.assertEqual(stock_entry.products[0].s_warehouse, asset_repair.warehouse)
		self.assertEqual(stock_entry.products[0].product_code, asset_repair.stock_products[0].product_code)
		self.assertEqual(stock_entry.products[0].qty, asset_repair.stock_products[0].consumed_quantity)

	def test_serialized_product_consumption(self):
		...
		stock_entry = make_serialized_product()
		serial_nos = stock_entry.get("products")[0].serial_no
		serial_no = serial_nos.split("\n")[0]

		# should not raise any error
		create_asset_repair(
			stock_consumption=1,
			product_code=stock_entry.get("products")[0].product_code,
	...
	def test_increase_in_asset_value_due_to_stock_consumption(self):
		...
		self.assertEqual(asset_repair.stock_products[0].total_value, increase_in_asset_value)
	...
	def test_gl_entries_with_perpetual_inventory(self):
		...
		pi_expense_account = (
			frappe.get_doc("Purchase Invoice", asset_repair.purchase_invoice).products[0].expense_account
		)
		stock_entry_expense_account = (
			frappe.get_doc("Stock Entry", asset_repair.stock_entry).get("products")[0].expense_account
		)
...
def create_asset_repair(**args):
	...
	if args.stock_consumption:
		...
		asset_repair.append(
			"stock_products",
			{
				"product_code": args.product_code or "_Test Stock Product",
		...
		if args.stock_consumption:
			...
			stock_entry.append(
				"products",
				{
					...
					"product_code": asset_repair.stock_products[0].product_code,
					"qty": asset_repair.stock_products[0].consumed_quantity,
