...
class Asset(AccountsController):
	...
	def validate_product(self):
		product = frappe.get_cached_value(
			"Product", self.product_code, ["is_fixed_asset", "is_stock_product", "disabled"], as_dict=1
		)
		if not product:
			frappe.throw(_("Product {0} does not exist").format(self.product_code))
		elif product.disabled:
			frappe.throw(_("Product {0} has been disabled").format(self.product_code))
		elif not product.is_fixed_asset:
			frappe.throw(_("Product {0} must be a Fixed Asset Product").format(self.product_code))
		elif product.is_stock_product:
			frappe.throw(_("Product {0} must be a non-stock product").format(self.product_code))
	...
	def set_missing_values(self):
		if not self.asset_category:
			self.asset_category = frappe.get_cached_value("Product", self.product_code, "asset_category")

		if self.product_code and not self.get("finance_books"):
			finance_books = get_product_details(self.product_code, self.asset_category)
	...
	def validate_asset_values(self):
		...
		if is_cwip_accounting_enabled(self.asset_category):
			if not self.is_existing_asset and not (self.purchase_receipt or self.purchase_invoice):
				frappe.throw(
					_("Please create purchase receipt or purchase invoice for the product {0}").format(
						self.product_code
					)
				)
	...
	def cancel_movement_entries(self):
		movements = frappe.db.sql(
			...
			WHERE asm_product.parent=asm.name and asm_product.asset=%s and asm.docstatus=1""",
			self.name,
			as_dict=1,
		)
...
@frappe.whitelist()
def make_sales_invoice(asset, product_code, company, serial_no=None):
...
@frappe.whitelist()
def create_asset_maintenance(asset, product_code, product_name, asset_category, company):
...
@frappe.whitelist()
def get_product_details(product_code, asset_category):
