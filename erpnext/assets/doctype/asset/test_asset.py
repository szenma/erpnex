...
class AssetSetup(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		make_purchase_receipt(product_code="Macbook Pro", qty=1, rate=100000.0, location="Test Location")
...
class TestAsset(AssetSetup):
	def test_asset_category_is_fetched(self):
		"""Tests if the Product's Asset Category value is assigned to the Asset, if the field is empty."""

		asset = create_asset(product_code="Macbook Pro", do_not_save=1)
	...
	def test_available_for_use_date_is_after_purchase_date(self):
		asset = create_asset(product_code="Macbook Pro", calculate_depreciation=1, do_not_save=1)
		asset.is_existing_asset = 0
	...
	def test_product_exists(self):
		asset = create_asset(product_code="MacBook", do_not_save=1)

		self.assertRaises(frappe.ValidationError, asset.save)

	def test_validate_product(self):
		asset = create_asset(product_code="MacBook Pro", do_not_save=1)
		product = frappe.get_doc("Product", "MacBook Pro")
		...
		product.is_fixed_asset = 0
		self.assertRaises(frappe.ValidationError, asset.save)
		product.is_fixed_asset = 1
	...
	def test_purchase_asset(self):
		pr = make_purchase_receipt(
			product_code="Macbook Pro", qty=1, rate=100000.0, location="Test Location"
		)
	...
	def test_purchase_of_grouped_asset(self):
		create_fixed_asset_product("Rack", is_grouped_asset=1)
		pr = make_purchase_receipt(product_code="Rack", qty=3, rate=100000.0, location="Test Location")
	...
	def test_is_fixed_asset_set(self):
		...
		doc.append("products", {"product_code": "Macbook Pro", "qty": 1, "asset": asset.name})

		doc.set_missing_values()
		self.assertEqual(doc.products[0].is_fixed_asset, 1)
	...
	def test_gle_made_by_asset_sale(self):
		...
		si = make_sales_invoice(asset=asset.name, product_code="Macbook Pro", company="_Test Company")
		si.customer = "_Test Customer"
		si.due_date = nowdate()
		si.get("products")[0].rate = 25000
		si.insert()
		si.submit()
	...
	def test_gle_made_by_asset_sale_for_existing_asset(self):
		...
		si = create_sales_invoice(
			product_code="Macbook Pro", asset=asset.name, qty=1, rate=40000, posting_date=getdate("2023-05-23")
		)
		asset.load_from_db()
	...
	def test_expense_head(self):
		pr = make_purchase_receipt(
			product_code="Macbook Pro", qty=2, rate=200000.0, location="Test Location"
		)
		doc = make_invoice(pr.name)

		self.assertEqual("Asset Received But Not Billed - _TC", doc.products[0].expense_account)

	# CWIP: Capital Work In Progress
	def test_cwip_accounting(self):
		pr = make_purchase_receipt(
			product_code="Macbook Pro", qty=1, rate=5000, do_not_submit=True, location="Test Location"
		)
	...
	def test_asset_cwip_toggling_cases(self):
		...
		pi = make_purchase_invoice(
			product_code="Macbook Pro", qty=1, rate=200000.0, location="Test Location", update_stock=1
		)
		...
		pr = make_purchase_receipt(
			product_code="Macbook Pro", qty=1, rate=200000.0, location="Test Location"
		)
...
class TestDepreciationBasics(AssetSetup):
	...
	def test_get_depreciation_amount(self):
		...
		asset = create_asset(product_code="Macbook Pro", available_for_use_date="2019-12-31")
	...
	def test_check_is_pro_rata(self):
		...
		asset = create_asset(product_code="Macbook Pro", available_for_use_date="2019-12-31", do_not_save=1)
	...
	asset = frappe.get_doc(
		{
			...
			"product_code": args.product_code or "Macbook Pro",
...
def create_fixed_asset_product(product_code=None, auto_create_assets=1, is_grouped_asset=0):
	...
	try:
		product = frappe.get_doc(
			{
				"doctype": "Product",
				"product_code": product_code or "Macbook Pro",
