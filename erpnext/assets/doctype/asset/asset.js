...
frappe.ui.form.on('Asset', {
	...
	setup: function(frm) {
		...
		frm.set_query("purchase_receipt", (doc) => {
			return {
				...
				filters: { product_code: doc.product_code }
			}
		});
	...
	product_code: function(frm) {
		if(frm.doc.product_code && frm.doc.calculate_depreciation) {
	...
	set_finance_book: function(frm) {
		frappe.call({
			method: "erpnext.assets.doctype.asset.asset.get_product_details",
			args: {
		...
		if (frm.doc.purchase_receipt) {
			if (frm.doc.product_code) {
				...
				frappe.msgprint({
					...
					message: __("Please select Product Code first")
				});
			}
		}
	},
	...
	set_values_from_purchase_doc: function(frm, doctype, purchase_doc) {
		...
		const product = purchase_doc.products.find(product => product.product_code === frm.doc.product_code);
		if (!product) {
			doctype_field = frappe.scrub(doctype)
			frm.set_value(doctype_field, '');
			frappe.msgprint({
				title: __('Invalid {0}', [__(doctype)]),
				message: __('The selected {0} does not contain the selected Asset Product.', [__(doctype)]),
				indicator: 'red'
			});
		}
		frm.set_value('gross_purchase_amount', product.base_net_rate + product.product_tax_amount);
		frm.set_value('purchase_receipt_amount', product.base_net_rate + product.product_tax_amount);
		product.asset_location && frm.set_value('location', product.asset_location);
		frm.set_value('cost_center', product.cost_center || purchase_doc.cost_center);
	},
