...
class TestAssetCapitalization(unittest.TestCase):
	...
	def test_capitalization_with_perpetual_inventory(self):
		...
		asset_capitalization = create_asset_capitalization(
			...
			target_product_code="Macbook Pro",
		...
		self.assertEqual(asset_capitalization.stock_products[0].valuation_rate, stock_rate)
		self.assertEqual(asset_capitalization.stock_products[0].amount, stock_amount)
		self.assertEqual(asset_capitalization.stock_products_total, stock_amount)

		self.assertEqual(asset_capitalization.asset_products[0].asset_value, consumed_asset_value)
		self.assertEqual(asset_capitalization.asset_products_total, consumed_asset_value)

		self.assertEqual(asset_capitalization.service_products[0].amount, service_amount)
		self.assertEqual(asset_capitalization.service_products_total, service_amount)
		...
		expected_sle = {
			("Capitalization Source Stock Product", "_Test Warehouse - TCP1"): {
	...
	def test_capitalization_with_periodical_inventory(self):
		...
		# Test Stock Ledger Entries
		expected_sle = {
			("Capitalization Source Stock Product", "_Test Warehouse - _TC"): {
				"actual_qty": -stock_qty,
				"stock_value_difference": -stock_amount,
			}
		}
	...
	def test_decapitalization_with_depreciation(self):
		...
		asset_capitalization = create_asset_capitalization(
			...
			target_product_code="Capitalization Target Stock Product",
		...
		self.assertEqual(
			asset_capitalization.asset_products[0].current_asset_value, consumed_asset_current_value
		)
		self.assertEqual(
			asset_capitalization.asset_products[0].asset_value, consumed_asset_value_before_disposal
		)
		self.assertEqual(asset_capitalization.asset_products_total, consumed_asset_value_before_disposal)
...
def create_asset_capitalization_data():
	create_product(
		"Capitalization Target Stock Product", is_stock_product=1, is_fixed_asset=0, is_purchase_product=0
	)
	create_product(
		"Capitalization Source Stock Product", is_stock_product=1, is_fixed_asset=0, is_purchase_product=0
	)
	create_product(
		"Capitalization Source Service Product", is_stock_product=0, is_fixed_asset=0, is_purchase_product=0
	)
...
def create_asset_capitalization(**args):
	...
	target_product_code = target_asset.product_code or args.target_product_code
	...
	if flt(args.stock_rate):
		asset_capitalization.append(
			"stock_products",
			{
				"product_code": args.stock_product or "Capitalization Source Stock Product",
	...
	if flt(args.service_rate):
		asset_capitalization.append(
			"service_products",
			{
				"product_code": args.service_product or "Capitalization Source Service Product",
...
def create_stock_reconciliation(asset_capitalization, stock_rate=0):
	...
	if not asset_capitalization.get("stock_products"):
		return

	try:
		create_stock_reconciliation(
			product_code=asset_capitalization.stock_products[0].product_code,
			warehouse=asset_capitalization.stock_products[0].warehouse,
			qty=flt(asset_capitalization.stock_products[0].stock_qty),
			rate=flt(stock_rate),
...
def create_depreciation_asset(**args):
	...
	asset.product_code = args.product_code or "Macbook Pro"
	asset.asset_name = args.asset_name or asset.product_code
...
def get_actual_sle_dict(name):
	...
	sle_dict = {}
	for d in sles:
		sle_dict[(d.product_code, d.warehouse)] = {
