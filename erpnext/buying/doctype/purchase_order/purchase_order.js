...
frappe.ui.form.on("Purchase Order", {
	setup: function(frm) {
		...
		if (frm.doc.is_old_subcontracting_flow) {
			frm.set_query("reserve_warehouse", "supplied_products", function() {
				return {
					filters: {
		...
		frm.set_query("fg_product", "products", function() {
			return {
				filters: {
					'is_stock_product': 1,
	...
	get_materials_from_supplier: function(frm) {
		...
		if (frm.doc.supplied_products && (frm.doc.per_received == 100 || frm.doc.status === 'Closed')) {
			frm.doc.supplied_products.forEach(d => {
...
frappe.ui.form.on("Purchase Order Product", {
...
erpnext.buying.PurchaseOrderController = class PurchaseOrderController extends erpnext.buying.BuyingController {
	...
	refresh(doc, cdt, cdn) {
		...
		for (var i in cur_frm.doc.products) {
			var product = cur_frm.doc.products[i];
		...
		if(doc.docstatus == 1) {
			this.frm.fields_dict.products_section.wrapper.addClass("hide-border");
			if(!this.frm.doc.set_warehouse) {
				this.frm.fields_dict.products_section.wrapper.removeClass("hide-border");
			}

			if(!in_list(["Closed", "Delivered"], doc.status)) {
				if(this.frm.doc.status !== 'Closed' && flt(this.frm.doc.per_received) < 100 && flt(this.frm.doc.per_billed) < 100) {
					// Don't add Update Products button if the PO is following the new subcontracting flow.
					if (!(this.frm.doc.is_subcontracted && !this.frm.doc.is_old_subcontracting_flow)) {
						this.frm.add_custom_button(__('Update Products'), () => {
	...
	get_products_from_open_material_requests() {
	...
	has_unsupplied_products() {
		return this.frm.doc['supplied_products'].some(product => product.required_qty > product.supplied_qty);
	}
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(__('Material Request'),
			function() {
				erpnext.utils.map_current_doc({
					...
					allow_child_product_selection: true,
					child_fieldname: "products",
					child_columns: ["product_code", "qty"]
				})
		...
		function() {
			var my_products = [];
			for (var i in me.frm.doc.products) {
				if(!me.frm.doc.products[i].material_request){
					my_products.push(me.frm.doc.products[i].product_code);
				}
			}
			frappe.call({
				...
				callback: function(r) {
					...
					var i = 0;
					var product_length = me.frm.doc.products.length;
					while (i < product_length) {
						var qty = me.frm.doc.products[i].qty;
						(r.message[0] || []).forEach(function(d) {
							if (d.qty > 0 && qty > 0 && me.frm.doc.products[i].product_code == d.product_code && !me.frm.doc.products[i].material_request_product)
							{
								me.frm.doc.products[i].material_request = d.mr_name;
								me.frm.doc.products[i].material_request_product = d.mr_product;
								var my_qty = Math.min(qty, d.qty);
								qty = qty - my_qty;
								d.qty = d.qty  - my_qty;
								me.frm.doc.products[i].stock_qty = my_qty * me.frm.doc.products[i].conversion_factor;
								me.frm.doc.products[i].qty = my_qty;

								frappe.msgprint("Assigning " + d.mr_name + " to " + d.product_code + " (row " + me.frm.doc.products[i].idx + ")");
								if (qty > 0) {
									frappe.msgprint("Splitting " + qty + " units of " + d.product_code);
									var new_row = frappe.model.add_child(me.frm.doc, me.frm.doc.products[i].doctype, "products");
									product_length++;

									for (var key in me.frm.doc.products[i]) {
										new_row[key] = me.frm.doc.products[i][key];
									}

									new_row.idx = product_length;
									new_row["stock_qty"] = new_row.conversion_factor * qty;
									new_row["qty"] = qty;
									new_row["material_request"] = "";
									new_row["material_request_product"] = "";
								}
							}
						});
						i++;
...
cur_frm.fields_dict['products'].grid.get_field('project').get_query = function(doc, cdt, cdn) {
	return {
		filters:[
...
if (cur_frm.doc.is_old_subcontracting_flow) {
	cur_frm.fields_dict['products'].grid.get_field('bom').get_query = function(doc, cdt, cdn) {
		var d = locals[cdt][cdn]
		return {
			filters: [
				['BOM', 'product', '=', d.product_code],
				['BOM', 'is_active', '=', '1'],
				['BOM', 'docstatus', '=', '1'],
