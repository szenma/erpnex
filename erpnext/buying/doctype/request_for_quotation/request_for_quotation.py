...
class RequestforQuotation(BuyingController):
	...
	def get_link(self):
		...
		route = frappe.db.get_value(
			"Portal Menu Product", {"reference_doctype": "Request for Quotation"}, ["route"]
		)
	...
	def update_supplier_part_no(self, supplier):
		...
		for product in self.products:
			product.supplier_part_no = frappe.db.get_value(
				"Product Supplier", {"parent": product.product_code, "supplier": supplier}, "supplier_part_no"
			)
			...
			if sup_name == None or supplier.supplier == sup_name:
				...
				for product in self.products:
					sqi_count = frappe.db.sql(
						...
						{"supplier": supplier.supplier, "rqi": product.name},
						as_dict=1,
					)[0]
...
def check_portal_enabled(reference_doctype):
	if not frappe.db.get_value(
		"Portal Menu Product", {"reference_doctype": reference_doctype}, "enabled"
	):
		frappe.throw(
			_(
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			"Request for Quotation Product": {
				...
				"field_map": {"name": "request_for_quotation_product", "parent": "request_for_quotation"},
			},
		},
		target_doc,
		postprocess,
...
def create_supplier_quotation(doc):
	...
	try:
		...
		add_products(sq_doc, doc.get("supplier"), doc.get("products"))
...
def add_products(sq_doc, supplier, products):
	for data in products:
		if data.get("qty") > 0:
			if isinstance(data, dict):
				data = frappe._dict(data)

			create_rfq_products(sq_doc, supplier, data)


def create_rfq_products(sq_doc, supplier, data):
	args = {}
	...
	args.update(
		{
			"request_for_quotation_product": data.name,
			"request_for_quotation": data.parent,
			"supplier_part_no": frappe.db.get_value(
				"Product Supplier", {"parent": data.product_code, "supplier": supplier}, "supplier_part_no"
			),
		}
	)

	sq_doc.append("products", args)
...
@frappe.whitelist()
def get_product_from_material_requests_based_on_supplier(source_name, target_doc=None):
	mr_products_list = frappe.db.sql(
		"""
		SELECT
			mr.name, mr_product.product_code
		FROM
			`tabProduct` as product,
			`tabProduct Supplier` as product_supp,
			`tabMaterial Request Product` as mr_product,
			`tabMaterial Request`  as mr
		WHERE product_supp.supplier = %(supplier)s
			AND product.name = product_supp.parent
			AND mr_product.parent = mr.name
			AND mr_product.product_code = product.name
	...
	material_requests = {}
	for d in mr_products_list:
		material_requests.setdefault(d.name, []).append(d.product_code)

	for mr, products in material_requests.products():
		target_doc = get_mapped_doc(
			...
			mr,
			{
				...
				"Material Request Product": {
					...
					"condition": lambda row: row.product_code in products,
					"field_map": [
