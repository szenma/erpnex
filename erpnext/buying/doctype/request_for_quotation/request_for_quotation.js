...
frappe.ui.form.on("Request for Quotation",{
	...
	onload: function(frm) {
		if(!frm.doc.message_for_supplier) {
			frm.set_value("message_for_supplier", __("Please supply the specified products at the best possible rates"))
		}
	},
	...
	schedule_date(frm) {
		if(frm.doc.schedule_date){
			frm.doc.products.forEach((product) => {
				product.schedule_date = frm.doc.schedule_date;
			})
		}
...
frappe.ui.form.on("Request for Quotation Product", {
...
erpnext.buying.RequestforQuotationController = class RequestforQuotationController extends erpnext.buying.BuyingController {
	refresh() {
		...
		if (this.frm.doc.docstatus===0) {
			...
			// Get products from open Material Requests based on supplier
					...
					fields: [
						{
							...
							label:'Supplier',
							reqd:1,
							description: __("Get Products from Material Requests against this Supplier")
						}
					],
					primary_action_label: __("Get Products"),
					primary_action: (args) => {
						if(!args) return;
						dialog.hide();

						erpnext.utils.map_current_doc({
							method: "erpnext.buying.doctype.request_for_quotation.request_for_quotation.get_product_from_material_requests_based_on_supplier",
