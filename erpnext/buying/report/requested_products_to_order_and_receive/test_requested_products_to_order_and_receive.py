...
from erpnext.buying.report.requested_products_to_order_and_receive.requested_products_to_order_and_receive import (
	get_data,
)
...
class TestRequestedProductsToOrderAndReceive(FrappeTestCase):
	def setUp(self) -> None:
		create_product("Test MR Report Product")
		...
		self.filters = frappe._dict(
			...
			product_code="Test MR Report Product",
		)
	...
	def setup_material_request(self, order=False, receive=False, days=0):
		...
		for row in mr.products:
			row.product_code = "Test MR Report Product"
			row.product_name = "Test MR Report Product"
			row.description = "Test MR Report Product"
			row.uom = "Nos"
