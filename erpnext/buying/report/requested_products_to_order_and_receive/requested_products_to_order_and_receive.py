...
def get_data(filters):
	...
	query = (
		frappe.qb.from_(mr)
		.join(mr_product)
		.on(mr_product.parent == mr.name)
		.select(
			mr.name.as_("material_request"),
			mr.transaction_date.as_("date"),
			mr_product.schedule_date.as_("required_date"),
			mr_product.product_code.as_("product_code"),
			Sum(Coalesce(mr_product.stock_qty, 0)).as_("qty"),
			Coalesce(mr_product.stock_uom, "").as_("uom"),
			Sum(Coalesce(mr_product.ordered_qty, 0)).as_("ordered_qty"),
			Sum(Coalesce(mr_product.received_qty, 0)).as_("received_qty"),
			(Sum(Coalesce(mr_product.stock_qty, 0)) - Sum(Coalesce(mr_product.received_qty, 0))).as_(
				"qty_to_receive"
			),
			Sum(Coalesce(mr_product.received_qty, 0)).as_("received_qty"),
			(Sum(Coalesce(mr_product.stock_qty, 0)) - Sum(Coalesce(mr_product.ordered_qty, 0))).as_(
				"qty_to_order"
			),
			mr_product.product_name,
	...
	query = get_conditions(filters, query, mr, mr_product)  # add conditional conditions

	query = query.groupby(mr.name, mr_product.product_code).orderby(mr.transaction_date, mr.schedule_date)
	data = query.run(as_dict=True)
	return data


def get_conditions(filters, query, mr, mr_product):
	...
	if filters.get("product_code"):
		query = query.where(mr_product.product_code == filters.get("product_code"))
...
def prepare_data(data, filters):
	...
	material_request_map, product_qty_map = {}, {}

	for row in data:
		# product wise map for charts
		if not row["product_code"] in product_qty_map:
			product_qty_map[row["product_code"]] = {
				"qty": row["qty"],
			...
			product_entry = product_qty_map[row["product_code"]]
			update_qty_columns(product_entry, row)
	...
	chart_data = prepare_chart_data(product_qty_map)
...
def prepare_chart_data(product_data):
	labels, qty_to_order, ordered_qty, received_qty, qty_to_receive = [], [], [], [], []

	if len(product_data) > 30:
		product_data = dict(list(product_data.products())[:30])

	for row in product_data:
		mr_row = product_data[row]
		labels.append(row)
