...
def get_data(filters):
	...
	for po in purchase_order_entry:
		# fetch material records linked to the purchase order product
		mr_record = mr_records.get(po.material_request_product, [{}])[0]
		procurement_detail = {
...
def get_mapped_mr_details(filters):
	...
	child = frappe.qb.DocType("Material Request Product")
...
def get_mapped_pi_records():
	...
	pi_records = (
		...
		.inner_join(po)
		.on(pi_product.purchase_order == po.name)
		.select(pi_product.po_detail, pi_product.base_amount)
		.where(
			(pi_product.docstatus == 1)
			& (po.status.notin(("Closed", "Completed", "Cancelled")))
			& (pi_product.po_detail.isnotnull())
		)
	).run()
...
def get_mapped_pr_records():
	...
	pr_records = (
		frappe.qb.from_(pr)
		.from_(pr_product)
		.select(pr_product.purchase_order_product, pr.posting_date)
		.where(
			(pr.docstatus == 1)
			& (pr.name == pr_product.parent)
			& (pr_product.purchase_order_product.isnotnull())
...
def get_po_entries(filters):
	...
	child = frappe.qb.DocType("Purchase Order Product")
	...
	query = (
		...
		.select(
			...
			child.material_request_product,
			child.product_code,
			child.stock_uom,
			child.qty,
			child.amount,
		...
		.groupby(parent.name, child.product_code)
	)
	query = apply_filters_on_query(filters, parent, child, query)
