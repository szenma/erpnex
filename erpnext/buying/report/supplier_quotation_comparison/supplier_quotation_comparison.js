...
frappe.query_reports["Supplier Quotation Comparison"] = {
	filters: [
			...
			get_query: () => {
				...
				if (quote != "") {
					return {
						...
						filters: {
							"from": "Supplier Quotation Product",
							"parent": quote
						}
					}
				}
		...
		},
		{
			...
			"label": __("Group by"),
			"fieldtype": "Select",
			"options": [__("Group by Supplier"), __("Group by Product")],
	...
	make_default_supplier_dialog: (report) => {
		// Get the name of the product to change
		if(!report.data) return;

		let filters = report.get_values();
		let product_code = filters.product_code;
		...
		dialog.set_primary_action(__("Set Default Supplier"), () => {
			...
			if(values) {
				// Set the default_supplier field of the appropriate Product to the selected supplier
				frappe.call({
