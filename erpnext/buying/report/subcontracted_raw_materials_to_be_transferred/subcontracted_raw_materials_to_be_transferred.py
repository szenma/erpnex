...
def get_columns(filters):
	return [
		...
		{"label": _("Product Code"), "fieldtype": "Data", "fieldname": "rm_product_code", "width": 150},
...
def get_data(filters):
	order_rm_product_details = get_order_products_to_supply(filters)

	data = []
	for row in order_rm_product_details:
...
def get_order_products_to_supply(filters):
	...
	return frappe.db.get_all(
		...
		fields=[
			...
			f"`tab{supplied_products_table}`.rm_product_code as rm_product_code",
			f"`tab{supplied_products_table}`.required_qty as reqd_qty",
			f"`tab{supplied_products_table}`.supplied_qty as transferred_qty",
		],
