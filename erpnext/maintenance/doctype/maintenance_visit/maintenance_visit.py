...
class MaintenanceVisit(TransactionBase):
	...
	def validate_purpose_table(self):
		if not self.purposes:
			frappe.throw(_("Add Products in the Purpose Table"), title=_("Purposes Required"))

	def validate_maintenance_date(self):
		if self.maintenance_type == "Scheduled" and self.maintenance_schedule_detail:
			product_ref = frappe.db.get_value(
				"Maintenance Schedule Detail", self.maintenance_schedule_detail, "product_reference"
			)
			if product_ref:
				start_date, end_date = frappe.db.get_value(
					"Maintenance Schedule Product", product_ref, ["start_date", "end_date"]
				)
