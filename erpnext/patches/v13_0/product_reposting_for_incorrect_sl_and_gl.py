...
def execute():
	doctypes_to_reload = [
		("setup", "company"),
		("stock", "repost_product_valuation"),
		("stock", "stock_entry_detail"),
		("stock", "purchase_receipt_product"),
		("stock", "delivery_note_product"),
		("stock", "packed_product"),
		("accounts", "sales_invoice_product"),
		("accounts", "purchase_invoice_product"),
		("buying", "purchase_receipt_product_supplied"),
		("subcontracting", "subcontracting_receipt_product"),
		("subcontracting", "subcontracting_receipt_supplied_product"),
	]
	...
	data = frappe.db.sql(
		"""
		SELECT
			name, product_code, warehouse, voucher_type, voucher_no, posting_date, posting_time, company
		FROM
...
def get_creation_time():
	return frappe.db.sql(
		...
		INFORMATION_SCHEMA.TABLES where TABLE_NAME = "tabRepost Product Valuation" """,
		as_list=1,
	)[0][0]
