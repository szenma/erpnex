...
def execute():
	...
	for stock_entry_code in stock_entry_codes:
		...
		create_repost_product_valuation(patched_stock_entry)
...
def find_broken_stock_entries() -> List[StockEntryCode]:
	...
	stock_entries_to_patch = frappe.db.sql(
		"""
		select se.name, sum(sed.additional_cost) as product_additional_cost, se.total_additional_costs
		...
		group by
			sed.parent
		having
			product_additional_cost != se.total_additional_costs
	""",
		period_closing_date,
		as_dict=True,
	)
...
def create_repost_product_valuation(stock_entry):
