...
def execute():
	# This fix is not related to Party Specific Product,
	# but it is needed for code introduced after Party Specific Product was
	# If your DB doesn't have this doctype yet, you should be fine
	if not frappe.db.exists("DocType", "Party Specific Product"):
		return
	...
	for doctype in ("Purchase Invoice", "Sales Invoice"):
		...
		payment_schedule_products = frappe.get_all(
			"Payment Schedule",
			fields=("due_date", "payment_amount", "base_payment_amount", "parent"),
			filters={"parent": ("in", invoices_to_update)},
		)

		for product in payment_schedule_products:
			invoices_to_update[product.parent].setdefault("payment_schedule", []).append(product)
		...
		for status, docs in status_map.products():
