...
def execute():
	...
	fields = ["name", "production_product"]
	...
	stock_entries = frappe.db.sql_list(
		...
		WHERE
			...
			and not exists(
				select name from `tabStock Entry Detail` sed where sed.parent = se.name and sed.is_finished_product = 1
			)
		ORDER BY
...
def set_expense_account(doc):
	for row in doc.products:
		if row.is_finished_product and not row.expense_account:
...
def repost_stock_entry(doc):
	doc.db_update()
	for child_row in doc.products:
		if child_row.is_finished_product:
			child_row.db_update()

	sl_entries = []
	finished_product_row = doc.get_finished_product_row()
	get_sle_for_target_warehouse(doc, sl_entries, finished_product_row)
...
def get_sle_for_target_warehouse(doc, sl_entries, finished_product_row):
	for d in doc.get("products"):
		if cstr(d.t_warehouse) and finished_product_row and d.name == finished_product_row.name:
