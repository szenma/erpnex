...
def execute():
	...
	for data in frappe.get_all(
		"Product Quality Inspection Parameter", fields=["distinct parent"], filters={"parenttype": "Product"}
	):
		...
		frappe.db.set_value(
			"Product", data.parent, "quality_inspection_template", qc_doc.name, update_modified=False
		)
		frappe.db.sql(
			""" update `tabProduct Quality Inspection Parameter`
			set parentfield = 'product_quality_inspection_parameter', parenttype = 'Quality Inspection Template',
			parent = %s where parenttype = 'Product' and parent = %s""",
			(qc_doc.name, data.parent),
		)

	# update field in product variant settings
	frappe.db.sql(
