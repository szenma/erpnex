...
def execute():
	...
	for doctype in ["Sales Invoice", "Delivery Note"]:
		if not frappe.get_meta(doctype + " Product").get_field("target_warehouse").hidden:
			continue
		...
		data = frappe.db.sql(
			...
			FROM
				`tab{doctype}` parent_doc, `tab{doctype} Product` child_doc
			WHERE
		...
		if data:
			...
			frappe.db.sql(
				""" UPDATE `tab{0} Product` set target_warehouse = null
			...
			frappe.db.sql(
				""" UPDATE `tabPacked Product` set target_warehouse = null
	...
	if frappe.get_meta("Sales Order Product").get_field("target_warehouse").hidden:
		frappe.db.sql(
			""" UPDATE `tabSales Order Product` set target_warehouse = null
