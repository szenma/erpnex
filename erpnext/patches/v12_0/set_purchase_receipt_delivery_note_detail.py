...
def execute():
	...
	frappe.reload_doc("stock", "doctype", "delivery_note_product", force=True)
	frappe.reload_doc("stock", "doctype", "purchase_receipt_product", force=True)

	def map_rows(doc_row, return_doc_row, detail_field, doctype):
		"""Map rows after identifying similar ones."""

		frappe.db.sql(
			""" UPDATE `tab{doctype} Product` set {detail_field} = '{doc_row_name}'
	...
	def row_is_mappable(doc_row, return_doc_row, detail_field):
		...
		if doc_row.product_code == return_doc_row.product_code and not return_doc_row.get(detail_field):
			if (
	...
	def set_document_detail_in_return_document(doctype):
		...
		detail_field = "purchase_receipt_product" if doctype == "Purchase Receipt" else "dn_detail"

		child_doc = frappe.scrub("{0} Product".format(doctype))
		...
		for docname in return_document_map:
			doc_products = frappe.get_cached_doc(doctype, docname).get("products")
			for return_doc in return_document_map[docname]:
				return_doc_products = frappe.get_cached_doc(doctype, return_doc).get("products")

				# iterate through return document products and original document products for mapping
				for return_product in return_doc_products:
					for doc_product in doc_products:
						if (
							row_is_mappable(doc_product, return_product, detail_field) and doc_product.get("name") not in mapped
						):
							map_rows(doc_product, return_product, detail_field, doctype)
							mapped.append(doc_product.get("name"))
							break
						else:
							continue
