...
def add_gst_hsn_code_field():
	custom_fields = {
		"Stock Entry Detail": [
			dict(
				...
				label="HSN/SAC",
				fieldtype="Data",
				fetch_from="product_code.gst_hsn_code",
	...
	frappe.db.sql(
		""" update `tabStock Entry Detail`, `tabProduct`
		SET
			`tabStock Entry Detail`.gst_hsn_code = `tabProduct`.gst_hsn_code
		Where
			`tabProduct`.name = `tabStock Entry Detail`.product_code and `tabProduct`.gst_hsn_code is not null
	"""
	)
