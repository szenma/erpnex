...
def verify_transaction(**kwargs):
	...
	if transaction_response["ResultCode"] == 0:
		if integration_request.reference_doctype and integration_request.reference_docname:
			try:
				product_response = transaction_response["CallbackMetadata"]["Product"]
				amount = fetch_param_value(product_response, "Amount", "Name")
				mpesa_receipt = fetch_param_value(product_response, "MpesaReceiptNumber", "Name")
				pr = frappe.get_doc(
...
def get_completed_integration_requests_info(reference_doctype, reference_docname, checkout_id):
	...
	for out in output_of_other_completed_requests:
		...
		product_response = out["CallbackMetadata"]["Product"]
		completed_amount = fetch_param_value(product_response, "Amount", "Name")
		completed_mpesa_receipt = fetch_param_value(product_response, "MpesaReceiptNumber", "Name")
