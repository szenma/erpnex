...
class TallyMigration(Document):
	...
	def dump_processed_data(self, data):
		for key, value in data.products():
			f = frappe.get_doc(
				{
					"doctype": "File",
					"file_name": key + ".json",
	...
	def _process_master_data(self):
		...
		def get_stock_products_uoms(collection):
			uoms = []
			for uom in collection.find_all("UNIT"):
				uoms.append({"doctype": "UOM", "uom_name": uom.NAME.string.strip()})

			products = []
			for product in collection.find_all("STOCKPRODUCT"):
				stock_uom = product.BASEUNITS.string.strip() if product.BASEUNITS else self.default_uom
				products.append(
					{
						"doctype": "Product",
						"product_code": product.NAME.string.strip(),
						"stock_uom": stock_uom.strip(),
						"is_stock_product": 0,
						"product_group": "All Product Groups",
						"product_defaults": [{"company": self.erpnext_company}],
					}
				)
		...
		try:
			...
			self.publish("Process Master Data", _("Processing Products and UOMs"), 4, 5)
			products, uoms = get_stock_products_uoms(collection)
			data = {
	...
	def _import_master_data(self):
		...
		def create_products_uoms(products_file_url, uoms_file_url):
			...
			products_file = frappe.get_doc("File", {"file_url": products_file_url})
			for product in json.loads(products_file.get_content()):
				try:
					product_doc = frappe.get_doc(product)
					product_doc.insert()
				except Exception:
					self.log(product_doc)
		...
		try:
			...
			self.publish("Import Master Data", _("Importing Products and UOMs"), 3, 4)
			create_products_uoms(self.products, self.uoms)
	...
	def _process_day_book_data(self):
		...
		def voucher_to_invoice(voucher):
			...
			invoice = {
				...
				"products": get_voucher_products(voucher, doctype),
		...
		def get_voucher_products(voucher, doctype):
			inventory_entries = (
				voucher.find_all("INVENTORYENTRIES.LIST")
				+ voucher.find_all("ALLINVENTORYENTRIES.LIST")
				+ voucher.find_all("INVENTORYENTRIESIN.LIST")
			...
			for entry in inventory_entries:
				...
				products.append(
					{
						"product_code": entry.STOCKPRODUCTNAME.string.strip(),
						"description": entry.STOCKPRODUCTNAME.string.strip(),
						"qty": qty.strip(),
						"uom": uom.strip(),
						"conversion_factor": 1,
