...
class TaxJarSettings(Document):
	def on_update(self):
		...
		fields_already_exist = frappe.db.exists(
			"Custom Field",
			{"dt": ("in", ["Product", "Sales Invoice Product"]), "fieldname": "product_tax_category"},
		)
		fields_hidden = frappe.get_value(
			"Custom Field", {"dt": ("in", ["Sales Invoice Product"])}, "hidden"
		)
...
def toggle_tax_category_fields(hidden):
	frappe.set_value(
		"Custom Field",
		{"dt": "Sales Invoice Product", "fieldname": "product_tax_category"},
		"hidden",
		hidden,
	)
	frappe.set_value(
		"Custom Field", {"dt": "Product", "fieldname": "product_tax_category"}, "hidden", hidden
	)
	...
	custom_fields = {
		"Sales Invoice Product": [
			dict(
				...
				fetch_from="product_code.product_tax_category",
			),
			dict(
