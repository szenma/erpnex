...
class QuickBooksMigrator(Document):
	...
	def _migrate(self):
		try:
			...
			# Also add a company field to Customer Supplier and Product
			...
			# (When Product table is not present, This appens when Invoice is attached with a "StatementCharge" "ReimburseCharge
	...
	def _make_custom_fields(self):
		...
		doctypes_for_company_field = ["Customer", "Product", "Supplier"]
	...
	def _save_product(self, product):
		try:
			if not frappe.db.exists(
				{"doctype": "Product", "quickbooks_id": product["Id"], "company": self.company}
			):
				if product["Type"] in ("Service", "Inventory"):
					product_dict = {
						"doctype": "Product",
						"quickbooks_id": product["Id"],
						"product_code": encode_company_abbr(product["Name"], self.company),
						"stock_uom": "Unit",
						"is_stock_product": 0,
						"product_group": "All Product Groups",
						"company": self.company,
						"product_defaults": [{"company": self.company, "default_warehouse": self.default_warehouse}],
					}
					if "ExpenseAccountRef" in product:
						expense_account = self._get_account_name_by_id(product["ExpenseAccountRef"]["value"])
						product_dict["product_defaults"][0]["expense_account"] = expense_account
					if "IncomeAccountRef" in product:
						income_account = self._get_account_name_by_id(product["IncomeAccountRef"]["value"])
						product_dict["product_defaults"][0]["income_account"] = income_account
					frappe.get_doc(product_dict).insert()
		except Exception as e:
			self._log_error(e, product)
	...
	def _save_invoice(self, invoice):
		...
		# product list is populated from the corresponding transaction, these products are not shown in api response
		...
		try:
			if not frappe.db.exists(
				...
				invoice_dict = {
					...
					"products": self._get_si_products(invoice, is_return=is_return),
	...
	def _get_si_products(self, invoice, is_return=False):
		products = []
		for line in invoice["Line"]:
			if line["DetailType"] == "SalesProductLineDetail":
				if line["SalesProductLineDetail"]["TaxCodeRef"]["value"] != "TAX":
					tax_code = line["SalesProductLineDetail"]["TaxCodeRef"]["value"]
				else:
					if "TxnTaxCodeRef" in invoice["TxnTaxDetail"]:
						tax_code = invoice["TxnTaxDetail"]["TxnTaxCodeRef"]["value"]
					else:
						tax_code = "NON"
				if line["SalesProductLineDetail"]["ProductRef"]["value"] != "SHIPPING_PRODUCT_ID":
					product = frappe.db.get_all(
						"Product",
						filters={
							"quickbooks_id": line["SalesProductLineDetail"]["ProductRef"]["value"],
							"company": self.company,
						},
					...
					products.append(
						{
							...
							"description": line.get("Description", line["SalesProductLineDetail"]["ProductRef"]["name"]),
							"qty": line["SalesProductLineDetail"]["Qty"],
							"price_list_rate": line["SalesProductLineDetail"]["UnitPrice"],
							"cost_center": self.default_cost_center,
							"warehouse": self.default_warehouse,
							"product_tax_rate": json.dumps(self._get_product_taxes(tax_code)),
						}
					)
				else:
					products.append(
						{
							...
							"expense_account": self._get_account_name_by_id(
								"TaxRate - {}".format(line["SalesProductLineDetail"]["TaxCodeRef"]["value"])
							),
							"uom": "Unit",
	...
	def _get_product_taxes(self, tax_code):
				...
				if rate_list_type in tax_code:
					for tax_rate_detail in tax_code[rate_list_type]["TaxRateDetail"]:
						if tax_rate_detail["TaxTypeApplicable"] == "TaxOnAmount":
							...
							product_taxes[tax_head] = tax_rate["RateValue"]
		...
		try:
			if not frappe.db.exists(
				...
				invoice_dict = {
					...
					"products": self._get_pi_products(invoice, is_return=is_return),
					"taxes": self._get_taxes(invoice),
					"set_posting_time": 1,
					"disable_rounded_total": 1,
					"is_return": is_return,
	...
	def _get_pi_products(self, purchase_invoice, is_return=False):
		products = []
		for line in purchase_invoice["Line"]:
			if line["DetailType"] == "ProductBasedExpenseLineDetail":
				if line["ProductBasedExpenseLineDetail"]["TaxCodeRef"]["value"] != "TAX":
					tax_code = line["ProductBasedExpenseLineDetail"]["TaxCodeRef"]["value"]
				else:
				...
				product = frappe.db.get_all(
					"Product",
					filters={
						"quickbooks_id": line["ProductBasedExpenseLineDetail"]["ProductRef"]["value"],
						"company": self.company,
					},
					fields=["name", "stock_uom"],
				)[0]
				products.append(
					{
						...
						"description": line.get(
							"Description", line["ProductBasedExpenseLineDetail"]["ProductRef"]["name"]
						),
						"qty": line["ProductBasedExpenseLineDetail"]["Qty"],
						"price_list_rate": line["ProductBasedExpenseLineDetail"]["UnitPrice"],
						"warehouse": self.default_warehouse,
						"cost_center": self.default_cost_center,
						"product_tax_rate": json.dumps(self._get_product_taxes(tax_code)),
					}
	...
	def _save_purchase(self, purchase):
		try:
			...
			for line in purchase["Line"]:
				...
				elif line["DetailType"] == "ProductBasedExpenseLineDetail":
					account = (
						frappe.get_doc(
							"Product",
							{
