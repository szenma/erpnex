...
def get_tax_data(doc):
	...
	line_products = [get_line_product_dict(product, doc.docstatus) for product in doc.products]
...
def get_line_product_dict(product, docstatus):
	tax_dict = dict(
		id=product.get("idx"),
		quantity=product.get("qty"),
		unit_price=product.get("rate"),
		product_tax_code=product.get("product_tax_category"),
	)

	if docstatus == 1:
		tax_dict.update({"sales_tax": product.get("tax_collectable")})
...
def set_sales_tax(doc, method):
	...
	if tax_data is not None:
		if not tax_data.amount_to_collect:
			...
			# Assigning values to tax_collectable and taxable_amount fields in sales product table
			for product in tax_data.breakdown.line_products:
				doc.get("products")[cint(product.id) - 1].tax_collectable = product.tax_collectable
				doc.get("products")[cint(product.id) - 1].taxable_amount = product.taxable_amount
...
def check_for_nexus(doc, tax_dict):
	...
	if not frappe.db.get_value("TaxJar Nexus", {"region_code": tax_dict["to_state"]}):
		for product in doc.get("products"):
			product.tax_collectable = flt(0)
			product.taxable_amount = flt(0)
...
def sanitize_error_response(response):
	...
	for k, v in sanitized_responses.products():
