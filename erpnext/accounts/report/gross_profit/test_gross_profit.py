...
class TestGrossProfit(FrappeTestCase):
	...
	def create_product(self):
		product = create_product(
			product_code="_Test GP Product", is_stock_product=1, company=self.company, warehouse=self.warehouse
		)
		self.product = product if isinstance(product, str) else product.product_code

	def create_bundle(self):
		from erpnext.selling.doctype.product_bundle.test_product_bundle import make_product_bundle

		product2 = create_product(
			product_code="_Test GP Product 2", is_stock_product=1, company=self.company, warehouse=self.warehouse
		)
		self.product2 = product2 if isinstance(product2, str) else product2.product_code

		# This will be parent product
		bundle = create_product(
			product_code="_Test GP bundle", is_stock_product=0, company=self.company, warehouse=self.warehouse
		)
		self.bundle = bundle if isinstance(bundle, str) else bundle.product_code

		# Create Product Bundle
		self.product_bundle = make_product_bundle(parent=self.bundle, products=[self.product, self.product2])
	...
	def create_delivery_note(
		self, product=None, qty=1, rate=100, posting_date=nowdate(), do_not_save=False, do_not_submit=False
	):
		"""
	...
	def test_invoice_without_only_delivery_note(self):
		...
		product = se.products[0]
		se.append(
			"products",
			{
				"product_code": product.product_code,
				"s_warehouse": product.s_warehouse,
				"t_warehouse": product.t_warehouse,
				"qty": 1,
				"basic_rate": 200,
				"conversion_factor": product.conversion_factor or 1.0,
				"transfer_qty": flt(product.qty) * (flt(product.conversion_factor) or 1.0),
	...
	def test_bundled_delivery_note_with_different_warehouses(self):
		"""
		Test Delivery Note with bundled product. Packed Product from the bundle having different warehouses
		"""
		se = make_stock_entry(
			company=self.company,
			product_code=self.product,
		...
		# Packed Products will have different warehouses
		dnote = self.create_delivery_note(product=self.bundle, qty=1, rate=200, do_not_submit=True)
		dnote.packed_products[1].warehouse = self.finished_warehouse
		dnote = dnote.submit()
	...
	def test_crnote_against_invoice_with_multiple_instances_of_same_product(self):
		"""
		Product Qty for Sales Invoices with multiple instances of same product go in the -ve. Ideally, the credit noteshould cancel out the invoice products.
		"""
		from erpnext.accounts.doctype.sales_invoice.sales_invoice import make_sales_return

		# Invoice with an product added twice
		sinv = self.create_sales_invoice(qty=1, rate=100, posting_date=nowdate(), do_not_submit=True)
		sinv.append("products", frappe.copy_doc(sinv.products[0], ignore_no_copy=False))
		sinv = sinv.save().submit()
		...
		gp_entry = [x for x in data if x.parent_invoice == sinv.name]
		# Both products of Invoice should have '0' qty
