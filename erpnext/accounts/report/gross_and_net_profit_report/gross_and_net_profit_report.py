...
def get_revenue(data, period_list, include_in_gross=1):
	revenue = [
		product for product in data if product["include_in_gross"] == include_in_gross or product["is_group"] == 1
	]
	...
	for d in reversed(data):
		if d.get("is_group"):
			for period in period_list:
				...
				d[period.key] = sum(
					product[period.key] for product in data if product.get("parent_account") == d.get("account")
				)
		else:
...
def set_total(node, value, complete_list, totals):
	...
	if not parent == "":
		return set_total(
			next(product for product in complete_list if product["account"] == parent), value, complete_list, totals
		)
