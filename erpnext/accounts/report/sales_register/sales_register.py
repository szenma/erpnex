...
def get_columns(invoice_list, additional_table_columns):
	...
	if invoice_list:
		income_accounts = frappe.db.sql_list(
			...
			from `tabSales Invoice Product` where docstatus = 1 and parent in (%s)
...
def get_conditions(filters):
	...
	def get_sales_invoice_product_field_condition(field, table="Sales Invoice Product") -> str:
	...
	conditions += get_sales_invoice_product_field_condition("mode_of_payment", "Sales Invoice Payment")
	conditions += get_sales_invoice_product_field_condition("cost_center")
	conditions += get_sales_invoice_product_field_condition("warehouse")
	conditions += get_sales_invoice_product_field_condition("brand")
	conditions += get_sales_invoice_product_field_condition("product_group")

	if accounting_dimensions:
		common_condition = """
			and exists(select name from `tabSales Invoice Product`
				where parent=`tabSales Invoice`.name
			"""
...
def get_invoice_income_map(invoice_list):
	income_details = frappe.db.sql(
		...
		from `tabSales Invoice Product` where parent in (%s) group by parent, income_account"""
		% ", ".join(["%s"] * len(invoice_list)),
		tuple(inv.name for inv in invoice_list),
		as_dict=1,
	)
...
def get_invoice_so_dn_map(invoice_list):
	si_products = frappe.db.sql(
		...
		from `tabSales Invoice Product` where parent in (%s)
	...
	for d in si_products:
		...
		if d.delivery_note:
			...
			delivery_note_list = frappe.db.sql_list(
				"""select distinct parent from `tabDelivery Note Product`
