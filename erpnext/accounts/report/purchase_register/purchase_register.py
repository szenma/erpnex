...
def get_columns(invoice_list, additional_table_columns):
	...
	if invoice_list:
		expense_accounts = frappe.db.sql_list(
			...
			from `tabPurchase Invoice Product` where docstatus = 1
...
def get_conditions(filters):
	...
	if filters.get("cost_center"):
		conditions += """ and exists(select name from `tabPurchase Invoice Product`
			 where parent=`tabPurchase Invoice`.name
			 	and ifnull(`tabPurchase Invoice Product`.cost_center, '') = %(cost_center)s)"""

	if filters.get("warehouse"):
		conditions += """ and exists(select name from `tabPurchase Invoice Product`
			 where parent=`tabPurchase Invoice`.name
			 	and ifnull(`tabPurchase Invoice Product`.warehouse, '') = %(warehouse)s)"""

	if filters.get("product_group"):
		conditions += """ and exists(select name from `tabPurchase Invoice Product`
			 where parent=`tabPurchase Invoice`.name
			 	and ifnull(`tabPurchase Invoice Product`.product_group, '') = %(product_group)s)"""

	accounting_dimensions = get_accounting_dimensions(as_list=False)

	if accounting_dimensions:
		common_condition = """
			and exists(select name from `tabPurchase Invoice Product`
				where parent=`tabPurchase Invoice`.name
			"""
	...
	for d in pi_products:
		...
		if d.purchase_receipt:
			...
			pr_list = frappe.db.sql_list(
				"""select distinct parent from `tabPurchase Receipt Product`
				where docstatus=1 and purchase_order_product=%s""",
				d.po_detail,
			)
