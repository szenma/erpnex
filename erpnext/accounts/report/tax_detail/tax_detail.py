...
required_sql_fields = {
	...
	("Purchase Invoice Product", "Sales Invoice Product"): [
		"base_net_amount",
		"product_tax_rate",
...
def execute(filters=None):
	...
	gl_entries = frappe.db.sql(
		...
		left join `tabSales Invoice Product` sii on
		...
		left join `tabPurchase Invoice Product` pii on
...
def run_report(report_name, data):
	...
	new_data = []
	summary = []
	for section_name, section in sections.products():
		report[section_name] = {"rows": [], "subtotal": 0.0}
		for component_name, component in section.products():
			if component["type"] == "filter":
				for row in data:
					matched = True
					for colno, filter_string in component["filters"].products():
...
def get_fieldstr(fieldlist):
	fields = []
	for doctypes, docfields in fieldlist.products():
...
def modify_report_columns(doctype, field, column):
	...
	if doctype in ["Sales Invoice Product", "Purchase Invoice Product"]:
		if field in ["product_tax_rate", "base_net_amount"]:
			return None
	...
	for line in data:
		...
		if "Invoice" in line.voucher_type:
			if line.account_type not in ("Tax", "Round Off"):
				new_data += [line]
				if line.product_tax_rate:
					tax_rates = json.loads(line.product_tax_rate)
					for account, rate in tax_rates.products():
						tax_line = line.copy()
