...
erpnext.accounts.SalesInvoiceController = class SalesInvoiceController extends erpnext.selling.SellingController {
	...
	refresh(doc, dt, dn) {
		...
		if(doc.docstatus==1 && !doc.is_return) {
			...
			is_delivered_by_supplier = cur_frm.doc.products.some(function(product){
				return product.is_delivered_by_supplier ? true : false;
			})
			...
			if(cint(doc.update_stock)!=1) {
				...
				from_delivery_note = cur_frm.doc.products
					.some(function(product) {
						return product.delivery_note ? true : false;
					});
	...
	products_add(doc, cdt, cdn) {
		...
		this.frm.script_manager.copy_from_first_row("products", row, ["income_account", "discount_account", "cost_center"]);
	}
...
// --------------------------------
cur_frm.set_query("income_account", "products", function(doc) {
	return{
...
// -----------------------------
cur_frm.fields_dict["products"].grid.get_field("cost_center").get_query = function(doc) {
	return {
		filters: {
			'company': doc.company,
			"is_group": 0
...
cur_frm.cscript.income_account = function(doc, cdt, cdn) {
	erpnext.utils.copy_value_in_all_rows(doc, cdt, cdn, "products", "income_account");
}

cur_frm.cscript.expense_account = function(doc, cdt, cdn) {
	erpnext.utils.copy_value_in_all_rows(doc, cdt, cdn, "products", "expense_account");
}

cur_frm.cscript.cost_center = function(doc, cdt, cdn) {
	erpnext.utils.copy_value_in_all_rows(doc, cdt, cdn, "products", "cost_center");
}
...
cur_frm.set_query("asset", "products", function(doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	return {
		filters: [
			["Asset", "product_code", "=", d.product_code],
			["Asset", "docstatus", "=", 1],
...
frappe.ui.form.on('Sales Invoice', {
	setup: function(frm){
		...
		// discount account
		frm.fields_dict['products'].grid.get_field('discount_account').get_query = function(doc) {
			return {
				filters: {
					'report_type': 'Profit and Loss',
					'company': doc.company,
		...
		frm.fields_dict['products'].grid.get_field('deferred_revenue_account').get_query = function(doc) {
			return {
				filters: {
					'root_type': 'Liability',
					'company': doc.company,
	...
	update_stock: function(frm, dt, dn) {
		...
		frm.fields_dict.products.grid.toggle_reqd("product_code", frm.doc.update_stock);
