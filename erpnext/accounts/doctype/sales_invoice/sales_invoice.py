...
class SalesInvoice(SellingController):
	def __init__(self, *args, **kwargs):
		...
		self.status_updater = [
			{
				"source_dt": "Sales Invoice Product",
	...
	def validate(self):
		...
		self.validate_product_cost_centers()
		...
		if cint(self.update_stock):
			self.validate_dropship_product()
	...
	def validate_product_cost_centers(self):
		for product in self.products:
			cost_center_company = frappe.get_cached_value("Cost Center", product.cost_center, "company")
			if cost_center_company != self.company:
				frappe.throw(
					_("Row #{0}: Cost Center {1} does not belong to company {2}").format(
						frappe.bold(product.idx), frappe.bold(product.cost_center), frappe.bold(self.company)
					)
				)

	def validate_income_account(self):
		for product in self.get("products"):
			validate_account_head(product.idx, product.income_account, self.company, "Income")
			...
			self.status_updater.append(
				{
					...
					"keyword": "Delivered",
					"second_source_dt": "Delivery Note Product",
					"second_source_field": "qty",
					"second_join_field": "so_detail",
					"overflow_type": "delivery",
					"extra_cond": """ and exists(select name from `tabSales Invoice`
					where name=`tabSales Invoice Product`.parent and update_stock = 1)""",
				}
			)
				...
				self.status_updater.append(
					{
						...
						"second_join_field": "so_detail",
						"extra_cond": """ and exists (select name from `tabSales Invoice` where name=`tabSales Invoice Product`.parent and update_stock=1 and is_return=1)""",
					}
				)
	...
	def on_update_after_submit(self):
		if hasattr(self, "repost_required"):
			...
			# validate if deferred revenue is enabled for any product
			# Don't allow to update the invoice if deferred revenue is enabled
			for product in self.get("products"):
				if product.enable_deferred_revenue:
					frappe.throw(
						_(
							"Deferred Revenue is enabled for product {0}. You cannot update the invoice after submission."
	...
	def check_if_child_table_updated(
		...
		# Check if any field affecting accounting entry is altered
		for index, product in enumerate(self.get(child_table)):
			for field in fields_to_check:
				if doc_before_update.get(child_table)[index].get(field) != product.get(field):
					return True

			for dimension in accounting_dimensions:
				if doc_before_update.get(child_table)[index].get(dimension) != product.get(dimension):
					return True
	...
	def set_pos_fields(self, for_validate=False):
		...
		from erpnext.stock.get_product_details import get_pos_profile, get_pos_profile_product_details
		...
		if pos:
			...
			# set pos values in products
			for product in self.get("products"):
				if product.get("product_code"):
					profile_details = get_pos_profile_product_details(
						pos, frappe._dict(product.as_dict()), pos, update_data=True
					)
					for fname, val in profile_details.products():
						if (not for_validate) or (for_validate and not product.get(fname)):
							product.set(fname, val)
		...
		super(SalesInvoice, self).validate_with_previous_doc(
			{
				...
				"Sales Order Product": {
					...
					"compare_fields": [["product_code", "="], ["uom", "="], ["conversion_factor", "="]],
					"is_child_table": True,
	...
	def so_dn_required(self):
		...
		for key, value in prev_doc_field_map.products():
			if frappe.db.get_single_value("Selling Settings", value[0]) == "Yes":
				...
				for d in self.get("products"):
					if d.product_code and not d.get(key.lower().replace(" ", "_")) and not self.get(value[1]):
						msgprint(_("{0} is mandatory for Product {1}").format(key, d.product_code), raise_exception=1)
	...
	def validate_product_code(self):
		for d in self.get("products"):
			if not d.product_code and self.is_opening == "No":
				msgprint(_("Product Code required at Row No {0}").format(d.idx), raise_exception=True)

	def validate_warehouse(self):
		super(SalesInvoice, self).validate_warehouse()

		for d in self.get_product_list():
			if (
				not d.warehouse
				and d.product_code
				and frappe.get_cached_value("Product", d.product_code, "is_stock_product")
			):
				frappe.throw(_("Warehouse required for stock Product {0}").format(d.product_code))
	...
	def validate_dropship_product(self):
		for product in self.products:
			if product.sales_order:
				if frappe.db.get_value("Sales Order Product", product.so_detail, "delivered_by_supplier"):
					frappe.throw(_("Could not update stock, invoice contains drop shipping product."))

	def update_current_stock(self):
		for d in self.get("products"):
			if d.product_code and d.warehouse:
				bin = frappe.db.sql(
					"select actual_qty from `tabBin` where product_code = %s and warehouse = %s",
					(d.product_code, d.warehouse),
					as_dict=1,
				)
				d.actual_qty = bin and flt(bin[0]["actual_qty"]) or 0

		for d in self.get("packed_products"):
			bin = frappe.db.sql(
				"select actual_qty, projected_qty from `tabBin` where product_code =	%s and warehouse = %s",
				(d.product_code, d.warehouse),
				as_dict=1,
			)
			d.actual_qty = bin and flt(bin[0]["actual_qty"]) or 0
	...
	def make_product_gl_entries(self, gl_entries):
		...
		for product in self.get("products"):
			if flt(product.base_net_amount, product.precision("base_net_amount")):
				if product.is_fixed_asset:
					asset = self.get_asset(product)

					if self.is_return:
						fixed_asset_gl_entries = get_gl_entries_on_asset_regain(
							asset,
							product.base_net_amount,
							product.finance_book,
							self.get("doctype"),
							self.get("name"),
					...
					if not self.is_internal_transfer():
						...
						amount, base_amount = self.get_amount_and_base_amount(product, enable_discount_accounting)
							...
							self.get_gl_dict(
								{
									...
									"against": self.customer,
									"credit": flt(base_amount, product.precision("base_net_amount")),
									"credit_in_account_currency": (
										flt(base_amount, product.precision("base_net_amount"))
										if account_currency == self.company_currency
										else flt(amount, product.precision("net_amount"))
									),
	...
	def get_asset(self, product):
		if product.get("asset"):
			asset = frappe.get_doc("Asset", product.asset)
		else:
			frappe.throw(
				_("Row #{0}: You must select an Asset for Product {1}.").format(product.idx, product.product_name),
				title=_("Missing Asset"),
			)

		self.check_finance_books(product, asset)
		return asset
		...
		for d in self.get("products"):
			if d.dn_detail:
				billed_amt = frappe.db.sql(
					"""select sum(amount) from `tabSales Invoice Product`
					where dn_detail=%s and docstatus=1""",
					d.dn_detail,
				)
				billed_amt = billed_amt and billed_amt[0][0] or 0
				frappe.db.set_value(
					"Delivery Note Product", d.dn_detail, "billed_amt", billed_amt, update_modified=update_modified
				)
	...
	def update_serial_no(self, in_cancel=False):
		...
		for product in self.products:
			...
			for serial_no in get_serial_nos(product.serial_no):
				if serial_no and frappe.db.get_value("Serial No", serial_no, "product_code") == product.product_code:
	...
	def set_serial_no_against_delivery_note(self):
		for product in self.products:
			if product.serial_no and product.delivery_note and product.qty != len(get_serial_nos(product.serial_no)):
				product.serial_no = get_delivery_note_serial_no(product.product_code, product.qty, product.delivery_note)

	def validate_serial_against_delivery_note(self):
		"""
		validate if the serial numbers in Sales Invoice Products are same as in
		Delivery Note Product
		"""

		for product in self.products:
			if not product.delivery_note or not product.dn_detail:
				continue

			serial_nos = frappe.db.get_value("Delivery Note Product", product.dn_detail, "serial_no") or ""
			dn_serial_nos = set(get_serial_nos(serial_nos))

			serial_nos = product.serial_no or ""
			si_serial_nos = set(get_serial_nos(serial_nos))
			serial_no_diff = si_serial_nos - dn_serial_nos

			if serial_no_diff:
				dn_link = frappe.utils.get_link_to_form("Delivery Note", product.delivery_note)
			...
			if product.serial_no and cint(product.qty) != len(si_serial_nos):
				frappe.throw(
					_("Row #{0}: {1} Serial numbers required for Product {2}. You have provided {3}.").format(
						product.idx, product.qty, product.product_code, len(si_serial_nos)
					)
				)
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	if doctype in ["Sales Invoice", "Sales Order"]:
		...
		target_detail_field = "sales_invoice_product" if doctype == "Sales Invoice" else "sales_order_product"
		source_document_warehouse_field = "target_warehouse"
		target_document_warehouse_field = "from_warehouse"
		received_products = get_received_products(source_name, target_doctype, target_detail_field)
	else:
		source_doc = frappe.get_doc(doctype, source_name)
	...
	def update_product(source, target, source_parent):
		target.qty = flt(source.qty) - received_products.get(source.name, 0.0)
		if source.doctype == "Purchase Order Product" and target.doctype == "Sales Order Product":
			target.purchase_order = source.parent
			target.purchase_order_product = source.name
			target.material_request = source.material_request
			target.material_request_product = source.material_request_product

		if (
			source.get("purchase_order")
			and source.get("purchase_order_product")
			and target.doctype == "Purchase Invoice Product"
		):
			target.purchase_order = source.purchase_order
			target.po_detail = source.purchase_order_product

	product_field_map = {
		"doctype": target_doctype + " Product",
	...
	if doctype in ["Sales Invoice", "Sales Order"]:
		product_field_map["field_map"].update(
			{
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			doctype + " Product": product_field_map,
		},
		target_doc,
		set_missing_values,
	)
...
def get_received_products(reference_name, doctype, reference_fieldname):
	...
	received_products_map = frappe._dict(
		frappe.get_all(
			doctype + " Product",
...
def set_purchase_references(doc):
	...
	if doc.is_internal_transfer():
		if doc.doctype == "Purchase Receipt":
			so_product_map = get_delivery_note_details(doc.inter_company_invoice_reference)

			if so_product_map:
				pd_product_map, parent_child_map, warehouse_map = get_pd_details(
					"Purchase Order Product", so_product_map, "sales_order_product"
				)

				update_pr_products(doc, so_product_map, pd_product_map, parent_child_map, warehouse_map)

		elif doc.doctype == "Purchase Invoice":
			dn_product_map, so_product_map = get_sales_invoice_details(doc.inter_company_invoice_reference)
			# First check for Purchase receipt
			if list(dn_product_map.values()):
				pd_product_map, parent_child_map, warehouse_map = get_pd_details(
					"Purchase Receipt Product", dn_product_map, "delivery_note_product"
				)
...
def update_pi_products(
	...
	for product in doc.get("products"):
		product.set(detail_field, purchase_product_map.get(sales_product_map.get(product.sales_invoice_product)))
		product.set(parent_field, parent_child_map.get(sales_product_map.get(product.sales_invoice_product)))
		if doc.update_stock:
			product.warehouse = warehouse_map.get(sales_product_map.get(product.sales_invoice_product))
			if not product.warehouse and product.get("purchase_order") and product.get("purchase_order_product"):
				product.warehouse = frappe.db.get_value(
					"Purchase Order Product", product.purchase_order_product, "warehouse"
				)


def update_pr_products(doc, sales_product_map, purchase_product_map, parent_child_map, warehouse_map):
	for product in doc.get("products"):
		product.warehouse = warehouse_map.get(sales_product_map.get(product.delivery_note_product))
		if not product.warehouse and product.get("purchase_order") and product.get("purchase_order_product"):
			product.warehouse = frappe.db.get_value(
				"Purchase Order Product", product.purchase_order_product, "warehouse"
			)
...
def get_delivery_note_details(internal_reference):
	si_product_details = frappe.get_all(
		"Delivery Note Product", fields=["name", "so_detail"], filters={"parent": internal_reference}
	)

	return {d.name: d.so_detail for d in si_product_details if d.so_detail}
...
def get_sales_invoice_details(internal_reference):
	...
	for d in si_product_details:
		if d.dn_detail:
			dn_product_map.setdefault(d.name, d.dn_detail)
		if d.so_detail:
			so_product_map.setdefault(d.name, d.so_detail)

	return dn_product_map, so_product_map
...
def get_pd_details(doctype, sd_detail_map, sd_detail_field):
	...
	pd_product_details = frappe.get_all(
		doctype,
		fields=[sd_detail_field, "name", "warehouse", "parent"],
		filters={sd_detail_field: ("in", list(sd_detail_map.values()))},
	)

	for d in pd_product_details:
		pd_product_map.setdefault(d.get(sd_detail_field), d.name)
	...
	return pd_product_map, parent_child_map, accepted_warehouse_map
...
def update_address(doc, address_field, address_display_field, address_name):
	...
	for key, value in fetch_values.products():
		doc.set(key, value)
