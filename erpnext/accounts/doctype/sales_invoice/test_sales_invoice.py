...
from erpnext.controllers.taxes_and_totals import get_productised_tax_breakup_data
...
class TestSalesInvoice(unittest.TestCase):
	def setUp(self):
		from erpnext.stock.doctype.stock_ledger_entry.test_stock_ledger_entry import create_products

		create_products(["_Test Internal Transfer Product"], uoms=[{"uom": "Box", "conversion_factor": 10}])
	...
	def test_sales_invoice_calculation_base_currency(self):
		...
		expected_values = {
			...
			"_Test Product Home Desktop 100": [50, 0, 50, 500, 50, 50, 500],
			"_Test Product Home Desktop 200": [150, 0, 150, 750, 150, 150, 750],
		}

		# check if children are saved
		self.assertEqual(len(si.get("products")), len(expected_values) - 1)

		# check if product values are calculated
		for d in si.get("products"):
			for i, k in enumerate(expected_values["keys"]):
				self.assertEqual(d.get(k), expected_values[d.product_code][i])
	...
	def test_sales_invoice_calculation_export_currency(self):
		...
		si.get("products")[0].rate = 1
		si.get("products")[0].price_list_rate = 1
		si.get("products")[1].rate = 3
		si.get("products")[1].price_list_rate = 3
		...
		expected_values = {
			...
			"_Test Product Home Desktop 100": [1, 0, 1, 10, 50, 50, 500],
			"_Test Product Home Desktop 200": [3, 0, 3, 15, 150, 150, 750],
		}
	...
	def test_sales_invoice_with_discount_and_inclusive_tax(self):
		...
		# with inclusive tax
		self.assertEqual(si.products[0].net_amount, 3947.368421052631)
	...
	def test_sales_invoice_discount_amount(self):
		...
		# check if children are saved
		self.assertEqual(len(si.get("products")), len(expected_values))

		# check if product values are calculated
		for i, d in enumerate(si.get("products")):
			for k, v in expected_values[i].products():
	...
	def test_discount_amount_gl_entry(self):
		...
		expected_values = dict(
			...
			for d in [
				...
				[test_records[3]["products"][0]["income_account"], 0.0, 1163.45],
	...
	def test_tax_calculation_with_multiple_products(self):
		si = create_sales_invoice(qty=84, rate=4.6, do_not_save=True)
		product_row = si.get("products")[0]
		for qty in (54, 288, 144, 430):
			product_row_copy = copy.deepcopy(product_row)
			product_row_copy.qty = qty
			si.append("products", product_row_copy)
	...
	def test_tax_calculation_with_product_tax_template(self):
		si = create_sales_invoice(qty=84, rate=4.6, do_not_save=True)
		product_row = si.get("products")[0]
		...
		add_products = [
			...
			(430, "_Test Product Tax Template 1 - _TC"),
		]
		for qty, product_tax_template in add_products:
			product_row_copy = copy.deepcopy(product_row)
			product_row_copy.qty = qty
			product_row_copy.product_tax_template = product_tax_template
			si.append("products", product_row_copy)
	...
	def test_tax_calculation_with_multiple_products_and_discount(self):
		si = create_sales_invoice(qty=1, rate=75, do_not_save=True)
		product_row = si.get("products")[0]
		for rate in (500, 200, 100, 50, 50):
			product_row_copy = copy.deepcopy(product_row)
			product_row_copy.price_list_rate = rate
			product_row_copy.rate = rate
			si.append("products", product_row_copy)
	...
	def test_inclusive_rate_validations(self):
		...
		si.get("products")[0].price_list_rate = 62.5
		si.get("products")[0].price_list_rate = 191
		for i in range(6):
	...
	def test_sales_invoice_calculation_base_currency_with_tax_inclusive_price(self):
		...
		expected_values = {
			...
			"_Test Product Home Desktop 100": [
				62.5,
				0,
				62.5,
				625.0,
			...
			"_Test Product Home Desktop 200": [
				190.66,
				0,
				190.66,
				953.3,
	...
	def test_sales_invoice_calculation_export_currency_with_tax_inclusive_price(self):
		...
		si.currency = "USD"
		si.conversion_rate = 50
		si.get("products")[0].price_list_rate = 55.56
		si.get("products")[0].discount_percentage = 10
		si.get("products")[1].price_list_rate = 187.5
		si.get("products")[1].discount_percentage = 20
		...
		# check if product values are calculated
		for i, d in enumerate(si.get("products")):
			for key, val in expected_values[i].products():
	...
	def test_sales_invoice_gl_entry_without_perpetual_inventory(self):
		...
		expected_values = dict(
			...
			for d in [
				...
				[test_records[1]["products"][0]["income_account"], 0.0, 500.0],
	...
	def validate_pos_gl_entry(self, si, pos, cash_amount, validate_without_change_gle=False):
		...
		self.assertTrue(sle)
		self.assertEqual(
			[sle.product_code, sle.warehouse, sle.actual_qty], ["_Test FG Product", "Stores - TCP1", -1.0]
		)
		...
		expected_gl_entries = sorted(
			[
				...
				[pos.products[0].income_account, 0.0, 89.09],
				["Round Off - TCP1", 0.0, 0.01],
				[pos.taxes[0].account_head, 0.0, 10.69],
				[pos.taxes[1].account_head, 0.0, 0.21],
				[stock_in_hand, 0.0, abs(sle.stock_value_difference)],
				[pos.products[0].expense_account, abs(sle.stock_value_difference), 0.0],
	...
	def test_bin_details_of_packed_product(self):
		...
		packed_product = si.packed_products[0]
	...
	def test_sales_invoice_gl_entry_with_perpetual_inventory_no_product_code(self):
		si = create_sales_invoice(
			company="_Test Company with perpetual inventory",
			debit_to="Debtors - TCP1",
			income_account="Sales - TCP1",
		...
		si.get("products")[0].product_code = None
		si.insert()
		si.submit()
	...
	def test_sales_invoice_gl_entry_with_perpetual_inventory_non_stock_product(self):
		si = create_sales_invoice(product="_Test Non Stock Product")
		...
		expected_values = dict(
			...
			for d in [
				[si.debit_to, 100.0, 0.0],
				[test_records[1]["products"][0]["income_account"], 0.0, 100.0],
			]
		)
		for i, gle in enumerate(gl_entries):
			self.assertEqual(expected_values[gle.account][0], gle.account)
	...
	def test_serialized(self):
		...
		si = frappe.copy_doc(test_records[0])
		si.update_stock = 1
		si.get("products")[0].product_code = "_Test Serialized Product With Series"
		si.get("products")[0].qty = 1
		si.get("products")[0].serial_no = serial_nos[0]
		si.insert()
		si.submit()
	...
	def test_serialized_cancel(self):
		...
		serial_nos = get_serial_nos(si.get("products")[0].serial_no)
	...
	def test_serialize_status(self):
		...
		si.get("products")[0].product_code = "_Test Serialized Product With Series"
		si.get("products")[0].qty = 1
		si.get("products")[0].serial_no = serial_no.name
		si.insert()
	...
	def test_serial_numbers_against_delivery_note(self):
		"""
		check if the sales invoice product serial numbers and the delivery note products
		...
		dn = create_delivery_note(product=se.get("products")[0].product_code, serial_no=serial_nos[0])
		dn.submit()

		si = make_sales_invoice(dn.name)
		si.save()

		self.assertEqual(si.get("products")[0].serial_no, dn.get("products")[0].serial_no)

	def test_return_sales_invoice(self):
		make_stock_entry(product_code="_Test Product", target="Stores - TCP1", qty=50, basic_rate=100)

		actual_qty_0 = get_qty_after_transaction(product_code="_Test Product", warehouse="Stores - TCP1")
		...
		actual_qty_1 = get_qty_after_transaction(product_code="_Test Product", warehouse="Stores - TCP1")
		...
		actual_qty_2 = get_qty_after_transaction(product_code="_Test Product", warehouse="Stores - TCP1")
		...
		stock_in_hand_account = get_inventory_account(
			"_Test Company with perpetual inventory", si1.products[0].warehouse
		)
	...
	def test_gle_made_when_asset_is_returned(self):
		create_asset_data()
		asset = create_asset(product_code="Macbook Pro")

		si = create_sales_invoice(product_code="Macbook Pro", asset=asset.name, qty=1, rate=90000)
		return_si = create_sales_invoice(
			is_return=1,
			return_against=si.name,
			product_code="Macbook Pro",
			asset=asset.name,
			qty=-1,
			rate=90000,
		)
	...
	def test_discount_on_net_total(self):
		...
		expected_values = {
			...
			"_Test Product Home Desktop 100": [50, 0, 50, 500, 50, 50, 500, 25, 25, 250, 250],
			"_Test Product Home Desktop 200": [150, 0, 150, 750, 150, 150, 750, 75, 75, 375, 375],
		}
	...
	def test_create_so_with_margin(self):
		si = create_sales_invoice(product_code="_Test Product", qty=1, do_not_submit=True)
		price_list_rate = flt(100) * flt(si.plc_conversion_rate)
		si.products[0].price_list_rate = price_list_rate
		si.products[0].margin_type = "Percentage"
		si.products[0].margin_rate_or_amount = 25
		si.products[0].discount_amount = 0.0
		si.products[0].discount_percentage = 0.0
		si.save()
		self.assertEqual(si.get("products")[0].rate, flt((price_list_rate * 25) / 100 + price_list_rate))
	...
	def test_multiple_uom_in_selling(self):
		frappe.db.sql(
			"""delete from `tabProduct Price`
			where price_list='_Test Price List' and product_code='_Test Product'"""
		)
		product_price = frappe.new_doc("Product Price")
		product_price.price_list = "_Test Price List"
		product_price.product_code = "_Test Product"
		product_price.price_list_rate = 100
		product_price.insert()

		si = frappe.copy_doc(test_records[1])
		si.products[0].uom = "_Test UOM 1"
		si.products[0].conversion_factor = None
		si.products[0].price_list_rate = None
		si.save()
		...
		expected_values = {
			...
			"_Test Product": [1000, "_Test UOM", "_Test UOM 1", 10.0, 1000, 1000, 1000, 1000, 1000],
		}
	...
	def test_product_wise_tax_breakup(self):
		frappe.flags.country = "United States"

		si = self.create_si_to_test_tax_breakup()

		productised_tax, productised_taxable_amount = get_productised_tax_breakup_data(si)

		expected_productised_tax = {
			"_Test Product": {"Service Tax": {"tax_rate": 10.0, "tax_amount": 1000.0}},
			"_Test Product 2": {"Service Tax": {"tax_rate": 10.0, "tax_amount": 500.0}},
		}
		expected_productised_taxable_amount = {"_Test Product": 10000.0, "_Test Product 2": 5000.0}

		self.assertEqual(productised_tax, expected_productised_tax)
		self.assertEqual(productised_taxable_amount, expected_productised_taxable_amount)
	...
	def test_credit_note(self):
		...
		si = create_sales_invoice(product_code="_Test Product", qty=(5 * -1), rate=500, is_return=1)
	...
	def test_sales_invoice_with_project_link(self):
		...
		product_project = make_project(
			{
				"project_name": "Sales Invoice Product Project",
				"project_template_name": "Test Project Template",
		...
		sales_invoice.products[0].project = product_project.name
		...
		expected_values = {
			...
			"Sales - _TC": {"project": product_project.name},
		}
	...
	def test_deferred_revenue(self):
		...
		si = create_sales_invoice(product=product.name, posting_date="2019-01-10", do_not_submit=True)
		si.products[0].enable_deferred_revenue = 1
		si.products[0].service_start_date = "2019-01-10"
		si.products[0].service_end_date = "2019-03-15"
	...
	def test_fixed_deferred_revenue(self):
		...
		si = create_sales_invoice(
			product=product.name, posting_date="2019-01-16", rate=50000, do_not_submit=True
		)
		si.products[0].enable_deferred_revenue = 1
		si.products[0].service_start_date = "2019-01-16"
		si.products[0].service_end_date = "2019-03-31"
		si.products[0].deferred_revenue_account = deferred_account
		si.save()
		si.submit()
	...
	def test_inter_company_transaction_without_default_warehouse(self):
		...
		self.assertIsNone(target_doc.products[0].warehouse)
		self.assertEqual(target_doc.products[0].expense_account, "Stock Received But Not Billed - _TC1")

		target_doc.products[0].update({"cost_center": "Main - _TC1"})

		# missing warehouse is validated on save, after mapping
		self.assertRaises(WarehouseMissingError, target_doc.save)

		target_doc.products[0].update({"warehouse": "Stores - _TC1"})
		target_doc.save()

		# after warehouse is set, linked account or default inventory account is set
		self.assertEqual(target_doc.products[0].expense_account, "Stock In Hand - _TC1")
	...
	def test_sle_for_target_warehouse(self):
		...
		si.get("products")[0].warehouse = "Finished Goods - _TC"
		si.get("products")[0].target_warehouse = "Stores - _TC"
		si.insert()
		si.submit()
	...
	def test_internal_transfer_gl_entry(self):
		...
		si.selling_price_list = "_Test Price List Rest of the World"
		si.update_stock = 1
		si.products[0].target_warehouse = "Work In Progress - TCP1"
		...
		target_doc.products[0].warehouse = "Finished Goods - TCP1"
		...
		self.assertEqual(si.products[0].rate, rate)
		self.assertEqual(target_doc.products[0].rate, rate)

		check_gl_entries(self, target_doc.name, pi_gl_entries, add_days(nowdate(), -1))

	def test_internal_transfer_gl_precision_issues(self):
		# Make a stock queue of an product with two valuations

		# Remove all existing stock for this
		if get_stock_balance("_Test Internal Transfer Product", "Stores - TCP1", "2022-04-10"):
			create_stock_reconciliation(
				product_code="_Test Internal Transfer Product",
				warehouse="Stores - TCP1",
				qty=0,
				rate=0,
				company="_Test Company with perpetual inventory",
		...
		si.update_stock = 1
		si.products[0].target_warehouse = "Work In Progress - TCP1"
		si.products[0].conversion_factor = 10
		si.save()
		si.submit()
	...
	def test_product_tax_net_range(self):
		product = create_product("T Shirt")
		...
		sales_invoice = create_sales_invoice(product="T Shirt", rate=700, do_not_submit=True)
		self.assertEqual(
			sales_invoice.products[0].product_tax_template, "_Test Account Excise Duty @ 12 - _TC"
		)
		...
		sales_invoice.save()
		self.assertEqual(
			sales_invoice.products[0].product_tax_template, "_Test Account Excise Duty @ 10 - _TC"
		)
	...
	def test_additional_discount_for_sales_invoice_with_discount_accounting_enabled(self):
		...
		si.load_from_db()
		si.products[0].income_account = "Service - _TC"
	...
	def test_asset_depreciation_on_sale_with_pro_rata(self):
		...
		create_asset_data()
		asset = create_asset(product_code="Macbook Pro", calculate_depreciation=1, submit=1)
		post_depreciation_entries(getdate("2021-09-30"))

		create_sales_invoice(
			product_code="Macbook Pro", asset=asset.name, qty=1, rate=90000, posting_date=getdate("2021-09-30")
		)
		asset.load_from_db()
	...
	def test_asset_depreciation_on_sale_without_pro_rata(self):
		...
		create_sales_invoice(
			product_code="Macbook Pro", asset=asset.name, qty=1, rate=90000, posting_date=getdate("2021-12-31")
		)
		asset.load_from_db()
	...
	def test_sales_commission(self):
		...
		frappe.db.set_value("Product", si.get("products")[0].product_code, "grant_commission", 1)
		frappe.db.set_value("Product", si.get("products")[1].product_code, "grant_commission", 0)

		product = copy.deepcopy(si.get("products")[0])
		product.update(
			{
				"qty": 1,
				"rate": 500,
		...
		product = copy.deepcopy(si.get("products")[1])
		product.update(
			{
				"qty": 1,
				"rate": 500,
	...
	def test_over_billing_case_against_delivery_note(self):
		"""
		Test a case where duplicating the product with qty = 1 in the invoice
		...
		si = make_sales_invoice(dn.name)
		# make a copy of first product and add it to invoice
		product_copy = frappe.copy_doc(si.products[0])
		si.append("products", product_copy)
		si.save()
	...
	def test_multi_currency_deferred_revenue_via_journal_entry(self):
		...
		si.products[0].enable_deferred_revenue = 1
		si.products[0].service_start_date = "2019-01-01"
		si.products[0].service_end_date = "2019-03-30"
		si.products[0].deferred_expense_account = deferred_account
		si.save()
		si.submit()
		...
		gl_entries = frappe.db.sql(
			...
			(si.products[0].name, si.posting_date),
			as_dict=1,
		)
	...
	def test_standalone_serial_no_return(self):
		si = create_sales_invoice(
			product_code="_Test Serialized Product With Series", update_stock=True, is_return=True, qty=-1
		)
		si.reload()
		self.assertTrue(si.products[0].serial_no)
	...
	def test_batch_expiry_for_sales_invoice_return(self):
		...
		product = make_product(
			"_Test Batch Product For Return Check",
			{
		...
		pr = make_purchase_receipt(qty=1, product_code=product.name)

		batch_no = pr.products[0].batch_no
		si = create_sales_invoice(qty=1, product_code=product.name, update_stock=1, batch_no=batch_no)

		si.load_from_db()
		batch_no = si.products[0].batch_no
...
def create_sales_invoice(**args):
	...
	si.append(
		"products",
		{
			...
			"product_name": args.product_name or "_Test Product",
			"description": args.description or "_Test Product",
