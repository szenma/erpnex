...
frappe.ui.form.on('Pricing Rule', {
	setup: function(frm) {
		...
		['products', 'product_groups', 'brands'].forEach(d => {
			frm.fields_dict[d].grid.get_field('uom').get_query = function(doc, cdt, cdn){
				var row = locals[cdt][cdn];
				return {
					query:"erpnext.accounts.doctype.pricing_rule.pricing_rule.get_product_uoms",
		...
		var help_content =
			`<table class="table table-bordered" style="background-color: var(--scrollbar-track-color);">
				...
				<tr><td>
					...
					<ol>
						<li>
							{{__("Pricing Rule is first selected based on 'Apply On' field, which can be Product, Product Group or Brand.")}}
						</li>
						<li>
						...
						<li>
							...
							<ul>
								<li>
									{{__('Product Code > Product Group > Brand')}}
								</li>
								<li>
