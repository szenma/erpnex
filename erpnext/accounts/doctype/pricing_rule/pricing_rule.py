...
apply_on_dict = {"Product Code": "products", "Product Group": "product_groups", "Brand": "brands"}

other_fields = ["other_product_code", "other_product_group", "other_brand"]
...
class PricingRule(Document):
	...
	def validate_mandatory(self):
		for apply_on, field in apply_on_dict.products():
	...
	def validate_recursion(self):
		if self.price_or_product_discount != "Product":
			return
		if self.free_product or self.same_product:
	...
	def cleanup_fields_value(self):
		...
		if self.mixed_conditions and self.get("same_product"):
			self.same_product = 0
	...
	def validate_rate_or_discount(self):
		...
		if self.price_or_product_discount == "Product" and not self.free_product:
			if self.mixed_conditions:
				frappe.throw(_("Free product code is not selected"))
			else:
				self.same_product = 1

	def validate_max_discount(self):
		if self.rate_or_discount == "Discount Percentage" and self.get("products"):
			for d in self.products:
				max_discount = frappe.get_cached_value("Product", d.product_code, "max_discount")
				if max_discount and flt(self.discount_percentage) > flt(max_discount):
					throw(_("Max discount allowed for product: {0} is {1}%").format(d.product_code, max_discount))
...
def apply_pricing_rule(args, doc=None):
	...
	product_code_list = tuple(product.get("product_code") for product in product_list)
	query_products = frappe.get_all(
		"Product",
		fields=["product_code", "has_serial_no"],
		filters=[["product_code", "in", product_code_list]],
		as_list=1,
	)
	serialized_products = dict()
	for product_code, val in query_products:
		serialized_products.setdefault(product_code, val)

	for product in product_list:
		args_copy = copy.deepcopy(args)
		args_copy.update(product)
		data = get_pricing_rule_for_product(args_copy, doc=doc)
		out.append(data)

		if (
			serialized_products.get(product.get("product_code"))
			and not product.get("serial_no")
			and set_serial_nos_based_on_fifo
			and not args.get("is_return")
		):
			out[0].update(get_serial_no_for_product(args_copy))

	return out


def get_serial_no_for_product(args):
	from erpnext.stock.get_product_details import get_serial_no
	...
	if args.get("parenttype") in ("Sales Invoice", "Delivery Note") and flt(args.stock_qty) > 0:
		product_details.serial_no = get_serial_no(args)
	return product_details


def update_pricing_rule_uom(pricing_rule, args):
	child_doc = {"Product Code": "products", "Product Group": "product_groups", "Brand": "brands"}.get(
...
def get_pricing_rule_for_product(args, doc=None, for_validate=False):
	from erpnext.accounts.doctype.pricing_rule.utils import (
		get_applied_pricing_rules,
		get_pricing_rule_products,
		get_pricing_rules,
	...
	if args.get("is_free_product") or args.get("parenttype") == "Material Request":
		return {}
	...
	if args.ignore_pricing_rule or not args.product_code:
		if frappe.db.exists(args.doctype, args.name) and args.get("pricing_rules"):
			product_details = remove_pricing_rule_for_product(
	...
	if pricing_rules:
		...
		for pricing_rule in pricing_rules:
			...
			if isinstance(pricing_rule, str):
				...
				pricing_rule.apply_rule_on_other_products = get_pricing_rule_products(pricing_rule) or []

			if pricing_rule.get("suggestion"):
				continue

			product_details.validate_applied_rule = pricing_rule.get("validate_applied_rule", 0)
			product_details.price_or_product_discount = pricing_rule.get("price_or_product_discount")
			...
			if pricing_rule.mixed_conditions or pricing_rule.apply_rule_on_other:
				...
				if pricing_rule.apply_rule_on_other_products:
					product_details["apply_rule_on_other_products"] = json.dumps(pricing_rule.apply_rule_on_other_products)
			...
			if not pricing_rule.validate_applied_rule:
				if pricing_rule.price_or_product_discount == "Price":
					apply_price_discount_rule(pricing_rule, product_details, args)
				else:
					get_product_discount_rule(pricing_rule, product_details, args, doc)

		if not product_details.get("has_margin"):
			product_details.margin_type = None
			product_details.margin_rate_or_amount = 0.0

		product_details.has_pricing_rule = 1

		product_details.pricing_rules = frappe.as_json([d.pricing_rule for d in rules])
...
def update_args_for_pricing_rule(args):
	if not (args.product_group and args.brand):
		product = frappe.get_cached_value("Product", args.product_code, ("product_group", "brand"))
		if not product:
			return

		args.product_group, args.brand = product

		if not args.product_group:
			frappe.throw(_("Product Group not mentioned in product master for product {0}").format(args.product_code))
...
def apply_price_discount_rule(pricing_rule, product_details, args):
	product_details.pricing_rule_for = pricing_rule.rate_or_discount

	if (
		pricing_rule.margin_type in ["Amount", "Percentage"] and pricing_rule.currency == args.currency
	) or (pricing_rule.margin_type == "Percentage"):
		product_details.margin_type = pricing_rule.margin_type
		product_details.has_margin = True

		if pricing_rule.apply_multiple_pricing_rules and product_details.margin_rate_or_amount is not None:
			product_details.margin_rate_or_amount += pricing_rule.margin_rate_or_amount
		else:
			product_details.margin_rate_or_amount = pricing_rule.margin_rate_or_amount
	...
	if pricing_rule.rate_or_discount == "Rate":
		...
		if pricing_rule_rate:
			...
			# Override already set price list rate (from product price)
		...
		product_details.update({"discount_percentage": 0.0})
	...
	for apply_on in ["Discount Amount", "Discount Percentage"]:
		...
		if pricing_rule.apply_discount_on_rate and product_details.get("discount_percentage"):
			# Apply discount on discounted rate
			product_details[field] += (100 - product_details[field]) * (pricing_rule.get(field, 0) / 100)
		else:
			if field not in product_details:
				product_details.setdefault(field, 0)

			product_details[field] += pricing_rule.get(field, 0) if pricing_rule else args.get(field, 0)


def remove_pricing_rule_for_product(pricing_rules, product_details, product_code=None, rate=None):
	from erpnext.accounts.doctype.pricing_rule.utils import (
		get_applied_pricing_rules,
		get_pricing_rule_products,
	)
	...
	for d in get_applied_pricing_rules(pricing_rules):
		...
		if pricing_rule.price_or_product_discount == "Price":
			if pricing_rule.rate_or_discount == "Discount Percentage":
				product_details.discount_percentage = 0.0
				product_details.discount_amount = 0.0
				product_details.rate = rate or 0.0
			...
			if pricing_rule.margin_type in ["Percentage", "Amount"]:
				product_details.margin_rate_or_amount = 0.0
				product_details.margin_type = None
		elif pricing_rule.get("free_product"):
			product_details.remove_free_product = (
				product_code if pricing_rule.get("same_product") else pricing_rule.get("free_product")
			)

		if pricing_rule.get("mixed_conditions") or pricing_rule.get("apply_rule_on_other"):
			products = get_pricing_rule_products(pricing_rule, other_products=True)
			product_details.apply_on = (
				frappe.scrub(pricing_rule.apply_rule_on_other)
				if pricing_rule.apply_rule_on_other
				else frappe.scrub(pricing_rule.get("apply_on"))
			)
			product_details.applied_on_products = ",".join(products)

	product_details.pricing_rules = ""
	product_details.pricing_rule_removed = True

	return product_details


@frappe.whitelist()
def remove_pricing_rules(product_list):
	...
	for product in product_list:
		...
		if product.get("pricing_rules"):
			out.append(
				remove_pricing_rule_for_product(
					product.get("pricing_rules"), product, product.product_code, product.get("price_list_rate")
				)
			)
...
def get_product_uoms(doctype, txt, searchfield, start, page_len, filters):
	products = [filters.get("value")]
	if filters.get("apply_on") != "Product Code":
		field = frappe.scrub(filters.get("apply_on"))
		products = [d.name for d in frappe.db.get_all("Product", filters={field: filters.get("value")})]

	return frappe.get_all(
		"UOM Conversion Detail",
		filters={"parent": ("in", products), "uom": ("like", "{0}%".format(txt))},
		fields=["distinct uom"],
		as_list=1,
	)
