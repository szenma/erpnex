...
class Subscription(Document):
	...
	def create_invoice(self, prorate):
		...
		# Subscription is better suited for service products. I won't update `update_stock`
		# for that reason
		products_list = self.get_products_from_plans(self.plans, prorate)
		for product in products_list:
			product["cost_center"] = self.cost_center
			invoice.append("products", product)
	...
	def get_products_from_plans(self, plans, prorate=0):
		"""
		Returns the `Product`s linked to `Subscription Plan`
		"""
		if prorate:
		...
		for plan in plans:
			...
			deferred = frappe.db.get_value("Product", product_code, deferred_field)
			...
			for dimension in accounting_dimensions:
				if plan_doc.get(dimension):
					product.update({dimension: plan_doc.get(dimension)})
