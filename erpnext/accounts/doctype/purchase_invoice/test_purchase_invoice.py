...
test_dependencies = ["Product", "Cost Center", "Payment Term", "Payment Terms Template"]
...
class TestPurchaseInvoice(unittest.TestCase, StockTestMixin):
	...
	def test_purchase_invoice_received_qty(self):
		...
		pi.products[0].received_qty = 0
		pi.save()
		self.assertEqual(pi.products[0].received_qty, 2)
	...
	def test_gl_entries_for_non_stock_products_with_perpetual_inventory(self):
		pi = make_purchase_invoice(
			product_code="_Test Non Stock Product",
			company="_Test Company with perpetual inventory",
			warehouse="Stores - TCP1",
			cost_center="Main - TCP1",
			expense_account="_Test Account Cost for Goods Sold - TCP1",
	...
	def test_purchase_invoice_calculation(self):
		...
		expected_values = [
			["_Test Product Home Desktop 100", 90, 59],
			["_Test Product Home Desktop 200", 135, 177],
		]
		for i, product in enumerate(pi.get("products")):
			self.assertEqual(product.product_code, expected_values[i][0])
			self.assertEqual(product.product_tax_amount, expected_values[i][1])
			self.assertEqual(product.valuation_rate, expected_values[i][2])
	...
	def test_total_purchase_cost_for_project(self):
		...
		existing_purchase_cost = frappe.db.sql(
			...
			from `tabPurchase Invoice Product`
			where project = '{0}'
	...
	def test_standalone_return_using_pi(self):
		...
		make_stock_entry(product_code=product, target=warehouse, qty=50, rate=120)
	...
	def test_return_with_lcv(self):
		...
		# Create landed cost voucher - will increase valuation of received product by 10
	...
	def test_purchase_invoice_update_stock_gl_entry_with_perpetual_inventory(self):
		...
		self.assertTrue(gl_entries)
		stock_in_hand_account = get_inventory_account(pi.company, pi.get("products")[0].warehouse)
	...
	def test_auto_batch(self):
		product_code = frappe.db.get_value("Product", {"has_batch_no": 1, "create_new_batch": 1}, "name")
			...
			doc = frappe.get_doc(
				{
					"doctype": "Product",
					"is_stock_product": 1,
					"product_code": "test batch product",
		...
		self.assertTrue(frappe.db.get_value("Batch", {"product": product_code, "reference_name": pi.name}))
	...
	def test_rejected_serial_no(self):
		...
		self.assertEqual(
			frappe.db.get_value("Serial No", pi.get("products")[0].serial_no, "warehouse"),
			pi.get("products")[0].warehouse,
		)

		self.assertEqual(
			frappe.db.get_value("Serial No", pi.get("products")[0].rejected_serial_no, "warehouse"),
			pi.get("products")[0].rejected_warehouse,
		)
	...
	def test_debit_note(self):
		...
		pi = make_purchase_invoice(product_code="_Test Product", qty=(5 * -1), rate=500, is_return=1)
		pi.load_from_db()
	...
	def test_purchase_invoice_with_project_link(self):
		...
		product_project = make_project(
			{
				"project_name": "Purchase Invoice Product Project",
				"project_template_name": "Test Project Template",
		...
		pi.products[0].project = product_project.name
		...
		expected_values = {
			...
			"_Test Account Cost for Goods Sold - _TC": {"project": product_project.name},
		}
	...
	def test_deferred_expense_via_journal_entry(self):
		...
		product = create_product("_Test Product for Deferred Accounting", is_purchase_product=True)
		product.enable_deferred_expense = 1
		product.deferred_expense_account = deferred_account
		product.save()

		pi = make_purchase_invoice(product=product.name, qty=1, rate=100, do_not_save=True)
		pi.set_posting_time = 1
		pi.posting_date = "2019-01-10"
		pi.products[0].enable_deferred_expense = 1
		pi.products[0].service_start_date = "2019-01-10"
		pi.products[0].service_end_date = "2019-03-15"
		pi.products[0].deferred_expense_account = deferred_account
		pi.save()
		pi.submit()
		...
		gl_entries = gl_entries = frappe.db.sql(
			...
			(pi.products[0].name, pi.posting_date),
			as_dict=1,
		)
	...
	def test_purchase_invoice_advance_taxes(self):
		...
		po = create_purchase_order(
			...
			rate=3000,
			product="_Test Non Stock Product",
		...
		purchase_invoice.products[0].product_code = "_Test Non Stock Product"
		purchase_invoice.products[0].expense_account = "_Test Account Cost for Goods Sold - _TC"
	...
	def test_provisional_accounting_entry(self):
		create_product("_Test Non Stock Product", is_stock_product=0)
		...
		company.enable_provisional_accounting_for_non_stock_products = 1
		company.default_provisional_account = provisional_account
		company.save()

		pr = make_purchase_receipt(
			product_code="_Test Non Stock Product", posting_date=add_days(nowdate(), -2)
		)

		pi = create_purchase_invoice_from_receipt(pr.name)
		pi.set_posting_time = 1
		pi.posting_date = add_days(pr.posting_date, -1)
		pi.products[0].expense_account = "Cost of Goods Sold - _TC"
		pi.save()
		pi.submit()

		self.assertEquals(pr.products[0].provisional_expense_account, "Provision Account - _TC")
		...
		company.enable_provisional_accounting_for_non_stock_products = 0
		company.save()
	...
	def test_product_less_defaults(self):
		...
		pi.save()
		self.assertEqual(pi.products[0].conversion_factor, 1000)
	...
	def test_gl_entries_for_standalone_debit_note(self):
		...
		self.assertAlmostEqual(returned_inv.products[0].rate, rate)
