...
erpnext.accounts.PurchaseInvoice = class PurchaseInvoice extends erpnext.buying.BuyingController {
	setup(doc) {
		...
		// formatter for purchase invoice product
		if(this.frm.doc.update_stock) {
			this.frm.set_indicator_formatter('product_code', function(doc) {
	...
	products_add(doc, cdt, cdn) {
		...
		this.frm.script_manager.copy_from_first_row("products", row,
			["expense_account", "discount_account", "cost_center", "project"]);
	}

	on_submit() {
		$.each(this.frm.doc["products"] || [], function(i, row) {
...
function hide_fields(doc) {
	...
	var product_fields_stock = ['warehouse_section', 'received_qty', 'rejected_qty'];

	cur_frm.fields_dict['products'].grid.set_column_disp(product_fields_stock,
...
cur_frm.fields_dict['products'].grid.get_field("product_code").get_query = function(doc, cdt, cdn) {
	return {
...
cur_frm.set_query("expense_account", "products", function(doc) {
	return {
...
frappe.ui.form.on("Purchase Invoice", {
	setup: function(frm) {
		...
		frm.fields_dict['products'].grid.get_field('deferred_expense_account').get_query = function(doc) {
			return {
				filters: {
					'root_type': 'Asset',
					'company': doc.company,
	...
	update_stock: function(frm) {
		hide_fields(frm.doc);
		frm.fields_dict.products.grid.toggle_reqd("product_code", frm.doc.update_stock? true: false);
	},
