...
frappe.ui.form.on('Payment Entry', {
	...
	initialize_taxes: function(frm) {
		$.each(frm.doc["taxes"] || [], function(i, tax) {
			...
			let tax_fields = ["total",  "tax_fraction_for_current_product",
	...
	validate_inclusive_tax: function(tax) {
		let actual_type_error = function() {
			let msg = __("Actual type tax cannot be included in Product rate in row {0}", [tax.idx])
			frappe.throw(msg);
		};

		let on_previous_row_error = function(row_range) {
			let msg = __("For row {0} in {1}. To include {2} in Product rate, rows {3} must also be included",
	...
	determine_exclusive_rate: function(frm) {
		...
		$.each(frm.doc["taxes"] || [], function(i, tax) {
			tax.tax_fraction_for_current_product = frm.events.get_current_tax_fraction(frm, tax);
	...
	get_current_tax_fraction: function(frm, tax) {
		...
		if(cint(tax.included_in_paid_amount)) {
			...
			if(tax.charge_type == "On Paid Amount") {
				...
				current_tax_fraction = (tax_rate / 100.0) *
					frm.doc["taxes"][cint(tax.row_id) - 1].tax_fraction_for_current_product;
			} else if(tax.charge_type == "On Previous Row Total") {
				current_tax_fraction = (tax_rate / 100.0) *
					frm.doc["taxes"][cint(tax.row_id) - 1].grand_total_fraction_for_current_product;
			}
		}
