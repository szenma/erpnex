...
class PaymentEntry(AccountsController):
	...
	def set_missing_ref_details(
		...
		for d in self.get("references"):
			if d.allocated_amount:
				...
				for field, value in ref_details.products():
	...
	def validate_paid_invoices(self):
		...
		for reference_doctype, references in no_oustanding_refs.products():
			frappe.msgprint(
				_(
	...
	def update_payment_schedule(self, cancel=0):
		...
		for idx, (key, allocated_amount) in enumerate(invoice_payment_amount_map.products(), 1):
			if not invoice_paid_amount_map.get(key):
	...
	def initialize_taxes(self):
		for tax in self.get("taxes"):
			...
			tax_fields = ["total", "tax_fraction_for_current_product", "grand_total_fraction_for_current_product"]
	...
	def determine_exclusive_rate(self):
		...
		for i, tax in enumerate(self.get("taxes")):
			tax.tax_fraction_for_current_product = self.get_current_tax_fraction(tax)
			if i == 0:
				...
				tax.grand_total_fraction_for_current_product = (
					self.get("taxes")[i - 1].grand_total_fraction_for_current_product
...
def get_outstanding_reference_documents(args):
	...
	for fieldname, date_fields in date_fields_dict.products():
...
def split_invoices_based_on_payment_terms(outstanding_invoices):
	...
	if invoice_ref_based_on_payment_terms:
		for idx, ref in invoice_ref_based_on_payment_terms.products():
...
def get_payment_entry(
	...
	if dt in ["Purchase Order", "Sales Order", "Sales Invoice", "Purchase Invoice"]:
		pe.project = doc.get("project") or reduce(
			lambda prev, cur: prev or cur, [x.get("project") for x in doc.get("products")], None
		)  # get first non-empty project from products
...
def add_tax_discount_loss(pe, doc, total_discount_percentage) -> float:
	...
	for account, loss in tax_discount_loss.products():
