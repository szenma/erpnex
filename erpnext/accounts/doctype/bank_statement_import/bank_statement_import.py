...
def update_mapping_db(bank, template_options):
	...
	for d in json.loads(template_options)["column_to_field_map"].products():
...
def write_xlsx(data, sheet_name, wb=None, column_widths=None, file_path=None):
	...
	for row in data:
		clean_row = []
		for product in row:
			if isinstance(product, str) and (sheet_name not in ["Data Import Template", "Data Export"]):
				value = handle_html(product)
			else:
				value = product

			if isinstance(product, str) and next(ILLEGAL_CHARACTERS_RE.finditer(value), None):
