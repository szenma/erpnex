...
def validate_expense_against_budget(args, expense_amount=0):
	...
	if not (args.get("account") and args.get("cost_center")) and args.product_code:
		args.cost_center, args.account = get_product_details(args)
...
def get_requested_amount(args, budget):
	...
	data = frappe.db.sql(
		...
		from `tabMaterial Request Product` child, `tabMaterial Request` parent where parent.name = child.parent and
		child.product_code = %s and parent.docstatus = 1 and child.stock_qty > child.ordered_qty and {0} and
...
def get_ordered_amount(args, budget):
	...
	data = frappe.db.sql(
		...
		from `tabPurchase Order Product` child, `tabPurchase Order` parent where
		parent.name = child.parent and child.product_code = %s and parent.docstatus = 1 and child.amount > child.billed_amt
...
def get_product_details(args):
	...
	if args.product_code:
		product_defaults = frappe.db.get_value(
			"Product Default",
			{"parent": args.product_code, "company": args.get("company")},
			["buying_cost_center", "expense_account"],
		)
		if product_defaults:
			cost_center, expense_account = product_defaults

	if not (cost_center and expense_account):
		for doctype in ["Product Group", "Company"]:
