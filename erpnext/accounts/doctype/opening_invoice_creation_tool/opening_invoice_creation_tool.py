...
class OpeningInvoiceCreationTool(Document):
	...
	def set_missing_values(self, row):
		...
		row.product_name = row.product_name or _("Opening Invoice Product")
	...
	def get_invoice_dict(self, row=None):
		def get_product_dict():
			...
			product_dict = frappe._dict(
				{
					"uom": default_uom,
					"rate": rate or 0.0,
					"qty": row.qty,
					"conversion_factor": 1.0,
					"product_name": row.product_name or "Opening Invoice Product",
					"description": row.product_name or "Opening Invoice Product",
			...
			for dimension in get_accounting_dimensions():
				product_dict.update({dimension: row.get(dimension)})
		...
		for dimension in accounting_dimension:
			invoice.update({dimension: self.get(dimension) or product.get(dimension)})
