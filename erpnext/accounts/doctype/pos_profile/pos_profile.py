...
class POSProfile(Document):
	...
	def validate_all_link_fields(self):
		...
		for link_dt, dn_list in accounts.products():
	...
	def validate_duplicate_groups(self):
		product_groups = [d.product_group for d in self.product_groups]
		customer_groups = [d.customer_group for d in self.customer_groups]

		if len(product_groups) != len(set(product_groups)):
			frappe.throw(
				_("Duplicate product group found in the product group table"), title=_("Duplicate Product Group")
			)
...
def get_product_groups(pos_profile):
	product_groups = []
	pos_profile = frappe.get_cached_doc("POS Profile", pos_profile)

	if pos_profile.get("product_groups"):
		# Get products based on the product groups defined in the POS profile
		for data in pos_profile.get("product_groups"):
			product_groups.extend(
				["%s" % frappe.db.escape(d.name) for d in get_child_nodes("Product Group", data.product_group)]
			)

	return list(set(product_groups))
