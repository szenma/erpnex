...
class TestTaxWithholdingCategory(unittest.TestCase):
	...
	def test_single_threshold_tds(self):
		...
		for d in gl_entries:
			...
			elif d.account == pi.products[0].get("expense_account"):
			...
			[
				{
					...
					"product_code": frappe.db.get_value("Product", {"product_name": "TDS Product"}, "name"),
					"qty": 1,
					"rate": 20000,
					"cost_center": "Main - _TC",
...
def create_purchase_invoice(**args):
	...
	product = frappe.db.get_value("Product", {"product_name": "TDS Product"}, "name")
...
def create_sales_invoice(**args):
	# return sales invoice doc object
	product = frappe.db.get_value("Product", {"product_name": "TCS Product"}, "name")
...
def create_records():
	...
	# create product
	if not frappe.db.exists("Product", "TDS Product"):
		frappe.get_doc(
			{
				"doctype": "Product",
				"product_code": "TDS Product",
				"product_name": "TDS Product",
				"product_group": "All Product Groups",
				"is_stock_product": 0,
			}
		).insert()

	if not frappe.db.exists("Product", "TCS Product"):
		frappe.get_doc(
			{
				"doctype": "Product",
				"product_code": "TCS Product",
				"product_name": "TCS Product",
				"product_group": "All Product Groups",
				"is_stock_product": 1,
			}
		).insert()
