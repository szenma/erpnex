...
class POSInvoice(SalesInvoice):
	...
	def validate(self):
		...
		self.validate_serialised_or_batched_product()
		self.validate_stock_availablility()
		self.validate_return_products_qty()
		self.set_status()
	...
	def validate_pos_reserved_serial_nos(self, product):
		serial_nos = get_serial_nos(product.serial_no)
		filters = {"product_code": product.product_code, "warehouse": product.warehouse}
		if product.batch_no:
			filters["batch_no"] = product.batch_no
		...
		if len(invalid_serial_nos) == 1:
			frappe.throw(
				...
				).format(product.idx, bold_invalid_serial_nos),
				title=_("Product Unavailable"),
			)
	...
	def validate_pos_reserved_batch_qty(self, product):
		filters = {"product_code": product.product_code, "warehouse": product.warehouse, "batch_no": product.batch_no}

		available_batch_qty = get_batch_qty(product.batch_no, product.warehouse, product.product_code)
		reserved_batch_qty = get_pos_reserved_batch_qty(filters)

		bold_product_name = frappe.bold(product.product_name)
		bold_extra_batch_qty_needed = frappe.bold(
			abs(available_batch_qty - reserved_batch_qty - product.stock_qty)
		)
		bold_invalid_batch_no = frappe.bold(product.batch_no)

		if (available_batch_qty - reserved_batch_qty) == 0:
			frappe.throw(
				_(
					"Row #{}: Batch No. {} of product {} has no stock available. Please select valid batch no."
				).format(product.idx, bold_invalid_batch_no, bold_product_name),
				title=_("Product Unavailable"),
			)
		elif (available_batch_qty - reserved_batch_qty - product.stock_qty) < 0:
			frappe.throw(
				_(
					"Row #{}: Batch No. {} of product {} has less than required stock available, {} more required"
				).format(
					product.idx, bold_invalid_batch_no, bold_product_name, bold_extra_batch_qty_needed
				),
				title=_("Product Unavailable"),
			)

	def validate_delivered_serial_nos(self, product):
		delivered_serial_nos = get_delivered_serial_nos(product.serial_no)
		...
		if delivered_serial_nos:
			...
			frappe.throw(
				...
				).format(product.idx, bold_delivered_serial_nos),
				title=_("Product Unavailable"),
			)

	def validate_invalid_serial_nos(self, product):
		serial_nos = get_serial_nos(product.serial_no)
		error_msg = []
		invalid_serials, msg = "", ""
		...
		msg = _("Row #{}: Following Serial numbers for product {} are <b>Invalid</b>: {}").format(
			product.idx, frappe.bold(product.get("product_code")), frappe.bold(invalid_serials)
		)
		if invalid_serials:
			error_msg.append(msg)

		if error_msg:
			frappe.throw(error_msg, title=_("Invalid Product"), as_list=True)
	...
	def validate_stock_availablility(self):
		...
		for d in self.get("products"):
			if d.serial_no:
				...
				if is_negative_stock_allowed(product_code=d.product_code):
					return

				available_stock, is_stock_product = get_stock_availability(d.product_code, d.warehouse)

				product_code, warehouse, qty = (
					frappe.bold(d.product_code),
					frappe.bold(d.warehouse),
					frappe.bold(d.qty),
				)
				if is_stock_product and flt(available_stock) <= 0:
					frappe.throw(
						_("Row #{}: Product Code: {} is not available under warehouse {}.").format(
							d.idx, product_code, warehouse
						),
						title=_("Product Unavailable"),
					)
				elif is_stock_product and flt(available_stock) < flt(d.stock_qty):
					frappe.throw(
						_(
							"Row #{}: Stock quantity not enough for Product Code: {} under warehouse {}. Available quantity {}."
						).format(d.idx, product_code, warehouse, available_stock),
						title=_("Product Unavailable"),
					)

	def validate_serialised_or_batched_product(self):
		error_msg = []
		for d in self.get("products"):
			...
			msg = ""
			product_code = frappe.bold(d.product_code)
			serial_nos = get_serial_nos(d.serial_no)
			if serialized and batched and (no_batch_selected or no_serial_selected):
				msg = _(
					"Row #{}: Please select a serial no and batch against product: {} or remove it to complete transaction."
				).format(d.idx, product_code)
			elif serialized and no_serial_selected:
				msg = _(
					"Row #{}: No serial number selected against product: {}. Please select one or remove it to complete transaction."
				).format(d.idx, product_code)
			elif batched and no_batch_selected:
				msg = _(
					"Row #{}: No batch selected against product: {}. Please select a batch or remove it to complete transaction."
				).format(d.idx, product_code)
			elif serialized and not no_serial_selected and len(serial_nos) != d.qty:
				msg = _("Row #{}: You must select {} serial numbers for product {}.").format(
					d.idx, frappe.bold(cint(d.qty)), product_code
				)
	...
	def validate_return_products_qty(self):
		...
		for d in self.get("products"):
			if d.get("qty") > 0:
				frappe.throw(
					_(
						"Row #{}: You cannot add postive quantities in a return invoice. Please remove product {} to complete the return."
					).format(d.idx, frappe.bold(d.product_code)),
			...
			for product in self.get("products"):
				if product.get("product_code"):
					profile_details = get_pos_profile_product_details(
						profile.get("company"), frappe._dict(product.as_dict()), profile
					)
...
def get_stock_availability(product_code, warehouse):
	if frappe.db.get_value("Product", product_code, "is_stock_product"):
		is_stock_product = True
		bin_qty = get_bin_qty(product_code, warehouse)
		pos_sales_qty = get_pos_reserved_qty(product_code, warehouse)
		return bin_qty - pos_sales_qty, is_stock_product
	else:
		is_stock_product = True
		if frappe.db.exists("Product Bundle", product_code):
			return get_bundle_availability(product_code, warehouse), is_stock_product
		else:
			is_stock_product = False
			# Is a service product or non_stock product
			return 0, is_stock_product


def get_bundle_availability(bundle_product_code, warehouse):
	product_bundle = frappe.get_doc("Product Bundle", bundle_product_code)

	bundle_bin_qty = 1000000
	for product in product_bundle.products:
		product_bin_qty = get_bin_qty(product.product_code, warehouse)
		product_pos_reserved_qty = get_pos_reserved_qty(product.product_code, warehouse)
		available_qty = product_bin_qty - product_pos_reserved_qty

		max_available_bundles = available_qty / product.stock_qty
		if bundle_bin_qty > max_available_bundles and frappe.get_value(
			"Product", product.product_code, "is_stock_product"
		):
			bundle_bin_qty = max_available_bundles

	pos_sales_qty = get_pos_reserved_qty(bundle_product_code, warehouse)
	return bundle_bin_qty - pos_sales_qty


def get_bin_qty(product_code, warehouse):
...
def get_pos_reserved_qty(product_code, warehouse):
	p_inv = frappe.qb.DocType("POS Invoice")
	p_product = frappe.qb.DocType("POS Invoice Product")

	reserved_qty = (
		frappe.qb.from_(p_inv)
		.from_(p_product)
		.select(Sum(p_product.qty).as_("qty"))
		.where(
			(p_inv.name == p_product.parent)
			& (IfNull(p_inv.consolidated_invoice, "") == "")
			& (p_inv.is_return == 0)
			& (p_product.docstatus == 1)
			& (p_product.product_code == product_code)
			& (p_product.warehouse == warehouse)
		)
	).run(as_dict=True)
