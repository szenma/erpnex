...
class POSInvoiceMergeLog(Document):
	...
	def validate_duplicate_pos_invoices(self):
		...
		error_list = []
		for key, value in pos_occurences.products():
			if len(value) > 1:
				error_list.append(
			...
			for product in doc.get("products"):
				...
				for i in products:
					if (
						...
						and not i.serial_no
						and not i.batch_no
						and i.uom == product.uom
						and i.net_rate == product.net_rate
						and i.warehouse == product.warehouse
					):
						found = True
						i.qty = i.qty + product.qty
						i.amount = i.amount + product.net_amount
						i.net_amount = i.amount
						i.base_amount = i.base_amount + product.base_net_amount
						i.base_net_amount = i.base_amount

				if not found:
					product.rate = product.net_rate
					product.amount = product.net_amount
					product.base_amount = product.base_net_amount
					product.price_list_rate = 0
					si_product = map_child_doc(product, invoice, {"doctype": "Sales Invoice Product"})
					products.append(si_product)
			...
			for tax in doc.get("taxes"):
				...
				for t in taxes:
					if t.account_head == tax.account_head and t.cost_center == tax.cost_center:
						...
						update_product_wise_tax_detail(t, tax)
						found = True
				if not found:
					...
					tax.product_wise_tax_detail = tax.product_wise_tax_detail
					taxes.append(tax)
...
def update_product_wise_tax_detail(consolidate_tax_row, tax_row):
	consolidated_tax_detail = json.loads(consolidate_tax_row.product_wise_tax_detail)
	tax_row_detail = json.loads(tax_row.product_wise_tax_detail)

	if not consolidated_tax_detail:
		consolidated_tax_detail = {}

	for product_code, tax_data in tax_row_detail.products():
		if consolidated_tax_detail.get(product_code):
			consolidated_tax_data = consolidated_tax_detail.get(product_code)
			consolidated_tax_detail.update(
				{product_code: [consolidated_tax_data[0], consolidated_tax_data[1] + tax_data[1]]}
			)
		else:
			consolidated_tax_detail.update({product_code: [tax_data[0], tax_data[1]]})

	consolidate_tax_row.product_wise_tax_detail = json.dumps(
...
def split_invoices(invoices):
	...
	for pos_invoice in pos_return_docs:
		for product in pos_invoice.products:
...
def create_merge_logs(invoice_by_customer, closing_entry=None):
	try:
		for customer, invoices in invoice_by_customer.products():
