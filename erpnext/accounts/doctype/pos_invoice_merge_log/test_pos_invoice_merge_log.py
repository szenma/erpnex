...
class TestPOSInvoiceMergeLog(unittest.TestCase):
	...
	def test_consolidated_invoice_product_taxes(self):
		frappe.db.sql("delete from `tabPOS Invoice`")
		...
		try:
			...
			inv2.get("products")[0].product_code = "_Test Product 2"
			inv2.append(
				"taxes",
				{
					"account_head": "_Test Account VAT - _TC",
			...
			product_wise_tax_detail = json.loads(consolidated_invoice.get("taxes")[0].product_wise_tax_detail)

			tax_rate, amount = product_wise_tax_detail.get("_Test Product")
			self.assertEqual(tax_rate, 9)
			self.assertEqual(amount, 9)

			tax_rate2, amount2 = product_wise_tax_detail.get("_Test Product 2")
