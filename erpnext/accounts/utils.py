...
def reconcile_against_document(args, skip_ref_details_update_for_pe=False):  # nosemgrep
	...
	for key, entries in reconciled_entries.products():
		voucher_type = key[0]
		voucher_no = key[1]
...
def get_stock_rbnb_difference(posting_date, company):
	stock_products = frappe.db.sql_list(
	...
	pr_valuation_amount = frappe.db.sql(
		"""
		select sum(pr_product.valuation_rate * pr_product.qty * pr_product.conversion_factor)
		from `tabPurchase Receipt Product` pr_product, `tabPurchase Receipt` pr
		where pr.name = pr_product.parent and pr.docstatus=1 and pr.company=%s
		and pr.posting_date <= %s and pr_product.product_code in (%s)"""
		% ("%s", "%s", ", ".join(["%s"] * len(stock_products))),
		tuple([company, posting_date] + stock_products),
	)[0][0]

	pi_valuation_amount = frappe.db.sql(
		"""
		select sum(pi_product.valuation_rate * pi_product.qty * pi_product.conversion_factor)
		from `tabPurchase Invoice Product` pi_product, `tabPurchase Invoice` pi
		where pi.name = pi_product.parent and pi.docstatus=1 and pi.company=%s
		and pi.posting_date <= %s and pi_product.product_code in (%s)"""
		% ("%s", "%s", ", ".join(["%s"] * len(stock_products))),
		tuple([company, posting_date] + stock_products),
	)[0][0]
...
def update_gl_entries_after(
	...
	stock_vouchers = get_future_stock_vouchers(
		posting_date, posting_time, for_warehouses, for_products, company
	)
...
def repost_gle_for_stock_vouchers(
	...
	warehouse_account=None,
	repost_doc: Optional["RepostProductValuation"] = None,
):
...
def get_future_stock_vouchers(
	posting_date, posting_time, for_warehouses=None, for_products=None, company=None
):

	values = []
	condition = ""
	if for_products:
		condition += " and product_code in ({})".format(", ".join(["%s"] * len(for_products)))
		values += for_products
...
def get_stock_and_account_balance(account=None, posting_date=None, company=None):
	...
	related_warehouses = [
		wh
		for wh, wh_details in warehouse_account.products()
