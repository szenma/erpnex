...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	if sl_entries:
		...
		for sle in sl_entries:
			...
			if cancel:
				...
				if sle["actual_qty"] < 0 and not sle.get("outgoing_rate"):
					sle["outgoing_rate"] = get_incoming_outgoing_rate_for_cancel(
						sle.product_code, sle.voucher_type, sle.voucher_no, sle.voucher_detail_no
					)
			...
			is_stock_product = frappe.get_cached_value("Product", args.get("product_code"), "is_stock_product")
			if is_stock_product:
				bin_name = get_or_make_bin(args.get("product_code"), args.get("warehouse"))
				repost_current_voucher(args, allow_negative_stock, via_landed_cost_voucher)
				update_bin_qty(bin_name, args)
			else:
				frappe.msgprint(
					_("Product {0} ignored since it is not a stock product").format(args.get("product_code"))
				)
	...
	if args[0].get("is_cancelled"):
		...
		if repost_entry:
			if repost_entry.status == "In Progress":
				frappe.throw(
					_(
						"Cannot cancel the transaction. Reposting of product valuation on submission is not completed yet."
					)
				)
			if repost_entry.status == "Queued":
				doc = frappe.get_doc("Repost Product Valuation", repost_entry.name)
				doc.status = "Skipped"
...
def repost_future_sle(
	...
	products_to_be_repost = get_products_to_be_repost(
		voucher_type=voucher_type, voucher_no=voucher_no, doc=doc
	)
	if products_to_be_repost:
		args = products_to_be_repost

	distinct_product_warehouses = get_distinct_product_warehouse(args, doc)
	affected_transactions = get_affected_transactions(doc)

	i = get_current_index(doc) or 0
	while i < len(args):
		validate_product_warehouse(args[i])

		obj = update_entries_after(
			{
				"product_code": args[i].get("product_code"),
				"warehouse": args[i].get("warehouse"),
				"posting_date": args[i].get("posting_date"),
				"posting_time": args[i].get("posting_time"),
				"creation": args[i].get("creation"),
				"distinct_product_warehouses": distinct_product_warehouses,
			},
			allow_negative_stock=allow_negative_stock,
			via_landed_cost_voucher=via_landed_cost_voucher,
		)
		...
		distinct_product_warehouses[
			(args[i].get("product_code"), args[i].get("warehouse"))
		].reposting_status = True

		if obj.new_products_found:
			for product_wh, data in distinct_product_warehouses.products():
		...
		if doc:
			update_args_in_repost_product_valuation(
				doc, i, args, distinct_product_warehouses, affected_transactions
			)


def validate_product_warehouse(args):
	for field in ["product_code", "warehouse", "posting_date", "posting_time"]:
		if args.get(field) in [None, ""]:
			validation_msg = f"The field {frappe.unscrub(field)} is required for the reposting"
			frappe.throw(_(validation_msg))


def update_args_in_repost_product_valuation(
	doc, index, args, distinct_product_warehouses, affected_transactions
):
	doc.db_set(
		{
			"products_to_be_repost": json.dumps(args, default=str),
			"distinct_product_and_warehouse": json.dumps(
				{str(k): v for k, v in distinct_product_warehouses.products()}, default=str
			),
			"current_index": index,
	...
	frappe.publish_realtime(
		"product_reposting_progress",
		{"name": doc.name, "products_to_be_repost": json.dumps(args, default=str), "current_index": index},
	)


def get_products_to_be_repost(voucher_type=None, voucher_no=None, doc=None):
	products_to_be_repost = []
	if doc and doc.products_to_be_repost:
		products_to_be_repost = json.loads(doc.products_to_be_repost) or []

	if not products_to_be_repost and voucher_type and voucher_no:
		products_to_be_repost = frappe.db.get_all(
			"Stock Ledger Entry",
			filters={"voucher_type": voucher_type, "voucher_no": voucher_no},
			fields=["product_code", "warehouse", "posting_date", "posting_time", "creation"],
			order_by="creation asc",
			group_by="product_code, warehouse",
		)

	return products_to_be_repost or []


def get_distinct_product_warehouse(args=None, doc=None):
	distinct_product_warehouses = {}
	if doc and doc.distinct_product_and_warehouse:
		distinct_product_warehouses = json.loads(doc.distinct_product_and_warehouse)
		distinct_product_warehouses = {
			frappe.safe_eval(k): frappe._dict(v) for k, v in distinct_product_warehouses.products()
		}
	else:
		for i, d in enumerate(args):
			distinct_product_warehouses.setdefault(
				(d.product_code, d.warehouse), frappe._dict({"reposting_status": False, "sle": d, "args_idx": i})
			)

	return distinct_product_warehouses
...
class update_entries_after(object):
	...
	def __init__(
		...
		self.set_precision()
		self.valuation_method = get_valuation_method(self.product_code)

		self.new_products_found = False
		self.distinct_product_warehouses = args.get("distinct_product_warehouses", frappe._dict())
	...
	def initialize_previous_data(self, args):
		"""
		Get previous sl entries for current product for each related warehouse
	...
	def get_future_entries_to_fix(self):
		...
		args = self.data[self.args.warehouse].previous_sle or frappe._dict(
			{"product_code": self.product_code, "warehouse": self.args.warehouse}
		)
	...
	def get_dependent_entries_to_fix(self, entries_to_fix, sle):
		...
		if not dependant_sle:
			...
			dependant_sle.product_code == self.product_code and dependant_sle.warehouse == self.args.warehouse
		):
			return entries_to_fix
		elif dependant_sle.product_code != self.product_code:
			self.update_distinct_product_warehouses(dependant_sle)
			return entries_to_fix
		elif dependant_sle.product_code == self.product_code and dependant_sle.warehouse in self.data:
			return entries_to_fix
		else:
	...
	def update_distinct_product_warehouses(self, dependant_sle):
		key = (dependant_sle.product_code, dependant_sle.warehouse)
		val = frappe._dict({"sle": dependant_sle, "dependent_voucher_detail_nos": []})

		if key not in self.distinct_product_warehouses:
			self.distinct_product_warehouses[key] = val
			self.new_products_found = True
		else:
			existing_sle_posting_date = (
				self.distinct_product_warehouses[key].get("sle", {}).get("posting_date")
			)
	...
	def get_dependent_voucher_detail_nos(self, key):
		if "dependent_voucher_detail_nos" not in self.distinct_product_warehouses[key]:
			self.distinct_product_warehouses[key].dependent_voucher_detail_nos = []

		return self.distinct_product_warehouses[key].dependent_voucher_detail_nos
	...
	def process_sle(self, sle):
		...
		if (sle.serial_no and not self.via_landed_cost_voucher) or not cint(self.allow_negative_stock):
			# validate negative stock for serialized products, fifo valuation
	...
	def reset_actual_qty_for_stock_reco(self, sle):
		current_qty = frappe.get_cached_value(
			"Stock Reconciliation Product", sle.voucher_detail_no, "current_qty"
		)
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		# Sales and Purchase Return
			...
			if frappe.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				...
				product_code, incoming_rate = frappe.db.get_value(
					sle.voucher_type + " Product", sle.voucher_detail_no, ["product_code", rate_field]
				)

				if product_code == sle.product_code:
					rate = incoming_rate
				else:
					if sle.voucher_type in ("Delivery Note", "Sales Invoice"):
						ref_doctype = "Packed Product"
					elif sle == "Subcontracting Receipt":
						ref_doctype = "Subcontracting Receipt Supplied Product"
					else:
						ref_doctype = "Purchase Receipt Product Supplied"

					rate = frappe.db.get_value(
						ref_doctype,
						{"parent_detail_docname": sle.voucher_detail_no, "product_code": sle.product_code},
						rate_field,
					)
	...
	def update_outgoing_rate_on_transaction(self, sle):
		...
		In case of Stock Entry, also calculate FG Product rate and total incoming/outgoing amount
		"""
	...
	def update_rate_on_stock_entry(self, sle, outgoing_rate):
		...
		# Update outgoing product's rate, recalculate FG Product's rate and total incoming/outgoing amount
	...
	def recalculate_amounts_in_stock_entry(self, voucher_no):
		...
		for d in stock_entry.products:
			d.db_update()

	def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
		# Update product's incoming rate on transaction
		product_code = frappe.db.get_value(sle.voucher_type + " Product", sle.voucher_detail_no, "product_code")
		if product_code == sle.product_code:
			frappe.db.set_value(
				sle.voucher_type + " Product", sle.voucher_detail_no, "incoming_rate", outgoing_rate
			)
		else:
			# packed product
			frappe.db.set_value(
	...
	def update_rate_on_purchase_receipt(self, sle, outgoing_rate):
		if frappe.db.exists(sle.voucher_type + " Product", sle.voucher_detail_no):
			if sle.voucher_type in ["Purchase Receipt", "Purchase Invoice"] and frappe.get_cached_value(
				...
				frappe.db.set_value(
					f"{sle.voucher_type} Product", sle.voucher_detail_no, "valuation_rate", sle.outgoing_rate
				)
		else:
			frappe.db.set_value(
				"Purchase Receipt Product Supplied", sle.voucher_detail_no, "rate", outgoing_rate
			)

		# Recalculate subcontracted product's rate in case of subcontracted purchase receipt/invoice
		if frappe.get_cached_value(sle.voucher_type, sle.voucher_no, "is_subcontracted"):
			doc = frappe.get_doc(sle.voucher_type, sle.voucher_no)
			doc.update_valuation_rate(reset_outgoing_rate=False)
			for d in doc.products + doc.supplied_products:
				d.db_update()

	def update_rate_on_subcontracting_receipt(self, sle, outgoing_rate):
		if frappe.db.exists("Subcontracting Receipt Product", sle.voucher_detail_no):
			frappe.db.set_value("Subcontracting Receipt Product", sle.voucher_detail_no, "rate", outgoing_rate)
		else:
			frappe.db.set_value(
				"Subcontracting Receipt Supplied Product",
		...
		scr.calculate_products_qty_and_amount()
		scr.db_update()
		for d in scr.products:
			d.db_update()
	...
	def update_rate_on_stock_reconciliation(self, sle):
		if not sle.serial_no and not sle.batch_no:
			...
			for product in sr.products:
				# Skip for Serial and Batch Products
				if product.name != sle.voucher_detail_no or product.serial_no or product.batch_no:
					continue
				...
				product.current_qty = previous_sle.get("qty_after_transaction") or 0.0
				product.current_valuation_rate = previous_sle.get("valuation_rate") or 0.0
				product.current_amount = flt(product.current_qty) * flt(product.current_valuation_rate)

				product.amount = flt(product.qty) * flt(product.valuation_rate)
				product.quantity_difference = product.qty - product.current_qty
				product.amount_difference = product.amount - product.current_amount
			else:
				sr.difference_amount = sum([product.amount_difference for product in sr.products])
			sr.db_update()
	...
	def get_moving_average_values(self, sle):
		...
		if new_stock_qty >= 0:
			...
			# Get valuation rate from previous SLE or Product master, if product does not have the
	...
	def check_if_allow_zero_valuation_rate(self, voucher_type, voucher_detail_no):
		...
		if voucher_type == "Stock Entry":
			ref_product_dt = voucher_type + " Detail"
		elif voucher_type in ["Purchase Invoice", "Sales Invoice", "Delivery Note", "Purchase Receipt"]:
			ref_product_dt = voucher_type + " Product"

		if ref_product_dt:
			return frappe.db.get_value(ref_product_dt, voucher_detail_no, "allow_zero_valuation_rate")
		else:
			return 0
	...
	def raise_exceptions(self):
		msg_list = []
		for warehouse, exceptions in self.exceptions.products():
			...
			if (
				...
				msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
					abs(deficiency),
					frappe.get_desk_link("Product", exceptions[0]["product_code"]),
	...
	def update_bin_data(self, sle):
		bin_name = get_or_make_bin(sle.product_code, sle.warehouse)
		values_to_update = {
	...
	def update_bin(self):
		...
		for warehouse, data in self.data.products():
			bin_name = get_or_make_bin(self.product_code, warehouse)
...
def get_batch_incoming_rate(
	product_code, warehouse, batch_no, posting_date, posting_time, creation=None
):
...
def get_valuation_rate(
	...
	if warehouse and batch_no and frappe.db.get_value("Batch", batch_no, "use_batchwise_valuation"):
		last_valuation_rate = frappe.db.sql(
			...
			""",
			(product_code, warehouse, batch_no, voucher_no, voucher_type),
		)

	# Get valuation rate from last sle for the same product and warehouse
	if not last_valuation_rate or last_valuation_rate[0][0] is None:
		last_valuation_rate = frappe.db.sql(
			...
			(product_code, warehouse, voucher_no, voucher_type),
		)

	if last_valuation_rate:
		return flt(last_valuation_rate[0][0])

	# If negative stock allowed, and product delivered without any incoming entry,
	# system does not found any SLE, then take valuation rate from Product
	valuation_rate = frappe.db.get_value("Product", product_code, "valuation_rate")

	if not valuation_rate:
		# try Product Standard rate
		valuation_rate = frappe.db.get_value("Product", product_code, "standard_rate")

		if not valuation_rate:
			# try in price list
			valuation_rate = frappe.db.get_value(
				"Product Price", dict(product_code=product_code, buying=1, currency=currency), "price_list_rate"
			)
	...
	if (
		...
		form_link = get_link_to_form("Product", product_code)

		message = _(
			"Valuation Rate for the Product {0}, is required to do accounting entries for {1} {2}."
		).format(form_link, voucher_type, voucher_no)
		message += "<br><br>" + _("Here are the options to proceed:")
		solutions = (
			"<li>"
			+ _(
				"If the product is transacting as a Zero Valuation Rate product in this entry, please enable 'Allow Zero Valuation Rate' in the {0} Product table."
			).format(voucher_type)
			+ "</li>"
		)
		solutions += (
		...
		sub_solutions = "<ul><li>" + _("Create an incoming stock transaction for the Product.") + "</li>"
		sub_solutions += "<li>" + _("Mention Valuation Rate in the Product master.") + "</li></ul>"
...
def regenerate_sle_for_batch_stock_reco(detail):
	...
	doc.recalculate_current_qty(detail.product_code, detail.batch_no)

	if not frappe.db.exists(
		"Repost Product Valuation", {"voucher_no": doc.name, "status": "Queued", "docstatus": "1"}
	):
...
def get_next_stock_reco(kwargs):
	...
	query = (
		...
		.where(
			(sle.product_code == kwargs.get("product_code"))
...
def validate_negative_qty_in_future_sle(args, allow_negative_stock=False):
	if allow_negative_stock or is_negative_stock_allowed(product_code=args.product_code):
		return
	...
	if is_negative_with_precision(neg_sle):
		message = _(
			...
			frappe.get_desk_link("Product", args.product_code),
...
def is_negative_stock_allowed(*, product_code: Optional[str] = None) -> bool:
	if cint(frappe.db.get_single_value("Stock Settings", "allow_negative_stock", cache=True)):
		return True
	if product_code and cint(frappe.db.get_value("Product", product_code, "allow_negative_stock", cache=True)):
		return True
	return False
...
def get_incoming_rate_for_inter_company_transfer(sle) -> float:
	...
	field = "delivery_note_product" if sle.voucher_type == "Purchase Receipt" else "sales_invoice_product"

	doctype = "Delivery Note Product" if sle.voucher_type == "Purchase Receipt" else "Sales Invoice Product"

	reference_name = frappe.get_cached_value(sle.voucher_type + " Product", sle.voucher_detail_no, field)
