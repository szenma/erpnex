...
def get_stock_value_from_bin(warehouse=None, product_code=None):
	values = {}
	conditions = ""
	if warehouse:
	...
	if product_code:
		conditions += " and `tabBin`.product_code = %(product_code)s"

		values["product_code"] = product_code

	query = (
		"""select sum(stock_value) from `tabBin`, `tabProduct` where 1 = 1
		and `tabProduct`.name = `tabBin`.product_code and ifnull(`tabProduct`.disabled, 0) = 0 %s"""
		% conditions
	)
...
def get_stock_value_on(
	warehouses: list | str = None, posting_date: str = None, product_code: str = None
) -> float:
	if not posting_date:
	...
	if product_code:
		query = query.where(sle.product_code == product_code)
...
def get_serial_nos_data_after_transactions(args):
	...
	stock_ledger_entries = (
		...
		.where(
			(sle.product_code == args.product_code)
...
@frappe.whitelist()
def get_latest_stock_qty(product_code, warehouse=None):
	values, condition = [product_code], ""
	if warehouse:
...
def get_latest_stock_balance():
	...
	for d in frappe.db.sql(
		"""SELECT product_code, warehouse, stock_value as stock_value
		FROM tabBin""",
		as_dict=1,
	):
		bin_map.setdefault(d.warehouse, {}).setdefault(d.product_code, flt(d.stock_value))

	return bin_map


def get_bin(product_code, warehouse):
	bin = frappe.db.get_value("Bin", {"product_code": product_code, "warehouse": warehouse})
	if not bin:
		bin_obj = _create_bin(product_code, warehouse)
	else:
...
def get_or_make_bin(product_code: str, warehouse: str) -> str:
	bin_record = frappe.get_cached_value("Bin", {"product_code": product_code, "warehouse": warehouse})
...
def _create_bin(product_code, warehouse):
	...
	try:
		...
		bin_obj = frappe.get_doc(doctype="Bin", product_code=product_code, warehouse=warehouse)
		bin_obj.flags.ignore_permissions = 1
		bin_obj.insert()
	except frappe.UniqueValidationError:
		frappe.db.rollback(save_point=bin_creation_savepoint)  # preserve transaction in postgres
		bin_obj = frappe.get_last_doc("Bin", {"product_code": product_code, "warehouse": warehouse})
...
def get_incoming_rate(args, raise_error_if_no_rate=True):
	...
	if (args.get("serial_no") or "").strip():
		...
		valuation_method = get_valuation_method(args.get("product_code"))
...
def get_valuation_method(product_code):
	"""get valuation method from product or default"""
	val_method = frappe.db.get_value("Product", product_code, "valuation_method", cache=True)
	if not val_method:
		val_method = (
...
def get_valid_serial_nos(sr_nos, qty=0, product_code=""):
	...
	if qty and len(valid_serial_nos) != abs(qty):
		frappe.throw(_("{0} valid serial nos for Product {1}").format(abs(qty), product_code))
...
def update_included_uom_in_report(columns, result, include_uom, conversion_factors):
	...
	for row_idx, row in enumerate(result):
		data = row.products() if is_dict_obj else enumerate(row)
...
def get_available_serial_nos(args):
	return frappe.db.sql(
		...
		WHERE product_code = %(product_code)s and warehouse = %(warehouse)s
...
def add_additional_uom_columns(columns, result, include_uom, conversion_factors):
	...
	for row_idx, row in enumerate(result):
		for convertible_col, data in convertible_column_map.products():
			conversion_factor = conversion_factors.get(row.get("product_code")) or 1.0
...
def get_incoming_outgoing_rate_for_cancel(product_code, voucher_type, voucher_no, voucher_detail_no):
	outgoing_rate = frappe.db.sql(
		...
		WHERE voucher_type = %s and voucher_no = %s
			and product_code = %s and voucher_detail_no = %s
			ORDER BY CREATION DESC limit 1""",
		(voucher_type, voucher_no, product_code, voucher_detail_no),
	)
...
def is_reposting_product_valuation_in_progress():
	reposting_in_progress = frappe.db.exists(
		"Repost Product Valuation", {"docstatus": 1, "status": ["in", ["Queued", "In Progress"]]}
	)
	if reposting_in_progress:
		frappe.msgprint(
			_("Product valuation reposting in progress. Report might show incorrect product valuation."), alert=1
		)
...
def check_pending_reposting(posting_date: str, throw_error: bool = True) -> bool:
	...
	reposting_pending = frappe.db.exists("Repost Product Valuation", filters)
...
def scan_barcode(search_value: str) -> BarcodeScanResult:
	...
	barcode_data = frappe.db.get_value(
		...
		["barcode", "parent as product_code", "uom"],
		as_dict=True,
	)
	if barcode_data:
		_update_product_info(barcode_data)
	...
	serial_no_data = frappe.db.get_value(
		"Serial No",
		search_value,
		["name as serial_no", "product_code", "batch_no"],
		as_dict=True,
	)
	if serial_no_data:
		_update_product_info(serial_no_data)
	...
	batch_no_data = frappe.db.get_value(
		"Batch",
		search_value,
		["name as batch_no", "product as product_code"],
		as_dict=True,
	)
	if batch_no_data:
		_update_product_info(batch_no_data)
...
def _update_product_info(scan_result: Dict[str, Optional[str]]) -> Dict[str, Optional[str]]:
	if product_code := scan_result.get("product_code"):
		if product_info := frappe.get_cached_value(
			"Product",
			product_code,
			["has_batch_no", "has_serial_no"],
			as_dict=True,
		):
			scan_result.update(product_info)
	return scan_result
