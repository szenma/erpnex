...
class StockSettings(Document):
	def validate(self):
		...
		set_by_naming_series(
			"Product",
			"product_code",
			self.get("product_naming_by") == "Naming Series",
	...
	def cant_change_valuation_method(self):
		...
		if db_valuation_method and db_valuation_method != self.valuation_method:
			# check if there are any stock ledger entries against products
			# which does not have it's own valuation method
			sle = frappe.db.sql(
				"""select name from `tabStock Ledger Entry` sle
				where exists(select name from tabProduct
					where name=sle.product_code and (valuation_method is null or valuation_method='')) limit 1
			"""
			)
			...
			if sle:
				frappe.throw(
					_(
						"Can't change the valuation method, as there are transactions against some products which do not have its own valuation method"
					)
				)
...
def clean_all_descriptions():
	for product in frappe.get_all("Product", ["name", "description"]):
		if product.description:
			clean_description = clean_html(product.description)
		if product.description != clean_description:
			frappe.db.set_value("Product", product.name, "description", clean_description)
