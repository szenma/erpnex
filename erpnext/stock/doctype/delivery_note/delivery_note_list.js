frappe.listview_settings['Delivery Note'] = {
	...
	onload: function (doclist) {
		const action = () => {
			const selected_docs = doclist.get_checked_products();
			const docnames = doclist.get_checked_products(true);
			...
			if (selected_docs.length > 0) {
				...
				frappe.new_doc("Delivery Trip")
					.then(() => {
						...
						// the dialog to select more products. We just want the mapper
		...
		// doclist.page.add_actions_menu_product(__('Create Delivery Trip'), action, false);

		doclist.page.add_action_product(__('Create Delivery Trip'), action);

		doclist.page.add_action_product(__("Sales Invoice"), ()=>{
			erpnext.bulk_transaction_processing.create(doclist, "Delivery Note", "Sales Invoice");
		});

		doclist.page.add_action_product(__("Packaging Slip From Delivery Note"), ()=>{
