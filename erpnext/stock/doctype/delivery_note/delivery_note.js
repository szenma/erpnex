...
frappe.ui.form.on("Delivery Note", {
	setup: function(frm) {
		...
		frm.set_query('expense_account', 'products', function(doc, cdt, cdn) {
		...
		frm.set_query('cost_center', 'products', function(doc, cdt, cdn) {
			if (erpnext.is_perpetual_inventory_enabled(doc.company)) {
				return {
					filters: {
						'company': doc.company,
...
frappe.ui.form.on("Delivery Note Product", {
	expense_account: function(frm, dt, dn) {
		var d = locals[dt][dn];
		frm.update_in_all_rows('products', 'expense_account', d.expense_account);
	},
	cost_center: function(frm, dt, dn) {
		var d = locals[dt][dn];
		frm.update_in_all_rows('products', 'cost_center', d.cost_center);
	}
});
...
erpnext.stock.DeliveryNoteController = class DeliveryNoteController extends erpnext.selling.SellingController {
	...
	refresh(doc, dt, dn) {
		...
		if (!doc.is_return && doc.status!="Closed") {
			...
			if(doc.docstatus==0 && !doc.__islocal) {
				if (doc.__onload && doc.__onload.has_unpacked_products) {
		...
		if(doc.docstatus==1 && !doc.is_return && doc.status!="Closed" && flt(doc.per_billed) < 100) {
			...
			from_sales_invoice = me.frm.doc.products.some(function(product) {
				return product.against_sales_invoice ? true : false;
			});
	...
	packed_products_on_form_rendered(doc, grid_row) {
		erpnext.setup_serial_or_batch_no();
	}
...
frappe.ui.form.on('Delivery Note', {
	...
	unhide_account_head: function(frm) {
		...
		frm.fields_dict["products"].grid.set_column_disp(["expense_account", "cost_center"], aii_enabled);
	}
})
...
erpnext.stock.delivery_note.set_print_hide = function(doc, cdt, cdn){
	...
	var dn_product_fields = frappe.meta.docfield_map['Delivery Note Product'];
	var dn_fields_copy = dn_fields;
	var dn_product_fields_copy = dn_product_fields;
	if (doc.print_without_amount) {
		dn_fields['currency'].print_hide = 1;
		dn_product_fields['rate'].print_hide = 1;
		dn_product_fields['discount_percentage'].print_hide = 1;
		dn_product_fields['price_list_rate'].print_hide = 1;
		dn_product_fields['amount'].print_hide = 1;
		dn_product_fields['discount_amount'].print_hide = 1;
		dn_fields['taxes'].print_hide = 1;
	} else {
		if (dn_fields_copy['currency'].print_hide != 1)
			dn_fields['currency'].print_hide = 0;
		if (dn_product_fields_copy['rate'].print_hide != 1)
			dn_product_fields['rate'].print_hide = 0;
		if (dn_product_fields_copy['amount'].print_hide != 1)
			dn_product_fields['amount'].print_hide = 0;
		if (dn_product_fields_copy['discount_amount'].print_hide != 1)
			dn_product_fields['discount_amount'].print_hide = 0;
...
frappe.tour['Delivery Note'] = [
	...
	},
	{
		fieldname: "products",
		title: __("Products"),
		description: __("This table is used to set details about the 'Product', 'Qty', 'Basic Rate', etc.") + " " +
