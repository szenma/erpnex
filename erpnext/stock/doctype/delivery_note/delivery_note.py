...
class DeliveryNote(SellingController):
	def __init__(self, *args, **kwargs):
		...
		self.status_updater = [
			{
				"source_dt": "Delivery Note Product",
				...
				"keyword": "Delivered",
				"second_source_dt": "Sales Invoice Product",
			...
			{
				"source_dt": "Delivery Note Product",
				"target_dt": "Sales Invoice Product",
		...
		if cint(self.is_return):
			self.status_updater.extend(
				[
					...
					where name=`tabDelivery Note Product`.parent and is_return=1)""",
						"second_source_extra_cond": """ and exists (select name from `tabSales Invoice`
					where name=`tabSales Invoice Product`.parent and is_return=1 and update_stock=1)""",
					},
					{
						"source_dt": "Delivery Note Product",
	...
	def onload(self):
		if self.docstatus == 0:
			self.set_onload("has_unpacked_products", self.has_unpacked_products())
	...
	def before_print(self, settings=None):
		...
		product_meta = frappe.get_meta("Delivery Note Product")
		print_hide_fields = {
			"parent": ["grand_total", "rounded_total", "in_words", "currency", "total", "taxes"],
			"products": ["rate", "amount", "discount_amount", "price_list_rate", "discount_percentage"],
		}

		for key, fieldname in print_hide_fields.products():
			for f in fieldname:
				toggle_print_hide(self.meta if key == "parent" else product_meta, f)
		...
		if frappe.db.get_single_value("Selling Settings", "so_required") == "Yes":
			for d in self.get("products"):
				if not d.against_sales_order:
					frappe.throw(_("Sales Order required for Product {0}").format(d.product_code))
	...
	def validate(self):
		...
		if self._action != "submit" and not self.is_return:
			...
			set_batch_nos(self, "warehouse", throw=True, child_table="packed_products")
	...
	def validate_warehouse(self):
		...
		for d in self.get_product_list():
			if not d["warehouse"] and frappe.get_cached_value("Product", d["product_code"], "is_stock_product") == 1:
		...
		if self.get("_action") and self._action != "update_after_submit":
			for d in self.get("products"):
				d.actual_qty = frappe.db.get_value(
					"Bin", {"product_code": d.product_code, "warehouse": d.warehouse}, "actual_qty"
				)

			for d in self.get("packed_products"):
				bin_qty = frappe.db.get_value(
					"Bin",
					{"product_code": d.product_code, "warehouse": d.warehouse},
	...
	def validate_packed_qty(self):
		...
		if frappe.db.exists("Packing Slip", {"docstatus": 1, "delivery_note": self.name}):
			...
			for product in self.products + self.packed_products:
				if (
					product.product_code not in product_bundle_list
					and flt(product.packed_qty)
					and flt(product.packed_qty) != flt(product.qty)
				):
					frappe.throw(
						_("Row {0}: Packed Qty must be equal to {1} Qty.").format(
							product.idx, frappe.bold(product.doctype)
						)
					)
	...
	def check_next_docstatus(self):
		...
		submit_in = frappe.db.sql(
			"""select t1.name
			from `tabInstallation Note` t1, `tabInstallation Note Product` t2
	...
	def has_unpacked_products(self):
		product_bundle_list = self.get_product_bundle_list()

		for product in self.products + self.packed_products:
			if product.product_code not in product_bundle_list and flt(product.packed_qty) < flt(product.qty):
				return True

		return False

	def get_product_bundle_list(self):
		products_list = [product.product_code for product in self.products]
		return frappe.db.get_all(
			"Product Bundle",
			filters={"new_product_code": ["in", products_list]},
			pluck="name",
		)
...
def update_billed_amount_based_on_so(so_detail, update_modified=True):
	...
	si_product = frappe.qb.DocType("Sales Invoice Product").as_("si_product")
	sum_amount = Sum(si_product.amount).as_("amount")

	billed_against_so = (
		frappe.qb.from_(si_product)
		.select(sum_amount)
		.where(
			(si_product.so_detail == so_detail)
			& ((si_product.dn_detail.isnull()) | (si_product.dn_detail == ""))
			& (si_product.docstatus == 1)
		)
		.run()
	)
	billed_against_so = billed_against_so and billed_against_so[0][0] or 0

	# Get all Delivery Note Product rows against the Sales Order Product row
	dn = frappe.qb.DocType("Delivery Note").as_("dn")
	dn_product = frappe.qb.DocType("Delivery Note Product").as_("dn_product")

	dn_details = (
		frappe.qb.from_(dn)
		.from_(dn_product)
		.select(dn_product.name, dn_product.amount, dn_product.si_detail, dn_product.parent)
		.where(
			(dn.name == dn_product.parent)
			& (dn_product.so_detail == so_detail)
			& (dn.docstatus == 1)
			& (dn.is_return == 0)
		)
...
def get_invoiced_qty_map(delivery_note):
	...
	for dn_detail, qty in frappe.db.sql(
		"""select dn_detail, qty from `tabSales Invoice Product`
...
def get_returned_qty_map(delivery_note):
	...
	returned_qty_map = frappe._dict(
		frappe.db.sql(
			"""select dn_product.dn_detail, abs(dn_product.qty) as qty
		from `tabDelivery Note Product` dn_product, `tabDelivery Note` dn
		where dn.name = dn_product.parent
			and dn.docstatus = 1
			and dn.is_return = 1
...
def make_sales_invoice(source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		if len(target.get("products")) == 0:
			frappe.throw(_("All these products have already been Invoiced/Returned"))
	...
	def update_product(source_doc, target_doc, source_parent):
		...
		if source_doc.serial_no and source_parent.per_billed > 0 and not source_parent.is_return:
			target_doc.serial_no = get_delivery_note_serial_no(
				source_doc.product_code, target_doc.qty, source_parent.name
			)

	def get_pending_qty(product_row):
		pending_qty = product_row.qty - invoiced_qty_map.get(product_row.name, 0)

		returned_qty = 0
		if returned_qty_map.get(product_row.name, 0) > 0:
			returned_qty = flt(returned_qty_map.get(product_row.name, 0))
			returned_qty_map[product_row.name] -= pending_qty
		...
		to_make_invoice_qty_map[product_row.name] = pending_qty
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			"Delivery Note Product": {
				...
				"condition": lambda product: product.parent not in delivery_notes,
...
def make_installation_note(source_name, target_doc=None):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			"Delivery Note Product": {
				"doctype": "Installation Note Product",
				"field_map": {
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			"Delivery Note Product": {
				...
				"postprocess": update_product,
				"condition": lambda product: (
					not frappe.db.exists("Product Bundle", {"new_product_code": product.product_code})
					and flt(product.packed_qty) < flt(product.qty)
				),
			},
			"Packed Product": {
				...
				"postprocess": update_product,
				"condition": lambda product: (flt(product.packed_qty) < flt(product.qty)),
			},
		},
		target_doc,
		set_missing_values,
