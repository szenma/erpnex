...
class TestWarehouse(FrappeTestCase):
	def setUp(self):
		super().setUp()
		if not frappe.get_value("Product", "_Test Product"):
	...
	def test_unlinking_warehouse_from_product_defaults(self):
		...
		product_names = [f"_Test Product {i} for Unlinking" for i in range(2)]
		for product, warehouse in zip(product_names, warehouse_ids):
			create_product(product, warehouse=warehouse, company=company)
		...
		for product in product_names:
			self.assertTrue(bool(frappe.db.exists("Product", product)), f"{product} doesn't exist")

			product_doc = frappe.get_doc("Product", product)
			for product_default in product_doc.product_defaults:
				self.assertNotIn(
					product_default.default_warehouse,
					warehouse_ids,
					f"{product} linked to {product_default.default_warehouse} in {warehouse_ids}.",
				)
	...
	def test_group_non_group_conversion(self):
		...
		make_stock_entry(product_code="_Test Product", target=warehouse.name, qty=1)
		# SLE exists
		self.assertRaises(frappe.ValidationError, convert_to_group_or_ledger, warehouse.name)
