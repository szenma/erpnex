...
class Warehouse(NestedSet):
	...
	def on_trash(self):
		...
		for d in bins:
			if (
				...
				throw(
					_("Warehouse {0} can not be deleted as quantity exists for Product {1}").format(
						self.name, d["product_code"]
					)
				)
	...
	def unlink_from_products(self):
		frappe.db.set_value("Product Default", {"default_warehouse": self.name}, "default_warehouse", None)
