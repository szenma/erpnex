...
class QualityInspection(Document):
	def validate(self):
		if not self.readings and self.product_code:
			self.get_product_specification_details()

		if self.inspection_type == "In Process" and self.reference_type == "Job Card":
			product_qi_template = frappe.db.get_value("Product", self.product_code, "quality_inspection_template")
			parameters = get_template_details(product_qi_template)
	...
	@frappe.whitelist()
	def get_product_specification_details(self):
		if not self.quality_inspection_template:
			self.quality_inspection_template = frappe.db.get_value(
				"Product", self.product_code, "quality_inspection_template"
			)
	...
	def get_quality_inspection_template(self):
		...
		if not template:
			template = frappe.db.get_value("BOM", self.product_code, "quality_inspection_template")
		...
		if self.reference_type == "Job Card":
			if self.reference_name:
				frappe.db.sql(
					...
					WHERE name = %s and production_product = %s
				""".format(
						doctype=self.reference_type
					),
					(quality_inspection, self.modified, self.reference_name, self.product_code),
				)

		else:
			args = [quality_inspection, self.modified, self.reference_name, self.product_code]
			doctype = self.reference_type + " Product"
...
def product_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters.get("parent"):
		if (
			from_doctype in ["Purchase Invoice Product", "Purchase Receipt Product"]
			and filters.get("inspection_type") != "In Process"
		):
			cond = """and product_code in (select name from `tabProduct` where
				inspection_required_before_purchase = 1)"""
		elif (
			from_doctype in ["Sales Invoice Product", "Delivery Note Product"]
			and filters.get("inspection_type") != "In Process"
		):
			cond = """and product_code in (select name from `tabProduct` where
		...
		if from_doctype in ["Supplier Quotation Product"]:
			qi_condition = ""
		...
		return frappe.db.sql(
			"""
				SELECT product_code
				FROM `tab{doc}`
				WHERE parent=%(parent)s and docstatus < 2 and product_code like %(txt)s
				{qi_condition} {cond} {mcond}
				ORDER BY product_code limit {page_len} offset {start}
			""".format(
				doc=from_doctype,
				cond=cond,
				mcond=mcond,
		...
		return frappe.db.sql(
			"""
				SELECT production_product
				FROM `tab{doc}`
				WHERE name = %(reference_name)s and docstatus < 2 and production_product like %(txt)s
				{qi_condition} {cond} {mcond}
				ORDER BY production_product
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			"BOM": {
				...
				"field_map": {"name": "bom_no", "product": "product_code", "stock_uom": "uom", "stock_qty": "qty"},
			}
		},
		target_doc,
		postprocess,
