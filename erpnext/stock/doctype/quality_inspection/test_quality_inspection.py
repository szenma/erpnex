...
class TestQualityInspection(FrappeTestCase):
	def setUp(self):
		super().setUp()
		create_product("_Test Product with QA")
		frappe.db.set_value("Product", "_Test Product with QA", "inspection_required_before_delivery", 1)

	def test_qa_for_delivery(self):
		make_stock_entry(
			product_code="_Test Product with QA", target="_Test Warehouse - _TC", qty=1, basic_rate=100
		)
		dn = create_delivery_note(product_code="_Test Product with QA", do_not_submit=True)
	...
	def test_qa_not_submit(self):
		...
		dn.products[0].quality_inspection = qa.name
	...
	def test_make_quality_inspections_from_linked_document(self):
		...
		for product in dn.products:
			product.sample_size = product.qty
		quality_inspections = make_quality_inspections(dn.doctype, dn.name, dn.products)
		self.assertEqual(len(dn.products), len(quality_inspections))
	...
	def test_rejected_qi_validation(self):
		...
		se = make_stock_entry(
			product_code="_Test Product with QA",
...
def create_quality_inspection(**args):
	...
	qa.product_code = args.product_code or "_Test Product with QA"
	qa.sample_size = 1
