...
class TestInventoryDimension(FrappeTestCase):
	...
	def test_inventory_dimension(self):
		...
		inward.products[0].to_shelf = "Shelf 1"
		inward.save()
		inward.submit()
		inward.load_from_db()
		...
		self.assertEqual(inward.products[0].to_shelf, "Shelf 1")
		...
		outward.products[0].shelf = "Shelf 1"
		outward.save()
		outward.submit()
	...
	def test_inventory_dimension_for_purchase_receipt_and_delivery_note(self):
		...
		frappe.reload_doc("stock", "doctype", "purchase_receipt_product")
		frappe.reload_doc("stock", "doctype", "delivery_note_product")
		...
		self.assertEqual(pr_doc.products[0].rack, "Rack 1")
		sle_rack = frappe.db.get_value(
			"Stock Ledger Entry",
			{"voucher_detail_no": pr_doc.products[0].name, "voucher_type": pr_doc.doctype},
			"rack",
		)
		...
		self.assertEqual(dn_doc.products[0].rack, "Rack 1")
		sle_rack = frappe.db.get_value(
			"Stock Ledger Entry",
			{"voucher_detail_no": dn_doc.products[0].name, "voucher_type": dn_doc.doctype},
			"rack",
		)
	...
	def test_for_purchase_sales_and_stock_transaction(self):
		...
		product_code = "Test Inventory Dimension Product"
		...
		pr_doc = make_purchase_receipt(
			product_code=product_code, warehouse=warehouse, qty=10, rate=100, do_not_submit=True
		)

		pr_doc.products[0].store = "Store 1"
		pr_doc.save()
		pr_doc.submit()
		...
		se_doc = make_stock_entry(
			product_code=product_code, qty=10, from_warehouse=warehouse, to_warehouse=warehouse, do_not_save=True
		)

		se_doc.products[0].store = "Store 1"
		se_doc.products[0].to_store = "Store 2"
		...
		dn_doc = create_delivery_note(product_code=product_code, qty=10, warehouse=warehouse, do_not_save=True)

		dn_doc.products[0].store = "Store 2"
		dn_doc.save()
		dn_doc.submit()
		...
		se_doc.products[0].store = "Store 2"
		se_doc.products[0].to_store = "Store 1"
	...
	def test_inter_transfer_return_against_inventory_dimension(self):
		...
		dn_doc.products[0].store = "Inter Transfer Store 1"
		dn_doc.products[0].to_store = "Inter Transfer Store 2"
		dn_doc.save()
		dn_doc.submit()
		...
		pr_doc.products[0].warehouse = data.store_warehouse
		pr_doc.products[0].from_store = "Inter Transfer Store 2"
		pr_doc.products[0].store = "Inter Transfer Store 3"
		pr_doc.save()
		pr_doc.submit()
		...
		return_doc1.products[0].target_warehouse = dn_doc.products[0].target_warehouse
		return_doc1.products[0].warehouse = dn_doc.products[0].warehouse
		return_doc1.save()
		return_doc1.submit()
...
def prepare_data_for_internal_transfer():
	...
	pr_doc.products[0].store = "Inter Transfer Store 1"
	pr_doc.submit()
