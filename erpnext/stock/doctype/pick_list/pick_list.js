...
frappe.ui.form.on('Pick List', {
	setup: (frm) => {
		...
		frm.set_query('product_code', 'locations', () => {
	...
	set_product_locations:(frm, save) => {
		if (!(frm.doc.locations && frm.doc.locations.length)) {
			frappe.msgprint(__('Add products in the Product Locations table'));
		} else {
			frappe.call({
				method: "set_product_locations",
				doc: frm.doc,
				args: {
					"save": save,
				},
				freeze: 1,
				freeze_message: __("Setting Product Locations..."),
			});
		}
	},
	get_product_locations: (frm) => {
		// Button on the form
		frm.events.set_product_locations(frm, false);
	},
	refresh: (frm) => {
		frm.trigger('add_get_products_button');
	...
	work_order: (frm) => {
		frappe.db.get_value('Work Order',
			...
			frappe.prompt({
				fieldtype: 'Float',
				label: __('Qty of Finished Goods Product'),
				fieldname: 'qty',
			...
			}, __('Select Quantity'), __('Get Products'));
		});
	},
	...
	update_pick_list_stock: (frm) => {
		frm.events.set_product_locations(frm, true);
	},
	add_get_products_button: (frm) => {
		...
		frm.get_products_btn = frm.add_custom_button(__('Get Products'), () => {
			erpnext.utils.map_current_doc({
...
frappe.ui.form.on('Pick List Product', {
	product_code: (frm, cdt, cdn) => {
		let row = frappe.get_doc(cdt, cdn);
		if (row.product_code) {
			get_product_details(row.product_code).then(data => {
	...
	uom: (frm, cdt, cdn) => {
		let row = frappe.get_doc(cdt, cdn);
		if (row.uom) {
			get_product_details(row.product_code, row.uom).then(data => {
...
function get_product_details(product_code, uom=null) {
	if (product_code) {
		return frappe.xcall('erpnext.stock.doctype.pick_list.pick_list.get_product_details', {
			product_code,
			uom
		});
	}
