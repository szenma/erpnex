...
from erpnext.assets.doctype.asset.test_asset import create_asset_category, create_fixed_asset_product
...
class TestLandedCostVoucher(FrappeTestCase):
	def test_landed_cost_voucher(self):
		...
		pr_lc_value = frappe.db.get_value(
			"Purchase Receipt Product", {"parent": pr.name}, "landed_cost_voucher_amount"
		)
		...
		products, warehouses = pr.get_products_and_warehouses()
		update_gl_entries_after(pr.posting_date, pr.posting_time, warehouses, products, company=pr.company)
	...
	def assertPurchaseReceiptLCVGLEntries(self, pr):
		...
		fixed_asset_account = get_inventory_account(pr.company, pr.get("products")[1].warehouse)
	...
	def test_landed_cost_voucher_stock_impact(self):
		...
		product = make_product("LCV Stock Product", {"is_stock_product": 1})
	...
	def test_landed_cost_voucher_for_zero_purchase_rate(self):
		...
		lcv.get_products_from_purchase_receipts()
		lcv.products[0].applicable_charges = 100
		lcv.save()
		lcv.submit()
	...
	def test_landed_cost_voucher_against_purchase_invoice(self):
		...
		pi_lc_value = frappe.db.get_value(
			"Purchase Invoice Product", {"parent": pi.name}, "landed_cost_voucher_amount"
		)
	...
	def test_landed_cost_voucher_for_serialized_product(self):
		frappe.db.sql(
		...
		pr.products[0].product_code = "_Test Serialized Product"
		pr.products[0].serial_no = "SN001\nSN002\nSN003\nSN004\nSN005"
		pr.submit()
	...
	def test_landed_cost_voucher_for_odd_numbers(self):
		...
		pr.products[0].cost_center = "Main - TCP1"
		for x in range(2):
			pr.append(
				"products",
				{
		...
		self.assertEqual(flt(lcv.products[0].applicable_charges, 2), 41.07)
		self.assertEqual(flt(lcv.products[2].applicable_charges, 2), 41.08)
	...
	def test_multiple_landed_cost_voucher_against_pr(self):
		...
		lcv1.insert()
		lcv1.set("products", [lcv1.get("products")[0]])
		distribute_landed_cost_on_products(lcv1)
		...
		lcv2.insert()
		lcv2.set("products", [lcv2.get("products")[1]])
		distribute_landed_cost_on_products(lcv2)

		lcv2.submit()

		pr.load_from_db()

		self.assertEqual(pr.products[0].landed_cost_voucher_amount, 100)
		self.assertEqual(pr.products[1].landed_cost_voucher_amount, 100)
	...
	def test_multi_currency_lcv(self):
		...
		self.assertEqual(pr.products[0].landed_cost_voucher_amount, 729)
	...
	def test_asset_lcv(self):
		...
		if not frappe.db.exists("Product", "Macbook Pro"):
			create_fixed_asset_product()

		pr = make_purchase_receipt(product_code="Macbook Pro", qty=1, rate=50000)
...
def create_landed_cost_voucher(receipt_document_type, receipt_document, company, charges=50):
	...
	distribute_landed_cost_on_products(lcv)
...
def distribute_landed_cost_on_products(lcv):
	based_on = lcv.distribute_charges_based_on.lower()
	total = sum(flt(d.get(based_on)) for d in lcv.get("products"))

	for product in lcv.get("products"):
		product.applicable_charges = flt(product.get(based_on)) * flt(lcv.total_taxes_and_charges) / flt(total)
		product.applicable_charges = flt(product.applicable_charges, lcv.precision("applicable_charges", product))
