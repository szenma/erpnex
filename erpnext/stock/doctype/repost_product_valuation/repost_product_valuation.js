...
frappe.ui.form.on('Repost Product Valuation', {
	setup: function(frm) {
	...
	based_on: function(frm) {
		...
		if (frm.doc.based_on == 'Transaction') {
			fields_to_reset = ['product_code', 'warehouse'];
		} else if (frm.doc.based_on == 'Product and Warehouse') {
	...
	setup_realtime_progress: function(frm) {
		frappe.realtime.on('product_reposting_progress', data => {
			...
			if (frm.doc.status == 'In Progress') {
				...
				frm.doc.products_to_be_repost = data.products_to_be_repost;
	...
	execute_reposting(frm) {
		frm.add_custom_button(__("Start Reposting"), () => {
			frappe.call({
				method: 'erpnext.stock.doctype.repost_product_valuation.repost_product_valuation.execute_repost_product_valuation',
				callback: function() {
	...
	show_reposting_progress: function(frm) {
		...
		let total_count = frm.doc.products_to_be_repost ? JSON.parse(frm.doc.products_to_be_repost).length : 0;
