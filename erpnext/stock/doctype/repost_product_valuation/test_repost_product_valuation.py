...
from erpnext.controllers.stock_controller import create_product_wise_repost_entries
from erpnext.stock.doctype.product.test_product import make_product
from erpnext.stock.doctype.purchase_receipt.test_purchase_receipt import make_purchase_receipt
from erpnext.stock.doctype.repost_product_valuation.repost_product_valuation import (
...
class TestRepostProductValuation(FrappeTestCase, StockTestMixin):
	def tearDown(self):
	...
	def test_clear_old_logs(self):
		...
		logs = frappe.get_all("Repost Product Valuation", filters={"status": "Skipped"})
		self.assertTrue(len(logs) > 10)

		from erpnext.stock.doctype.repost_product_valuation.repost_product_valuation import RepostProductValuation

		RepostProductValuation.clear_old_logs(days=1)

		logs = frappe.get_all("Repost Product Valuation", filters={"status": "Skipped"})
		self.assertTrue(len(logs) == 0)

	def test_create_product_wise_repost_product_valuation_entries(self):
		...
		rivs = create_product_wise_repost_entries(pr.doctype, pr.name)
		self.assertGreaterEqual(len(rivs), 2)
		self.assertIn("_Test Product", [d.product_code for d in rivs])
	...
	def test_prevention_of_cancelled_transaction_riv(self):
		...
		warehouse = "_Test Warehouse - _TC"
		old = make_stock_entry(product_code=product.name, to_warehouse=warehouse, qty=2, rate=5)
		_new = make_stock_entry(product_code=product.name, to_warehouse=warehouse, qty=5, rate=10)

		old.cancel()

		riv = frappe.get_last_doc(
			"Repost Product Valuation", {"voucher_type": old.doctype, "voucher_no": old.name}
		)
	...
	def test_gl_repost_progress(self):
		...
		doc = frappe.new_doc("Repost Product Valuation")
	...
	def test_gl_complete_gl_reposting(self):
		...
		for _ in range(10):
			make_stock_entry(product=product, company=company, qty=1, rate=10, target="Stores - TCP1")

		# consume
		consumption = make_stock_entry(product=product, company=company, qty=1, source="Stores - TCP1")
	...
	def test_create_repost_entry_for_cancelled_document(self):
		...
		self.assertFalse(frappe.db.exists("Repost Product Valuation", {"voucher_no": pr.name}))

		pr.load_from_db()

		pr.cancel()
		self.assertTrue(pr.docstatus == 2)
		self.assertTrue(frappe.db.exists("Repost Product Valuation", {"voucher_no": pr.name}))

	def test_repost_product_valuation_for_closing_stock_balance(self):
		...
		riv = frappe.new_doc("Repost Product Valuation")
		riv.update(
			{
				"product_code": "_Test Product",
				"warehouse": "_Test Warehouse - _TC",
