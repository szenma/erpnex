...
class RepostProductValuation(Document):
	@staticmethod
	def clear_old_logs(days=None):
		days = days or 90
		table = DocType("Repost Product Valuation")
		frappe.db.delete(
			table,
			filters=(
	...
	def validate_period_closing_voucher(self):
		...
		if year_end_date and getdate(self.posting_date) <= getdate(year_end_date):
			...
			msg = f"Due to period closing, you cannot repost product valuation before {date}"
			frappe.throw(_(msg))
		...
		if closing_stock and closing_stock[0].name:
			...
			msg = f"Due to closing stock balance {name}, you cannot repost product valuation before {to_date}"
			frappe.throw(_(msg))
	...
	def get_closing_stock_balance(self):
		...
		for field in ["warehouse", "product_code"]:
			if self.get(field):
	...
	def validate_accounts_freeze(self):
		...
		if getdate(self.posting_date) <= getdate(acc_settings.acc_frozen_upto):
			if (
				...
				return
			frappe.throw(
				_("You cannot repost product valuation before {}").format(acc_settings.acc_frozen_upto)
			)
	...
	def on_submit(self):
		...
		Exceptions:
		        1. "Repost Product Valuation" document has self.flags.dont_run_in_test
	...
	def restart_reposting(self):
		...
		self.distinct_product_and_warehouse = None
		self.products_to_be_repost = None
		self.gl_reposting_index = 0
		self.db_update()

	def deduplicate_similar_repost(self):
		"""Deduplicate similar reposts based on product-warehouse-posting combination."""
		if self.based_on != "Product and Warehouse":
			return
		...
		frappe.db.sql(
			"""
			update `tabRepost Product Valuation`
			set status = 'Skipped'
			WHERE product_code = %(product_code)s
				...
				and docstatus = 1
				and status = 'Queued'
				and based_on = 'Product and Warehouse'
				""",
			filters,
		)
...
def on_doctype_update():
	frappe.db.add_index("Repost Product Valuation", ["warehouse", "product_code"], "product_warehouse")


def repost(doc):
	try:
		if not frappe.db.exists("Repost Product Valuation", doc.name):
			return
		...
		doc.log_error("Unable to repost product valuation")
...
def _get_directly_dependent_vouchers(doc):
	...
	i.e. any one product-warehouse is present in the stock transaction"""
	...
	if doc.based_on == "Transaction":
		...
		doc_products, doc_warehouses = ref_doc.get_products_and_warehouses()
		products.update(doc_products)
		warehouses.update(doc_warehouses)

		sles = get_products_to_be_repost(doc.voucher_type, doc.voucher_no)
		sle_products = {sle.product_code for sle in sles}
		sle_warehouses = {sle.warehouse for sle in sles}
		products.update(sle_products)
		warehouses.update(sle_warehouses)
	else:
		products.add(doc.product_code)
...
def notify_error_to_stock_managers(doc, traceback):
	...
	subject = _("Error while reposting product valuation")
	message = (
		_("Hi,")
		+ "<br>"
		+ _("An error has been appeared while reposting product valuation via {0}").format(
...
def repost_entries():
	"""
	Reposts 'Repost Product Valuation' entries in queue.
	Called hourly via hooks.py.
	"""
	if not in_configured_timeslot():
		return

	riv_entries = get_repost_product_valuation_entries()

	for row in riv_entries:
		doc = frappe.get_doc("Repost Product Valuation", row.name)
...
def get_repost_product_valuation_entries():
	return frappe.db.sql(
		""" SELECT name from `tabRepost Product Valuation`
...
@frappe.whitelist()
def execute_repost_product_valuation():
	"""Execute repost product valuation via scheduler."""
	frappe.get_doc("Scheduled Job Type", "repost_product_valuation.repost_entries").enqueue(force=True)
