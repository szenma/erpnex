...
frappe.ui.form.on('Stock Entry', {
	setup: function(frm) {
		frm.set_indicator_formatter('product_code', function(doc) {
			if (!doc.s_warehouse) {
				return 'blue';
			} else {
		...
		frappe.db.get_value('Stock Settings', {name: 'Stock Settings'}, 'sample_retention_warehouse', (r) => {
			if (r.sample_retention_warehouse) {
				...
				frm.set_query("s_warehouse", "products", function() {
					return {
						filters: filters
					};
				});
		...
		frm.set_query('batch_no', 'products', function(doc, cdt, cdn) {
			...
			if(!product.product_code) {
				...
				if (frm.doc.purpose != "Material Receipt") {
					filters["warehouse"] = product.s_warehouse || product.t_warehouse;
				}
		...
		let batch_field = frm.get_docfield('products', 'batch_no');
		if (batch_field) {
		...
		attach_bom_products(frm.doc.bom_no);

		if(!check_should_not_attach_bom_products(frm.doc.bom_no)) {
	...
	setup_quality_inspection: function(frm) {
		...
		let quality_inspection_field = frm.get_docfield("products", "quality_inspection");
		...
		frm.set_query("quality_inspection", "products", function(doc, cdt, cdn) {
	...
	outgoing_stock_entry: function(frm) {
		frappe.call({
			doc: frm.doc,
			method: "set_products_for_stock_in",
			callback: function() {
	...
	refresh: function(frm) {
		frm.trigger("get_products_from_transit_entry");
		...
		if(!frm.doc.docstatus) {
			...
			frm.add_custom_button(__('Material Request'), function() {
				frappe.model.with_doctype('Material Request', function() {
					...
					var products = frm.get_field('products').grid.get_selected_children();
					if(!products.length) {
					...
					products.forEach(function(product) {
						var mr_product = frappe.model.add_child(mr, 'products');
						mr_product.product_code = product.product_code;
						mr_product.product_name = product.product_name;
						mr_product.uom = product.uom;
						mr_product.stock_uom = product.stock_uom;
						mr_product.conversion_factor = product.conversion_factor;
						mr_product.product_group = product.product_group;
						mr_product.description = product.description;
						mr_product.image = product.image;
						mr_product.qty = product.qty;
						mr_product.warehouse = product.s_warehouse;
						mr_product.required_date = frappe.datetime.nowdate();
					});
			...
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__('Alternate Product'), () => {
					erpnext.utils.select_alternate_products({
						...
						condition: (d) => {
							if (d.s_warehouse && d.allow_alternative_product) {return true;}
						}
					})
				});
			}
		...
		if (frm.doc.docstatus===0 && frm.doc.purpose == "Material Issue") {
			frm.add_custom_button(__('Expired Batches'), function() {
				frappe.call({
					method: "erpnext.stock.doctype.stock_entry.stock_entry.get_expired_batch_products",
					callback: function(r) {
						if (!r.exc && r.message) {
							frm.set_value("products", []);
							r.message.forEach(function(element) {
								let d = frm.add_child("products");
								d.product_code = element.product;
		...
		if(frm.doc.docstatus==1 && frm.doc.purpose == "Material Receipt" && frm.get_sum('products', 			'sample_quantity')) {
	...
	get_products_from_transit_entry: function(frm) {
		if (frm.doc.docstatus===0) {
	...
	before_save: function(frm) {
		frm.doc.products.forEach((product) => {
			product.uom = product.uom || product.stock_uom;
		})
	},

	stock_entry_type: function(frm){
		frm.remove_custom_button('Bill of Materials', "Get Products From");
		frm.events.show_bom_custom_button(frm);
	...
	set_serial_no: function(frm, cdt, cdn, callback) {
		...
		if(!d.product_code && !d.s_warehouse && !d.qty) return;
		var	args = {
			'product_code'	: d.product_code,
		...
		frappe.call({
			method: "erpnext.stock.get_product_details.get_serial_no",
			args: {"args": args},
			callback: function(r) {
				if (!r.exe && r.message){
	...
	toggle_display_account_head: function(frm) {
		...
		frm.fields_dict["products"].grid.set_column_disp(["cost_center", "expense_account"], enabled);
	},

	set_basic_rate: function(frm, cdt, cdn) {
		const product = locals[cdt][cdn];
		product.transfer_qty = flt(product.qty) * flt(product.conversion_factor);

		const args = {
			'product_code'			: product.product_code,
			'posting_date'		: frm.doc.posting_date,
			'posting_time'		: frm.doc.posting_time,
			'warehouse'			: cstr(product.s_warehouse) || cstr(product.t_warehouse),
			'serial_no'			: product.serial_no,
			'batch_no'          : product.batch_no,
			'company'			: frm.doc.company,
			'qty'				: product.s_warehouse ? -1*flt(product.transfer_qty) : flt(product.transfer_qty),
		...
		if (product.product_code || product.serial_no) {
			frappe.call({
				...
				callback: function(r) {
					...
					frm.events.calculate_basic_amount(frm, product);
				}
			});
		}
	},
	...
	show_bom_custom_button: function(frm){
		if (frm.doc.docstatus === 0 &&
			...
			frm.add_custom_button(__('Bill of Materials'), function() {
				frm.events.get_products_from_bom(frm);
			}, __("Get Products From"));
		}
	},
	...
	get_products_from_bom: function(frm) {
		...
		let fields = [
			...
			{"fieldname":"fetch", "label":__("Get Products from BOM"), "fieldtype":"Button"}
		]
				...
				callback: function(r) {
					if (!r.message) {
						...
						$.each(r.message, function(i, product) {
							let d = frappe.model.add_child(cur_frm.doc, "Stock Entry Detail", "products");
							d.product_code = product.product_code;
							d.product_name = product.product_name;
							d.product_group = product.product_group;
							...
							d.conversion_factor = product.conversion_factor ? product.conversion_factor : 1;
							d.qty = product.qty;
							d.expense_account = product.expense_account;
	...
	calculate_basic_amount: function(frm, product) {
		product.basic_amount = flt(flt(product.transfer_qty) * flt(product.basic_rate),
			precision("basic_amount", product));
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			erpnext.utils.map_current_doc({
				method: 'erpnext.stock.doctype.stock_entry.stock_entry.get_products_from_subcontract_order',
...
frappe.ui.form.on('Stock Entry Detail', {
				...
				callback: function(r) {
					if(r.message) {
						...
						if (d.has_batch_no && !d.has_serial_no) {
							// check only batch_no for batched product
	...
	expense_account: function(frm, cdt, cdn) {
		erpnext.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "products", "expense_account");
	},
	cost_center: function(frm, cdt, cdn) {
		erpnext.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "products", "cost_center");
	},
...
erpnext.stock.StockEntry = class StockEntry extends erpnext.stock.StockController {
	setup() {
		...
		this.frm.fields_dict.products.grid.get_field('product_code').get_query = function() {
		...
		this.frm.fields_dict.products.grid.get_field('expense_account').get_query = function() {
	...
	refresh() {
		...
		erpnext.utils.add_product(this.frm);
	}
	...
	set_default_account(callback) {
		...
		if(this.frm.doc.company && erpnext.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			return this.frm.call({
				...
				callback: function(r) {
					if (!r.exc) {
						$.each(me.frm.doc.products || [], function(i, d) {
	...
	get_products() {
		...
		if(this.frm.doc.work_order || this.frm.doc.bom_no) {
			// if work order / bom is mentioned, get products
			return this.frm.call({
				doc: me.frm.doc,
				freeze: true,
				method: "get_products",
				callback: function(r) {
					if(!r.exc) refresh_field("products");
					if(me.frm.doc.bom_no) {
						attach_bom_products(me.frm.doc.bom_no);
	...
	products_add(doc, cdt, cdn) {
		...
		if (!(row.expense_account && row.cost_center)) {
			this.frm.script_manager.copy_from_first_row("products", row, ["expense_account", "cost_center"]);
		}
	...
	from_warehouse(doc) {
		...
		this.set_warehouse_in_children(doc.products, "s_warehouse", doc.from_warehouse);
	}

	to_warehouse(doc) {
		this.set_warehouse_in_children(doc.products, "t_warehouse", doc.to_warehouse);
	}
	...
	products_on_form_rendered(doc, grid_row) {
	...
	toggle_related_fields(doc) {
		...
		this.frm.fields_dict["products"].grid.set_column_disp("retain_sample", doc.purpose=='Material Receipt');
		this.frm.fields_dict["products"].grid.set_column_disp("sample_quantity", doc.purpose=='Material Receipt');
		...
		this.frm.fields_dict["products"].grid.set_column_disp("additional_cost", doc.purpose!='Material Issue');
	}
...
erpnext.stock.select_batch_and_serial_no = (frm, product) => {
	let get_warehouse_type_and_name = (product) => {
		let value = '';
		if(frm.fields_dict.from_warehouse.disp_status === "Write") {
			value = cstr(product.s_warehouse) || '';
			return {
				type: 'Source Warehouse',
				name: value
			};
		} else {
			value = cstr(product.t_warehouse) || '';
			return {
	...
	if(product && !product.has_serial_no && !product.has_batch_no) return;
	...
	frappe.require("assets/erpnext/js/utils/serial_no_batch_selector.js", function() {
		...
		frm.batch_selector = new erpnext.SerialNoBatchSelector({
			frm: frm,
			product: product,
			warehouse_details: get_warehouse_type_and_name(product),
		});
	});

}

function attach_bom_products(bom_no) {
	if (!bom_no) {
		return
	}

	if (check_should_not_attach_bom_products(bom_no)) return
	frappe.db.get_doc("BOM",bom_no).then(bom => {
		const {name, products} = bom
		erpnext.stock.bom = {name, products:{}}
		products.forEach(product => {
			erpnext.stock.bom.products[product.product_code] = product;
		});
	});
}

function check_should_not_attach_bom_products(bom_no) {
  return (
	bom_no === undefined ||
