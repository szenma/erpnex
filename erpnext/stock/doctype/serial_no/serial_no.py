...
from erpnext.stock.get_product_details import get_reserved_qty_for_so
...
class SerialNoProductError(ValidationError):
	pass
...
class SerialNo(StockController):
	...
	def validate_warehouse(self):
		if not self.get("__islocal"):
			product_code, warehouse = frappe.db.get_value("Serial No", self.name, ["product_code", "warehouse"])
			if not self.via_stock_ledger and product_code != self.product_code:
				frappe.throw(_("Product Code cannot be changed for Serial No."), SerialNoCannotCannotChangeError)
			if not self.via_stock_ledger and warehouse != self.warehouse:
				frappe.throw(_("Warehouse cannot be changed for Serial No."), SerialNoCannotCannotChangeError)

	def validate_product(self):
		"""
		Validate whether serial no is required for this product
		"""
		product = frappe.get_cached_doc("Product", self.product_code)
		if product.has_serial_no != 1:
			frappe.throw(
				_("Product {0} is not setup for Serial Nos. Check Product master").format(self.product_code)
			)

		self.product_group = product.product_group
		self.description = product.description
		self.product_name = product.product_name
		self.brand = product.brand
		self.warranty_period = product.warranty_period
	...
	def on_trash(self):
		sl_entries = frappe.db.sql(
			...
			where serial_no like %s and product_code=%s and is_cancelled=0""",
			("%%%s%%" % self.name, self.product_code),
			as_dict=True,
		)
...
def process_serial_no(sle):
	product_det = get_product_details(sle.product_code)
	validate_serial_no(sle, product_det)
	update_serial_nos(sle, product_det)


def validate_serial_no(sle, product_det):
	serial_nos = get_serial_nos(sle.serial_no) if sle.serial_no else []
	validate_material_transfer_entry(sle)

	if product_det.has_serial_no == 0:
		if serial_nos:
			frappe.throw(
				_("Product {0} is not setup for Serial Nos. Column must be blank").format(sle.product_code),
		...
		if serial_nos:
			if cint(sle.actual_qty) != flt(sle.actual_qty):
				frappe.throw(
					_("Serial No {0} quantity {1} cannot be a fraction").format(sle.product_code, sle.actual_qty)
				)

			if len(serial_nos) and len(serial_nos) != abs(cint(sle.actual_qty)):
				frappe.throw(
					_("{0} Serial Numbers required for Product {1}. You have provided {2}.").format(
						abs(sle.actual_qty), sle.product_code, len(serial_nos)
					),
					SerialNoQtyError,
				)
			...
			if len(serial_nos) != len(set(serial_nos)):
				frappe.throw(
					_("Duplicate Serial No entered for Product {0}").format(sle.product_code), SerialNoDuplicateError
				)
			...
			for serial_no in serial_nos:
				if frappe.db.exists("Serial No", serial_no):
					...
					if sr.product_code != sle.product_code:
						if not allow_serial_nos_with_different_product(serial_no, sle):
							frappe.throw(
								_("Serial No {0} does not belong to Product {1}").format(serial_no, sle.product_code),
								SerialNoProductError,
							)
					...
					if not allow_existing_serial_no and sle.voucher_type in [
						...
						if sle.voucher_type == "Stock Entry":
							...
							if se_purpose in ["Manufacture", "Material Receipt"]:
								msg = f"Cannot create a {sle.voucher_type} ({se_purpose}) for the Product {frappe.bold(sle.product_code)} with the existing Serial No {frappe.bold(serial_no)}."
						else:
							msg = f"Cannot create a {sle.voucher_type} for the Product {frappe.bold(sle.product_code)} with the existing Serial No {frappe.bold(serial_no)}."
					...
					if cint(sle.actual_qty) < 0:
						...
						if sle.voucher_type in ("Delivery Note", "Sales Invoice"):
							...
							if sr.sales_order:
								if sle.voucher_type == "Sales Invoice":
									if not frappe.db.exists(
										...
										{"parent": sle.voucher_no, "product_code": sle.product_code, "sales_order": sr.sales_order},
									):
										frappe.throw(
											_(
												"Cannot deliver Serial No {0} of product {1} as it is reserved to fullfill Sales Order {2}"
											).format(sr.name, sle.product_code, sr.sales_order)
										)
									...
									if not frappe.db.exists(
										...
										invoice = frappe.db.get_value(
											"Delivery Note Product",
											{"parent": sle.voucher_no, "product_code": sle.product_code},
											"against_sales_invoice",
										)
										if not invoice or frappe.db.exists(
											"Sales Invoice Product",
											{"parent": invoice, "product_code": sle.product_code, "sales_order": sr.sales_order},
										):
											frappe.throw(
												_(
													"Cannot deliver Serial No {0} of product {1} as it is reserved to fullfill Sales Order {2}"
							...
							# if Sales Order reference in Delivery Note or Invoice validate SO reservations for product
							if sle.voucher_type == "Sales Invoice":
								...
								if sales_order and get_reserved_qty_for_so(sales_order, sle.product_code):
								...
								if sales_order and get_reserved_qty_for_so(sales_order, sle.product_code):
									...
									if sales_invoice:
										sales_order = frappe.db.get_value(
											"Sales Invoice Product",
											{"parent": sales_invoice, "product_code": sle.product_code},
											"sales_order",
										)
										if sales_order and get_reserved_qty_for_so(sales_order, sle.product_code):
											validate_so_serial_no(sr, sales_order)
					...
					# transfer out
					frappe.throw(_("Serial No {0} not in stock").format(serial_no), SerialNoNotExistsError)
		elif cint(sle.actual_qty) < 0 or not product_det.serial_no_series:
			frappe.throw(
				_("Serial Nos Required for Serialized Product {0}").format(sle.product_code), SerialNoRequiredError
			)
	elif serial_nos:
...
def validate_so_serial_no(sr, sales_order):
	if not sr.sales_order or sr.sales_order != sales_order:
		msg = _(
			"Sales Order {0} has reservation for the product {1}, you can only deliver reserved {1} against {0}."
		).format(sales_order, sr.product_code)
...
def allow_serial_nos_with_different_product(sle_serial_no, sle):
	"""
...
def update_serial_nos(sle, product_det):
	...
	if (
		...
		and product_det.has_serial_no == 1
		and product_det.serial_no_series
	):
		serial_nos = get_auto_serial_nos(product_det.serial_no_series, sle.actual_qty)
...
def auto_make_serial_nos(args):
	...
	if len(form_links) == 1:
		...
		message = _("The following serial numbers were created: <br><br> {0}").format(
			get_products_html(form_links, product_code)
		)
		frappe.msgprint(message, multiple_title)


def get_products_html(serial_nos, product_code):
	...
	""".format(
		product_code, len(serial_nos), body
	)
...
def get_product_details(product_code):
	return frappe.db.sql(
		...
		is_stock_product, has_serial_no, serial_no_series
		from tabProduct where name=%s""",
		product_code,
		as_dict=True,
	)[0]
...
def update_args_for_serial_no(serial_no_doc, serial_no, args, is_new=False):
	for field in ["product_code", "work_order", "company", "batch_no", "supplier", "location"]:
		if args.get(field):
	...
	serial_no_doc.validate_product()
...
def get_delivery_note_serial_no(product_code, qty, delivery_note):
	serial_nos = ""
	dn_serial_nos = frappe.db.sql_list(
		""" select name from `tabSerial No`
		where product_code = %(product_code)s and delivery_document_no = %(delivery_note)s
		and sales_invoice is null limit {0}""".format(
			cint(qty)
		),
		{"product_code": product_code, "delivery_note": delivery_note},
	)
...
def auto_fetch_serial_number(
	...
	filters = frappe._dict({"product_code": product_code, "warehouse": warehouse})
...
def get_pos_reserved_serial_nos(filters):
	...
	POSInvoiceProduct = frappe.qb.DocType("POS Invoice Product")
	query = (
		frappe.qb.from_(POSInvoice)
		.from_(POSInvoiceProduct)
		.select(POSInvoice.is_return, POSInvoiceProduct.serial_no)
		.where(
			(POSInvoice.name == POSInvoiceProduct.parent)
			& (POSInvoice.docstatus == 1)
			& (POSInvoiceProduct.docstatus == 1)
			& (POSInvoiceProduct.product_code == filters.get("product_code"))
			& (POSInvoiceProduct.warehouse == filters.get("warehouse"))
			& (POSInvoiceProduct.serial_no.isnotnull())
			& (POSInvoiceProduct.serial_no != "")
		)
	)
...
def fetch_serial_numbers(filters, qty, do_not_include=None):
	...
	query = (
		...
		.where(
			(serial_no.product_code == filters["product_code"])
