...
class TestSerialNo(FrappeTestCase):
	...
	def test_cannot_create_direct(self):
		...
		sr.product_code = "_Test Serialized Product"
	...
	def test_inter_company_transfer(self):
		se = make_serialized_product(target_warehouse="_Test Warehouse - _TC")
		serial_nos = get_serial_nos(se.get("products")[0].serial_no)

		dn = create_delivery_note(
			product_code="_Test Serialized Product With Series", qty=1, serial_no=serial_nos[0]
		)
	...
	def test_auto_creation_of_serial_no(self):
		...
		product_code = make_product(
			"_Test Auto Serial Product ", {"has_serial_no": 1, "serial_no_series": "XYZ.###"}
		).product_code

		# Reserve XYZ005
		pr_1 = make_purchase_receipt(product_code=product_code, qty=1, serial_no="XYZ005")
		# XYZ005 is already used and will throw an error if used again
		pr_2 = make_purchase_receipt(product_code=product_code, qty=10)

		self.assertEqual(get_serial_nos(pr_1.get("products")[0].serial_no)[0], "XYZ005")
		for serial_no in get_serial_nos(pr_2.get("products")[0].serial_no):
	...
	def test_serial_no_sanitation(self):
		...
		se.get("products")[0].product_code = product_code
		se.get("products")[0].qty = 4
		se.get("products")[0].serial_no = " _TS1, _TS2 , _TS3  , _TS4 - 2021"
		se.get("products")[0].transfer_qty = 4
		se.set_stock_entry_type()
		se.insert()
		se.submit()

		self.assertEqual(se.get("products")[0].serial_no, "_TS1\n_TS2\n_TS3\n_TS4 - 2021")
	...
	def test_correct_serial_no_incoming_rate(self):
		...
		in1 = make_stock_entry(
			product_code=product_code, to_warehouse=warehouse, qty=1, rate=42, serial_no=serial_nos[0]
		)
		in2 = make_stock_entry(
			product_code=product_code, to_warehouse=warehouse, qty=1, rate=113, serial_no=serial_nos[1]
		)

		out = create_delivery_note(
			product_code=product_code, qty=1, serial_no=serial_nos[0], do_not_submit=True
		)

		# change serial no
		out.products[0].serial_no = serial_nos[1]
		out.save()
		out.submit()
	...
	def test_auto_fetch(self):
		...
		in1 = make_stock_entry(product_code=product_code, to_warehouse=warehouse, qty=5)
		in2 = make_stock_entry(product_code=product_code, to_warehouse=warehouse, qty=5)

		in1.reload()
		in2.reload()

		batch1 = in1.products[0].batch_no
		batch2 = in2.products[0].batch_no

		batch_wise_serials = {
			batch1: get_serial_nos(in1.products[0].serial_no),
			batch2: get_serial_nos(in2.products[0].serial_no),
		}

		# Test FIFO
		first_fetch = auto_fetch_serial_number(5, product_code, warehouse)
		self.assertEqual(first_fetch, batch_wise_serials[batch1])

		# partial FIFO
		partial_fetch = auto_fetch_serial_number(2, product_code, warehouse)
		self.assertTrue(
		...
		remaining = auto_fetch_serial_number(
			3, product_code, warehouse, exclude_sr_nos=json.dumps(partial_fetch)
		)
		self.assertEqual(sorted(remaining + partial_fetch), first_fetch)

		# batchwise
		for batch, expected_serials in batch_wise_serials.products():
			fetched_sr = auto_fetch_serial_number(5, product_code, warehouse, batch_nos=batch)
			self.assertEqual(fetched_sr, sorted(expected_serials))

		# non existing warehouse
		self.assertEqual(auto_fetch_serial_number(10, product_code, warehouse="Nonexisting"), [])

		# multi batch
		all_serials = [sr for sr_list in batch_wise_serials.values() for sr in sr_list]
		fetched_serials = auto_fetch_serial_number(
			10, product_code, warehouse, batch_nos=list(batch_wise_serials.keys())
		)
		...
		non_expired_serials = auto_fetch_serial_number(
			5, product_code, warehouse, posting_date="2021-01-01", batch_nos=batch1
		)
