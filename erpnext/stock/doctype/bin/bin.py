...
class Bin(Document):
	def before_save(self):
		if self.get("__islocal") or not self.stock_uom:
			self.stock_uom = frappe.get_cached_value("Product", self.product_code, "stock_uom")
	...
	def update_reserved_qty_for_production(self):
		"""Update qty reserved for production from Production Product tables
		in open work orders"""
	...
	def update_reserved_qty_for_sub_contracting(self, subcontract_doctype="Subcontracting Order"):
		...
		supplied_product = frappe.qb.DocType(
		...
		conditions = (
			(supplied_product.rm_product_code == self.product_code)
			& (subcontract_order.name == supplied_product.parent)
			& (subcontract_order.per_received < 100)
			& (supplied_product.reserve_warehouse == self.warehouse)
			& (
				(
		...
		reserved_qty_for_sub_contract = (
			...
			.from_(supplied_product)
			.select(Sum(Coalesce(supplied_product.required_qty, 0)))
			.where(conditions)
		).run()[0][0] or 0.0

		se = frappe.qb.DocType("Stock Entry")
		se_product = frappe.qb.DocType("Stock Entry Detail")

		if frappe.db.field_exists("Stock Entry", "is_return"):
			qty_field = (
				Case().when(se.is_return == 1, se_product.transfer_qty * -1).else_(se_product.transfer_qty)
			)
		else:
			qty_field = se_product.transfer_qty

		conditions = (
			(se.docstatus == 1)
			& (se.purpose == "Send to Subcontractor")
			& ((se_product.product_code == self.product_code) | (se_product.original_product == self.product_code))
			& (se.name == se_product.parent)
			& (subcontract_order.docstatus == 1)
			& (subcontract_order.per_received < 100)
			& (
				(
...
def on_doctype_update():
	frappe.db.add_unique("Bin", ["product_code", "warehouse"], constraint_name="unique_product_warehouse")
...
def update_qty(bin_name, args):
	...
	if future_sle_exists(args):
		last_sle_qty = (
			...
			.where(
				(sle.product_code == args.get("product_code"))
