...
frappe.ui.form.on('Product Variant Settings', {
	refresh: function(frm) {
		const allow_fields = [];

		const existing_fields = frm.doc.fields.map(row => row.field_name);
		const exclude_fields = [...existing_fields, "naming_series", "product_code", "product_name",
		...
		frappe.model.with_doctype('Product', () => {
			...
			frappe.get_meta('Product').fields.forEach(d => {
