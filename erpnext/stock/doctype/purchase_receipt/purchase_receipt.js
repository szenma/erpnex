...
frappe.ui.form.on("Purchase Receipt", {
	...
	toggle_display_account_head: function(frm) {
		...
		frm.fields_dict["products"].grid.set_column_disp(["cost_center"], enabled);
	}
});
		...
		frappe.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					frappe.msgprint(__("Purchase Receipt doesn't have any Product for which Retain Sample is enabled."));
				}
			}
		});
	}
...
frappe.ui.form.on('Purchase Receipt Product', {
	product_code: function(frm, cdt, cdn) {
		var d = locals[cdt][cdn];
		frappe.db.get_value('Product', {name: d.product_code}, 'sample_quantity', (r) => {
