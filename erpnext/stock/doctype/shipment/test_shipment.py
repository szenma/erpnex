...
def create_test_delivery_note():
	...
	product = get_shipment_product(company.name)
	posting_date = date.today() + timedelta(days=1)

	create_material_receipt(product, company.name)
...
def get_shipment_product(company_name):
	product_name = "Testing Shipment product"
	...
	if len(products):
		...
		return create_shipment_product(product_name, company_name)
...
def create_material_receipt(product, company):
	...
	stock.append(
		"products",
		{
			...
			"qty": 5,
			"uom": "Nos",
			"basic_rate": product.standard_rate,
			"cost_center": "Main - _TC",
		},
	)
	stock.insert()
...
def create_shipment_product(product_name, company_name):
	product = frappe.new_doc("Product")
	product.product_name = product_name
	product.product_code = product_name
	product.product_group = "All Product Groups"
	product.stock_uom = "Nos"
	product.standard_rate = 50
	product.append("product_defaults", {"company": company_name, "default_warehouse": "Stores - _TC"})
	product.insert()
	return product
