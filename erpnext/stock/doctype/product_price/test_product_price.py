...
from erpnext.stock.doctype.product_price.product_price import ProductPriceDuplicateProduct
from erpnext.stock.get_product_details import get_price_list_rate_for, process_args
...
class TestProductPrice(FrappeTestCase):
	def setUp(self):
		...
		make_test_records_for_doctype("Product Price", force=True)

	def test_template_product_price(self):
		from erpnext.stock.doctype.product.test_product import make_product

		product = make_product(
			"Test Template Product 1",
			{
				"has_variants": 1,
	...
	def test_duplicate_product(self):
		...
		self.assertRaises(ProductPriceDuplicateProduct, doc.save)
	...
	def test_price_in_a_qty(self):
		...
		price = get_price_list_rate_for(process_args(args), doc.product_code)
	...
	def test_price_with_no_qty(self):
		...
		price = get_price_list_rate_for(args, doc.product_code)
	...
	def test_invalid_product(self):
		...
		doc.product_code = "This is not an product code"
		# Valid product codes must already exist
		self.assertRaises(frappe.ValidationError, doc.save)
...
test_records = frappe.get_test_records("Product Price")
