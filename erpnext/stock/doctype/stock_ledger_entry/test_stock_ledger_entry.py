...
class TestStockLedgerEntry(FrappeTestCase, StockTestMixin):
	def setUp(self):
		...
		# delete SLE and BINs for all products
		frappe.db.sql(
			"delete from `tabStock Ledger Entry` where product_code in (%s)"
			% (", ".join(["%s"] * len(products))),
			products,
		)
		frappe.db.sql(
			"delete from `tabBin` where product_code in (%s)" % (", ".join(["%s"] * len(products))), products
		)

	def tearDown(self):
		frappe.db.rollback()

	def test_product_cost_reposting(self):
		company = "_Test Company"

		# _Test Product for Reposting at Stores warehouse on 10-04-2020: Qty = 50, Rate = 100
		create_stock_reconciliation(
			product_code="_Test Product for Reposting",
		...
		# _Test Product for Reposting at FG warehouse on 20-04-2020: Qty = 10, Rate = 200
		create_stock_reconciliation(
			product_code="_Test Product for Reposting",
		...
		# _Test Product for Reposting transferred from Stores to FG warehouse on 30-04-2020
		se = make_stock_entry(
			product_code="_Test Product for Reposting",
			source="Stores - _TC",
		...
		finished_product_sle = frappe.db.get_value(
			"Stock Ledger Entry",
			{
				"product_code": "_Test Finished Product for Reposting",
				"warehouse": "Finished Goods - _TC",
				"voucher_type": "Stock Entry",
		...
		self.assertEqual(finished_product_sle.get("incoming_rate"), 540)
		self.assertEqual(finished_product_sle.get("valuation_rate"), 540)

		# Reconciliation for _Test Product for Reposting at Stores on 12-04-2020: Qty = 50, Rate = 150
		...
		# Check valuation rate of repacked product after back-dated entry at Stores
		finished_product_sle = frappe.db.get_value(
			"Stock Ledger Entry",
			{
				"product_code": "_Test Finished Product for Reposting",
		...
		self.assertEqual(finished_product_sle.get("incoming_rate"), 790)
		self.assertEqual(finished_product_sle.get("valuation_rate"), 790)

		# Check updated rate in Repack entry
		repack.reload()
		self.assertEqual(repack.products[0].get("basic_rate"), 150)
		self.assertEqual(repack.products[1].get("basic_rate"), 750)
	...
	def test_sales_return_valuation_reposting(self):
		...
		self.assertEqual(dn.products[0].incoming_rate, 100)
		self.assertEqual(outgoing_rate, 100)
		...
		self.assertEqual(return_dn.products[0].incoming_rate, 100)
		...
		dn.reload()
		self.assertEqual(dn.products[0].incoming_rate, 110)
		...
		return_dn.reload()
		self.assertEqual(return_dn.products[0].incoming_rate, 110)
	...
	def test_reposting_of_sales_return_for_packed_product(self):
		company = "_Test Company"
		packed_product_code = "_Test Product for Reposting"
		bundled_product = "_Test Bundled Product for Reposting"
		create_product_bundle_product(bundled_product, [[packed_product_code, 4]])
		...
		pr = make_purchase_receipt(
			...
			posting_date="2020-04-10",
			warehouse="Stores - _TC",
			product_code=packed_product_code,
			qty=50,
			rate=100,
		)
		...
		self.assertEqual(dn.packed_products[0].incoming_rate, 100)
		self.assertEqual(outgoing_rate, 100)
		...
		self.assertEqual(return_dn.packed_products[0].incoming_rate, 100)
		self.assertEqual(incoming_rate, 100)
		...
		dn.reload()
		self.assertEqual(dn.packed_products[0].incoming_rate, 101)
		...
		return_dn.reload()
		self.assertEqual(return_dn.packed_products[0].incoming_rate, 101)
	...
	def test_batchwise_product_valuation_moving_average(self):
		product, warehouses, batches = setup_product_valuation_test(valuation_method="Moving Average")

		# Incoming Entries for Stock Value check
		pr_entry_list = [
			(product, warehouses[0], batches[0], 1, 100),
			(product, warehouses[0], batches[1], 1, 50),
			(product, warehouses[0], batches[0], 1, 150),
			(product, warehouses[0], batches[1], 1, 100),
		]
		prs = create_purchase_receipt_entries_for_batchwise_product_valuation_test(pr_entry_list)
		...
		dn_entry_list = [
			(product, warehouses[0], batches[1], 1, 200),
			(product, warehouses[0], batches[0], 1, 200),
			(product, warehouses[0], batches[1], 1, 200),
			(product, warehouses[0], batches[0], 1, 200),
		]
		dns = create_delivery_note_entries_for_batchwise_product_valuation_test(dn_entry_list)
		...
		for dn, incoming_rate in zip(dns, expected_incoming_rates):
			self.assertEqual(
				dn.products[0].incoming_rate,
				incoming_rate,
				"Incorrect 'Incoming Rate' values fetched for DN products",
			)

	def test_batchwise_product_valuation_stock_reco(self):
		product, warehouses, batches = setup_product_valuation_test()
		...
		osr1 = create_stock_reconciliation(
			warehouse=warehouses[0], product_code=product, qty=10, rate=100, batch_no=batches[1]
		)
		expected_sles = [
		...
		osr2 = create_stock_reconciliation(
			warehouse=warehouses[0], product_code=product, qty=13, rate=200, batch_no=batches[0]
		)
		expected_sles = [
		...
		sr1 = create_stock_reconciliation(
			warehouse=warehouses[0], product_code=product, qty=5, rate=50, batch_no=batches[1]
		)
		...
		sr2 = create_stock_reconciliation(
			warehouse=warehouses[0], product_code=product, qty=20, rate=75, batch_no=batches[0]
		)
		expected_sles = [
	...
	def test_batch_wise_valuation_across_warehouse(self):
		product_code, warehouses, batches = setup_product_valuation_test()
		source = warehouses[0]
		target = warehouses[1]

		unrelated_batch = make_stock_entry(
			product_code=product_code, target=source, batch_no=batches[1], qty=5, rate=10
		)
		self.assertSLEs(
			unrelated_batch,
			[
		...
		reciept = make_stock_entry(
			product_code=product_code, target=source, batch_no=batches[0], qty=5, rate=10
		)
		self.assertSLEs(
			reciept,
			[
		...
		transfer = make_stock_entry(
			product_code=product_code, source=source, target=target, batch_no=batches[0], qty=5
		)
		self.assertSLEs(
			transfer,
			[
		...
		transfer_unrelated = make_stock_entry(
			product_code=product_code, source=source, target=target, batch_no=batches[1], qty=5
		)
		self.assertSLEs(
			transfer_unrelated,
			[
	...
	def test_intermediate_average_batch_wise_valuation(self):
		...
		create_today = make_stock_entry(
			product_code=product_code, target=warehouse, batch_no=batches[0], qty=1, rate=20
		)
		self.assertSLEs(
			create_today,
			[
		...
		consume_today = make_stock_entry(
			product_code=product_code, source=warehouse, batch_no=batches[0], qty=1
		)
		self.assertSLEs(
			consume_today,
			[
	...
	def test_legacy_product_valuation_stock_entry(self):
		columns = [
			"stock_value_difference",
			"stock_value",
			"actual_qty",
		...
		product, warehouses, batches = setup_product_valuation_test(use_batchwise_valuation=0)
		...
		se_entry_list_mr = [
			(product, None, warehouses[0], batches[0], 1, 50, "2021-01-21"),
			(product, None, warehouses[0], batches[1], 1, 100, "2021-01-23"),
		]
		ses = create_stock_entry_entries_for_batchwise_product_valuation_test(
		...
		se_entry_list_mi = [
			(product, warehouses[0], None, batches[1], 1, None, "2021-01-29"),
		]
		ses = create_stock_entry_entries_for_batchwise_product_valuation_test(
	...
	def test_mixed_valuation_batches_fifo(self):
		product_code, warehouses, batches = setup_product_valuation_test(use_batchwise_valuation=0)
		warehouse = warehouses[0]
		...
		old1 = make_stock_entry(
			product_code=product_code, target=warehouse, batch_no=batches[0], qty=10, rate=10
		)
		self.assertSLEs(
			old1,
			update_invariants(
		...
		old2 = make_stock_entry(
			product_code=product_code, target=warehouse, batch_no=batches[1], qty=10, rate=20
		)
		self.assertSLEs(
			old2,
			update_invariants(
		...
		old3 = make_stock_entry(
			product_code=product_code, target=warehouse, batch_no=batches[0], qty=5, rate=15
		)
		...
		new1 = make_stock_entry(product_code=product_code, target=warehouse, qty=10, rate=40)
		batches.append(new1.products[0].batch_no)
		...
		new2 = make_stock_entry(product_code=product_code, target=warehouse, qty=10, rate=42)
		batches.append(new2.products[0].batch_no)
		self.assertSLEs(
			new2,
			update_invariants(
				[
		...
		consume_old1 = make_stock_entry(
			product_code=product_code, source=warehouse, qty=15, batch_no=batches[0]
		)
		self.assertSLEs(
			consume_old1,
			update_invariants(
		...
		consume_new2 = make_stock_entry(
			product_code=product_code, source=warehouse, qty=10, batch_no=batches[-1]
		)
		self.assertSLEs(
			consume_new2,
			update_invariants(
		...
		consume_new1 = make_stock_entry(
			product_code=product_code, source=warehouse, qty=10, batch_no=batches[-2]
		)
		self.assertSLEs(
			consume_new1,
			update_invariants(
	...
	def test_fifo_dependent_consumption(self):
		product = make_product("_TestFifoTransferRates")
		source = "_Test Warehouse - _TC"
		target = "Stores - _TC"

		rates = [10 * i for i in range(1, 20)]

		receipt = make_stock_entry(product_code=product.name, target=source, qty=10, do_not_save=True, rate=10)
		for rate in rates[1:]:
			row = frappe.copy_doc(receipt.products[0], ignore_no_copy=False)
			row.basic_rate = rate
			receipt.append("products", row)
		...
		transfer = make_stock_entry(
			product_code=product.name, source=source, target=target, qty=10, do_not_save=True, rate=10
		)
		for rate in rates[1:]:
			row = frappe.copy_doc(transfer.products[0], ignore_no_copy=False)
			transfer.append("products", row)
	...
	def test_fifo_multi_product_repack_consumption(self):
		rm = make_product("_TestFifoRepackRM")
		packed = make_product("_TestFifoRepackFinished")
		warehouse = "_Test Warehouse - _TC"

		rates = [10 * i for i in range(1, 5)]

		receipt = make_stock_entry(
			product_code=rm.name, target=warehouse, qty=10, do_not_save=True, rate=10
		)
		for rate in rates[1:]:
			row = frappe.copy_doc(receipt.products[0], ignore_no_copy=False)
			row.basic_rate = rate
		...
		repack = make_stock_entry(
			product_code=rm.name, source=warehouse, qty=10, do_not_save=True, rate=10, purpose="Repack"
		)
		for rate in rates[1:]:
			row = frappe.copy_doc(repack.products[0], ignore_no_copy=False)
			repack.append("products", row)

		repack.append(
			"products",
			{
				"product_code": packed.name,
		...
		# same exact queue should be transferred
		self.assertSLEs(
			repack, [{"incoming_rate": sum(rates) * 10}], sle_filters={"product_code": packed.name}
		)
	...
	def test_negative_fifo_valuation(self):
		...
		"""
		product = make_product(properties={"allow_negative_stock": 1}).name
		warehouse = "_Test Warehouse - _TC"

		receipt = make_stock_entry(product_code=product, target=warehouse, qty=10, rate=10)
		consume1 = make_stock_entry(product_code=product, source=warehouse, qty=15)

		self.assertSLEs(consume1, [{"stock_value": -5 * 10, "stock_queue": [[-5, 10]]}])

		consume2 = make_stock_entry(product_code=product, source=warehouse, qty=5)
		self.assertSLEs(consume2, [{"stock_value": -10 * 10, "stock_queue": [[-10, 10]]}])

		receipt2 = make_stock_entry(product_code=product, target=warehouse, qty=15, rate=15)
	...
	def test_dependent_gl_entry_reposting(self):
		...
		make_stock_entry(product_code=product, to_warehouse=A, qty=5, rate=10, posting_date=_day(0))
		make_stock_entry(product_code=product, from_warehouse=A, to_warehouse=B, qty=5, posting_date=_day(1))
		depdendent_consumption = make_stock_entry(
			product_code=product, from_warehouse=B, qty=5, posting_date=_day(2)
		)
		...
		bd_receipt = make_stock_entry(
			product_code=product, to_warehouse=A, qty=5, rate=20, posting_date=_day(-1)
		)
		...
		bd_receipt2 = make_stock_entry(
			product_code=product, to_warehouse=A, qty=2, rate=20, posting_date=_day(-2)
		)
		...
		make_stock_entry(product_code=product, from_warehouse=B, to_warehouse=C, qty=5, posting_date=_day(3))
		final_consumption = make_stock_entry(
			product_code=product, from_warehouse=C, qty=5, posting_date=_day(4)
		)
	...
	def test_tie_breaking(self):
		...
		def ordered_qty_after_transaction():
			return (
				...
				.where((sle.product_code == product) & (sle.warehouse == warehouse) & (sle.is_cancelled == 0))
	...
	def test_transfer_invariants(self):
		...
		transfer = make_stock_entry(
			product=product, from_warehouse=source_warehouse, to_warehouse=target_warehouse, qty=1_728.0
		)
	...
	def test_negative_qty_with_precision(self):
		...
		product_code = "ProductPrecisionTest"
		warehouse = "_Test Warehouse - _TC"
		create_product(product_code, is_stock_product=1, stock_uom="Kg")

		create_stock_reconciliation(product_code=product_code, warehouse=warehouse, qty=559.8327, rate=100)

		make_stock_entry(product_code=product_code, source=warehouse, qty=470.84, rate=100)
		self.assertEqual(get_stock_balance(product_code, warehouse), 88.9927)
		...
		make_stock_entry(product_code=product_code, target=warehouse, qty=11.007, rate=100)
		self.assertEqual(get_stock_balance(product_code, warehouse), 99.9997)
		...
		self.assertEqual(flt(get_stock_balance(product_code, warehouse), 3), 0.000)
	...
	def test_future_negative_qty_with_precision(self):
		...
		make_stock_entry(
			product_code=product_code, source=warehouse, qty=470.84, rate=100, posting_date=add_days(today(), -1)
		)
...
def create_product_bundle_product(new_product_code, packed_products):
	if not frappe.db.exists("Product Bundle", new_product_code):
		product = frappe.new_doc("Product Bundle")
		product.new_product_code = new_product_code

		for d in packed_products:
			product.append("products", {"product_code": d[0], "qty": d[1]})

		product.save()


def create_products(products=None, uoms=None):
	if not products:
		products = [
			"_Test Product for Reposting",
			"_Test Finished Product for Reposting",
			"_Test Subcontracted Product for Reposting",
			"_Test Bundled Product for Reposting",
		]

	for d in products:
		properties = {"valuation_method": "FIFO"}
		if d == "_Test Bundled Product for Reposting":
			properties.update({"is_stock_product": 0})
		elif d == "_Test Subcontracted Product for Reposting":
			properties.update({"is_sub_contracted_product": 1})

		make_product(d, properties=properties, uoms=uoms)

	return products


def setup_product_valuation_test(
	...
	product = make_product(
		f"IV - Test Product {valuation_method} {suffix}",
	...
	for i, batch_id in enumerate(batches):
		if not frappe.db.exists("Batch", batch_id):
			...
			batch = frappe.get_doc(
				frappe._dict(
					doctype="Batch", batch_id=batch_id, product=product.product_code, use_batchwise_valuation=ubw
				)
			).insert()
			batch.use_batchwise_valuation = ubw
			batch.db_update()

	return product.product_code, warehouses, batches


def create_purchase_receipt_entries_for_batchwise_product_valuation_test(pr_entry_list):
	from erpnext.stock.doctype.purchase_receipt.test_purchase_receipt import make_purchase_receipt

	prs = []

	for product, warehouse, batch_no, qty, rate in pr_entry_list:
		pr = make_purchase_receipt(product=product, warehouse=warehouse, qty=qty, rate=rate, batch_no=batch_no)
		prs.append(pr)

	return prs


def create_delivery_note_entries_for_batchwise_product_valuation_test(dn_entry_list):
	from erpnext.selling.doctype.sales_order.sales_order import make_delivery_note
	from erpnext.selling.doctype.sales_order.test_sales_order import make_sales_order

	dns = []
	for product, warehouse, batch_no, qty, rate in dn_entry_list:
		so = make_sales_order(
			rate=rate, qty=qty, product=product, warehouse=warehouse, against_blanket_order=0
		)

		dn = make_delivery_note(so.name)
		dn.products[0].batch_no = batch_no
		dn.insert()
		dn.submit()
		dns.append(dn)
	return dns
...
def create_stock_entry_entries_for_batchwise_product_valuation_test(se_entry_list, purpose):
	ses = []
	for product, source, target, batch, qty, rate, posting_date in se_entry_list:
		args = dict(
			product_code=product,
			qty=qty,
			company="_Test Company",
...
class TestDeferredNaming(FrappeTestCase):
	...
	def setUp(self) -> None:
		self.product = make_product().name
	...
	def test_deferred_naming(self):
		se = make_stock_entry(
			product_code=self.product, to_warehouse=self.warehouse, qty=10, rate=100, company=self.company
		)
