...
from erpnext.controllers.product_variant import ProductTemplateCannotHaveStock
...
class StockLedgerEntry(Document):
	...
	def validate_product(self):
		product_det = frappe.db.sql(
			"""select name, product_name, has_batch_no, docstatus,
			is_stock_product, has_variants, stock_uom, create_new_batch
		...
		if product_det.is_stock_product != 1:
			frappe.throw(_("Product {0} must be a stock Product").format(self.product_code))

		# check if batch number is valid
		if product_det.has_batch_no == 1:
			batch_product = (
				self.product_code
				if self.product_code == product_det.product_name
				else self.product_code + ":" + product_det.product_name
			)
			if not self.batch_no:
				frappe.throw(_("Batch number is mandatory for Product {0}").format(batch_product))
			elif not frappe.db.get_value("Batch", {"product": self.product_code, "name": self.batch_no}):
				frappe.throw(
					_("{0} is not a valid Batch Number for Product {1}").format(self.batch_no, batch_product)
				)

		elif product_det.has_batch_no == 0 and self.batch_no and self.is_cancelled == 0:
			frappe.throw(_("The Product {0} cannot have Batch").format(self.product_code))

		if product_det.has_variants:
			frappe.throw(
				_("Stock cannot exist for Product {0} since has variants").format(self.product_code),
				ProductTemplateCannotHaveStock,
			)

		self.stock_uom = product_det.stock_uom
		...
		if self.batch_no and self.voucher_type != "Stock Entry":
			...
			if expiry_date:
				if getdate(self.posting_date) > getdate(expiry_date):
					frappe.throw(_("Batch {0} of Product {1} has expired.").format(self.batch_no, self.product_code))
		...
		if authorized_role:
			...
			if authorized_users and frappe.session.user not in authorized_users:
				last_transaction_time = frappe.db.sql(
					...
					where docstatus = 1 and is_cancelled = 0 and product_code = %s
					and warehouse = %s""",
					(self.product_code, self.warehouse),
				)[0][0]
				...
				if last_transaction_time and get_datetime(cur_doc_posting_datetime) < get_datetime(
					...
					msg = _("Last Stock Transaction for product {0} under warehouse {1} was on {2}.").format(
						frappe.bold(self.product_code), frappe.bold(self.warehouse), frappe.bold(last_transaction_time)
					)

					msg += "<br><br>" + _(
						"You are not authorized to make/edit Stock Transactions for Product {0} under warehouse {1} before this time."
					).format(frappe.bold(self.product_code), frappe.bold(self.warehouse))
...
def on_doctype_update():
	...
	frappe.db.add_index("Stock Ledger Entry", ["batch_no", "product_code", "warehouse"])
	frappe.db.add_index("Stock Ledger Entry", ["warehouse", "product_code"], "product_warehouse")
