...
def execute(filters=None, consolidated=False):
	data, columns = DelayedProductReport(filters).run()

	return data, columns


class DelayedProductReport(object):
	...
	def get_data(self, consolidated=False):
		...
		if self.filters.get("product_group"):
			query = query.where(child.product_group == self.filters.get("product_group"))
	...
	def filter_transactions_data(self, consolidated=False):
		...
		if not consolidated:
			sales_order_products = [d.so_detail for d in self.transactions]
			doctype = "Sales Order Product"
			filters = {"parent": ("in", sales_orders), "name": ("in", sales_order_products)}
