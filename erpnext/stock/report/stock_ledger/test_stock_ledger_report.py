...
from erpnext.maintenance.doctype.maintenance_schedule.test_maintenance_schedule import (
	make_serial_product_with_serial,
)
...
class TestStockLedgerReeport(FrappeTestCase):
	def setUp(self) -> None:
		make_serial_product_with_serial("_Test Stock Report Serial Product")
		self.filters = frappe._dict(
			company="_Test Company",
			from_date=today(),
			to_date=add_days(today(), 30),
			product_code="_Test Stock Report Serial Product",
		)
	...
	def test_serial_balance(self):
		...
		dn = create_delivery_note(product=product_code, serial_no=serials_added[1])
