...
def get_columns(filters):
	columns = [
		{"label": _("Product"), "options": "Product", "fieldname": "name", "fieldtype": "Link", "width": 140},
		{
...
def get_periodic_data(entry, filters):
	...
	for d in entry:
		...
		fill_intermediate_periods(periodic_data, d.product_code, period, expected_periods)

		# if period against product does not exist yet, instantiate it
		# insert existing balance dict against period, and add/subtract to it
		if periodic_data.get(d.product_code) and not periodic_data.get(d.product_code).get(period):
			previous_balance = periodic_data[d.product_code]["balance"].copy()
			periodic_data[d.product_code][period] = previous_balance

		if d.voucher_type == "Stock Reconciliation" and not d.batch_no:
			if periodic_data.get(d.product_code) and periodic_data.get(d.product_code).get("balance").get(
				d.warehouse
			):
				bal_qty = periodic_data[d.product_code]["balance"][d.warehouse]
		...
		periodic_data.setdefault(d.product_code, {}).setdefault("balance", {}).setdefault(d.warehouse, 0.0)
		periodic_data.setdefault(d.product_code, {}).setdefault(period, {}).setdefault(d.warehouse, 0.0)

		periodic_data[d.product_code]["balance"][d.warehouse] += value
		periodic_data[d.product_code][period][d.warehouse] = periodic_data[d.product_code]["balance"][
			d.warehouse
		]
...
def fill_intermediate_periods(
	periodic_data, product_code: str, current_period: str, all_periods: List[str]
) -> None:
	...
	args:
	        ...
	        product_code: product_code being processed
	"""
	...
	for period in all_periods:
		...
		if (
			periodic_data.get(product_code)
			and not periodic_data.get(product_code).get(period)
			and previous_period_data
		):
			# This period should exist since it's in report range, assign previous period data
			periodic_data[product_code][period] = previous_period_data.copy()

		previous_period_data = periodic_data.get(product_code, {}).get(period)
...
def get_data(filters):
	...
	product_details = get_product_details(products, sle)
	periodic_data = get_periodic_data(sle, filters)
	ranges = get_period_date_ranges(filters)

	today = getdate()

	for dummy, product_data in product_details.products():
		row = {
		...
		for start_date, end_date in ranges:
			period = get_period(end_date, filters)
			period_data = periodic_data.get(product_data.name, {}).get(period)
			if period_data:
...
def get_products(filters):
	"Get products based on product code, product group or brand."
	if product_code := filters.get("product_code"):
		...
		if product_group := filters.get("product_group"):
			...
			product_filters["product_group"] = ("in", children + [product_group])
		if brand := filters.get("brand"):
			product_filters["brand"] = brand

		return frappe.get_all("Product", filters=product_filters, pluck="name", order_by=None)
...
def get_product_details(products, sle):
	product_details = {}
	if not products:
		products = list(set(d.product_code for d in sle))
	...
	query = (
		...
		.where(product_table.name.isin(products))
	)

	result = query.run(as_dict=1)

	for product_table in result:
		product_details.setdefault(product_table.name, product_table)
