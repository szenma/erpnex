...
def get_data(filters):
	if not any([filters.warehouse, filters.product_code, filters.product_group]):
		frappe.throw(_("Any one of following filters required: warehouse, Product Code, Product Group"))
...
def get_stock_ledger_entries(filters):
	...
	if filters.product_code:
		sle_filters["product_code"] = filters.product_code
	elif filters.get("product_group"):
		product_group = filters.get("product_group")
		children = get_descendants_of("Product Group", product_group)
		product_group_filter = {"product_group": ("in", children + [product_group])}
		sle_filters["product_code"] = (
			"in",
			frappe.get_all("Product", filters=product_group_filter, pluck="name", order_by=None),
		)
...
def find_first_bad_queue(sles):
	product_warehouse_sles = {}
	for sle in sles:
		product_warehouse_sles.setdefault((sle.product_code, sle.warehouse), []).append(sle)

	data = []

	for _product_wh, sles in product_warehouse_sles.products():
