...
def get_data(report_filters):
	...
	for d in data:
		voucher_wise_dict.setdefault((d.product_code, d.warehouse), []).append(d)

	closing_date = add_days(from_date, -1)
	for key, stock_data in voucher_wise_dict.products():
		prev_stock_value = get_stock_value_on(
			posting_date=closing_date, product_code=key[0], warehouses=key[1]
		)
