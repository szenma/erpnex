...
class TestStockAgeing(FrappeTestCase):
	...
	def test_normal_inward_outward_queue(self):
		...
		self.assertTrue(slots["Flask Product"]["fifo_queue"])
		result = slots["Flask Product"]
	...
	def test_sequential_stock_reco_different_warehouse(self):
		...
		product_wise_slots, product_wh_wise_slots = generate_product_and_product_wh_wise_slots(
			filters=self.filters, sle=sle
		)

		# test without 'show_warehouse_wise_stock'
		product_result = product_wise_slots["Flask Product"]
		queue = product_result["fifo_queue"]

		self.assertEqual(product_result["qty_after_transaction"], product_result["total_qty"])
		self.assertEqual(product_result["total_qty"], 1390.0)
		self.assertEqual(queue[0][0], 990.0)
		self.assertEqual(queue[1][0], 400.0)

		# test with 'show_warehouse_wise_stock' checked
		product_wh_balances = [
			product_wh_wise_slots.get(i).get("qty_after_transaction") for i in product_wh_wise_slots
		]
		self.assertEqual(sum(product_wh_balances), product_result["qty_after_transaction"])

	def test_repack_entry_same_product_split_rows(self):
		"""
		Split consumption rows and have single repacked product row (same warehouse).
		Ledger:
		Product	| Qty | Voucher
		------------------------
		Product 1  | 500 | 001
		Product 1  | -50 | 002 (repack)
		Product 1  | -50 | 002 (repack)
		Product 1  | 100 | 002 (repack)

		Case most likely for batch products. Test time bucket computation.
		"""
		sle = [
			frappe._dict(  # stock up product
				name="Flask Product",
				actual_qty=500,
		...
		slots = FIFOSlots(self.filters, sle).generate()
		product_result = slots["Flask Product"]
		queue = product_result["fifo_queue"]

		self.assertEqual(product_result["total_qty"], 500.0)
	...
	def test_repack_entry_same_product_overconsume(self):
		"""
		Over consume product and have less repacked product qty (same warehouse).
		Ledger:
		Product	| Qty  | Voucher
		------------------------
		Product 1  | 500  | 001
		...
		self.assertEqual(product_result["total_qty"], 450.0)
		self.assertEqual(queue[0][0], 400.0)
		self.assertEqual(queue[1][0], 50.0)
		# check if time buckets add up to balance qty
		self.assertEqual(sum([i[0] for i in queue]), 450.0)

	def test_repack_entry_same_product_overconsume_with_split_rows(self):
		"""
		Over consume product and have less repacked product qty (same warehouse).
		Ledger:
		Product	| Qty  | Voucher
		...
		self.assertEqual(product_result["total_qty"], -30.0)
		self.assertEqual(queue[0][0], -30.0)

		# check transfer bucket
		transfer_bucket = fifo_slots.transferred_product_details[("002", "Flask Product", "WH 1")]
		self.assertEqual(transfer_bucket[0][0], 50)

	def test_repack_entry_same_product_overproduce(self):
		"""
		Under consume product and have more repacked product qty (same warehouse).
		Ledger:
		Product	| Qty  | Voucher
		------------------------
		Product 1  | 500  | 001
		...
		self.assertEqual(product_result["total_qty"], 550.0)
	...
	def test_repack_entry_same_product_overproduce_with_split_rows(self):
		"""
		Over consume product and have less repacked product qty (same warehouse).
		Ledger:
		Product	| Qty  | Voucher
		...
		self.assertEqual(product_result["total_qty"], 70.0)
		self.assertEqual(queue[0][0], 20.0)
		self.assertEqual(queue[1][0], 50.0)
	...
	def test_negative_stock_same_voucher(self):
		...
		# check transfer bucket
		transfer_bucket = fifo_slots.transferred_product_details[("001", "Flask Product", "WH 1")]
		self.assertEqual(transfer_bucket[0][0], 20)
		self.assertEqual(transfer_bucket[1][0], 50)
		self.assertEqual(product_result["fifo_queue"][0][0], -70.0)
		...
		transfer_bucket = fifo_slots.transferred_product_details[("001", "Flask Product", "WH 1")]
		self.assertFalse(transfer_bucket)
		self.assertEqual(product_result["fifo_queue"][0][0], 10.0)
...
def generate_product_and_product_wh_wise_slots(filters, sle):
	"Return results with and without 'show_warehouse_wise_stock'"
	product_wise_slots = FIFOSlots(filters, sle).generate()

	filters.show_warehouse_wise_stock = True
	product_wh_wise_slots = FIFOSlots(filters, sle).generate()
	filters.show_warehouse_wise_stock = False

	return product_wise_slots, product_wh_wise_slots
