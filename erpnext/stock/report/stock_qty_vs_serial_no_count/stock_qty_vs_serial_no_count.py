...
def get_data(warehouse):
	serial_product_list = frappe.get_all(
		"Product",
		filters={
			"has_serial_no": True,
		},
	...
	data = []
	for product in serial_product_list:
		total_serial_no = frappe.db.count(
			"Serial No",
			filters={"product_code": product.product_code, "status": ("in", status_list), "warehouse": warehouse},
		)

		actual_qty = frappe.db.get_value(
			"Bin", fieldname=["actual_qty"], filters={"warehouse": warehouse, "product_code": product.product_code}
		)
