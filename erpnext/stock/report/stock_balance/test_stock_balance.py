...
class TestStockBalance(FrappeTestCase):
	...
	def assertInvariants(self, rows):
		last_balance = frappe.db.sql(
			...
			WITH last_balances AS (
				SELECT product_code, warehouse,
					...
					ROW_NUMBER() OVER (PARTITION BY product_code, warehouse
		...
		product_wh_stock = _dict()

		for line in last_balance:
			product_wh_stock.setdefault((line.product_code, line.warehouse), line)
		...
		for row in rows:
			...
			# check against SLE
			last_sle = product_wh_stock[(row.product_code, row.warehouse)]
	...
	def test_basic_stock_balance(self):
		"""Check very basic functionality and product info"""
		rows = stock_balance(self.filters)
		self.assertEqual(rows, [])

		self.generate_stock_ledger(self.product.name, [_dict(qty=5, rate=10)])
		...
		self.assertPartialDictEq(
			{
				...
				"stock_uom": self.product.stock_uom,
				"in_qty": 5,
				"in_val": 50,
				"val_rate": 10,
			},
	...
	def test_uom_converted_info(self):
		...
		self.product.append("uoms", {"conversion_factor": 5, "uom": "Box"})
		self.product.save()
	...
	def test_product_group(self):
		self.filters.pop("product_code", None)
		rows = stock_balance(self.filters.update({"product_group": self.product.product_group}))
		self.assertTrue(all(r.product_group == self.product.product_group for r in rows))

	def test_child_warehouse_balances(self):
		# This is default
		self.generate_stock_ledger(self.product.name, [_dict(qty=5, rate=10, to_warehouse="Stores - _TC")])

		self.filters.pop("product_code", None)
		rows = stock_balance(self.filters.update({"warehouse": "All Warehouses - _TC"}))

		self.assertTrue(
			any(r.product_code == self.product.name and r.warehouse == "Stores - _TC" for r in rows),
			msg=f"Expected child warehouse balances \n{rows}",
		)

	def test_show_product_attr(self):
		from erpnext.controllers.product_variant import create_variant

		self.product.has_variants = True
		self.product.append("attributes", {"attribute": "Test Size"})
		self.product.save()

		attributes = {"Test Size": "Large"}
		variant = create_variant(self.product.name, attributes)
		variant.save()

		self.generate_stock_ledger(variant.name, [_dict(qty=5, rate=10)])
		rows = stock_balance(
			self.filters.update({"show_variant_attributes": 1, "product_code": variant.name})
		)
