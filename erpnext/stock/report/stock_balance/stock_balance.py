...
class StockBalanceReport(object):
	...
	def prepare_new_data(self):
		...
		if self.filters.get("show_stock_ageing_data"):
			...
			product_wise_fifo_queue = FIFOSlots(self.filters, self.sle_entries).generate()

		_func = productgetter(1)

		self.product_warehouse_map = self.get_product_warehouse_map()

		variant_values = {}
		if self.filters.get("show_variant_attributes"):
			variant_values = self.get_variant_values_for()

		for key, report_data in self.product_warehouse_map.products():
			if variant_data := variant_values.get(report_data.product_code):
			...
			if self.filters.get("show_stock_ageing_data"):
				...
				fifo_queue = []
				if fifo_queue := product_wise_fifo_queue.get((report_data.product_code, report_data.warehouse)):
	...
	def get_product_warehouse_map(self):
		product_warehouse_map = {}
		self.opening_vouchers = self.get_opening_vouchers()

		for entry in self.sle_entries:
			group_by_key = self.get_group_by_key(entry)
			if group_by_key not in product_warehouse_map:
				self.initialize_data(product_warehouse_map, group_by_key, entry)

			self.prepare_product_warehouse_map(product_warehouse_map, entry, group_by_key)

			if self.opening_data.get(group_by_key):
				del self.opening_data[group_by_key]

		for group_by_key, entry in self.opening_data.products():
			if group_by_key not in product_warehouse_map:
				self.initialize_data(product_warehouse_map, group_by_key, entry)

		product_warehouse_map = filter_products_with_no_transactions(
			product_warehouse_map, self.float_precision, self.inventory_dimensions
		)

		return product_warehouse_map

	def prepare_product_warehouse_map(self, product_warehouse_map, entry, group_by_key):
		qty_dict = product_warehouse_map[group_by_key]
	...
	def initialize_data(self, product_warehouse_map, group_by_key, entry):
		opening_data = self.opening_data.get(group_by_key, {})

		product_warehouse_map[group_by_key] = frappe._dict(
			{
	...
	def get_group_by_key(self, row) -> tuple:
		group_by_key = [row.company, row.product_code, row.warehouse]
	...
	def get_closing_balance(self) -> List[Dict[str, Any]]:
		...
		for fieldname in ["warehouse", "product_code", "product_group", "warehouse_type"]:
			if self.filters.get(fieldname):
	...
	def prepare_stock_ledger_entries(self):
		...
		query = (
			frappe.qb.from_(sle)
			.inner_join(product_table)
			.on(sle.product_code == product_table.name)
			.select(
				sle.product_code,
				sle.warehouse,
				sle.posting_date,
				...
				sle.product_code.as_("name"),
				sle.voucher_no,
				sle.stock_value,
				sle.batch_no,
				sle.serial_no,
				product_table.product_group,
				product_table.stock_uom,
		...
		query = self.apply_products_filters(query, product_table)
	...
	def apply_products_filters(self, query, product_table) -> str:
		if product_group := self.filters.get("product_group"):
			children = get_descendants_of("Product Group", product_group, ignore_permissions=True)
			query = query.where(product_table.product_group.isin(children + [product_group]))

		for field in ["product_code", "brand"]:
			if not self.filters.get(field):
				continue

			query = query.where(product_table[field] == self.filters.get(field))
	...
	def get_columns(self):
		columns = [
			...
			{"label": _("Product Name"), "fieldname": "product_name", "width": 150},
			{
	...
	def add_additional_uom_columns(self):
		...
		conversion_factors = self.get_productwise_conversion_factor()
		add_additional_uom_columns(self.columns, self.data, self.filters.include_uom, conversion_factors)

	def get_productwise_conversion_factor(self):
		products = []
		if self.filters.product_code or self.filters.product_group:
			products = [d.product_code for d in self.data]
		...
		query = (
			...
			.where((table.parenttype == "Product") & (table.uom == self.filters.include_uom))
		)

		if products:
			query = query.where(table.parent.isin(products))
	...
	def get_variant_values_for(self):
		"""Returns variant values for products."""
		attribute_map = {}
		products = []
		if self.filters.product_code or self.filters.product_group:
			products = [d.product_code for d in self.data]
...
def filter_products_with_no_transactions(
	...
	for group_by_key in iwb_map:
		...
		for key, val in qty_dict.products():
...
def get_variants_attributes() -> List[str]:
	"""Return all product variant attributes."""
	return frappe.get_all("Product Attribute", pluck="name")
