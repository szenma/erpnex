...
def get_data(
	...
	filters = get_filters(product_code, warehouse, parent_warehouse, company)
...
def get_filters(product_code=None, warehouse=None, parent_warehouse=None, company=None):
	filters = [["disable", "=", 0]]
	if product_code:
...
def get_warehouse_capacity_data(filters, start):
	capacity_data = frappe.db.get_all(
		"Putaway Rule",
		fields=["product_code", "warehouse", "stock_capacity", "company"],
		filters=filters,
		limit_start=start,
	...
	for entry in capacity_data:
		balance_qty = get_stock_balance(entry.product_code, entry.warehouse, nowdate()) or 0
		entry.update(
			{
