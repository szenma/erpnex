...
erpnext.stock.ProductDashboard = class ProductDashboard {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	make() {
		...
		this.content = $(frappe.render_template('product_dashboard')).appendTo(this.parent);
		...
		this.content.on('click', '.btn-edit', function () {
			let product = unescape($(this).attr('data-product'));
		...
		function handle_move_add(element, action) {
			let product = unescape(element.attr('data-product'));
			...
			if (disable_quick_entry) {
				open_stock_entry(product, warehouse, entry_type);
			} else {
				if (action === "Add") {
					let rate = unescape($(this).attr('data-rate'));
					erpnext.stock.move_product(product, null, warehouse, actual_qty, rate, function () {
						me.refresh();
					});
				} else {
					erpnext.stock.move_product(product, warehouse, null, actual_qty, null, function () {
						me.refresh();
					});
				}
			}
		...
		function open_stock_entry(product, warehouse, entry_type) {
			frappe.model.with_doctype('Stock Entry', function () {
				...
				var row = frappe.model.add_child(doc, 'products');
	...
	render(data) {
		...
		if (this.page_name === "warehouse-capacity-summary") {
			...
			context = this.get_product_dashboard_data(data, this.max_count, true);
		}
	...
	get_product_dashboard_data(data, max_count, show_product) {
		...
		return {
			...
			max_count: max_count,
			can_write: can_write,
			show_product: show_product || false
		};
	}
...
erpnext.stock.move_product = function (product, source, target, actual_qty, rate, callback) {
	var dialog = new frappe.ui.Dialog({
		title: target ? __('Add Product') : __('Move Product'),
		fields: [{
	...
	dialog.show();
	dialog.get_field('product_code').set_input(product);
	...
	dialog.set_primary_action(__('Create Stock Entry'), function () {
		...
		frappe.model.with_doctype('Stock Entry', function () {
			...
			let row = frappe.model.add_child(doc, 'products');
			row.product_code = dialog.get_value('product_code');
