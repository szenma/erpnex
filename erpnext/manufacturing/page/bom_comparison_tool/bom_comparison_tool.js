...
erpnext.BOMComparisonTool = class BOMComparisonTool {
	...
	render(doctype, name1, name2, diff) {
		...
		let row_changes_by_fieldname = group_products(diff.row_changed, change => change[0]);
		...
		let table_changes = Object.keys(row_changes_by_fieldname).map(fieldname => {
			...
			let html = changes.map(change => {
				let [fieldname,, product_code, changes] = change;
				let df = frappe.meta.get_docfield(doctype, fieldname);
				...
				return values_changed.map((change, i) => {
					let [fieldname, value1, value2] = change;
					let th = i === 0
						? `<th rowspan="${values_changed.length}">${product_code}</th>`
						: '';
					return `
						<tr>
							${th}
		...
		let get_added_removed_html = (title, grouped_products) => {
			return Object.keys(grouped_products).map(fieldname => {
				let rows = grouped_products[fieldname];
				let df = frappe.meta.get_docfield(doctype, fieldname);
		...
		let added_by_fieldname = group_products(diff.added, change => change[0]);
		let removed_by_fieldname = group_products(diff.removed, change => change[0]);
...
function group_products(array, fn) {
