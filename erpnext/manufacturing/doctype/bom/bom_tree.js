frappe.treeview_settings["BOM"] = {
	...
	get_label: function(node) {
		if(node.data.qty) {
			return node.data.qty + " x " + node.data.product_code;
		} else {
			return node.data.product_code || node.data.value;
		}
	},
	onload: function(me) {
	...
	onrender: function(node) {
		if(node.is_root && node.data.value!="BOM") {
			frappe.model.with_doc("BOM", node.data.value, function() {
				...
				node.data.product_code = bom.product || "";
			});
		}
	},
	view_template: 'bom_product_preview'
}
