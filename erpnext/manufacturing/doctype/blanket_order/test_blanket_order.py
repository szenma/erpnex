...
class TestBlanketOrder(FrappeTestCase):
	...
	def test_sales_order_creation(self):
		...
		so.products[0].qty = 10
		so.submit()

		self.assertEqual(so.doctype, "Sales Order")
		self.assertEqual(len(so.get("products")), len(bo.get("products")))

		# check the rate, quantity and updation for the ordered quantity
		self.assertEqual(so.products[0].rate, bo.products[0].rate)

		bo = frappe.get_doc("Blanket Order", bo.name)
		self.assertEqual(so.products[0].qty, bo.products[0].ordered_qty)

		# test the quantity
		frappe.flags.args.doctype = "Sales Order"
		so1 = make_order(bo.name)
		so1.currency = get_company_currency(so1.company)
		self.assertEqual(so1.products[0].qty, (bo.products[0].qty - bo.products[0].ordered_qty))
	...
	def test_purchase_order_creation(self):
		...
		po.products[0].qty = 10
		po.submit()

		self.assertEqual(po.doctype, "Purchase Order")
		self.assertEqual(len(po.get("products")), len(bo.get("products")))

		# check the rate, quantity and updation for the ordered quantity
		self.assertEqual(po.products[0].rate, po.products[0].rate)

		bo = frappe.get_doc("Blanket Order", bo.name)
		self.assertEqual(po.products[0].qty, bo.products[0].ordered_qty)

		# test the quantity
		frappe.flags.args.doctype = "Purchase Order"
		po1 = make_order(bo.name)
		po1.currency = get_company_currency(po1.company)
		self.assertEqual(po1.products[0].qty, (bo.products[0].qty - bo.products[0].ordered_qty))
