...
class TestRouting(FrappeTestCase):
	@classmethod
	def setUpClass(cls):
		cls.product_code = "Test Routing Product - A"

	@classmethod
	def tearDownClass(cls):
		frappe.db.sql("delete from tabBOM where product=%s", cls.product_code)
	...
	def test_sequence_id(self):
		...
		bom_doc = setup_bom(product_code=self.product_code, routing=routing_doc.name)
		wo_doc = make_wo_order_test_record(production_product=self.product_code, bom_no=bom_doc.name)
	...
	def test_update_bom_operation_time(self):
		...
		bom_doc = setup_bom(product_code="_Testing Product", routing=routing_doc.name, currency="INR")
...
def setup_bom(**args):
	...
	if not frappe.db.exists("Product", args.product_code):
		make_product(args.product_code, {"is_stock_product": 1})

	if not args.raw_materials:
		if not frappe.db.exists("Product", "Test Extra Product N-1"):
			make_product(
				"Test Extra Product N-1",
				{
					"is_stock_product": 1,
				},
			)

		args.raw_materials = ["Test Extra Product N-1"]

	name = frappe.db.get_value("BOM", {"product": args.product_code}, "name")
	if not name:
		bom_doc = make_bom(
