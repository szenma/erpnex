...
frappe.ui.form.on('Production Plan', {
	...
	before_save(frm) {
		// preserve temporary names on production plan product to re-link sub-assembly products
		frm.doc.po_products.forEach(product => {
			product.temporary_name = product.name;
		});
	},
	setup(frm) {
		...
		frm.fields_dict['po_products'].grid.get_field('warehouse').get_query = function(doc) {
			return {
				filters: {
					company: doc.company
				}
		...
		frm.fields_dict['po_products'].grid.get_field('product_code').get_query = function(doc) {
			return {
		...
		frm.fields_dict['po_products'].grid.get_field('bom_no').get_query = function(doc, cdt, cdn) {
			var d = locals[cdt][cdn];
			if (d.product_code) {
				return {
					query: "erpnext.controllers.queries.bom",
					filters:{'product': cstr(d.product_code), 'docstatus': 1}
				}
			} else frappe.msgprint(__("Please enter Product first"));
		}

		frm.fields_dict['mr_products'].grid.get_field('warehouse').get_query = function(doc) {
			return {
				filters: {
					company: doc.company
				}
	...
	refresh(frm) {
		if (frm.doc.docstatus === 1) {
			...
			if (frm.doc.status !== "Completed") {
				...
				if (frm.doc.po_products && frm.doc.status !== "Closed") {
					frm.add_custom_button(__("Work Order / Subcontract PO"), ()=> {
						frm.trigger("make_work_order");
					}, __('Create'));
				}

				if (frm.doc.mr_products && frm.doc.mr_products.length && !in_list(['Material Requested', 'Closed'], frm.doc.status)) {
				...
				<h3 style="text-decoration: underline;">
					...
					<div>
						<ul>
							...
							<li>
								${__('Reserved Qty for Production: Raw materials quantity to make manufacturing products.')}
							</li>
							<li>
								${__('Reserved Qty for Subcontract: Raw materials quantity to make subcontracted products.')}
							</li>
						</ul>
					</div>
				</div>
	...
	get_products(frm) {
		...
		frappe.call({
			...
			doc: frm.doc,
			callback: function () {
				refresh_field('po_products');
			}
		});
	},
	combine_products(frm) {
		frm.clear_table("prod_plan_references");
		...
		frappe.call({
			...
			doc: frm.doc,
			callback: function() {
				frm.refresh_field("po_products");
				if (frm.doc.sub_assembly_products.length > 0) {
					frm.trigger("get_sub_assembly_products");
				}
			}
		});
	},

	combine_sub_products(frm) {
		if (frm.doc.sub_assembly_products.length > 0) {
			frm.clear_table("sub_assembly_products");
			frm.trigger("get_sub_assembly_products");
		}
	},

	get_sub_assembly_products(frm) {
		frm.dirty();

		frappe.call({
			method: "get_sub_assembly_products",
			freeze: true,
			doc: frm.doc,
			callback: function() {
				refresh_field("sub_assembly_products");
			}
		});
	},
	...
	get_products_for_mr(frm) {
		if (!frm.doc.for_warehouse) {
			frm.trigger("toggle_for_warehouse");
			frappe.throw(__("Select the Warehouse"));
		}

		frm.events.get_products_for_material_requests(frm, [{
	...
	transfer_materials(frm) {
		...
		if (frm.doc.ignore_existing_ordered_qty) {
			...
			dialog.set_primary_action(__("Get Products"), () => {
				let warehouses = dialog.get_values().warehouses;
				frm.events.get_products_for_material_requests(frm, warehouses);
				dialog.hide();
			});
		}
	},

	get_products_for_material_requests(frm, warehouses) {
		frappe.call({
			method: "erpnext.manufacturing.doctype.production_plan.production_plan.get_products_for_material_requests",
			freeze: true,
			args: {
				doc: frm.doc,
			...
			callback: function(r) {
				if(r.message) {
					frm.set_value('mr_products', []);
					r.message.forEach(row => {
						let d = frm.add_child('mr_products');
	...
	show_progress(frm) {
		...
		// produced qty
		let product_wise_qty = {};
		frm.doc.po_products.forEach((data) => {
			if(!product_wise_qty[data.product_code]) {
				product_wise_qty[data.product_code] = data.produced_qty;
			} else {
				product_wise_qty[data.product_code] += data.produced_qty;
			}
		})
		...
		if (product_wise_qty) {
			for (var key in product_wise_qty) {
				title += __('Product {0}: {1} qty produced. ', [key, product_wise_qty[key]]);
			}
		}
...
frappe.ui.form.on("Production Plan Product", {
	product_code(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.product_code) {
			frappe.call({
				method: "erpnext.manufacturing.doctype.production_plan.production_plan.get_product_data",
				args: {
...
frappe.ui.form.on("Material Request Plan Product", {
	warehouse(frm, cdt, cdn) {
		const row = locals[cdt][cdn];
		if (row.warehouse && row.product_code && frm.doc.company) {
			frappe.call({
...
frappe.tour['Production Plan'] = [
	{
		...
		title: "Get Products From",
		description: __("Select whether to get products from a Sales Order or a Material Request. For now select <b>Sales Order</b>.\n A Production Plan can also be created manually where you can select the Products to manufacture.")
	},
	{
	...
	},
	{
		...
		description: __("Click on 'Get Finished Goods for Manufacture' to fetch the products from the above Sales Orders. Products only for which a BOM is present will be fetched.")
	},
	{
		fieldname: "po_products",
		title: "Finished Goods",
		description: __("On expanding a row in the Products to Manufacture table, you'll see an option to 'Include Exploded Products'. Ticking this includes raw materials of the sub-assembly products in the production process.")
	},
	{
		fieldname: "include_non_stock_products",
		title: "Include Non Stock Products",
		description: __("To include non-stock products in the material request planning. i.e. Products for which 'Maintain Stock' checkbox is unticked.")
	},
	{
		fieldname: "include_subcontracted_products",
		title: "Include Subcontracted Products",
		description: __("To add subcontracted Product's raw materials if include exploded products is disabled.")
	}
];
