...
frappe.ui.form.on('Job Card', {
	...
	refresh: function(frm) {
		...
		let has_products = frm.doc.products && frm.doc.products.length;
		...
		if (!frm.is_new() && has_products && frm.doc.docstatus < 2) {
			...
			// in case of multiple products in JC
			let to_transfer = frm.doc.products.some((row) => row.transferred_qty < row.required_qty);
		...
		frm.set_query("quality_inspection", function() {
			return {
				...
				filters: {
					"product_code": frm.doc.production_product,
		...
		if (frm.doc.docstatus == 0 && !frm.is_new() &&
			...
			&& (frm.doc.products || !frm.doc.products.length || frm.doc.for_quantity == frm.doc.transferred_qty)) {
			...
			if (frm.doc.work_order) {
				frappe.db.get_value('Work Order', frm.doc.work_order, ['skip_transfer', 'status'], (result) => {
					if (result.skip_transfer === 1 || result.status == 'In Process' || frm.doc.transferred_qty > 0 || !frm.doc.products.length) {
		...
		if (frm.doc.work_order) {
			frappe.db.get_value('Work Order', frm.doc.work_order,
				...
				if (r.message.transfer_material_against == 'Work Order') {
					frm.set_df_property('products', 'hidden', 1);
				}
			});
		}
	},
	...
	setup_quality_inspection: function(frm) {
		...
		quality_inspection_field.get_route_options_for_new_doc = function(frm) {
			return  {
				...
				"product_serial_no": frm.doc.serial_no,
