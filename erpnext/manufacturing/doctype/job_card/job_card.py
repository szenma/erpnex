...
class JobCard(Document):
	...
	def get_required_products(self):
		...
		for d in doc.required_products:
			if not d.operation:
				frappe.throw(
					_("Row {0} : Operation is required against the raw material product {1}").format(
						d.idx, d.product_code
					)
				)
			...
			if self.get("operation") == d.operation:
				self.append(
					"products",
					{
						...
						"uom": frappe.db.get_value("Product", d.product_code, "stock_uom"),
	...
	def validate_transfer_qty(self):
		if self.products and self.transferred_qty < self.for_quantity:
			frappe.throw(
				_(
	...
	def validate_produced_quantity(self, for_quantity, process_loss_qty, wo):
		...
		if wo.produced_qty > for_quantity + process_loss_qty:
			first_part_msg = _(
				...
				frappe.bold(_("Job Card")), frappe.bold(self.name), frappe.bold(self.production_product)
			)
	...
	def set_transferred_qty_in_job_card_product(self, ste_doc):
		def _get_job_card_products_transferred_qty(ste_doc):
			from frappe.query_builder.functions import Sum

			job_card_products_transferred_qty = {}
			job_card_products = [
				x.get("job_card_product") for x in ste_doc.get("products") if x.get("job_card_product")
			]

			if job_card_products:
				...
				query = (
					...
					.select(sed.job_card_product, Sum(sed.qty))
					.where(
						(sed.job_card_product.isin(job_card_products))
						& (se.docstatus == 1)
						& (se.purpose == "Material Transfer for Manufacture")
					)
					.groupby(sed.job_card_product)
				)

				job_card_products_transferred_qty = frappe._dict(query.run(as_list=True))

			return job_card_products_transferred_qty

		def _validate_over_transfer(row, transferred_qty):
			"Block over transfer of products if not allowed in settings."
			required_qty = frappe.db.get_value("Job Card Product", row.job_card_product, "required_qty")
			is_excess = flt(transferred_qty) > flt(required_qty)
			if is_excess:
				frappe.throw(
					_(
						"Row #{0}: Cannot transfer more than Required Qty {1} for Product {2} against Job Card {3}"
					).format(
						row.idx, frappe.bold(required_qty), frappe.bold(row.product_code), ste_doc.job_card
					),
					title=_("Excess Transfer"),
					exc=JobCardOverTransferError,
				)

		job_card_products_transferred_qty = _get_job_card_products_transferred_qty(ste_doc) or {}
		allow_excess = frappe.db.get_single_value("Manufacturing Settings", "job_card_excess_transfer")

		for row in ste_doc.products:
			if not row.job_card_product:
				continue

			transferred_qty = flt(job_card_products_transferred_qty.get(row.job_card_product, 0.0))

			if not allow_excess:
				_validate_over_transfer(row, transferred_qty)

			frappe.db.set_value("Job Card Product", row.job_card_product, "transferred_qty", flt(transferred_qty))
	...
	def set_status(self, update_status=False):
		...
		if self.docstatus < 2:
			...
			if self.docstatus == 1 and (self.for_quantity <= self.total_completed_qty or not self.products):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			"Job Card Product": {
				...
				"field_map": {"required_qty": "qty", "uom": "stock_uom", "name": "job_card_product"},
...
def make_stock_entry(source_name, target_doc=None):
	...
	def set_missing_values(source, target):
		...
		wo_allows_alternate_product = frappe.db.get_value(
			"Work Order", target.work_order, "allow_alternative_product"
		)
		for product in target.products:
			product.allow_alternative_product = int(
				wo_allows_alternate_product
				and frappe.get_cached_value("Product", product.product_code, "allow_alternative_product")
			)
