...
class TestJobCard(FrappeTestCase):
	...
	def generate_required_stock(self, work_order: WorkOrder) -> None:
		"""Create twice the stock for all required products in work order."""
		for product in work_order.required_products:
			make_stock_entry(
				product_code=product.product_code,
				target=product.source_warehouse or self.source_warehouse,
				qty=product.required_qty * 2,
				basic_rate=100,
			)
	...
	def test_job_card_overlap(self):
		wo2 = make_wo_order_test_record(product="_Test FG Product 2", qty=2)
	...
	def test_job_card_multiple_materials_transfer(self):
		...
		del transfer_entry_1.products[1]  # transfer only 1 of 2 RMs
		...
		del transfer_entry_2.products[0]
	...
	def test_job_card_excess_material_transfer(self):
		...
		transfer_entry_2.products[0].qty = 5
		transfer_entry_2.products[1].qty = 3
		transfer_entry_2.insert()
		transfer_entry_2.submit()
	...
	def test_job_card_excess_material_transfer_with_no_reference(self):
		...
		# fully transfer both RMs
		transfer_entry_1 = make_stock_entry_from_jc(job_card_name)
		row = transfer_entry_1.products[0]

		# Add new row without reference of the job card product
	...
	def test_job_card_partial_material_transfer(self):
		...
		transfer_entry.get_products()
		transfer_entry.insert()
		transfer_entry.submit()

		job_card.reload()
		self.assertEqual(job_card.transferred_qty, 1)
		self.assertEqual(transfer_entry.products[0].qty, 5)
		self.assertEqual(transfer_entry.products[1].qty, 3)

		# transfer remaining
		transfer_entry_2 = make_stock_entry_from_jc(job_card.name)

		self.assertEqual(transfer_entry_2.fg_completed_qty, 1)
		self.assertEqual(transfer_entry_2.products[0].qty, 5)
		self.assertEqual(transfer_entry_2.products[1].qty, 3)
	...
	def test_job_card_material_transfer_correctness(self):
		"""
		1. Test if only current Job Card Products are pulled in a Stock Entry against a Job Card
		...
		self.assertEqual(len(job_card.products), 1)
		self.assertEqual(job_card.products[0].product_code, "_Test Product")

		# check if right products are mapped in transfer entry
		transfer_entry = make_stock_entry_from_jc(job_card_name)
		transfer_entry.insert()

		self.assertEqual(len(transfer_entry.products), 1)
		self.assertEqual(transfer_entry.products[0].product_code, "_Test Product")
		self.assertEqual(transfer_entry.products[0].qty, 4)

		# change 'For Qty' and check impact on products table
		# no.of products should be the same with qty change
		transfer_entry.fg_completed_qty = 2
		transfer_entry.get_products()

		self.assertEqual(len(transfer_entry.products), 1)
		self.assertEqual(transfer_entry.products[0].product_code, "_Test Product")
		self.assertEqual(transfer_entry.products[0].qty, 2)
	...
	def test_job_card_proccess_qty_and_completed_qty(self):
		...
		product_code = "Test Job Card Process Qty Product"
		for product in [product_code, product_code + "RM 1", product_code + "RM 2"]:
		...
		bom_doc = setup_bom(
			...
			raw_materials=[product_code + "RM 1", product_code + "RM 2"],
			source_warehouse=warehouse,
		)

		for row in bom_doc.products:
			make_stock_entry(
				product_code=row.product_code,
		...
		wo_doc = make_wo_order_test_record(
			production_product=product_code,
			bom_no=bom_doc.name,
			skip_transfer=1,
...
def make_wo_with_transfer_against_jc():
	...
	work_order.required_products[0].operation = "Test Operation A"
	work_order.required_products[1].operation = "_Test Operation 1"
	work_order.submit()
