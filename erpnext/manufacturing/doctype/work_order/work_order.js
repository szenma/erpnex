...
frappe.ui.form.on("Work Order", {
	setup: function(frm) {
		...
		frm.set_query("source_warehouse", "required_products", function() {
			return {
				filters: {
					'company': frm.doc.company,
				}
		...
		frm.set_query("bom_no", function() {
			if (frm.doc.production_product) {
				return {
					query: "erpnext.controllers.queries.bom",
					filters: {product: cstr(frm.doc.production_product)}
				};
			} else {
				frappe.msgprint(__("Please enter Production Product first"));
			}
		});

		// Set query for FG Product
		frm.set_query("production_product", function() {
			return {
		...
		frm.set_query("operation", "required_products", function() {
			return {
	...
	source_warehouse: function(frm) {
		...
		transaction_controller.autofill_warehouse(frm.doc.required_products, "source_warehouse", frm.doc.source_warehouse);
	},
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus === 0 && !frm.is_new()) {
			...
			frm.trigger("show_progress_for_products");
		...
		if(frm.doc.required_products && frm.doc.allow_alternative_product) {
			const has_alternative = frm.doc.required_products.find(i => i.allow_alternative_product === 1);
			if (frm.doc.docstatus == 0 && has_alternative) {
				frm.add_custom_button(__('Alternate Product'), () => {
					erpnext.utils.select_alternate_products({
						frm: frm,
						child_docname: "required_products",
						warehouse_field: "source_warehouse",
						child_doctype: "Work Order Product",
	...
	add_custom_button_to_return_components: function(frm) {
		if (frm.doc.docstatus === 1 && in_list(["Closed", "Completed"], frm.doc.status)) {
			let non_consumed_products = frm.doc.required_products.filter(d =>{
				return flt(d.consumed_qty) < flt(d.transferred_qty - d.returned_qty)
			});

			if (non_consumed_products && non_consumed_products.length) {
	...
	show_progress_for_products: function(frm) {
		var bars = [];
		var message = '';
		var added_min = false;

		// produced qty
		var title = __('{0} products produced', [frm.doc.produced_qty]);
		bars.push({
			'title': title,
		...
		if(!frm.doc.skip_transfer){
			...
			if(pending_complete) {
				...
				title = __('{0} products in progress', [pending_complete]);
				bars.push({
					'title': title,
	...
	production_product: function(frm) {
		if (frm.doc.production_product) {
			frappe.call({
				method: "erpnext.manufacturing.doctype.work_order.work_order.get_product_details",
				args: {
					product: frm.doc.production_product,
				...
				callback: function(r) {
					if(r.message) {
						...
						erpnext.in_production_product_onchange = true;

						$.each(["description", "stock_uom", "project", "bom_no", "allow_alternative_product",
							"transfer_material_against", "product_name"], function(i, field) {
						...
						erpnext.in_production_product_onchange = false;
					}
				}
			});
		}
	...
	project: function(frm) {
		if(!erpnext.in_production_product_onchange && !frm.doc.bom_no) {
			frm.trigger("production_product");
		}
	},
	...
	bom_no: function(frm) {
		return frm.call({
			doc: frm.doc,
			method: "get_products_and_operations_from_bom",
			freeze: true,
			callback: function(r) {
				if(r.message["set_scrap_wh_mandatory"]){
					frm.toggle_reqd("scrap_warehouse", true);
	...
	before_submit: function(frm) {
		frm.fields_dict.required_products.grid.toggle_reqd("source_warehouse", true);
	...
	set_sales_order: function(frm) {
		if(frm.doc.production_product) {
			frappe.call({
				...
				args: { production_product: frm.doc.production_product },
				callback: function(r) {
					frm.set_query("sales_order", function() {
						erpnext.in_production_product_onchange = true;
						return {
...
frappe.ui.form.on("Work Order Product", {
	source_warehouse: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		if(!row.product_code) {
			frappe.throw(__("Please set the Product Code first"));
	...
	product_code: function(frm, cdt, cdn) {
		...
		if (row.product_code) {
			frappe.call({
				method: "erpnext.stock.doctype.product.product.get_product_details",
				args: {
					product_code: row.product_code,
					company: frm.doc.company
				},
				callback: function(r) {
					if (r.message) {
						frappe.model.set_value(cdt, cdn, {
							...
							"allow_alternative_product": r.message.allow_alternative_product,
							"include_product_in_manufacturing": r.message.include_product_in_manufacturing
						});
					}
				}
			});
...
erpnext.work_order = {
	set_custom_buttons: function(frm) {
		...
		if (doc.docstatus === 1 && !in_list(["Closed", "Completed"], doc.status)) {
			...
			if (show_start_btn) {
				let pending_to_transfer = frm.doc.required_products.some(
					product => flt(product.transferred_qty) < flt(product.required_qty)
				);
			...
			if (frm.doc.status != 'Stopped') {
				...
				if (frm.doc.__onload && frm.doc.__onload.material_consumption == 1) {
					if (flt(doc.material_transferred_for_manufacturing) > 0 || frm.doc.skip_transfer) {
						...
						var counter = 0;
						var tbl = frm.doc.required_products || [];
						var tbl_lenght = tbl.length;
						for (var i = 0, len = tbl_lenght; i < len; i++) {
							let wo_product_qty = frm.doc.required_products[i].transferred_qty || frm.doc.required_products[i].required_qty;
							if (flt(wo_product_qty) > flt(frm.doc.required_products[i].consumed_qty)) {
								counter += 1;
							}
						}
						if (counter > 0) {
...
frappe.tour['Work Order'] = [
	{
		...
		title: "Product to Manufacture",
		description: __("Select the Product to be manufactured.")
	},
	{
		fieldname: "bom_no",
		title: "BOM No",
		description: __("The default BOM for that product will be fetched by the system. You can also change the BOM.")
	},
	{
		fieldname: "qty",
		title: "Qty to Manufacture",
		description: __("Enter the quantity to manufacture. Raw material Products will be fetched only when this is set.")
	},
	{
		fieldname: "use_multi_level_bom",
		title: "Use Multi-Level BOM",
		description: __("This is enabled by default. If you want to plan materials for sub-assemblies of the Product you're manufacturing leave this enabled. If you plan and manufacture the sub-assemblies separately, you can disable this checkbox.")
	},
	{
		fieldname: "source_warehouse",
		title: "Source Warehouse",
		description: __("The warehouse where you store your raw materials. Each required product can have a separate source warehouse. Group warehouse also can be selected as source warehouse. On submission of the Work Order, the raw materials will be reserved in these warehouses for production usage.")
	},
	{
		fieldname: "fg_warehouse",
		title: "Target Warehouse",
		description: __("The warehouse where you store finished Products before they are shipped.")
	},
	{
		fieldname: "wip_warehouse",
		title: "Work-in-Progress Warehouse",
		description: __("The warehouse where your Products will be transferred when you begin production. Group Warehouse can also be selected as a Work in Progress warehouse.")
	},
	{
	...
	},
	{
		...
		description: __("All the required products (raw materials) will be fetched from BOM and populated in this table. Here you can also change the Source Warehouse for any product. And during the production, you can track transferred raw materials from this table.")
	},
	{
