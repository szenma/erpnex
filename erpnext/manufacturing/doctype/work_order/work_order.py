...
from erpnext.manufacturing.doctype.bom.bom import (
	get_bom_product_rate,
	get_bom_products_as_dict,
	validate_bom_no,
)
...
from erpnext.stock.doctype.product.product import get_product_defaults, validate_end_of_life
...
class ProductHasVariantError(frappe.ValidationError):
	pass
...
class WorkOrder(Document):
	...
	def validate(self):
		self.validate_production_product()
		if self.bom_no:
			validate_bom_no(self.production_product, self.bom_no)
		...
		self.set_required_products(reset_only_qty=len(self.get("required_products")))
	...
	def validate_sales_order(self):
		if self.sales_order:
			...
			so = frappe.db.sql(
				"""
				select so.name, so_product.delivery_date, so.project
				from `tabSales Order` so
				inner join `tabSales Order Product` so_product on so_product.parent = so.name
				left join `tabProduct Bundle Product` pk_product on so_product.product_code = pk_product.parent
				where so.name=%s and so.docstatus = 1
					and so.skip_delivery_note  = 0 and (
					so_product.product_code=%s or
					pk_product.product_code=%s )
			""",
				(self.sales_order, self.production_product, self.production_product),
				as_dict=1,
			)
			...
			if not so:
				so = frappe.db.sql(
					"""
					select
						so.name, so_product.delivery_date, so.project
					from
						`tabSales Order` so, `tabSales Order Product` so_product, `tabPacked Product` packed_product
					where so.name=%s
						and so.name=so_product.parent
						and so.name=packed_product.parent
						and so.skip_delivery_note = 0
						and so_product.product_code = packed_product.parent_product
						and so.docstatus = 1 and packed_product.product_code=%s
				""",
					(self.sales_order, self.production_product),
					as_dict=1,
				)
	...
	def validate_warehouse_belongs_to_company(self):
		...
		for d in self.get("required_products"):
	...
	def validate_work_order_against_so(self):
		...
		ordered_qty_against_so = frappe.db.sql(
			...
			where production_product = %s and sales_order = %s and docstatus < 2 and name != %s""",
			(self.production_product, self.sales_order, self.name),
		)[0][0]

		total_qty = flt(ordered_qty_against_so) + flt(self.qty)

		# get qty from Sales Order Product table
		so_product_qty = frappe.db.sql(
			"""select sum(stock_qty) from `tabSales Order Product`
			where parent = %s and product_code = %s""",
			(self.sales_order, self.production_product),
		)[0][0]
		# get qty from Packing Product table
		dnpi_qty = frappe.db.sql(
			"""select sum(qty) from `tabPacked Product`
			where parent = %s and parenttype = 'Sales Order' and product_code = %s""",
			(self.sales_order, self.production_product),
		)[0][0]
		# total qty in SO
		so_qty = flt(so_product_qty) + flt(dnpi_qty)
		...
		if total_qty > so_qty + (allowance_percentage / 100 * so_qty):
			frappe.throw(
				_("Cannot produce more Product {0} than Sales Order quantity {1}").format(
					self.production_product, so_qty
				),
				OverProductionError,
			)
	...
	def update_work_order_qty(self):
		...
		for purpose, fieldname in (
			...
			if self.sales_order and self.sales_order_product:
				update_produced_qty_in_so_product(self.sales_order, self.sales_order_product)
	...
	def update_production_plan_status(self):
		...
		produced_qty = 0
		if self.production_plan_product:
			total_qty = frappe.get_all(
				...
				filters={
					...
					"production_plan_product": self.production_plan_product,
				},
				as_list=1,
			)
		...
		production_plan.run_method(
			"update_produced_pending_qty", produced_qty, self.production_plan_product
		)
	...
	def on_submit(self):
		...
		if self.production_plan and frappe.db.exists(
			"Production Plan Product Reference", {"parent": self.production_plan}
		):
	...
	def create_serial_no_batch_no(self):
		...
		args = {"product_code": self.production_product, "work_order": self.name}
	...
	def create_batch_for_finished_good(self):
		...
		batch_auto_creation = frappe.get_cached_value("Product", self.production_product, "create_new_batch")
		if not batch_auto_creation:
			frappe.msgprint(
				_("Batch not created for product {} since it does not have a batch series.").format(
					frappe.bold(self.production_product)
				),
				alert=True,
				indicator="orange",
			)
		...
		while total_qty > 0:
			...
			make_batch(
				frappe._dict(
					{
						"product": self.production_product,
						"qty_to_produce": qty,
	...
	def make_serial_nos(self, args):
		...
		serial_no_series = frappe.get_cached_value("Product", self.production_product, "serial_no_series")
		if serial_no_series:
		...
		if serial_nos_length != self.qty:
			frappe.throw(
				_("{0} Serial Numbers required for Product {1}. You have provided {2}.").format(
					self.qty, self.production_product, serial_nos_length
				),
				SerialNoQtyError,
			)
	...
	def update_planned_qty(self):
		update_bin_qty(
			self.production_product,
			self.fg_warehouse,
			{"planned_qty": get_planned_qty(self.production_product, self.fg_warehouse)},
		)

		if self.material_request:
			mr_obj = frappe.get_doc("Material Request", self.material_request)
			mr_obj.update_requested_qty([self.material_request_product])

	def update_ordered_qty(self):
		if (
			self.production_plan
			and self.production_plan_product
			and not self.production_plan_sub_assembly_product
		):
			...
			query = (
				...
				.where(
					...
					& (table.production_plan_product == self.production_plan_product)
					& (table.docstatus == 1)
				)
			).run()
			...
			frappe.db.set_value("Production Plan Product", self.production_plan_product, "ordered_qty", qty)
	...
	def update_work_order_qty_in_so(self):
		if not self.sales_order and not self.sales_order_product:
			return

		total_bundle_qty = 1
		if self.product_bundle_product:
			total_bundle_qty = frappe.db.sql(
				""" select sum(qty) from
				`tabProduct Bundle Product` where parent = %s""",
				(frappe.db.escape(self.product_bundle_product)),
			)[0][0]

			if not total_bundle_qty:
				# product bundle is 0 (product bundle allows 0 qty for products)
				total_bundle_qty = 1

		cond = "product_bundle_product = %s" if self.product_bundle_product else "production_product = %s"
		...
		qty = frappe.db.sql(
			...
			(self.sales_order, (self.product_bundle_product or self.production_product)),
			as_list=1,
		)
	...
	def update_work_order_qty_in_combined_so(self):
		...
		product_reference = frappe.get_value(
			"Production Plan Product", self.production_plan_product, "sales_order_product"
		)

		for plan_reference in prod_plan.prod_plan_references:
			work_order_qty = 0.0
			if plan_reference.product_reference == product_reference:
				if self.docstatus == 1:
					work_order_qty = flt(plan_reference.qty) / total_bundle_qty
				frappe.db.set_value(
					"Sales Order Product", plan_reference.sales_order_product, "work_order_qty", work_order_qty
				)

	def update_completed_qty_in_material_request(self):
		if self.material_request:
			frappe.get_doc("Material Request", self.material_request).update_completed_qty(
				[self.material_request_product]
			)
	...
	def validate_production_product(self):
		if frappe.get_cached_value("Product", self.production_product, "has_variants"):
			frappe.throw(_("Work Order cannot be raised against a Product Template"), ProductHasVariantError)

		if self.production_product:
			validate_end_of_life(self.production_product)
	...
	def validate_qty(self):
		...
		if (
			...
			qty_dict = frappe.db.get_value(
				"Production Plan Product", self.production_plan_product, ["planned_qty", "ordered_qty"], as_dict=1
			)
			...
			if not max_qty > 0:
				frappe.throw(
					_("Cannot produce more product for {0}").format(self.production_product), OverProductionError
				)
			elif self.qty > max_qty:
				frappe.throw(
					_("Cannot produce more than {0} products for {1}").format(max_qty, self.production_product),
					OverProductionError,
				)
	...
	def update_required_products(self):
		"""
		update bin reserved_qty_for_production
		called from Stock Entry for production, after submit, cancel
		"""
		# calculate consumed qty based on submitted stock entries
		self.update_consumed_qty_for_required_products()

		if self.docstatus == 1:
			# calculate transferred qty based on submitted stock entries
			self.update_transferred_qty_for_required_products()
			self.update_returned_qty()

			# update in bin
			self.update_reserved_qty_for_production()

	def update_reserved_qty_for_production(self, products=None):
		"""update reserved_qty_for_production in bins"""
		for d in self.required_products:
			if d.source_warehouse:
				stock_bin = get_bin(d.product_code, d.source_warehouse)
				stock_bin.update_reserved_qty_for_production()

	@frappe.whitelist()
	def get_products_and_operations_from_bom(self):
		self.set_required_products()
	...
	def set_available_qty(self):
		for d in self.get("required_products"):
			if d.source_warehouse:
				d.available_qty_at_source_warehouse = get_latest_stock_qty(d.product_code, d.source_warehouse)

			if self.wip_warehouse:
				d.available_qty_at_wip_warehouse = get_latest_stock_qty(d.product_code, self.wip_warehouse)

	def set_required_products(self, reset_only_qty=False):
		"""set required_products for production to keep track of reserved qty"""
		...
		if self.bom_no and self.qty:
			...
			if reset_only_qty:
				for d in self.get("required_products"):
					if product_dict.get(d.product_code):
						d.required_qty = product_dict.get(d.product_code).get("qty")

					if not d.operation:
						d.operation = operation
			else:
				for product in sorted(product_dict.values(), key=lambda d: d["idx"] or float("inf")):
					self.append(
						"required_products",
						{
							"rate": product.rate,
							"amount": product.rate * product.qty,
							"operation": product.operation or operation,
							"product_code": product.product_code,
							"product_name": product.product_name,
							"description": product.description,
							"allow_alternative_product": product.allow_alternative_product,
							"required_qty": product.qty,
							"source_warehouse": product.source_warehouse or product.default_warehouse,
							"include_product_in_manufacturing": product.include_product_in_manufacturing,
						},
					)

					if not self.project:
						self.project = product.get("project")

			self.set_available_qty()

	def update_transferred_qty_for_required_products(self):
		...
		query = (
			...
			.select(
				ste_child.product_code,
				ste_child.original_product,
			...
			.groupby(ste_child.product_code)
		)

		data = query.run(as_dict=1) or []
		transferred_products = frappe._dict({d.original_product or d.product_code: d.qty for d in data})

		for row in self.required_products:
			row.db_set(
				"transferred_qty", (transferred_products.get(row.product_code) or 0.0), update_modified=False
			)
	...
	def update_returned_qty(self):
		...
		data = query.run(as_dict=1) or []
		returned_dict = frappe._dict({d.original_product or d.product_code: d.qty for d in data})

		for row in self.required_products:
			row.db_set("returned_qty", (returned_dict.get(row.product_code) or 0.0), update_modified=False)

	def update_consumed_qty_for_required_products(self):
		"""
		Update consumed qty from submitted stock entries
		against a work order for each stock product
		"""
			...
			consumed_qty = frappe.db.sql(
				...
				WHERE
					entry.work_order = %(name)s
						...
						AND (detail.product_code = %(product)s
							OR detail.original_product = %(product)s)
				""",
				{"name": self.name, "product": product.product_code},
			)[0][0]

			product.db_set("consumed_qty", flt(consumed_qty), update_modified=False)

	@frappe.whitelist()
	def make_bom(self):
		data = frappe.db.sql(
			""" select sed.product_code, sed.qty, sed.s_warehouse
		...
		bom.product = self.production_product
		bom.conversion_rate = 1

		for d in data:
			bom.append("products", {"product_code": d.product_code, "qty": d.qty, "source_warehouse": d.s_warehouse})
	...
	def update_batch_produced_qty(self, stock_entry_doc):
		...
		for row in stock_entry_doc.products:
			if row.batch_no and (row.is_finished_product or row.is_scrap_product):
				qty = frappe.get_all(
					"Stock Entry Detail",
					filters={"batch_no": row.batch_no, "docstatus": 1},
					or_filters={"is_finished_product": 1, "is_scrap_product": 1},
					fields=["sum(qty)"],
					as_list=1,
				)[0][0]
...
@frappe.whitelist()
def get_product_details(product, project=None, skip_bom_info=False):
	res = frappe.db.sql(
		"""
		select stock_uom, description, product_name, allow_alternative_product,
	...
	filters = {"product": product, "is_default": 1, "docstatus": 1}

	if project:
		filters = {"product": product, "project": project}

	res["bom_no"] = frappe.db.get_value("BOM", filters=filters)

	if not res["bom_no"]:
		variant_of = frappe.db.get_value("Product", product, "variant_of")

		if variant_of:
			res["bom_no"] = frappe.db.get_value("BOM", filters={"product": variant_of, "is_default": 1})

	if not res["bom_no"]:
		if project:
			res = get_product_details(product)
			frappe.msgprint(
				_("Default BOM not found for Product {0} and Project {1}").format(product, project), alert=1
			)
		else:
			frappe.throw(_("Default BOM for {0} not found").format(product))

	bom_data = frappe.db.get_value(
		"BOM",
		res["bom_no"],
		["project", "allow_alternative_product", "transfer_material_against", "product_name"],
		as_dict=1,
	)
...
@frappe.whitelist()
def make_work_order(bom_no, product, qty=0, project=None, variant_products=None):
	if not frappe.has_permission("Work Order", "write"):
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	product_details = get_product_details(product, project)

	wo_doc = frappe.new_doc("Work Order")
	wo_doc.production_product = product
	wo_doc.update(product_details)
	wo_doc.bom_no = bom_no

	if flt(qty) > 0:
		wo_doc.qty = flt(qty)
		wo_doc.get_products_and_operations_from_bom()

	if variant_products:
		add_variant_product(variant_products, wo_doc, bom_no, "required_products")

	return wo_doc


def add_variant_product(variant_products, wo_doc, bom_no, table_name="products"):
	if isinstance(variant_products, str):
		variant_products = json.loads(variant_products)

	for product in variant_products:
		args = frappe._dict(
			{
				"product_code": product.get("variant_product_code"),
				"required_qty": product.get("qty"),
				"qty": product.get("qty"),  # for bom
				"source_warehouse": product.get("source_warehouse"),
				"operation": product.get("operation"),
			}
		)

		bom_doc = frappe.get_cached_doc("BOM", bom_no)
		product_data = get_product_details(args.product_code, skip_bom_info=True)
		args.update(product_data)

		args["rate"] = get_bom_product_rate(
			{
		...
		if not args.source_warehouse:
			args["source_warehouse"] = get_product_defaults(
				product.get("variant_product_code"), wo_doc.company
			).default_warehouse

		args["amount"] = flt(args.get("required_qty")) * flt(args.get("rate"))
		args["uom"] = product_data.stock_uom
...
@frappe.whitelist()
def query_sales_order(production_product):
	out = frappe.db.sql_list(
		"""
		select distinct so.name from `tabSales Order` so, `tabSales Order Product` so_product
		where so_product.parent=so.name and so_product.product_code=%s and so.docstatus=1
	union
		select distinct so.name from `tabSales Order` so, `tabPacked Product` pi_product
		where pi_product.parent=so.name and pi_product.product_code=%s and so.docstatus=1
	""",
...
def create_pick_list(source_name, target_doc=None, for_qty=None):
	...
	def update_product_quantity(source, target, source_parent):
		...
		if qty:
			...
			target.uom = frappe.get_value("Product", source.product_code, "stock_uom")
...
def get_reserved_qty_for_production(
	product_code: str, warehouse: str, check_production_plan: bool = False
) -> float:
	"""Get total reserved quantity for any product in specified warehouse"""
	...
	query = (
		...
		.select(
			Sum(
				Case()
				.when(wo.skip_transfer == 0, wo_product.required_qty - wo_product.transferred_qty)
				.else_(wo_product.required_qty - wo_product.consumed_qty)
			)
		)
		.where(
			(wo_product.product_code == product_code)
			& (wo_product.parent == wo.name)
			& (wo.docstatus == 1)
			& (wo_product.source_warehouse == warehouse)
			& (wo.status.notin(["Stopped", "Completed", "Closed"]))
			& (
				(wo_product.required_qty > wo_product.transferred_qty)
				| (wo_product.required_qty > wo_product.consumed_qty)
			)
		)
	)
...
def make_stock_return_entry(work_order):
	...
	non_consumed_products = get_available_materials(work_order)
	if not non_consumed_products:
		return
	...
	stock_entry.add_transfered_raw_materials_in_products()
	stock_entry.set_stock_entry_type()
