...
class BOMUpdateLog(Document):
	...
	def validate_bom_products(self):
		current_bom_product = frappe.db.get_value("BOM", self.current_bom, "product")
		new_bom_product = frappe.db.get_value("BOM", self.new_bom, "product")

		if current_bom_product != new_bom_product:
			frappe.throw(_("The selected BOMs are not for the same product"))
