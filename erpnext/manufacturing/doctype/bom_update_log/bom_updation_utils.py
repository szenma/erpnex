...
def replace_bom(boms: Dict, log_name: str) -> None:
	...
	update_new_bom_in_bom_products(unit_cost, current_bom, new_bom)
	...
	for bom in parent_boms:
		...
		bom_obj.update_exploded_products()
...
def get_ancestor_boms(new_bom: str, bom_list: Optional[List] = None) -> List:
	...
	parents = (
		...
		.select(bom_product.parent)
		.where((bom_product.bom_no == new_bom) & (bom_product.docstatus < 2) & (bom_product.parenttype == "BOM"))
		.run(as_dict=True)
	)
...
def update_new_bom_in_bom_products(unit_cost: float, current_bom: str, new_bom: str) -> None:
	bom_product = frappe.qb.DocType("BOM Product")
	(
		frappe.qb.update(bom_product)
		.set(bom_product.bom_no, new_bom)
		.set(bom_product.rate, unit_cost)
		.set(bom_product.amount, (bom_product.stock_qty * unit_cost))
		.where(
			(bom_product.bom_no == current_bom) & (bom_product.docstatus < 2) & (bom_product.parenttype == "BOM")
		)
	).run()
...
def get_leaf_boms() -> List[str]:
	...
	return frappe.db.sql_list(
		...
		where docstatus=1 and is_active=1
			and not exists(select bom_no from `tabBOM Product`
...
def _generate_dependence_map() -> defaultdict:
	...
	bom_products = (
		frappe.qb.from_(bom_product)
		.join(bom)
		.on(bom_product.parent == bom.name)
		.select(bom_product.bom_no, bom_product.parent)
		.where(
			(bom_product.bom_no.isnotnull())
			& (bom_product.bom_no != "")
			& (bom.docstatus == 1)
			& (bom.is_active == 1)
			& (bom_product.parenttype == "BOM")
		)
	).run(as_dict=True)

	child_parent_map = defaultdict(list)
	parent_child_map = defaultdict(list)
	for row in bom_products:
...
def set_values_in_log(log_name: str, values: Dict[str, Any], commit: bool = False) -> None:
	...
	for key, value in values.products():
