...
def get_data(filters):
	...
	used_as_subassembly_products = get_bom_count(bom_data)

	for d in bom_wise_data:
		for row in bom_wise_data[d]:
			row.used_as_subassembly_products = used_as_subassembly_products.get(row.name, 0)
...
def get_filtered_data(filters):
	...
	if filters.get("product_code"):
		bom_ops_query = bom_ops_query.where(bom.product == filters.get("product_code"))
...
def get_args():
	return frappe._dict({"name": "", "product": "", "product_name": "", "uom": ""})
...
def get_columns(filters):
	return [
		...
		{"label": _("Product Name"), "fieldname": "product_name", "fieldtype": "Data", "width": 110},
		...
		{"label": _("Time (In Mins)"), "fieldname": "time_in_mins", "fieldtype": "Float", "width": 120},
		{
			...
			"fieldname": "used_as_subassembly_products",
			"fieldtype": "Int",
			"width": 200,
		},
	]
