...
class ExponentialSmoothingForecast(object):
	def forecast_future_data(self):
		for key, value in self.period_wise_data.products():
			forecast_data = []
...
class ForecastingReport(ExponentialSmoothingForecast):
	def __init__(self, filters=None):
		...
		self.child_doctype = self.doctype + " Product"
	...
	def prepare_periodical_data(self):
		...
		for entry in order_data:
			key = (entry.product_code, entry.warehouse)
	...
	def get_data_for_forecast(self):
		...
		if self.filters.product_code:
			query = query.where(child.product_code == self.filters.product_code)
	...
	def add_total(self):
		...
		total_row = {"product_code": _(frappe.bold("Total Quantity"))}
