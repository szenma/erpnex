...
class ProductionPlanReport(object):
	...
	def get_open_orders(self):
		...
		if doctype == "Work Order":
			query = (
				...
				.select(
					parent.production_product,
					parent.product_name.as_("production_product_name"),
			...
			child = frappe.qb.DocType(f"{doctype} Product")
			query = (
				frappe.qb.from_(parent)
				.from_(child)
				.select(
					...
					child.product_code.as_("production_product"),
					child.stock_qty.as_("qty_to_manufacture"),
					child.product_name.as_("production_product_name"),
				)
	...
	def get_raw_materials(self):
		...
		self.product_codes = [d.production_product for d in self.orders]
		...
		if self.filters.based_on == "Work Order":
			...
			raw_materials = (
				frappe.get_all(
					...
					fields=[
						"parent",
						"product_code",
						"product_name as raw_material_name",
			...
			for d in self.orders:
				bom_no = d.bom_no or frappe.get_cached_value("Product", d.production_product, "default_bom")
			...
			bom_product_doctype = (
				"BOM Explosion Product" if self.filters.include_subassembly_raw_materials else "BOM Product"
			)

			bom = frappe.qb.DocType("BOM")
			bom_product = frappe.qb.DocType(bom_product_doctype)

			if self.filters.include_subassembly_raw_materials:
				qty_field = bom_product.qty_consumed_per_unit
			else:
				qty_field = bom_product.qty / bom.quantity
			...
			raw_materials = (
				...
				.select(
					...
					bom_product.product_name.as_("raw_material_name"),
					qty_field.as_("required_qty_per_unit"),
				)
				.where((bom_product.parent.isin(bom_nos)) & (bom_product.parent == bom.name) & (bom.docstatus == 1))
			).run(as_dict=True)

		if not raw_materials:
			return

		self.product_codes.extend([d.product_code for d in raw_materials])
	...
	def get_product_details(self):
		if not (self.orders and self.product_codes):
			return
		...
		for d in frappe.get_all(
			...
			filters={"company": self.filters.company, "parent": ("in", self.product_codes)},
		):
			self.product_details[d.parent] = d
	...
	def get_bin_details(self):
		...
		for d in frappe.get_all(
			"Bin",
			fields=["warehouse", "product_code", "actual_qty", "ordered_qty", "projected_qty"],
			filters={"product_code": ("in", self.product_codes), "warehouse": ("in", self.warehouses)},
		):
	...
	def get_purchase_details(self):
		...
		purchased_products = frappe.get_all(
			"Purchase Order Product",
			fields=["product_code", "min(schedule_date) as arrival_date", "qty as arrival_qty", "warehouse"],
			filters={
				"product_code": ("in", self.product_codes),
				"warehouse": ("in", self.warehouses),
				"docstatus": 1,
			},
			group_by="product_code, warehouse",
		)
		for d in purchased_products:
			key = (d.product_code, d.warehouse)
	...
	def prepare_data(self):
		...
		for d in self.orders:
			...
			bin_data = self.bin_details.get((d.production_product, d.warehouse)) or {}
	...
	def update_raw_materials(self, data, key):
		...
		for d in self.raw_materials_dict.get(key):
			...
			if self.filters.based_on == "Work Order" and d.warehouse:
				...
				product_details = self.product_details.get(d.product_code)
				if product_details:
					warehouses = [product_details["default_warehouse"]]
	...
	def pick_materials_from_warehouses(self, args, order_data, warehouses):
		for index, warehouse in enumerate(warehouses):
			...
			key = (args.product_code, warehouse)
	...
	def get_columns(self):
		...
		self.columns = [
			...
			},
			{
				...
				"fieldname": "production_product_name",
				"fieldtype": "Data",
				"width": 130,
			},
			{
