...
def get_data(filters):
	...
	query = (
		frappe.qb.from_(wo)
		.select(wo.name.as_("work_order"), wo.qty, wo.produced_qty, wo.production_product, wo.bom_no)
	...
	for d in query.run(as_dict=True):
		...
		for data in frappe.get_all(
			"Work Order Product",
			fields=["product_code as raw_material_code", "required_qty", "consumed_qty"],
