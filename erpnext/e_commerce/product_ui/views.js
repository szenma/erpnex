erpnext.ProductView =  class {
	/* Options:
		...
		- Product Group: If its an Product Group page
	*/
	constructor(options) {
	...
	make(from_filters=false) {
		...
		this.get_product_filter_data(from_filters);
	}
	...
	get_product_filter_data(from_filters=false) {
		...
		frappe.call({
			...
			callback: function(result) {
				if (!result || result.exc || !result.message || result.message.exc) {
					...
					// Sub Category results are independent of Products
					if (me.product_group && result.message["sub_categories"].length) {
						me.render_product_sub_categories(result.message["sub_categories"]);
					}

					if (!result.message["products"].length) {
						// if result has no products or result is empty
						...
						// Render views
						me.render_list_view(result.message["products"], result.message["settings"]);
						me.render_grid_view(result.message["products"], result.message["settings"]);

						me.products = result.message["products"];
						me.product_count = result.message["products_count"];
					}
	...
	render_grid_view(products, settings) {
	...
	render_list_view(products, settings) {
		let me = this;
	...
	bind_view_toggler_actions() {
		$("#list").click(function() {
			...
			localStorage.setProduct("product_view", "List View");
		});
		...
		$("#image-view").click(function() {
			...
			localStorage.setProduct("product_view", "Grid View");
		});
	}
	...
	re_render_discount_filters(filter_data) {
		...
		// discount filters are rendered with Products (later)
	...
	change_route_with_filters() {
		...
		if (this.from_filters) {
			start = 0; // show products from first page if new filters are triggered
		}
	...
	render_product_sub_categories(categories) {
