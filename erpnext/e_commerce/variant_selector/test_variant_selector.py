...
class TestVariantSelector(FrappeTestCase):
	...
	def setUpClass(cls):
		...
		make_website_product(template_product)  # publish template not variants

	def test_product_attributes(self):
		"""
		Test if the right attributes are fetched in the popup.
		(Attributes must only come from active products)

		Attribute selection must not be linked to Website Products.
		"""
		...
		small_variant = frappe.get_doc("Product", "Test-Tshirt-Temp-S-R")
	...
	def test_next_product_variant_values(self):
		"""
		...
		filtered_products = next_values["filtered_products"]

		self.assertEqual(len(next_colours), 1)
		self.assertEqual(next_colours.pop(), "Red")
		self.assertEqual(len(filtered_products), 1)
		self.assertEqual(filtered_products.pop(), "Test-Tshirt-Temp-S-R")

	def test_exact_match_with_price(self):
		"""
		Test price fetching and matching of variant without Website Product
		"""
		from erpnext.e_commerce.doctype.website_product.test_website_product import make_web_product_price
		...
		make_web_product_price(product_code="Test-Tshirt-Temp-S-R", price_list_rate=100)
