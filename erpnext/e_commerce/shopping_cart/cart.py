...
from erpnext.utilities.product import get_web_product_qty_in_stock
...
def place_order():
	...
	cart_settings = frappe.db.get_value(
		"E Commerce Settings", None, ["company", "allow_products_not_in_stock"], as_dict=1
	)
	...
	if not cint(cart_settings.allow_products_not_in_stock):
		for product in sales_order.get("products"):
			product.warehouse = frappe.db.get_value(
				"Website Product", {"product_code": product.product_code}, "website_warehouse"
			)
			is_stock_product = frappe.db.get_value("Product", product.product_code, "is_stock_product")

			if is_stock_product:
				product_stock = get_web_product_qty_in_stock(product.product_code, "website_warehouse")
				if not cint(product_stock.in_stock):
					throw(_("{0} Not in Stock").format(product.product_code))
				if product.qty > product_stock.stock_qty[0][0]:
					throw(_("Only {0} in Stock for product {1}").format(product_stock.stock_qty[0][0], product.product_code))
...
@frappe.whitelist()
def update_cart(product_code, qty, additional_notes=None, with_products=False):
	quotation = _get_cart_quotation()

	empty_card = False
	qty = flt(qty)
	if qty == 0:
		quotation_products = quotation.get("products", {"product_code": ["!=", product_code]})
		if quotation_products:
			quotation.set("products", quotation_products)
		else:
			empty_card = True
		...
		if not quotation_products:
			...
			quotation_products[0].qty = qty
			quotation_products[0].additional_notes = additional_notes
	...
	if cint(with_products):
		context = get_cart_quotation(quotation)
		return {
			"products": frappe.render_template("templates/includes/cart/cart_products.html", context),
			"total": frappe.render_template("templates/includes/cart/cart_products_total.html", context),
...
def create_lead_for_product_inquiry(lead, subject, message):
...
def decorate_quotation_doc(doc):
	for d in doc.get("products", []):
		...
		fields = ["web_product_name", "thumbnail", "website_image", "description", "route"]
		...
		if not frappe.db.exists("Website Product", {"product_code": product_code}):
			variant_data = frappe.db.get_values(
				...
				fieldname=["variant_of", "product_name", "image"],
				as_dict=True,
			)[0]
			product_code = variant_data.variant_of
			fields = fields[1:]
			d.web_product_name = variant_data.product_name

			if variant_data.image:  # get image from variant or template web product
				d.thumbnail = variant_data.image
				fields = fields[2:]

		d.update(frappe.db.get_value("Website Product", {"product_code": product_code}, fields, as_dict=True))
...
def set_price_list_and_rate(quotation, cart_settings):
	...
	for product in quotation.get("products"):
		product.price_list_rate = product.discount_percentage = product.rate = product.amount = None

	# refetch values
	quotation.run_method("set_price_list_and_product_details")
