...
class TestShoppingCart(unittest.TestCase):
	...
	def setUp(self):
		...
		if not frappe.db.exists("Website Product", {"product_code": "_Test Product"}):
			make_website_product(frappe.get_cached_doc("Product", "_Test Product"))

		if not frappe.db.exists("Website Product", {"product_code": "_Test Product 2"}):
			make_website_product(frappe.get_cached_doc("Product", "_Test Product 2"))
	...
	def test_add_to_cart(self):
		...
		# add first product
		update_cart("_Test Product", 1)

		quotation = self.test_get_cart_customer()

		self.assertEqual(quotation.get("products")[0].product_code, "_Test Product")
		self.assertEqual(quotation.get("products")[0].qty, 1)
		self.assertEqual(quotation.get("products")[0].amount, 10)

		# add second product
		update_cart("_Test Product 2", 1)
		quotation = self.test_get_cart_customer()
		self.assertEqual(quotation.get("products")[1].product_code, "_Test Product 2")
		self.assertEqual(quotation.get("products")[1].qty, 1)
		self.assertEqual(quotation.get("products")[1].amount, 20)

		self.assertEqual(len(quotation.get("products")), 2)
	...
	def test_update_cart(self):
		...
		update_cart("_Test Product", 5)
		quotation = self.test_get_cart_customer()
		self.assertEqual(quotation.get("products")[0].product_code, "_Test Product")
		self.assertEqual(quotation.get("products")[0].qty, 5)
		self.assertEqual(quotation.get("products")[0].amount, 50)
	...
	def test_remove_from_cart(self):
		...
		update_cart("_Test Product", 0)
		quotation = self.test_get_cart_customer()

		self.assertEqual(quotation.get("products")[0].product_code, "_Test Product 2")
		self.assertEqual(quotation.get("products")[0].qty, 1)
		self.assertEqual(quotation.get("products")[0].amount, 20)
		self.assertEqual(quotation.net_total, 20)
		self.assertEqual(len(quotation.get("products")), 1)
	...
	def test_add_product_variant_without_web_product_to_cart(self):
		"Test adding Variants having no Website Products in cart via Template Web Product."
		...
		variant.save()
		make_website_product(template_product)  # publish template not variant
		...
		self.assertEqual(doc.get("products")[0].product_name, "Test-Tshirt-Temp-S-R")

		# test if products are rendered without error
		frappe.render_template("templates/includes/cart/cart_products.html", cart)
	...
	def create_quotation(self):
		...
		values = {
			...
			"products": [{"product_code": "_Test Product", "qty": 1}],
	...
	def enable_shopping_cart(self):
		...
		if not frappe.db.get_value(
			"Product Price", {"price_list": "_Test Price List India", "product_code": "_Test Product"}
		):
			frappe.get_doc(
				{
