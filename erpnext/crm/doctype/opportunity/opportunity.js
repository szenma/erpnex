...
frappe.ui.form.on("Opportunity", {
	...
	change_grid_labels: function(frm) {
		let company_currency = erpnext.get_currency(frm.doc.company);
		frm.set_currency_labels(["base_rate", "base_amount"], company_currency, "products");
		frm.set_currency_labels(["rate", "amount"], frm.doc.currency, "products");

		let product_grid = frm.fields_dict.products.grid;
		$.each(["base_rate", "base_amount"], function(i, fname) {
			if(frappe.meta.get_docfield(product_grid.doctype, fname))
				product_grid.set_column_disp(fname, frm.doc.currency != company_currency);
		});
		frm.refresh_fields();
	},
...
frappe.ui.form.on("Opportunity Product", {
...
cur_frm.cscript.product_code = function(doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.product_code) {
		return frappe.call({
			method: "erpnext.crm.doctype.opportunity.opportunity.get_product_details",
			args: {"product_code":d.product_code},
			callback: function(r, rt) {
				if(r.message) {
					$.each(r.message, function(k, v) {
						frappe.model.set_value(cdt, cdn, k, v);
					});
					refresh_field('image_view', d.name, 'products');
				}
			}
		})
	}
