...
class Opportunity(TransactionBase, CRMNote):
	...
	def calculate_totals(self):
		...
		for product in self.get("products"):
			product.amount = flt(product.rate) * flt(product.qty)
			product.base_rate = flt(self.conversion_rate) * flt(product.rate)
			product.base_amount = flt(self.conversion_rate) * flt(product.amount)
			total += product.amount
			base_total += product.base_amount
	...
	def has_active_quotation(self):
		if not self.get("products", []):
			...
			return frappe.db.sql(
				"""
				select q.name
				from `tabQuotation` q, `tabQuotation Product` qi
	...
	def validate_product_details(self):
		...
		# set missing values
		product_fields = ("product_name", "description", "product_group", "brand")

		for d in self.products:
			if not d.product_code:
				continue

			product = frappe.db.get_value("Product", d.product_code, product_fields, as_dict=True)
			for key in product_fields:
				if not d.get(key):
					d.set(key, product.get(key))


@frappe.whitelist()
def get_product_details(product_code):
	product = frappe.db.sql(
		"""select product_name, stock_uom, image, description, product_group, brand
		from `tabProduct` where name = %s""",
		product_code,
		as_dict=1,
	)
	return {
		"product_name": product and product[0]["product_name"] or "",
		"uom": product and product[0]["stock_uom"] or "",
		"description": product and product[0]["description"] or "",
		"image": product and product[0]["image"] or "",
		"product_group": product and product[0]["product_group"] or "",
		"brand": product and product[0]["brand"] or "",
	}
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			"Opportunity Product": {
				...
				"field_map": [["name", "opportunity_product"], ["parent", "opportunity"], ["uom", "uom"]],
...
def make_supplier_quotation(source_name, target_doc=None):
	doclist = get_mapped_doc(
		...
		source_name,
		{
			...
			"Opportunity Product": {"doctype": "Supplier Quotation Product", "field_map": {"uom": "stock_uom"}},
		},
		target_doc,
	)
