...
test_dependencies = ["Territory", "Customer Group", "Supplier Group", "Product"]
...
class TestUaeVat201(TestCase):
	def setUp(self):
		...
		make_product("_Test UAE VAT Product", properties={"is_zero_rated": 0, "is_exempt": 0})
		make_product("_Test UAE VAT Zero Rated Product", properties={"is_zero_rated": 1, "is_exempt": 0})
		make_product("_Test UAE VAT Exempt Product", properties={"is_zero_rated": 0, "is_exempt": 1})
...
def make_product(product_code, properties=None):
...
def make_sales_invoices():
	def make_sales_invoices_wrapper(emirate, product, tax=True, tourist_tax=False):
	...
	uae_product = "_Test UAE VAT Product"
	uae_exempt_product = "_Test UAE VAT Exempt Product"
	uae_zero_rated_product = "_Test UAE VAT Zero Rated Product"

	# Sales Invoice with standard rated expense in Dubai
	make_sales_invoices_wrapper("Dubai", uae_product)
	# Sales Invoice with standard rated expense in Sharjah
	make_sales_invoices_wrapper("Sharjah", uae_product)
	# Sales Invoice with Tourist Tax Return
	make_sales_invoices_wrapper("Dubai", uae_product, True, True)
	# Sales Invoice with Exempt Product
	make_sales_invoices_wrapper("Sharjah", uae_exempt_product, False)
	# Sales Invoice with Zero Rated Product
	make_sales_invoices_wrapper("Sharjah", uae_zero_rated_product, False)
