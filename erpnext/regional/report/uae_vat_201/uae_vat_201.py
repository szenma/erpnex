...
def get_total_emiratewise(filters):
	...
	try:
		return frappe.db.sql(
			...
			from
				`tabSales Invoice Product` i inner join `tabSales Invoice` s
			on
				i.parent = s.name
			where
...
def get_zero_rated_total(filters):
	"""Returns the sum of each Sales Invoice Product Amount which is zero rated."""
	conditions = get_conditions(filters)
	try:
		return (
			frappe.db.sql(
...
def get_exempt_total(filters):
	"""Returns the sum of each Sales Invoice Product Amount which is Vat Exempt."""
	conditions = get_conditions(filters)
	try:
		return (
			frappe.db.sql(
