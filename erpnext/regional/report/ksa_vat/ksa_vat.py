...
def get_tax_data_for_each_vat_setting(vat_setting, filters, doctype):
	...
	for invoice in invoices:
		invoice_products = frappe.get_all(
			f"{doctype} Product",
			filters={
				"docstatus": 1,
				"parent": invoice.name,
				"product_tax_template": vat_setting.product_tax_template,
			},
			fields=["product_code", "base_net_amount"],
		)

		for product in invoice_products:
			# Summing up total taxable amount
			if invoice.is_return == 0:
				total_taxable_amount += product.base_net_amount

			if invoice.is_return == 1:
				total_taxable_adjustment_amount += product.base_net_amount

			# Summing up total tax
			total_tax += get_tax_amount(product.product_code, vat_setting.account, doctype, invoice.name)
...
def get_tax_amount(product_code, account_head, doctype, parent):
	...
	product_wise_tax_detail = frappe.get_value(
		tax_doctype,
	...
	tax_amount = 0
	if product_wise_tax_detail and len(product_wise_tax_detail) > 0:
		product_wise_tax_detail = json.loads(product_wise_tax_detail)
		for key, value in product_wise_tax_detail.products():
			if key == product_code:
				tax_amount = value[1]
				break
