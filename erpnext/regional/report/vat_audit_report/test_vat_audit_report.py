...
class TestVATAuditReport(TestCase):
	def setUp(self):
		...
		make_product("_Test SA VAT Product")
		make_product("_Test SA VAT Zero Rated Product", properties={"is_zero_rated": 1})
...
def make_sales_invoices():
	def make_sales_invoices_wrapper(product, rate, tax_account, tax_rate, tax=True):
	...
	test_product = "_Test SA VAT Product"
	test_zero_rated_product = "_Test SA VAT Zero Rated Product"

	make_sales_invoices_wrapper(test_product, 100.0, "VAT - 15% - _TCSV", 15.0)
	make_sales_invoices_wrapper(test_zero_rated_product, 100.0, "VAT - 0% - _TCSV", 0.0)
