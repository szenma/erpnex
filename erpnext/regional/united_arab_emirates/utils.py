...
def update_productised_tax_data(doc):
	...
	for row in doc.products:
		...
		if row.product_tax_rate:
			product_tax_rate = frappe.parse_json(row.product_tax_rate)

		# First check if tax rate is present
		# If not then look up in product_wise_tax_detail
		if product_tax_rate:
			for account, rate in product_tax_rate.products():
				tax_rate += rate
		elif row.product_code and productised_tax.get(row.product_code):
...
def get_tax_accounts(company):
	...
	for tax_account in tax_accounts_list:
		for account, name in tax_account.products():
