...
from erpnext.controllers.taxes_and_totals import get_productised_tax
...
def update_productised_tax_data(doc):
	...
	for row in doc.products:
		tax_rate = 0.0
		if productised_tax.get(row.product_code):
			tax_rate = sum([tax.get("tax_rate", 0) for d, tax in productised_tax.get(row.product_code).products()])
...
def prepare_invoice(invoice, progressive_number):
	...
	invoice.e_invoice_products = [product for product in invoice.products]
	tax_data = get_invoice_summary(invoice.e_invoice_products, invoice.taxes)
	...
	for product in invoice.e_invoice_products:
		if product.tax_rate == 0.0 and product.tax_amount == 0.0 and tax_data.get("0.0"):
			product.tax_exemption_reason = tax_data["0.0"]["tax_exemption_reason"]

	customer_po_data = {}
	for d in invoice.e_invoice_products:
...
def get_invoice_summary(products, taxes):
	...
	for tax in taxes:
		...
		# Charges to appear as products in the e-invoice.
		if tax.charge_type in ["On Previous Row Total", "On Previous Row Amount"]:
			...
			if reference_row:
				products.append(
					frappe._dict(
						idx=len(products) + 1,
						product_code=reference_row.description,
						product_name=reference_row.description,
						...
						product_tax_rate={tax.account_head: tax.rate},
						charges=True,
					)
				)

		# Check product tax rates if tax rate is zero.
		if tax.rate == 0:
			for product in products:
				product_tax_rate = product.product_tax_rate
				if isinstance(product.product_tax_rate, str):
					product_tax_rate = json.loads(product.product_tax_rate)

				if product_tax_rate and tax.account_head in product_tax_rate:
					key = cstr(product_tax_rate[tax.account_head])
					...
					summary_data[key]["tax_amount"] += product.tax_amount
					summary_data[key]["taxable_amount"] += product.net_amount
					if key == "0.0":
			...
			if summary_data == {}:  # Implies that Zero VAT has not been set on any product.
				summary_data.setdefault(
					"0.0",
					{
						"tax_amount": 0.0,
			...
			product_wise_tax_detail = json.loads(tax.product_wise_tax_detail)
			for rate_product in [
				tax_product for tax_product in product_wise_tax_detail.products() if tax_product[1][0] == tax.rate
			]:
				key = cstr(tax.rate)
				if not summary_data.get(key):
					summary_data.setdefault(key, {"tax_amount": 0.0, "taxable_amount": 0.0})
				summary_data[key]["tax_amount"] += rate_product[1][1]
				summary_data[key]["taxable_amount"] += sum(
					[product.net_amount for product in products if product.product_code == rate_product[0]]
				)

			for product in products:
				key = cstr(tax.rate)
				if product.get("charges"):
					if not summary_data.get(key):
						summary_data.setdefault(key, {"taxable_amount": 0.0})
					summary_data[key]["taxable_amount"] += product.taxable_amount
...
def prepare_and_attach_invoice(doc, replace=False):
	...
	product_meta = frappe.get_meta("Sales Invoice Product")

	invoice_xml = frappe.render_template(
		"erpnext/regional/italy/e-invoice.xml",
		context={"doc": invoice, "product_meta": product_meta},
		is_path=True,
	)
...
def set_state_code(doc, method):
	...
	state_codes_lower = {key.lower(): value for key, value in state_codes.products()}
