...
$.extend(shopping_cart, {
		...
		if (frappe.session.user==="Guest") {
			...
			return frappe.call({
				...
				args: {
					...
					with_products: opts.with_products || 0
				},
				btn: opts.btn,
				callback: function(r) {
	...
	shopping_cart_update: function({product_code, qty, cart_dropdown, additional_notes}) {
		shopping_cart.update_cart({
			...
			callback: function(r) {
				if(!r.exc) {
					$(".cart-products").html(r.message.products);
	...
	bind_add_to_cart_action() {
		$('.page_content').on('click', '.btn-add-to-cart-list', (e) => {
			...
			const product_code = $btn.data('product-code');
