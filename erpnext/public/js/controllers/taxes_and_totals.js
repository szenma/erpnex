...
erpnext.taxes_and_totals = class TaxesAndTotals extends erpnext.payments {
	...
	apply_pricing_rule_on_product(product) {
		let effective_product_rate = product.price_list_rate;
		let product_rate = product.rate;
		if (in_list(["Sales Order", "Quotation"], product.parenttype) && product.blanket_order_rate) {
			effective_product_rate = product.blanket_order_rate;
		}
		if (product.margin_type == "Percentage") {
			product.rate_with_margin = flt(effective_product_rate)
				+ flt(effective_product_rate) * ( flt(product.margin_rate_or_amount) / 100);
		} else {
			product.rate_with_margin = flt(effective_product_rate) + flt(product.margin_rate_or_amount);
		}
		product.base_rate_with_margin = flt(product.rate_with_margin) * flt(this.frm.doc.conversion_rate);

		product_rate = flt(product.rate_with_margin , precision("rate", product));

		if (product.discount_percentage) {
			product.discount_amount = flt(product.rate_with_margin) * flt(product.discount_percentage) / 100;
		}

		if (product.discount_amount) {
			product_rate = flt((product.rate_with_margin) - (product.discount_amount), precision('rate', product));
			product.discount_percentage = 100 * flt(product.discount_amount) / flt(product.rate_with_margin);
		}

		frappe.model.set_value(product.doctype, product.name, "rate", product_rate);
	}
	...
	_calculate_taxes_and_totals() {
		...
		this.frm._products = is_quotation ? this.filtered_products() : this.frm.doc.products;

		this.validate_conversion_rate();
		this.calculate_product_values();
	...
	calculate_product_values() {
		...
		if (!this.discount_amount_applied) {
			for (const product of this.frm._products || []) {
				frappe.model.round_floats_in(product);
				product.net_rate = product.rate;
				product.qty = product.qty === undefined ? (me.frm.doc.is_return ? -1 : 1) : product.qty;

				if (!(me.frm.doc.is_return || me.frm.doc.is_debit_note)) {
					product.net_amount = product.amount = flt(product.rate * product.qty, precision("amount", product));
				}
				else {
					// allow for '0' qty on Credit/Debit notes
					let qty = product.qty || (me.frm.doc.is_debit_note ? 1 : -1);
					product.net_amount = product.amount = flt(product.rate * qty, precision("amount", product));
				}

				product.product_tax_amount = 0.0;
				product.total_weight = flt(product.weight_per_unit * product.stock_qty);

				me.set_in_company_currency(product, ["price_list_rate", "rate", "amount", "net_rate", "net_amount"]);
			}
		}
	}
	...
	initialize_taxes() {
		...
		$.each(this.frm.doc["taxes"] || [], function(i, tax) {
			...
			var tax_fields = ["total", "tax_amount_after_discount_amount",
				"tax_amount_for_current_product", "grand_total_for_current_product",
				"tax_fraction_for_current_product", "grand_total_fraction_for_current_product"];
	...
	determine_exclusive_rate() {
		...
		$.each(me.frm._products || [], function(n, product) {
			var product_tax_map = me._load_product_tax_rate(product.product_tax_rate);
			var cumulated_tax_fraction = 0.0;
			var total_inclusive_tax_amount_per_qty = 0;
			$.each(me.frm.doc["taxes"] || [], function(i, tax) {
				var current_tax_fraction = me.get_current_tax_fraction(tax, product_tax_map);
				tax.tax_fraction_for_current_product = current_tax_fraction[0];
				...
				if(i==0) {
					...
					tax.grand_total_fraction_for_current_product =
						me.frm.doc["taxes"][i-1].grand_total_fraction_for_current_product +
			...
			if(!me.discount_amount_applied && product.qty && (total_inclusive_tax_amount_per_qty || cumulated_tax_fraction)) {
				var amount = flt(product.amount) - total_inclusive_tax_amount_per_qty;
				product.net_amount = flt(amount / (1 + cumulated_tax_fraction));
				product.net_rate = product.qty ? flt(product.net_amount / product.qty, precision("net_rate", product)) : 0;

				me.set_in_company_currency(product, ["net_rate", "net_amount"]);
			}
		});
	}

	get_current_tax_fraction(tax, product_tax_map) {
		...
		if(cint(tax.included_in_print_rate)) {
			var tax_rate = this._get_tax_rate(tax, product_tax_map);
			...
			if(tax.charge_type == "On Net Total") {
				...
				current_tax_fraction = (tax_rate / 100.0) *
					this.frm.doc["taxes"][cint(tax.row_id) - 1].tax_fraction_for_current_product;

			} else if(tax.charge_type == "On Previous Row Total") {
				current_tax_fraction = (tax_rate / 100.0) *
					this.frm.doc["taxes"][cint(tax.row_id) - 1].grand_total_fraction_for_current_product;
			} else if (tax.charge_type == "On Product Quantity") {
	...
	_get_tax_rate(tax, product_tax_map) {
		return (Object.keys(product_tax_map).indexOf(tax.account_head) != -1) ?
			flt(product_tax_map[tax.account_head], precision("rate", tax)) : tax.rate;
	}
	...
	calculate_net_total() {
		...
		$.each(this.frm._products || [], function(i, product) {
			me.frm.doc.total += product.amount;
			me.frm.doc.total_qty += product.qty;
			me.frm.doc.base_total += product.base_amount;
			me.frm.doc.net_total += product.net_amount;
			me.frm.doc.base_net_total += product.base_net_amount;
		});
	}
	...
	calculate_taxes() {
		...
		$.each(this.frm._products || [], function(n, product) {
			var product_tax_map = me._load_product_tax_rate(product.product_tax_rate);
			$.each(me.frm.doc["taxes"] || [], function(i, tax) {
				// tax_amount represents the amount of tax for the current step
				var current_tax_amount = me.get_current_tax_amount(product, tax, product_tax_map);

				// Adjust divisional loss to the last product
				if (tax.charge_type == "Actual") {
					actual_tax_dict[tax.idx] -= current_tax_amount;
					if (n == me.frm._products.length - 1) {
				...
				if(i==0) {
					...
					tax.grand_total_for_current_product =
						flt(me.frm.doc["taxes"][i-1].grand_total_for_current_product + current_tax_amount);
				}
				...
				if (n == me.frm._products.length - 1) {
					...
					// in tax.total, accumulate grand total for each product
	...
	_load_product_tax_rate(product_tax_rate) {
		return product_tax_rate ? JSON.parse(product_tax_rate) : {};
	}

	get_current_tax_amount(product, tax, product_tax_map) {
		var tax_rate = this._get_tax_rate(tax, product_tax_map);
		...
		if(tax.charge_type == "Actual") {
			...
			current_tax_amount = this.frm.doc.net_total ?
				((product.net_amount / this.frm.doc.net_total) * actual) : 0.0;

		} else if(tax.charge_type == "On Net Total") {
			current_tax_amount = (tax_rate / 100.0) * product.net_amount;
		} else if(tax.charge_type == "On Previous Row Amount") {
			current_tax_amount = (tax_rate / 100.0) *
				this.frm.doc["taxes"][cint(tax.row_id) - 1].tax_amount_for_current_product;

		} else if(tax.charge_type == "On Previous Row Total") {
			current_tax_amount = (tax_rate / 100.0) *
				this.frm.doc["taxes"][cint(tax.row_id) - 1].grand_total_for_current_product;
		} else if (tax.charge_type == "On Product Quantity") {
			current_tax_amount = tax_rate * product.qty;
		}

		if (!tax.dont_recompute_tax) {
			this.set_product_wise_tax(product, tax, tax_rate, current_tax_amount);
		}

		return current_tax_amount;
	}

	set_product_wise_tax(product, tax, tax_rate, current_tax_amount) {
		// store tax breakup for each product
		let tax_detail = tax.product_wise_tax_detail;
		let key = product.product_code || product.product_name;

		if(typeof (tax_detail) == "string") {
			tax.product_wise_tax_detail = JSON.parse(tax.product_wise_tax_detail);
			tax_detail = tax.product_wise_tax_detail;
		}

		let product_wise_tax_amount = current_tax_amount * this.frm.doc.conversion_rate;
		if (tax_detail && tax_detail[key])
			product_wise_tax_amount += tax_detail[key][1];

		tax_detail[key] = [tax_rate, flt(product_wise_tax_amount, precision("base_tax_amount", tax))];
	}
	...
	_cleanup() {
		...
		let products = this.frm._products;

		if(products && products.length) {
			if(!frappe.meta.get_docfield(products[0].doctype, "product_tax_amount", this.frm.doctype)) {
				$.each(products || [], function(i, product) {
					delete product["product_tax_amount"];
				});
			}
		}

		if(this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			var temporary_fields = ["tax_amount_for_current_product", "grand_total_for_current_product",
				"tax_fraction_for_current_product", "grand_total_fraction_for_current_product"];
			...
			$.each(this.frm.doc["taxes"] || [], function(i, tax) {
				...
				if (!tax.dont_recompute_tax) {
					tax.product_wise_tax_detail = JSON.stringify(tax.product_wise_tax_detail);
				}
			});
		}
	}
	...
	apply_discount_amount() {
		...
		if (this.frm.doc.discount_amount) {
			...
			if (total_for_discount_amount) {
				$.each(this.frm._products || [], function(i, product) {
					distributed_amount = flt(me.frm.doc.discount_amount) * product.net_amount / total_for_discount_amount;
					product.net_amount = flt(product.net_amount - distributed_amount,
						precision("base_amount", product));
					net_total += product.net_amount;

					// discount amount rounding loss adjustment if no taxes
					if ((!(me.frm.doc.taxes || []).length || total_for_discount_amount==me.frm.doc.net_total || (me.frm.doc.apply_discount_on == "Net Total"))
							&& i == (me.frm._products || []).length - 1) {
						var discount_amount_loss = flt(me.frm.doc.net_total - net_total
							- me.frm.doc.discount_amount, precision("net_total"));
						product.net_amount = flt(product.net_amount + discount_amount_loss,
							precision("net_amount", product));
					}
					product.net_rate = product.qty ? flt(product.net_amount / product.qty, precision("net_rate", product)) : 0;
					me.set_in_company_currency(product, ["net_rate", "net_amount"]);
				});
	...
	get_total_for_discount_amount() {
		if(this.frm.doc.apply_discount_on == "Net Total") {
			...
			$.each(this.frm.doc["taxes"] || [], function(i, tax) {
				if (in_list(["Actual", "On Product Quantity"], tax.charge_type)) {
	...
	filtered_products() {
		return this.frm.doc.products.filter(product => !product["is_alternative"]);
	}
};
