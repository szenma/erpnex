...
$.extend(erpnext.queries, {
	...
	product: function(filters) {
		var args = { query: "erpnext.controllers.queries.product_query" };
...
/* 	if product code is selected in child table
...
erpnext.queries.setup_warehouse_query = function(frm){
	frm.set_query('warehouse', 'products', function(doc, cdt, cdn) {
		...
		if(row.product_code){
			...
			filters["filters"].push(["Bin", "product_code", "=", row.product_code]);
		}
		return filters
	});
}
