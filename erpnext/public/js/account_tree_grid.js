...
erpnext.AccountTreeGrid = class AccountTreeGrid extends frappe.views.TreeGridReport {
		...
		this.filters = [
			{fieldtype: "Select", label: __("Company"), link:"Company", fieldname: "company",
				...
				filter: function(val, product, opts, me) {
					if (product.company == val || val == opts.default_value) {
	...
	prepare_balances() {
		...
		$.each(gl, function(i, v) {
			...
			var account = me.product_by_name[v.account];
	...
	update_groups() {
		...
		$.each(this.data, function(i, account) {
			...
			if((account.is_group == 0) || (account.rgt - account.lft == 1)) {
				...
				while(parent) {
					var parent_account = me.product_by_name[parent];
