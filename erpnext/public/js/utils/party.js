...
erpnext.utils.add_product = function(frm) {
	if (frm.is_new()) {
		var prev_route = frappe.get_prev_route();
		if (prev_route[1]==='Product' && !(frm.doc.products && frm.doc.products.length)) {
			// add row
			var product = frm.add_child('products');
			frm.refresh_field('products');

			// set product
			frappe.model.set_value(product.doctype, product.name, 'product_code', prev_route[2]);
		}
	}
}
