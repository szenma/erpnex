erpnext.utils.BarcodeScanner = class BarcodeScanner {
	constructor(opts) {
		...
		this.products_table_name = opts.products_table_name || "products";
		this.products_table = this.frm.doc[this.products_table_name];
		...
		// {
		//     product_code: "HORSESHOE", // present if any product was found
		...
		return new Promise((resolve, reject) => {
			...
			this.scan_api_call(input, (r) => {
				...
				if (!data || Object.keys(data).length === 0) {
					this.show_alert(__("Cannot find Product with this Barcode"), "red");
					this.clean_up();
	...
	update_table(data) {
		return new Promise(resolve => {
			let cur_grid = this.frm.fields_dict[this.products_table_name].grid;

			const {product_code, barcode, batch_no, serial_no, uom} = data;

			let row = this.get_row_to_modify_on_scan(product_code, batch_no, uom, barcode);

			this.is_new_row = false;
			if (!row) {
				if (this.dont_allow_new_row) {
					this.show_alert(__("Maximum quantity scanned for product {0}.", [product_code]), "red");
					this.clean_up();
					reject();
					return;
				}
				...
				// add new row if new product/batch is scanned
				row = frappe.model.add_child(this.frm.doc, cur_grid.doctype, this.products_table_name);
				// trigger any row add triggers defined on child table.
				this.frm.script_manager.trigger(`${this.products_table_name}_add`, row.doctype, row.name);
				this.frm.has_products = false;
			}
			...
			frappe.run_serially([
				...
				() => this.set_product(row, product_code, barcode, batch_no, serial_no).then(qty => {
					this.show_scan_message(row.idx, row.product_code, qty);
				}),
	...
	// this flag on product row is used by transaction.js to avoid triggering selector
	...
	set_product(row, product_code, barcode, batch_no, serial_no) {
		return new Promise(resolve => {
			const increment = async (value = 1) => {
				const product_data = {product_code: product_code};
				product_data[this.qty_field] = Number((row[this.qty_field] || 0)) + Number(value);
				await frappe.model.set_value(row.doctype, row.name, product_data);
				return value;
			};

			if (this.prompt_qty) {
				frappe.prompt(__("Please enter quantity for product {0}", [product_code]), ({value}) => {
					increment(value).then((value) => resolve(value));
				});
			} else if (this.frm.has_products) {
				this.prepare_product_for_scan(row, product_code, barcode, batch_no, serial_no);
			} else {
	...
	prepare_product_for_scan(row, product_code, barcode, batch_no, serial_no) {
		var me = this;
		this.dialog = new frappe.ui.Dialog({
			title: __("Scan barcode for product {0}", [product_code]),
			fields: me.get_fields_for_dialog(row, product_code, barcode, batch_no, serial_no),
		})

		this.dialog.set_primary_action(__("Update"), () => {
			const product_data = {product_code: product_code};
			product_data[this.qty_field] = this.dialog.get_value("scanned_qty");
			product_data["has_product_scanned"] = 1;

			this.remaining_qty = flt(this.dialog.get_value("qty")) - flt(this.dialog.get_value("scanned_qty"));
			frappe.model.set_value(row.doctype, row.name, product_data);
	...
	get_fields_for_dialog(row, product_code, barcode, batch_no, serial_no) {
		let fields = [
			{
				fieldtype: "Data",
				...
				onchange: (e) => {
					...
					if (e.target.value) {
						this.scan_api_call(e.target.value, (r) => {
							if (r.message) {
								this.update_dialog_values(product_code, r);
							}
						})
					}
				}
	...
	update_dialog_values(scanned_product, r) {
		const {product_code, barcode, batch_no, serial_no} = r.message;

		this.dialog.set_value("barcode_scanner", "");
		if (product_code === scanned_product &&
	...
	add_child_for_remaining_qty(prev_row) {
		if (this.remaining_qty && this.remaining_qty >0) {
			let cur_grid = this.frm.fields_dict[this.products_table_name].grid;
			let row = frappe.model.add_child(this.frm.doc, cur_grid.doctype, this.products_table_name);

			let ignore_fields = ["name", "idx", "batch_no", "barcode",
				"received_qty", "serial_no", "has_product_scanned"];
			...
			this.frm.script_manager.trigger("product_code", row.doctype, row.name);
		}
	}
	...
	show_scan_message(idx, exist = null, qty = 1) {
		...
		if (exist) {
			...
			this.show_alert(__("Row #{0}: Product added", [idx]), "green")
		}
	}
	...
	get_row_to_modify_on_scan(product_code, batch_no, uom, barcode) {
		let cur_grid = this.frm.fields_dict[this.products_table_name].grid;
		...
		const matching_row = (row) => {
			const product_match = row.product_code == product_code;
			const batch_match = (!row[this.batch_no_field] || row[this.batch_no_field] == batch_no);
			const uom_match = !uom || row[this.uom_field] == uom;
			const qty_in_limit = flt(row[this.qty_field]) < flt(row[this.max_qty_field]);
			const product_scanned = row.has_product_scanned;
		...
		return this.products_table.find(matching_row) || this.get_existing_blank_row();
	}

	get_existing_blank_row() {
		return this.products_table.find((d) => !d.product_code);
	}
	...
	clean_up() {
		this.scan_barcode_field.set_value("");
		refresh_field(this.products_table_name);
	}
