...
erpnext.SerialNoBatchSelector = class SerialNoBatchSelector {
	constructor(opts, show_dialog) {
		...
		// frm, product, warehouse_details, has_batch, oldest
		let d = this.product;
	...
	make_dialog() {
		...
		let fields = [
			...
			},
			{
				...
				get_query: function() {
					return {
						...
						filters: [
							["Bin", "product_code", "=", me.product_code],
			...
			{fieldtype:'Column Break'},
			{
				...
				default: flt(me.product.stock_qty) || flt(me.product.transfer_qty),
			},
				...
				click: () => {
					...
					let numbers = frappe.call({
						...
						args: {
							...
							batch_nos: me.product.batch_no || null,
					...
					numbers.then((data) => {
						...
						if (!records_length) {
							...
							frappe.msgprint(
								__('Serial numbers unavailable for Product {0} under warehouse {1}. Please try changing warehouse.', [me.product.product_code.bold(), warehouse])
							);
						}
		...
		this.dialog.set_primary_action(__('Insert'), function() {
			...
			if(me.validate()) {
				frappe.run_serially([
					() => me.update_batch_products(),
					() => me.update_serial_no_product(),
					() => me.update_batch_serial_no_products(),
					() => {
						refresh_field("products");
						refresh_field("packed_products");
						if (me.callback) {
							return me.callback(me.product);
						}
					},
					() => me.dialog.hide()
				])
		...
		if(this.show_dialog) {
			let d = this.product;
			if (this.product.serial_no) {
				this.dialog.fields_dict.serial_no.set_value(this.product.serial_no);
			}

			if (this.has_batch && !this.has_serial_no && d.batch_no) {
				this.frm.doc.products.forEach(data => {
					if(data.product_code == d.product_code) {
	...
	on_close_dialog() {
		this.dialog.get_close_btn().on('click', () => {
			this.on_close && this.on_close(this.product);
		});
	}
	...
	validate() {
		...
		if(this.has_batch && !this.has_serial_no) {
			if(values.batches.length === 0 || !values.batches) {
				frappe.throw(__("Please select batches for batched product {0}", [values.product_code]));
			}
			...
			if (!serial_nos || !serial_nos.replace(/\s/g, '').length) {
				frappe.throw(__("Please enter serial numbers for serialized product {0}", [values.product_code]));
			}
			return true;
		}
	}

	update_batch_products() {
		// clones an products if muliple batches are selected.
		if(this.has_batch && !this.has_serial_no) {
			this.values.batches.map((batch, i) => {
				...
				if (i !== 0 && !this.batch_exists(batch_no)) {
					row = this.frm.add_child("products", { ...this.product });
				} else {
					row = this.frm.doc.products.find(i => i.batch_no === batch_no);
				}
	...
	update_serial_no_product() {
		// just updates serial no for the product
		if(this.has_serial_no && !this.has_batch) {
			this.map_row_values(this.product, this.values, 'serial_no', 'qty');
		}
	}

	update_batch_serial_no_products() {
	...
	batch_exists(batch) {
		const batches = this.frm.doc.products.map(data => data.batch_no);
	...
	get_serial_no_fields() {
		...
		if (this.product.batch_no) {
			serial_no_filters["batch_no"] = this.product.batch_no;
		}
...
function get_pending_qty_fields(me) {
	...
	const { frm: { doc: { fg_completed_qty }}, product: { product_code, stock_qty }} = me;
	const { qty_consumed_per_unit } = erpnext.stock.bom.products[product_code];
...
// get all products with same product code except row for which selector is open.
function get_rows_with_same_product_code(me) {
	const { frm: { doc: { products }}, product: { name, product_code }} = me;
	return products.filter(product => (product.name !== name) && (product.product_code === product_code))
}

function calc_total_selected_qty(me) {
	const totalSelectedQty = get_rows_with_same_product_code(me)
		.map(product => flt(product.qty))
		.reduce((i, j) => i + j, 0);
	return totalSelectedQty;
}

function get_selected_serial_nos(me) {
	const selected_serial_nos = get_rows_with_same_product_code(me)
...
function check_can_calculate_pending_qty(me) {
	const { frm: { doc }, product } = me;
	const docChecks = doc.bom_no
		&& doc.fg_completed_qty
		&& erpnext.stock.bom
		&& erpnext.stock.bom.name === doc.bom_no;
	const productChecks = !!product
		&& !product.original_product
		&& erpnext.stock.bom && erpnext.stock.bom.products
		&& (product.product_code in erpnext.stock.bom.products);
	return docChecks && productChecks;
}
