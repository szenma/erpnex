...
frappe.ui.form.ProductQuickEntryForm = class ProductQuickEntryForm extends frappe.ui.form.QuickEntryForm {
	...
	check_naming_series_based_on() {
		if (frappe.defaults.get_default("product_naming_by") === "Naming Series") {
			this.mandatory = this.mandatory.filter(d => d.fieldname !== "product_code");
		}
	}

	init_post_render_dialog_operations() {
		this.dialog.fields_dict.attribute_html.$wrapper.append(frappe.render_template("product_quick_entry"));
		this.init_for_create_variant_trigger();
		this.init_for_product_template_trigger();
		// explicitly hide manufacturing fields as hidden not working.
		this.toggle_manufacturer_fields();
		this.dialog.get_field("product_template").df.hidden = 1;
		this.dialog.get_field("product_template").refresh();
	}
	...
	register_primary_action() {
		...
		this.dialog.set_primary_action(__('Save'), function() {
			...
			if (me.dialog.fields_dict.create_variant.$input.prop("checked")) {
				...
				variant_values.product_group = me.template_doc.product_group;
			}
	...
	open_doc() {
		...
		if (this.dialog.fields_dict.create_variant.$input.prop("checked")) {
			var template = this.dialog.fields_dict.product_template.input.value;
			if (template)
	...
	preset_fields_for_template() {
		...
		let template_field = this.dialog.get_field("product_template");
		...
		['product_code', 'product_name', 'product_group', 'stock_uom'].forEach((d) => {
		...
		['product_code', 'stock_uom', 'product_group'].forEach((d) => {
			let f = this.dialog.get_field(d);
	...
	init_for_product_template_trigger() {
		var me = this;

		me.dialog.fields_dict["product_template"].df.onchange = () => {
			var template = me.dialog.fields_dict.product_template.input.value;
	...
	get_variant_doc() {
		...
		if (Object.keys(attribute).length) {
			frappe.call({
				method: "erpnext.controllers.product_variant.create_variant_doc_for_quick_entry",
				args: {
					"template": me.dialog.fields_dict.product_template.$input.val(),
					args: attribute
				},
				async: false,
				callback: function(r) {
					if (Object.prototype.toString.call(r.message) == "[object Object]") {
						...
						var msgprint_dialog = frappe.msgprint(__("Product Variant {0} already exists with same attributes", [repl('<a class="strong variant-click" data-product-code="%(product)s" \
								>%(product)s</a>', {
								product: r.message
							})]));
						...
						msgprint_dialog.$wrapper.find(".variant-click").on("click", function() {
							...
							me.dialog.hide();
							if (frappe._from_link) {
								frappe._from_link.set_value($(this).attr("data-product-code"));
							} else {
								frappe.set_route('Form', "Product", $(this).attr("data-product-code"));
							}
						});
					}
				}
