...
$.extend(wishlist, {
	...
	bind_move_to_cart_action: function() {
		...
		$('.page_content').on("click", ".btn-add-to-cart", (e) => {
			...
			let product_code = $move_to_cart_btn.data("product-code");
			...
			let args = { product_code: product_code };
	...
	bind_remove_action: function() {
		...
		$('.page_content').on("click", ".remove-wish", (e) => {
			...
			let product_code = $remove_wish_btn.data("product-code");
	...
	bind_wishlist_action() {
		// 'wish'('like') or 'unwish' product in product listing
	...
	wishlist_action(btn) {
		...
		if (frappe.session.user==="Guest") {
			if (localStorage) {
				localStorage.setProduct("last_visited", window.location.pathname);
			}
			this.redirect_guest();
			return;
		}
		...
		if ($wish_icon.hasClass('wished')) {
			...
			let args = { product_code: btn.data('product-code') };
	...
	add_remove_from_wishlist(action, args, success_action, failure_action, async=false) {
		/*	AJAX call to add or remove Product from Wishlist
			action: "add" or "remove"
			args: args for method (product_code, price, formatted_price),
