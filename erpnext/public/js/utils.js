...
$.extend(erpnext, {
	...
	setup_serial_or_batch_no: function() {
		...
		frappe.model.get_value('Product', {'name': grid_row.doc.product_code},
	...
	route_to_pending_reposts: (args) => {
		frappe.set_route('List', 'Repost Product Valuation', args);
	},
});
...
erpnext.utils.select_alternate_products = function(opts) {
	const frm = opts.frm;
	const warehouse_field = opts.warehouse_field || 'warehouse';
	const product_field = opts.product_field || 'product_code';

	this.data = [];
	const dialog = new frappe.ui.Dialog({
		title: __("Select Alternate Product"),
		fields: [
			{fieldtype:'Section Break', label: __('Products')},
			{
				fieldname: "alternative_products", fieldtype: "Table", cannot_add_rows: true,
				...
				fields: [{
					...
					fieldtype:'Link',
					fieldname:"alternate_product",
					options: 'Product',
					default: "",
					in_list_view: 1,
					label: __('Alternate Product'),
					onchange: function() {
						const product_code = this.get_value();
						const warehouse = this.grid_row.on_grid_fields_dict.warehouse.get_value();
						if (product_code && warehouse) {
							frappe.call({
					...
					get_query: (e) => {
						return {
							query: "erpnext.stock.doctype.product_alternative.product_alternative.get_alternative_products",
							filters: {
					...
					onchange: function() {
						...
						const product_code = this.grid_row.on_grid_fields_dict.product_code.get_value();
						if (product_code && warehouse) {
							frappe.call({
								method: "erpnext.stock.utils.get_latest_stock_qty",
								args: {
		...
		primary_action: function() {
			const args = this.get_values()["alternative_products"];
			const alternative_products = args.filter(d => {
				if (d.alternate_product && d.product_code != d.alternate_product) {
					return true;
				}
			});

			alternative_products.forEach(d => {
				let row = frappe.get_doc(opts.child_doctype, d.docname);
				let qty = null;
				if (row.doctype === 'Work Order Product') {
					qty = row.required_qty;
				} else {
					qty = row.qty;
				}
				row[product_field] = d.alternate_product;
				frappe.model.set_value(row.doctype, row.name, 'qty', qty);
				frappe.model.set_value(row.doctype, row.name, opts.original_product_field, d.product_code);
				frm.trigger(product_field, row.doctype, row.name);
			});
	...
	frm.doc[opts.child_docname].forEach(d => {
		if (!opts.condition || opts.condition(d)) {
			dialog.fields_dict.alternative_products.df.data.push({
				"docname": d.name,
				"product_code": d[product_field],
	...
	this.data = dialog.fields_dict.alternative_products.df.data;
	dialog.fields_dict.alternative_products.grid.refresh();
	dialog.show();
}

erpnext.utils.update_child_products = function(opts) {
	const frm = opts.frm;
	const cannot_add_row = (typeof opts.cannot_add_row === 'undefined') ? true : opts.cannot_add_row;
	const child_docname = (typeof opts.cannot_add_row === 'undefined') ? "products" : opts.child_docname;
	const child_meta = frappe.get_meta(`${frm.doc.doctype} Product`);
	...
	const fields = [{
		...
		onchange: function () {
			frappe.call({
				...
				args: { product_code: this.doc.product_code, uom: this.value },
				callback: r => {
					if(!r.exc) {
						if (this.doc.conversion_factor == r.message.conversion_factor) return;

						const docname = this.doc.docname;
						dialog.fields_dict.trans_products.df.data.some(doc => {
							if (doc.docname == docname) {
								doc.conversion_factor = r.message.conversion_factor;
								dialog.fields_dict.trans_products.grid.refresh();
								return true;
							}
						})
					}
	...
	new frappe.ui.Dialog({
		title: __("Update Products"),
		fields: [
			{
				fieldname: "trans_products",
				fieldtype: "Table",
				label: "Products",
		...
		primary_action: function() {
			const trans_products = this.get_values()["trans_products"].filter((product) => !!product.product_code);
			frappe.call({
				method: 'erpnext.controllers.accounts_controller.update_child_qty_rate',
				freeze: true,
				args: {
					'parent_doctype': frm.doc.doctype,
					'trans_products': trans_products,
...
erpnext.utils.map_current_doc = function(opts) {
	function _map() {
		if($.isArray(cur_frm.doc.products) && cur_frm.doc.products.length > 0) {
			// remove first product row if empty
			if(!cur_frm.doc.products[0].product_code) {
				cur_frm.doc.products = cur_frm.doc.products.splice(1);
			}

			// find the doctype of the products table
			var products_doctype = frappe.meta.get_docfield(cur_frm.doctype, 'products').options;

			// find the link fieldname from products table for the given
			// source_doctype
			var link_fieldname = null;
			frappe.get_meta(products_doctype).fields.forEach(function(d) {
				if(d.options===opts.source_doctype) link_fieldname = d.fieldname; });

			// search in existing products if the source_name is already set and full qty fetched
			var already_set = false;
			var product_qty_map = {};

			$.each(cur_frm.doc.products, function(i, d) {
				opts.source_name.forEach(function(src) {
					if(d[link_fieldname]==src) {
						already_set = true;
						if (product_qty_map[d.product_code])
							product_qty_map[d.product_code] += flt(d.qty);
						else
							product_qty_map[d.product_code] = flt(d.qty);
					}
				});
			...
			if(already_set) {
				opts.source_name.forEach(function(src) {
					frappe.model.with_doc(opts.source_doctype, src, function(r) {
						...
						$.each(source_doc.products || [], function(i, row) {
							if(row.qty > flt(product_qty_map[row.product_code])) {
								already_set = false;
								return false;
							}
						})
					...
					if(already_set) {
						frappe.msgprint(__("You have already selected products from {0} {1}",
	...
	if (opts.source_doctype) {
		const d = new frappe.ui.form.MultiSelectDialog({
			...
			add_filters_group: 1,
			allow_child_product_selection: opts.allow_child_product_selection,
			...
			action: function(selections, args) {
				...
				if (opts.allow_child_product_selection) {
...
frappe.form.link_formatters['Product'] = function(value, doc) {
	if (doc && value && doc.product_name && doc.product_name !== value && doc.product_code === value) {
		return value + ': ' + doc.product_name;
	} else if (!value && doc.doctype && doc.product_name) {
		// format blank value in child table
		return doc.product_name;
	} else {
		// if value is blank in report view or product code and name are the same, return as is
		return value;
	}
}
