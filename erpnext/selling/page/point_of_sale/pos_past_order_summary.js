erpnext.PointOfSale.PastOrderSummary = class {
	...
	prepare_dom() {
			...
			`<section class="past-order-summary">
				...
				<div class="invoice-summary-wrapper">
					<div class="abs-container">
						...
						<div class="products-container summary-container"></div>
		...
		this.$products_container = this.$summary_container.find('.products-container');
	...
	get_product_html(doc, product_data) {
		return `<div class="product-row-wrapper">
					<div class="product-name">${product_data.product_name}</div>
					<div class="product-qty">${product_data.qty || 0}</div>
					<div class="product-rate-disc">${get_rate_discount_html()}</div>
				</div>`;

		function get_rate_discount_html() {
			if (product_data.rate && product_data.price_list_rate && product_data.rate !== product_data.price_list_rate) {
				return `<span class="product-disc">(${product_data.discount_percentage}% off)</span>
						<div class="product-rate">${format_currency(product_data.rate, doc.currency)}</div>`;
			} else {
				return `<div class="product-rate">${format_currency(product_data.price_list_rate || product_data.rate, doc.currency)}</div>`;
			}
		}
	}
	...
	attach_products_info(doc) {
		this.$products_container.html('');
		doc.products.forEach(product => {
			const product_dom = this.get_product_html(doc, product);
			this.$products_container.append(product_dom);
			this.set_dynamic_rate_header_width();
		});
	}

	set_dynamic_rate_header_width() {
		const rate_cols = Array.from(this.$products_container.find(".product-rate-disc"));
		this.$products_container.find(".product-rate-disc").css("width", "");
		...
		this.$products_container.find(".product-rate-disc").css("width", max_width);
	}
