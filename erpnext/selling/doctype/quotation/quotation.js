...
erpnext.selling.QuotationController = class QuotationController extends erpnext.selling.SellingController {
	...
	refresh(doc, dt, dn) {
		...
		if (this.frm.doc.docstatus===0) {
			this.frm.add_custom_button(__('Opportunity'),
				...
				}, __("Get Products From"), "btn-default");
		}
	...
	make_sales_order() {
		...
		let has_alternative_product = this.frm.doc.products.some((product) => product.is_alternative);
		if (has_alternative_product) {
			this.show_alternative_products_dialog();
		} else {
	...
	show_alternative_products_dialog() {
		let me = this;
		...
		this.data = this.frm.doc.products.filter(
			(product) => product.is_alternative || product.has_alternative_product
		).map((product) => {
			return {
				"name": product.name,
				...
				"is_alternative": product.is_alternative,
			}
		});

		const dialog = new frappe.ui.Dialog({
			title: __("Select Alternative Products for Sales Order"),
			fields: [
				{
					fieldname: "info",
					fieldtype: "HTML",
				...
				{
					fieldname: "alternative_products",
					fieldtype: "Table",
					...
					reqd: 1,
					data: this.data,
					description: __("Select an product from each set to be used in the Sales Order."),
					get_data: () => {
						return this.data;
					},
					fields: table_fields
			...
			primary_action: function() {
				frappe.model.open_mapped_doc({
					...
					frm: me.frm,
					args: {
						selected_products: dialog.fields_dict.alternative_products.grid.get_selected_children()
					}
				});
				dialog.hide();
			},
...
frappe.ui.form.on("Quotation Product", "products_on_form_rendered", "packed_products_on_form_rendered", function(frm, cdt, cdn) {
	// enable tax_amount field if Actual
})

frappe.ui.form.on("Quotation Product", "stock_balance", function(frm, cdt, cdn) {
	var d = frappe.model.get_doc(cdt, cdn);
	frappe.route_options = {"product_code": d.product_code};
