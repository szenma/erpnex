...
def get_customer_outstanding(
	...
	unmarked_delivery_note_products = frappe.db.sql(
		"""select
			dn_product.name, dn_product.amount, dn.base_net_total, dn.base_grand_total
		from `tabDelivery Note` dn, `tabDelivery Note Product` dn_product
		where
			dn.name = dn_product.parent
			and dn.customer=%s and dn.company=%s
			and dn.docstatus = 1 and dn.status not in ('Closed', 'Stopped')
			and ifnull(dn_product.against_sales_order, '') = ''
			and ifnull(dn_product.against_sales_invoice, '') = ''
		""",
		(customer, company),
		as_dict=True,
	)

	if not unmarked_delivery_note_products:
		return outstanding_based_on_gle + outstanding_based_on_so

	si_amounts = frappe.db.sql(
		"""
		SELECT
			dn_detail, sum(amount) from `tabSales Invoice Product`
		WHERE
			docstatus = 1
			and dn_detail in ({})
		GROUP BY dn_detail""".format(
			", ".join(frappe.db.escape(dn_product.name) for dn_product in unmarked_delivery_note_products)
		)
	)

	si_amounts = {si_product[0]: si_product[1] for si_product in si_amounts}

	for dn_product in unmarked_delivery_note_products:
		dn_amount = flt(dn_product.amount)
		si_amount = flt(si_amounts.get(dn_product.name))

		if dn_amount > si_amount and dn_product.base_net_total:
			outstanding_based_on_dn += (
				(dn_amount - si_amount) / dn_product.base_net_total
			) * dn_product.base_grand_total
