...
def get_data_column(filters, partner_doctype):
	...
	for key, value in rows.products():
		value.update({frappe.scrub(partner_doctype): key[0], "product_group": key[1]})
...
def get_data(filters, period_list, partner_doctype):
	...
	if not sales_users_data:
		return
	sales_users = []
	sales_user_wise_product_groups = {}

	for d in sales_users_data:
		if d.parent not in sales_users:
			sales_users.append(d.parent)

		sales_user_wise_product_groups.setdefault(d.parent, [])
		if d.product_group:
			sales_user_wise_product_groups[d.parent].append(d.product_group)
		...
		for period in period_list:
			...
			for r in actual_data:
				if (
					...
					and (not sales_user_wise_product_groups.get(d.parent) or r.product_group == d.product_group)
				):
...
def get_actual_data(filters, sales_users_or_territory_data, date_field, sales_field):
	...
	child_table = "`tab{0}`".format(filters.get("doctype") + " Product")

	if sales_field == "sales_person":
		select_field = "`tabSales Team`.sales_person"
		child_table = "`tab{0}`, `tabSales Team`".format(filters.get("doctype") + " Product")
	...
	return frappe.db.sql(
		""" SELECT `tab{child_doc}`.product_group,
		...
		WHERE
			...
			child_doc=filters.get("doctype") + " Product",
		),
...
def get_parents_data(filters, partner_doctype):
	...
	return frappe.get_all(
		...
		fields=["parent", "product_group", target_qty_amt_field, "fiscal_year", "distribution_id"],
	)
