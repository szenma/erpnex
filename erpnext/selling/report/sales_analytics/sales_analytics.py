...
class Analytics(object):
	...
	def run(self):
		...
		if self.filters.tree_type in ["Supplier Group", "Product Group", "Customer Group", "Territory"]:
			skip_total_row = 1
	...
	def get_columns(self):
		...
		if self.filters.tree_type in ["Customer", "Supplier", "Product"]:
			self.columns.append(
				{
		...
		if self.filters.tree_type == "Product":
			self.columns.append(
				{
					"label": _("UOM"),
					"fieldname": "stock_uom",
	...
	def get_data(self):
		...
		elif self.filters.tree_type == "Product":
			self.get_sales_transactions_based_on_products()
			self.get_rows()
		...
		elif self.filters.tree_type == "Product Group":
			self.get_sales_transactions_based_on_product_group()
			self.get_rows_by_group()
	...
	def get_sales_transactions_based_on_products(self):
		...
		self.entries = frappe.db.sql(
			"""
			select i.product_code as entity, i.product_name as entity_name, i.stock_uom, i.{value_field} as value_field, s.{date_field}
			from `tab{doctype} Product` i , `tab{doctype}` s
	...
	def get_sales_transactions_based_on_product_group(self):
		if self.filters["value_quantity"] == "Value":
			value_field = "base_net_amount"
		else:
			value_field = "qty"
		...
		self.entries = frappe.db.sql(
			"""
			select i.product_group as entity, i.{value_field} as value_field, s.{date_field}
			from `tab{doctype} Product` i , `tab{doctype}` s
			where s.name = i.parent and i.docstatus = 1 and s.company = %s
			and s.{date_field} between %s and %s
		""".format(
	...
	def get_rows(self):
		...
		for entity, period_data in self.entity_periodic_data.products():
			row = {
				"entity": entity,
	...
	def get_groups(self):
		...
		if self.filters.tree_type == "Product Group":
			parent = "parent_product_group"
		if self.filters.tree_type == "Supplier Group":
