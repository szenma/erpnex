...
def get_data(conditions, filters):
	data = frappe.db.sql(
		...
		SELECT
			...
			so.status, so.customer, soi.product_code,
		...
		FROM
			`tabSales Order` so,
			`tabSales Order Product` soi
		LEFT JOIN `tabSales Invoice Product` sii
		...
		WHERE
			...
			{conditions}
		GROUP BY soi.name
		ORDER BY so.transaction_date ASC, soi.product_code ASC
	""".format(
	...
	if data:
		...
		query = (
			...
			.select(
				...
				soi.product_code.as_("so_product_code"),
		...
		for e in dn_elapsed_time:
			key = (e.sales_order, e.so_product_code)
...
def prepare_data(data, so_elapsed_time, filters):
	...
	for row in data:
		...
		row["time_taken_to_deliver"] = (
			so_elapsed_time.get((row.sales_order, row.product_code))
