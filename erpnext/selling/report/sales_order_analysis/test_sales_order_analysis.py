...
test_dependencies = ["Sales Order", "Product", "Sales Invoice", "Delivery Note"]
...
class TestSalesOrderAnalysis(FrappeTestCase):
	def create_sales_order(self, transaction_date, do_not_save=False, do_not_submit=False):
		product = create_product(product_code="_Test Excavator", is_stock_product=0)
		so = make_sales_order(
		...
		so.taxes = ""
		so.products[0].delivery_date = add_days(transaction_date, 15)
		if not do_not_save:
			so.save()
	...
	def test_01_so_to_deliver_and_bill(self):
		...
		product, so = self.create_sales_order(transaction_date)
		...
		for key, val in expected_value.products():
	...
	def test_07_so_delivered_with_multiple_delivery_notes(self):
		...
		dn1 = self.create_delivery_note(so, do_not_save=True)
		dn1.products[0].qty = 5
		dn1 = dn1.save().submit()

		dn2 = self.create_delivery_note(so, do_not_save=True)
		dn2.products[0].qty = 5
		dn2 = dn2.save().submit()
