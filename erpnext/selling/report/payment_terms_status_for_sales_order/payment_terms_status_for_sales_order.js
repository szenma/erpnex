	...
	let filters = [
		...
		},
		{
			...
			"get_query": () => {
				...
				return{
					"query": "erpnext.selling.report.payment_terms_status_for_sales_order.payment_terms_status_for_sales_order.get_customers_or_products",
					"filters": [
						['Customer', 'disabled', '=', '0'],
		...
		},
		{
			...
			"options": "Product",
			"get_query": () => {
				var product_group = frappe.query_report.get_filter_value('product_group');
				return{
					"query": "erpnext.selling.report.payment_terms_status_for_sales_order.payment_terms_status_for_sales_order.get_customers_or_products",
					"filters": [
						['Product', 'disabled', '=', '0'],
