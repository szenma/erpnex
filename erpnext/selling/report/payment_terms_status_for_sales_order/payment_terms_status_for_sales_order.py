...
def get_customers_or_products(doctype, txt, searchfield, start, page_len, filters):
	filter_list = []
	if isinstance(filters, list):
		for product in filters:
			if product[0] == doctype:
				filter_list.append(product)
			elif product[0] == "Customer Group":
				if product[3] != "":
					filter_list.append(
						[doctype, "customer_group", "in", get_descendants_of("Customer Group", product[3])]
					)
			elif product[0] == "Product Group":
				if product[3] != "":
					filter_list.append([doctype, "product_group", "in", get_descendants_of("Product Group", product[3])])
	...
	return frappe.desk.reportview.execute(
		doctype,
		filters=filter_list,
		fields=["name", "customer_group"] if doctype == "Customer" else ["name", "product_group"],
		limit_start=start,
...
def build_filter_criterions(filters):
	...
	if filters.product_group:
		qb_criterions.append(
			qb.DocType("Sales Order Product").product_group.isin(
				get_descendants_of("Product Group", filters.product_group)
			)
		)

	if filters.product:
		qb_criterions.append(qb.DocType("Sales Order Product").product_code == filters.product)
...
def get_so_with_invoices(filters):
	...
	soi = qb.DocType("Sales Order Product")
	...
	if sorders != []:
		...
		sii = qb.DocType("Sales Invoice Product")
		query_inv = (
			qb.from_(sii)
			.right_join(si)
