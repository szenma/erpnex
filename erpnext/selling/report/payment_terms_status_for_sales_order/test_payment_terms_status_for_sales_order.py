...
test_dependencies = ["Sales Order", "Product", "Sales Invoice", "Payment Terms Template", "Customer"]
...
class TestPaymentTermsStatusForSalesOrder(FrappeTestCase):
	...
	def test_01_payment_terms_status(self):
		...
		product = create_product(product_code="_Test Excavator 1", is_stock_product=0)
		so = make_sales_order(
	...
	def test_02_alternate_currency(self):
		...
		product = create_product(product_code="_Test Excavator 2", is_stock_product=0)
		so = make_sales_order(
	...
	def test_03_group_filters(self):
		transaction_date = "2021-06-15"
		self.create_payment_terms_template()
		product1 = create_product(product_code="_Test Excavator 1", is_stock_product=0)
		product1.product_group = "Products"
		product1.save()

		so1 = make_sales_order(
			transaction_date=transaction_date,
			delivery_date=add_days(transaction_date, -30),
			product=product1.product_code,
			qty=1,
			rate=1000000,
			do_not_save=True,
		)
		...
		product2 = create_product(product_code="_Test Steel", is_stock_product=0)
		product2.product_group = "Raw Material"
		product2.save()
