...
erpnext.selling.SellingController = class SellingController extends erpnext.TransactionController {
		...
		if(this.frm.fields_dict["products"].grid.get_field('product_code')) {
			this.frm.set_query("product_code", "products", function() {
				return {
					...
					filters: {'is_sales_product': 1, 'customer': cur_frm.doc.customer, 'has_variants': 0}
				}
			});
		}

		if(this.frm.fields_dict["packed_products"] &&
			this.frm.fields_dict["packed_products"].grid.get_field('batch_no')) {
			this.frm.set_query("batch_no", "packed_products", function(doc, cdt, cdn) {
	...
	discount_percentage(doc, cdt, cdn) {
		...
		this.apply_discount_on_product(doc, cdt, cdn, 'discount_percentage');
	}
	...
	discount_amount(doc, cdt, cdn) {
		...
		this.apply_discount_on_product(doc, cdt, cdn, 'discount_amount');
	}

	apply_discount_on_product(doc, cdt, cdn, field) {
		var product = frappe.get_doc(cdt, cdn);
	...
	warehouse(doc, cdt, cdn) {
		...
		if (product.serial_no) {
			let serial_nos = product.serial_no.split(`\n`).filter(sn => sn.trim()); // filter out whitespaces
			if (product.qty === serial_nos.length) return;
		}

		if (product.serial_no && !product.batch_no) {
			product.serial_no = null;
		}

		var has_batch_no;
		frappe.db.get_value('Product', {'product_code': product.product_code}, 'has_batch_no', (r) => {
			has_batch_no = r && r.has_batch_no;
			if(product.product_code && product.warehouse) {
				return this.frm.call({
					method: "erpnext.stock.get_product_details.get_bin_details_and_serial_nos",
					child: product,
					args: {
	...
	toggle_editable_price_list_rate() {
		var df = frappe.meta.get_docfield(this.frm.doc.doctype + " Product", "price_list_rate", this.frm.doc.name);
		var editable_price_list_rate = cint(frappe.defaults.get_default("editable_price_list_rate"));

		if(df && editable_price_list_rate) {
			const parent_field = frappe.meta.get_parentfield(this.frm.doc.doctype, this.frm.doc.doctype + " Product");
	...
	calculate_commission() {
		...
		this.frm.doc.amount_eligible_for_commission = this.frm.doc.products.reduce(
			(sum, product) => product.grant_commission ? sum + product.base_net_amount : sum, 0
		)
	...
	batch_no(doc, cdt, cdn) {
		...
		product.serial_no = null;
		var has_serial_no;
		frappe.db.get_value('Product', {'product_code': product.product_code}, 'has_serial_no', (r) => {
			has_serial_no = r && r.has_serial_no;
			if(product.warehouse && product.product_code && product.batch_no) {
				return this.frm.call({
					method: "erpnext.stock.get_product_details.get_batch_qty_and_serial_no",
					child: product,
					args: {
						"batch_no": product.batch_no,
						"stock_qty": product.stock_qty || product.qty, //if stock_qty field is not available fetch qty (in case of Packed Products table)
						"warehouse": product.warehouse,
						"product_code": product.product_code,
	...
	set_product_bundle_help(doc) {
		...
		if ((doc.packed_products || []).length) {
			$(cur_frm.fields_dict.packing_list.row.wrapper).toggle(true);

			if (in_list(['Delivery Note', 'Sales Invoice'], doc.doctype)) {
				var help_msg = "<div class='alert alert-warning'>" +
					__("For 'Product Bundle' products, Warehouse, Serial No and Batch No will be considered from the 'Packing List' table. If Warehouse and Batch No are same for all packing products for any 'Product Bundle' product, those values can be entered in the main Product table, values will be copied to 'Packing List' table.")+
				"</div>";
	...
	*/
	...
	_set_batch_number(doc) {
		...
		let args = {'product_code': doc.product_code, 'warehouse': doc.warehouse, 'qty': flt(doc.qty) * flt(doc.conversion_factor)};
		...
		if(frm.doc.project) {
			frappe.call({
				...
				callback: function(r, rt) {
					if(!r.exc) {
						$.each(frm.doc["products"] || [], function(i, row) {
							if(r.message) {
								frappe.model.set_value(row.doctype, row.name, "cost_center", r.message);
								frappe.msgprint(__("Cost Center For Product with Product Code {0} has been Changed to {1}", [row.product_name, r.message]));
							}
						})
					}
				}
