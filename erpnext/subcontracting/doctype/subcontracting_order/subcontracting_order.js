...
frappe.ui.form.on('Subcontracting Order', {
	setup: (frm) => {
		frm.get_field("products").grid.cannot_add_rows = true;
		frm.get_field("products").grid.only_sortable();
		...
		frm.set_query('expense_account', 'products', () => ({
		...
		frm.set_query('bom', 'products', (doc, cdt, cdn) => {
	...
	purchase_order: (frm) => {
		frm.set_value('service_products', null);
		frm.set_value('products', null);
		frm.set_value('supplied_products', null);
	...
	get_materials_from_supplier: function (frm) {
		...
		if (frm.doc.status != "Closed" && frm.doc.supplied_products && frm.doc.per_received > 0) {
...
erpnext.buying.SubcontractingOrderController = class SubcontractingOrderController {
	...
	set_warehouse(doc) {
		this.set_warehouse_in_children(doc.products, "warehouse", doc.set_warehouse);
	}

	set_reserve_warehouse(doc) {
		this.set_warehouse_in_children(doc.supplied_products, "reserve_warehouse", doc.set_reserve_warehouse);
	}
